...
export function SmartManager() {
  ...
  useEffect(() => {
    if (session && !session.demo) {
      setCompany({
        ...
        taxRate: session.company.taxRate ?? 18,
        timezone: session.company.timezone || "Africa/Dar_es_Salaam",
        businessScale: session.company.businessScale || "large",
        createdAt: session.company.created_at || null,
        receiptWidth: session.company.receiptWidth || "80mm",
        receiptFooter: session.company.receiptFooter || "",
        receiptShowLogo: session.company.receiptShowLogo !== false,
      });
      setCurrentUser({
  ...
  useEffect(() => {
    ...
    sb("company_modules")
      ...
      .catch(() => {
        /* keep the "everything enabled" default rather than block on a failed fetch */
      });
  }, [company.id]);
    ...
    if (IS_CONFIGURED && company.id) {
      ...
      try {
        await sb("company_modules")
          .insert({ company_id: company.id, module_key: id, enabled: !turningOff })
          .run();
      } catch (_e) {
        try {
  ...
  const inventory = useCompanyTable("inventory_items", inventorySeed, {
    order: { col: "name", ascending: true },
    mapRow: mapInventoryRow,
  });
  const invoices = useCompanyTable("sales_invoices", invoicesSeed, {
    select: "*,sales_invoice_items(*),sales_payments(*)",
    order: { col: "due_date", ascending: true },
    mapRow: mapInvoiceRow,
  });
  const crm = useCompanyTable("crm_leads", seedLeads, {
    order: { col: "created_at", ascending: false },
    mapRow: mapLeadRow,
  });
  const expenses = useCompanyTable("finance_expenses", expensesSeed, {
    order: { col: "expense_date", ascending: false },
    mapRow: mapExpenseRow,
  });
  const suppliers = useCompanyTable("inventory_suppliers", suppliersSeed, {
    order: { col: "name", ascending: true },
    mapRow: mapSupplierRow,
  });
  const posTransactions = useCompanyTable("pos_transactions", posTransactionsSeed, {
    select: "*,pos_transaction_items(*),profiles(full_name),pos_returns(*,pos_return_items(*))",
    order: { col: "created_at", ascending: false },
    mapRow: mapPosTransactionRow,
  });
  ...
  const quotations = useCompanyTable("sales_quotations", quotationsSeed, {
    select: "*,sales_quotation_items(*)",
    order: { col: "issue_date", ascending: false },
    mapRow: mapQuotationRow,
  });
  const scheduledWorkflows = useCompanyTable("scheduled_reports", scheduledReportsSeed, {
    mapRow: mapScheduledReportRow,
  });
  ...
  const visibleModules = MODULES.filter(
    (m) => enabledModules.has(m.id) && currentRole.allowedModules.includes(m.id),
  );
  ...
  useEffect(() => {
    const up = () => {
      setOnline(true);
      notify("Back online — new changes will save to the server again.");
    };
    const down = () => {
      setOnline(false);
      notify("Offline — keep working; changes stay on this device until you reconnect.", "error");
    };
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div style={{ animation: "fadeInUp .5s ease-out .15s both" }} className="text-center">
            <p className="text-[15px] font-bold tracking-tight" style={{ fontFamily: "'Poppins'" }}>
              <span className="text-[#111827]">SMART</span>{" "}
              <span className="text-[#16A34A]">MANAGER</span>
            </p>
  ...
  if (!session) {
    return authView === "login" ? (
      <LoginPage
        onAuthenticated={(s) => setSession(s || { demo: true })}
  ...
  return (
    <>
      ...
      <div
        className={`h-screen w-full flex text-slate-800 overflow-hidden relative text-size-${textSize} ${darkMode ? "dark bg-[#0F172A]" : "bg-[#F8FAFC]"} ${highContrast ? "high-contrast" : ""}`}
      ...
      >
          ...
          <div className="relative px-5 py-5 border-b border-[#F3F4F6] flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-col leading-tight">
                <span
                  className="text-[14.5px] font-semibold tracking-tight brand-wordmark"
            ...
            <button
              className="text-slate-400 hover:text-[#111827] transition-colors"
              onClick={() => setSidebarOpen(false)}
                ...
                >
                  ...
                  <span className="flex items-center gap-2.5">
                    <Icon
                      size={16}
                      strokeWidth={2}
                      className={isActive ? "" : "group-hover:text-[#22C55E] transition-colors"}
                      style={isActive ? { color: "#16A34A" } : undefined}
                    />
                    {m.label}
                  </span>
            ...
            >
              <span className="flex items-center gap-2.5">
                <Settings
                  ...
                  strokeWidth={2}
                  className={
                    active === "settings" ? "" : "group-hover:text-[#22C55E] transition-colors"
                  }
                  style={active === "settings" ? { color: "#16A34A" } : undefined}
                />{" "}
                Settings
        ...
        <div className="relative z-10 flex-1 flex flex-col min-w-0 w-full">
          {/* Topbar */}
          <header
            className={`h-16 shrink-0 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 sm:px-6 ${darkMode ? "dark-shell" : ""}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 text-[13px] text-slate-500">
                ...
                <span className="font-medium text-[#111827] truncate max-w-[140px] sm:max-w-none">
                  {company.name}
                </span>
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <span
                ...
                title={
                  IS_CONFIGURED
                    ...
                    : "Running on built-in demo data — connect Supabase to persist changes"
                }
              >
                <span
                  className="w-1.5 h-1.5 rounded-full"
                  style={{
                    backgroundColor: !online ? "#EF4444" : IS_CONFIGURED ? "#16A34A" : "#F59E0B",
                  }}
                />
              ...
              >
                ...
                <kbd className="hidden sm:inline-block text-[10px] font-mono bg-slate-100 px-1.5 py-0.5 rounded">
                  ⌘K
                </kbd>
              </button>
              ...
              {criticalAlerts.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-[11.5px] font-semibold px-2.5 py-1.5 rounded-xl animate-pulse"
          ...
          {paletteOpen && (
            <CommandPalette
              ...
              expenses={expenses}
              onNavigate={go}
              onNavigateWithIntent={goWithIntent}
          ...
          <nav
            className="sm:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-slate-200/80 flex"
            style={{ backdropFilter: "blur(12px)" }}
          >
            {[
              ...visibleModules.filter((m) =>
                ["dashboard", "sales", "inventory", "finance", "hr"].includes(m.id),
              ),
              ...visibleModules.filter(
                (m) => !["dashboard", "sales", "inventory", "finance", "hr"].includes(m.id),
              ),
            ]
              .slice(0, 5)
              .map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[9.5px] font-medium leading-none mt-1 truncate max-w-[48px]">
                      {m.label.split(" ")[0]}
                    </span>
                  </button>
                );
          ...
          <main
            key={active}
            className="module-fade flex-1 overflow-y-auto p-4 sm:p-6 pb-24 sm:pb-6"
          >
            {active === "dashboard" && (
              <Dashboard
                company={company}
                ...
                currentUser={currentUser}
                onQuickAction={goWithIntent}
                onNavigate={go}
              />
            )}
            {active === "crm" && (
              <CRM crm={crm} invoices={invoices} expenses={expenses} suppliers={suppliers} />
            )}
            {active === "sales" && (
              <Sales
                invoices={invoices}
                inventory={inventory}
                subscriptionsHook={subscriptions}
            ...
            {active === "inventory" && (
              <Inventory inventory={inventory} suppliersHook={suppliers} />
            )}
            {active === "procurement" && (
              <Procurement
                inventory={inventory}
                suppliersHook={suppliers}
            ...
            {active === "finance" && (
              <Finance
                invoices={invoices}
                expensesHook={expenses}
                posTransactionsHook={posTransactions}
            ...
            {active === "reports" && (
              <Reports
                ...
                expensesHook={expenses}
                company={company}
                schedulesHook={scheduledWorkflows}
            ...
            {active === "pos" && (
              <POS
                inventory={inventory}
                transactionsHook={posTransactions}
                company={company}
                currentUser={currentUser}
              />
            )}
            ...
            {active === "projects" && (
              <Projects filesHook={files} expensesHook={expenses} company={company} />
            )}
            ...
            {active === "hr" && (
              <HR
                employeesHook={employees}
                leaveRequestsHook={leaveRequests}
                expensesHook={expenses}
                intent={intent}
                clearIntent={clearIntent}
                currentUser={currentUser}
            ...
            {active === "healthcare" && (
              <HealthcareClinicModule currentUser={currentUser} company={company} />
            )}
            {active === "school" && (
              <SchoolManagementModule
                currentUser={currentUser}
                company={company}
                onFeesLoad={setSchFeesForAlerts}
              />
            )}
            {active === "pharmacy" && (
              <PharmacyManagementModule
                currentUser={currentUser}
                company={company}
                onStockLoad={setPhmStockForAlerts}
              />
            )}
            {active === "hotel" && (
              <HotelManagementModule
                currentUser={currentUser}
                company={company}
                onBookingsLoad={setHtlBookingsForAlerts}
              />
            )}
            {active === "fleet" && (
              <FleetManagementModule
                currentUser={currentUser}
                company={company}
                onVehiclesLoad={setVehiclesForAlerts}
              />
            )}
            {active === "banking" && (
              <BankingMFIModule
                currentUser={currentUser}
                company={company}
                onLoansLoad={setBankLoansForAlerts}
              />
            )}
            {active === "restaurant" && (
              <RestaurantModule
                currentUser={currentUser}
                company={company}
                onOrdersLoad={setRstOrdersForAlerts}
              />
            )}
