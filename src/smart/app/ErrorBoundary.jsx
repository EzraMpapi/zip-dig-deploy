...
export class ErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className="min-h-screen w-full flex items-center justify-center bg-[#F8FAFC] p-4"
      ...
      >
        <div className="w-full max-w-md text-center">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200/60 p-6 sm:p-8">
            <div
              className="w-11 h-11 rounded-xl mx-auto flex items-center justify-center mb-4"
              style={{ backgroundColor: "#FEE2E2" }}
            >
              <AlertCircle size={20} className="text-[#EF4444]" />
            </div>
            <h1
              className="text-[17px] font-semibold text-[#111827] mb-1.5"
              style={{ fontFamily: "'Poppins'" }}
            >
              Something went wrong
            </h1>
            <p className="text-[13px] text-slate-500 leading-relaxed mb-5">
              A part of the app hit an unexpected error and stopped safely rather than showing
              something incorrect. Your data on the server is untouched — this only affected what
              was on screen.
            </p>
            {this.state.error?.message && (
              <p className="text-[11px] font-mono text-slate-400 bg-slate-50 rounded-lg px-3 py-2 mb-5 text-left break-words">
                {this.state.error.message}
              </p>
            )}
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="btn-secondary text-[13px] font-medium rounded-lg py-2.5"
              >
                Reload the App
              </button>
              <button
                onClick={this.handleSignOutAndReload}
                className="text-[12px] text-slate-400 hover:text-slate-600 py-1.5"
              >
...
export async function signDocument({ docType, docRef, signerName, content }) {
  ...
  notify(
    biometric
      ? `Signed by ${signerName} — biometric verified, content hashed.`
      : `Signed by ${signerName} — content hashed (no biometric sensor on this device).`,
  );
  if (IS_CONFIGURED) {
    try {
      await sb("digital_signatures")
        .insert({
          doc_type: docType,
          doc_ref: docRef,
      ...
      notify("Signature recorded locally, but the server update failed.", "error");
    }
  }
  return record;
}
