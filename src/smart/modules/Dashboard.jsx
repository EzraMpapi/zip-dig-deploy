...
export function Dashboard({
  ...
  const financials = useMemo(() => {
    ...
    const expRows = expenses.rows.filter(
      (e) => !periodStart || (e.date || e.expenseDate || "") >= periodStart,
    );
    ...
    const pendingCash = outstanding.reduce(
  ...
  const financeKpis = [
    {
      ...
      value: `TZS ${money(Math.round(financials.revenue))}k`,
    ...
    {
      label: "Expenses",
      value: `TZS ${money(Math.round(financials.expenseTotal))}k`,
      delta: PERIOD_LABELS[period],
      up: false,
      icon: Wallet,
    },
    {
      label: "Profit",
      value: `TZS ${money(Math.round(financials.profit))}k`,
      delta: financials.profit >= 0 ? "Net positive" : "Net negative",
      up: financials.profit >= 0,
      icon: financials.profit >= 0 ? TrendingUp : TrendingDown,
    },
    {
      label: "Cash Flow",
      value: `TZS ${money(Math.round(financials.pendingCash))}k`,
      delta: `${financials.outstandingCount} invoices pending`,
      up: false,
      icon: Landmark,
    },
  ];
  ...
  const pipelineByStage = useMemo(() => {
    return STAGES.map((stage) => ({
      stage,
      value: crm.rows.filter((l) => l.stage === stage).length,
    }));
  }, [crm.rows]);
  ...
  const stockByCategory = useMemo(() => {
    ...
    inventory.rows.forEach((it) => {
      map[it.category] = (map[it.category] || 0) + it.qty * it.unitCost;
    });
    return Object.entries(map)
      .sort((a, b) => b[1] - a[1])
  ...
  const pendingLeave = useMemo(
    () => leaveRequests.rows.filter((l) => l.status === "Pending"),
    [leaveRequests.rows],
  );
  ...
  const recentActivity = useMemo(() => {
    ...
    expenses.rows.forEach((e) => {
      items.push({
        ...
        icon: Wallet,
        color: "#F59E0B",
        text: `Expense recorded — ${e.category}`,
        sub: `TZS ${money(e.amount)}k · ${e.vendor}`,
      });
    });
    leaveRequests.rows.forEach((l) => {
      items.push({
        date: l.startDate,
        icon: Clock,
        color: l.status === "Approved" ? "#16A34A" : "#F59E0B",
        text: `Leave ${l.status.toLowerCase()} — ${l.type}`,
        sub: l.employee,
      });
    });
  ...
  const quickActions = [
    {
      ...
      icon: ReceiptText,
      action: () => onQuickAction("sales", { tab: "invoices", openForm: true }),
    },
    ...
    { label: "Approve Leave", icon: Clock, action: () => onQuickAction("hr", { tab: "leave" }) },
    {
      ...
      icon: CreditCard,
      action: () => onQuickAction("finance", { tab: "receivables" }),
    },
    {
      label: "Record Expense",
      icon: Wallet,
      action: () => onQuickAction("finance", { tab: "expenses" }),
    },
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {pendingLeave.length > 0 && (
            <span className="text-[11px] font-mono text-slate-400">{pendingLeave.length}</span>
          )}
        </div>
        {pendingLeave.length === 0 ? (
          ...
          <div className="space-y-1">
            {pendingLeave.slice(0, 4).map((l) => (
              <button
                key={l.id}
                onClick={() => onQuickAction("hr", { tab: "leave" })}
                className="w-full flex items-center justify-between px-2 py-2 rounded-lg hover:bg-slate-50 text-left transition-colors"
              >
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        ...
        {recentActivity.length === 0 ? (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <p className="text-[12.5px] text-slate-500">Nothing recorded yet this session.</p>
          </div>
        ) : (
          <div className="space-y-1">
              ...
              return (
                <div key={i} className="flex items-center gap-2.5 px-2 py-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[12.5px] font-medium text-[#111827] truncate">{a.text}</p>
  ...
  const roleHeader = (focusLine) => (
    <div>
      <h1 className="text-[22px] font-semibold text-[#111827] tracking-tight">
        Hello, {company.owner}
      </h1>
      <p className="text-[13px] text-slate-500 mt-1">
        {currentUser.role} view — {focusLine}
      </p>
    </div>
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <button
          ...
          className="w-full bg-white rounded-xl border border-slate-200/80 shadow-sm p-6 flex items-center justify-between hover:border-[#16A34A]/40 transition-colors text-left"
        >
          <div>
            <p className="text-[15px] font-semibold text-[#111827] mb-1">Open {targetLabel}</p>
            <p className="text-[12.5px] text-slate-500">
              Tasks, timelines, and details for your role live there — this home screen doesn&apos;t
              duplicate that view.
            </p>
          </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-8 text-center">
          <div
            className="w-11 h-11 rounded-xl mx-auto flex items-center justify-center mb-3.5"
            style={{ backgroundColor: "#DCFCE7" }}
          >
            <Briefcase size={19} strokeWidth={1.75} className="text-[#16A34A]" />
          </div>
          <p className="text-[14.5px] font-semibold text-[#111827] mb-1">
            Welcome to {company.name}
          </p>
          <p className="text-[12.5px] text-slate-500 max-w-[380px] mx-auto leading-relaxed">
            Your access is scoped to{" "}
            {currentRole.allowedModules
              .map((m) => MODULES.find((mm) => mm.id === m)?.label)
              .filter(Boolean)
              .join(" and ")}{" "}
  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg,#0D2214 0%,#1a3a2a 55%,#16A34A 130%)" }}
      >
        ...
        <div className="relative px-5 sm:px-7 py-5">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10.5px] text-[rgba(255,255,255,.4)] font-mono">
              ...
              <h1 className="text-white text-[22px] font-black tracking-tight leading-none">
                ...
                , {(company.owner || "Welcome").split(" ")[0]} 👋
              </h1>
              <p className="text-[rgba(255,255,255,.5)] text-[12px] mt-1">
                {company.name} · {currentUser.role}
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <button
                onClick={() => onNavigate("ai")}
                className="flex items-center gap-1.5 text-[12px] font-bold text-[#111827] bg-[#16A34A] px-3.5 py-2 rounded-xl hover:bg-[#15803D]"
              >
                <Sparkles size={13} /> Ask AI
              </button>
              <button
                onClick={() =>
                  typeof window.__openDailyBrief === "function" && window.__openDailyBrief()
                }
          ...
          {(() => {
            ...
            const overdueInvs = invRows.filter(
              (i) => i.status !== "Paid" && i.dueDate < TODAY.toISOString().slice(0, 10),
            );
            const overdueAmt = overdueInvs.reduce(
              (s, i) => s + lineTotal(i.items || []).total - (i.amountPaid || 0),
              0,
            );
            const lowStock = inventory.rows.filter(
              (it) => it.qty <= it.reorder && it.reorder > 0,
            ).length;
            ...
            const activeEmployees = (employees?.rows || employees || []).filter(
              (e) => e.status === "Active",
            ).length;
            const activeSubs = subscriptions.rows.filter((s) => s.status === "Active");
            const MRR = activeSubs.reduce((s, sub) => {
              const mo = { Monthly: 1, Quarterly: 3, Annual: 12 }[sub.cycle] || 1;
              return s + sub.amount / mo;
            }, 0);
            ...
            return (
              <div className="grid grid-cols-4 lg:grid-cols-8 gap-px bg-[rgba(255,255,255,.06)] rounded-xl overflow-hidden">
                {[
                  {
                    l: "AR Billed",
                    v: "TZS " + money(Math.round(totalBilled / 1000)) + "k",
                    col: "#4ADE80",
                    sub: invRows.length + " invoices",
                  },
                  {
                    l: "Collected",
                    v: "TZS " + money(Math.round(totalCollected / 1000)) + "k",
                    col: "#60A5FA",
                    sub:
                      Math.round(totalBilled > 0 ? (totalCollected / totalBilled) * 100 : 0) +
                      "% rate",
                  },
                  {
                    l: "Overdue AR",
                    v: "TZS " + money(Math.round(overdueAmt / 1000)) + "k",
                    col: overdueAmt > 0 ? "#F87171" : "#4ADE80",
                    sub: overdueInvs.length + " invoices",
                  },
                  {
                    l: "Gross P&L",
                    v:
                      (grossProfit >= 0 ? "+" : "") +
                      money(Math.round(Math.abs(grossProfit) / 1000)) +
                      "k",
                    col: grossProfit >= 0 ? "#4ADE80" : "#F87171",
                    sub: "Collected − Exp",
                  },
                  {
                    l: "Inventory",
                  ...
                  },
                  {
                    l: "Low Stock",
                    v: String(lowStock),
                    col: lowStock > 0 ? "#F87171" : "#4ADE80",
                    sub: inventory.rows.filter((it) => it.qty <= 0).length + " out",
                  },
                  {
                    l: "Pipeline",
                    v:
                      money(
                        Math.round(
                          crm.rows
                            ...
                            .reduce((s, l) => s + (l.value || 0), 0) / 1000,
                        ),
                      ) + "k",
                    col: "#F9A8D4",
                    sub: openLeads + " open deals",
                  },
                  {
                    l: "MRR",
                    v: "TZS " + money(Math.round(MRR)) + "k",
                    col: "#34D399",
                    sub: activeSubs.length + " active subs",
                  },
                ].map(({ l, v, col, sub }) => (
                  <div key={l} className="bg-[rgba(0,0,0,.25)] px-3 py-3 text-center">
                    <p className="text-[9.5px] font-bold uppercase tracking-wide text-[rgba(255,255,255,.45)] mb-1">
                      {l}
                    </p>
                    <p className="text-[14px] font-black leading-tight" style={{ color: col }}>
                      {v}
                    </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          >
            ...
            {alerts.length > 0 && (
              <span className="text-[10.5px] font-black text-white bg-[#EF4444] px-2 py-0.5 rounded-full">
                {alerts.length}
              </span>
            )}
          </div>
            ...
            {alerts.length === 0 ? (
              ...
              alerts.slice(0, 8).map((a, i) => {
                ...
                return (
                  <button
                    key={a.id || i}
                    onClick={() => onNavigate(a.target || "dashboard")}
                  ...
                  >
                    <span className="text-[14px] shrink-0 mt-0.5">
                      {a.priority === "critical" ? "🚨" : a.priority === "high" ? "⚠️" : "ℹ️"}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] font-bold truncate" style={{ color: col }}>
                        {a.title}
                      </p>
          ...
          <div className="p-3 grid grid-cols-4 sm:grid-cols-6 gap-2">
            {[
              ...
              },
              {
                ...
                icon: Users,
                col: "#7C3AED",
                action: () => onQuickAction("crm", { tab: "leads" }),
              },
              {
                label: "Record Expense",
                icon: Wallet,
              ...
              },
              {
                ...
                icon: Clock,
                col: "#EF4444",
                action: () => onQuickAction("hr", { tab: "leave" }),
              },
              {
                label: "New Employee",
                icon: UserPlus,
                col: "#059669",
                action: () => onQuickAction("hr", { tab: "employees" }),
              },
              {
                label: "New Project",
                icon: FolderKanban,
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-[11.5px] text-slate-400">
                    Collected revenue · Operating expenses · TZS thousands
                  </p>
                </div>
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <ComposedChart data={data} margin={{ left: -10, right: 4, top: 4, bottom: 0 }}>
                  ...
                  />
                  <Tooltip
                    contentStyle={{ borderRadius: 10, border: "1px solid #EEF1F4", fontSize: 12 }}
                    formatter={(v, n) => [`TZS ${money(v)}k`, n]}
                  />
                ...
                const buckets = [
                  {
                    ...
                    days: 0,
                    col: "#16A34A",
                    items: unpaid.filter((i) => !i.dueDate || new Date(i.dueDate) >= TODAY),
                  },
                  {
                    label: "1–30 days",
                    days: 30,
                    ...
                    items: unpaid.filter(
                      (i) =>
                        i.dueDate &&
                        todayMs - new Date(i.dueDate).getTime() > 0 &&
                        todayMs - new Date(i.dueDate).getTime() <= 30 * 86400000,
                    ),
                  },
                  {
                    label: "31–60 days",
                    ...
                    items: unpaid.filter(
                      (i) =>
                        i.dueDate &&
                        todayMs - new Date(i.dueDate).getTime() > 30 * 86400000 &&
                        todayMs - new Date(i.dueDate).getTime() <= 60 * 86400000,
                    ),
                  },
                  {
                    label: "60+ days",
                    ...
                    col: "#EF4444",
                    items: unpaid.filter(
                      (i) => i.dueDate && todayMs - new Date(i.dueDate).getTime() > 60 * 86400000,
                    ),
                  },
                ];
                const totalAR = unpaid.reduce(
                  (s, i) => s + lineTotal(i.items || []).total - (i.amountPaid || 0),
                  0,
                );
                return (
                  <div className="space-y-3">
                      ...
                      return (
                        <div key={b.label}>
                          <div className="flex items-center justify-between text-[12px] mb-1">
                            <span className="font-semibold" style={{ color: b.col }}>
                              {b.label}
                            </span>
                            <span className="font-mono font-bold text-[#111827]">
                              TZS {money(Math.round(amt / 1000))}k
                            </span>
                          </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <span className="text-[13px] font-black text-[#EF4444]">
                        TZS {money(Math.round(totalAR / 1000))}k
                      </span>
                    </div>
                  </div>
                );
        ...
        <div className="p-3 grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-9 gap-2">
          {[
            { id: "dashboard", label: "Dashboard", icon: LayoutDashboard, status: "ok" },
            {
              ...
              label: "CRM",
              icon: Users,
              status: crm.rows.length === 0 ? "empty" : "ok",
            },
            {
              id: "sales",
              label: "Sales",
            ...
            },
            {
              ...
              icon: Package,
              status:
                inventory.rows.filter((it) => it.qty <= it.reorder && it.reorder > 0).length > 0
                  ? "warn"
            ...
            { id: "procurement", label: "Procurement", icon: ShoppingBag, status: "ok" },
            {
              ...
              label: "Finance",
              icon: CircleDollarSign,
              status: expenses.rows.length === 0 ? "empty" : "ok",
            },
            {
              id: "hr",
              label: "HR",
              icon: UserCheck,
              status:
                leaveRequests.rows.filter((l) => l.status === "Pending").length > 0 ? "warn" : "ok",
            },
            {
              id: "manufacturing",
              label: "Mfg",
              ...
              status:
                workOrders.rows.filter(
                  (w) => w.status !== "Completed" && w.dueDate < TODAY.toISOString().slice(0, 10),
                ).length > 0
                  ? "warn"
                  : "ok",
            },
            ...
            const cfg = {
              ok: { ring: "#16A34A", dot: "#16A34A", bg: "#F0FDF4" },
              warn: { ring: "#F59E0B", dot: "#F59E0B", bg: "#FFFBEB" },
              empty: { ring: "#94A3B8", dot: "#94A3B8", bg: "#F8FAFC" },
            };
            ...
            return (
              ...
              >
                <div
                  className="relative w-10 h-10 rounded-xl flex items-center justify-center border-2 transition-all group-hover:scale-105"
                ...
                <span className="text-[9.5px] font-semibold text-slate-500 text-center leading-tight">
                  {label}
                </span>
              </button>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
          ...
          {(() => {
            ...
            if (!custData.length)
              return <p className="text-slate-300 text-center py-8 text-[12px]">No invoices yet</p>;
            return (
          ...
          {(() => {
            ...
            const catData = Object.entries(cats)
              ...
              .map(([name, val], i) => ({
                ...
                value: Math.round(val / 1000),
                fill: ["#16A34A", "#2563EB", "#D97706", "#7C3AED", "#EF4444", "#0891B2"][i % 6],
              }));
            if (!catData.length)
              return (
                <p className="text-slate-300 text-center py-8 text-[12px]">No inventory yet</p>
              );
            return (
                ...
                <div className="flex-1 space-y-1.5">
                  {catData.map((d) => (
                    <div key={d.name} className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-bold" style={{ color: d.fill }}>
                        {money(d.value)}k
                      </span>
                    </div>
                  ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
          ...
          {(() => {
            ...
            const stageData = ["New", "Contacted", "Qualified", "Proposal", "Negotiation"]
              .map((s) => ({
                name: s,
                value: Math.round(
                  crm.rows
                    .filter((l) => l.stage === s)
                    .reduce((sum, l) => sum + (l.value || 0), 0) / 1000,
                ),
                fill: STAGE_COLORS[s],
              }))
              .filter((d) => d.value > 0);
            if (!stageData.length)
              return <p className="text-slate-300 text-center py-8 text-[12px]">No leads yet</p>;
            return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            {pendingLeave.length > 0 && (
              <span className="text-[10.5px] font-black text-white bg-[#F59E0B] px-2 py-0.5 rounded-full">
                {pendingLeave.length}
              </span>
            )}
          </div>
              ...
              recentActivity.map((a, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2.5 px-4 py-2.5">
                    ...
                    <span className="text-[10.5px] text-slate-300 shrink-0">
                      {relativeDay(a.date)}
                    </span>
                  </div>
                );
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {inventory.rows
              .filter((it) => it.qty <= it.reorder && it.reorder > 0)
              .slice(0, 3)
              .map((it) => (
                <button
                  key={it.id}
            ...
            {workOrders.rows
              .filter(
                (w) =>
                  w.status !== "Completed" &&
                  w.status !== "Cancelled" &&
                  w.dueDate < TODAY.toISOString().slice(0, 10),
              )
              .slice(0, 2)
              .map((w) => (
                <button
            ...
            {inventory.rows.filter((it) => it.qty <= it.reorder && it.reorder > 0).length === 0 &&
              workOrders.rows.filter(
                ...
                <div className="py-10 text-center">
                  <CheckCircle2 size={20} className="text-[#16A34A] mx-auto mb-2" />
                  <p className="text-[12px] text-slate-400">Everything looks good</p>
                </div>
              )}
          </div>
        </div>
...
export function GettingStartedChecklist({
  ...
  const steps = [
    ...
    {
      id: "payment",
      label: "Record your first payment or sale",
      done:
        invoices.rows.some((inv) => inv.status === "Paid" || (inv.amountPaid || 0) > 0) ||
        posTransactions.rows.length > 0,
      module: "finance",
    },
    {
      id: "expense",
      label: "Record your first expense",
      done: expenses.rows.length > 0,
      module: "finance",
    },
  ];
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
      <div className="flex items-start justify-between mb-1">
        <div>
          ...
          <p className="text-[12px] text-slate-500 mt-0.5">
            {completedCount} of {steps.length} done — real progress from your own actual records,
            not a checklist that just sits there.
          </p>
        </div>
        <button
      ...
      <div className="space-y-1">
        {steps.map((s) => (
          <button
            key={s.id}
            onClick={() => !s.done && onNavigate(s.module)}
            disabled={s.done}
          ...
          >
            ...
            <span
              className={`text-[13px] ${s.done ? "text-slate-400 line-through" : "text-[#111827] font-medium"}`}
            >
              {s.label}
            </span>
          </button>
...
export function BusinessHealthCard({ invoices, inventory, expenses, posTransactions }) {
  ...
  const health = useMemo(() => {
    ...
    factors.push({
      label: "Stock levels",
      pts: Math.round(stockPct * 25),
      max: 25,
      detail:
        items.length === 0
          ? "No inventory tracked"
          : `${low.length} of ${items.length} items at or below reorder level`,
    });
    ...
    factors.push({
      label: "Payables",
      pts: Math.round(payPct * 20),
      max: 20,
      detail:
        openExp.length === 0
          ? "No open expenses"
          : `${lateExp.length} of ${openExp.length} open expenses past due`,
    });
    ...
    const rThis = revOf(thisM),
      rLast = revOf(lastM);
    const momPct = rLast === 0 ? (rThis > 0 ? 1 : 0.5) : Math.max(0, Math.min(1, rThis / rLast));
    factors.push({
      label: "Sales momentum",
      pts: Math.round(momPct * 25),
      max: 25,
      detail:
        rLast === 0
          ? "No prior-month baseline yet"
          : `This month TZS ${money(Math.round(rThis))}k vs last month ${money(Math.round(rLast))}k`,
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5">
        <div className="relative shrink-0" style={{ width: 84, height: 84 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-[20px] font-bold font-mono text-[#111827] leading-none">
              {health.score}
            </span>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11.5px] text-slate-400 mt-0.5">
            Computed live from your real records right now — every point traceable, nothing
            hardcoded.
          </p>
        </div>
        <button
          onClick={() => setExpanded((e) => !e)}
          className="text-[11.5px] font-medium text-[#16A34A] hover:underline shrink-0 self-start sm:self-auto text-left"
        >
          {expanded ? "Hide breakdown" : "How is this computed?"}
        </button>
      </div>
      {(() => {
        ...
        return (
          <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-3">
            <div>
              <p className="text-[10.5px] text-slate-400">Revenue (month)</p>
              <p className="text-[14px] font-mono font-semibold text-[#111827]">
                TZS {money(Math.round(rev))}k
              </p>
            </div>
            <div>
              <p className="text-[10.5px] text-slate-400">Expenses (month)</p>
              <p className="text-[14px] font-mono font-semibold text-[#111827]">
                TZS {money(Math.round(exp))}k
              </p>
            </div>
            <div>
              <p className="text-[10.5px] text-slate-400">Profit (month)</p>
              <p
                className="text-[14px] font-mono font-semibold"
                style={{ color: profit >= 0 ? "#16A34A" : "#EF4444" }}
              >
                TZS {money(Math.round(profit))}k
              </p>
            </div>
          </div>
        );
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-[14.5px] font-semibold text-[#111827]">
              Built for {profile.label}
            </h3>
            <p className="text-[11.5px] text-slate-400">
              Real guidance for {company.category || "your industry"}, not a generic tip shown to
              everyone.
            </p>
          </div>
        </div>
        <button
          onClick={() => setDismissed(true)}
          className="text-slate-300 hover:text-slate-500"
          aria-label="Dismiss industry insights"
        >
          <X size={16} />
        </button>
      </div>
