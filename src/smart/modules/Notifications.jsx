...
export async function sendWebhookNotification(url, text) {
  ...
  try {
    ...
    return {
      ok: true,
      note: "Request sent. Browsers can't read the response for no-cors webhook calls — check the channel directly to confirm it arrived.",
    };
  } catch (e) {
    return {
      ok: false,
      note: "The request failed to leave the browser — check the URL and your connection.",
    };
  }
}
...
export function Notifications({
  ...
  const channels = useCompanyTable("notification_channels", notificationChannelsSeed, {
    mapRow: mapNotificationChannelRow,
  });
  const rules = useCompanyTable("notification_rules", notificationRulesSeed, {
    mapRow: mapNotificationRuleRow,
  });
  const log = useCompanyTable("notification_log", notificationLogSeed, {
    order: { col: "created_at", ascending: false },
    mapRow: mapNotificationLogRow,
  });
  ...
  const functionalEnabled = channels.rows.filter(
    (c) => c.enabled && NOTIFICATION_CHANNELS.find((n) => n.id === c.id)?.functional,
  ).length;
  ...
  async function logDispatch(entry) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        /* logging failures shouldn't block the notification flow itself */
      }
    }
  }
          ...
          <div className="space-y-2">
            {smartAlerts.map((alert) => {
              ...
              return (
                <div
                  key={alert.id}
                  className="flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer hover:shadow-sm transition-all"
                  style={{ background: ap.bg, borderColor: ap.border }}
                  onClick={() => onNavigate && onNavigate(alert.module)}
                >
                  <span className="text-[20px] shrink-0">{alert.icon}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[13px] font-semibold" style={{ color: ap.text }}>
                        {alert.title}
                      </p>
                      <span
                        className="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase"
                        style={{ background: ap.badge, color: ap.badgeText }}
                      >
                        {alert.priority}
                      </span>
                    ...
                    <p className="text-[12px] mt-0.5" style={{ color: ap.text + "CC" }}>
                      {alert.detail}
                    </p>
                  </div>
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Configure delivery channels and route real business alerts to them
          </p>
        </div>
          ...
          items={[
            {
              ...
              sub: `of ${NOTIFICATION_CHANNELS.length}`,
            },
            {
              label: "Functional Channels On",
              value: String(functionalEnabled),
              sub: "Slack / Teams only",
            },
            {
              label: "Active Business Alerts",
              value: String(alerts.length),
              color: alerts.length > 0 ? "text-[#F59E0B]" : undefined,
            },
...
export function NotificationChannels({ channels, onLog, canManage, currentUser }) {
  ...
  async function updateField(id, key, value) {
    ...
    if (IS_CONFIGURED) {
      ...
      try {
        await sb("notification_channels")
          .eq("channel_id", id)
          .update({ [key === "enabled" ? "enabled" : key]: value })
          .run();
      } catch (_e) {
        /* saved locally regardless; a background sync failure isn't worth interrupting typing */
      }
    }
  }
  ...
  async function sendTest(meta) {
    ...
    if (!config.enabled) {
      notify(`Enable ${meta.name} first.`, "error");
      return;
    }
    setTesting(meta.id);
    if (meta.functional) {
      const result = await sendWebhookNotification(
        config.webhookUrl,
        `Test notification from Smart Manager — if you can see this, ${meta.name} is connected correctly.`,
      );
      notify(result.note, result.ok ? undefined : "error");
      onLog({
        channel: meta.name,
        event: "Test notification",
        message: "Manual test send",
        status: result.ok ? "Sent" : "Failed",
        note: result.note,
      });
    } else {
      notify(
        `${meta.name} needs a backend integration to actually send — see the note below. Nothing was dispatched.`,
        "error",
      );
      onLog({
        channel: meta.name,
      ...
      {!canManage && (
        <div className="flex items-start gap-2.5 bg-[#F59E0B]/5 border border-[#F59E0B]/20 rounded-lg p-3">
          ...
          <p className="text-[12px] text-[#8a670a] leading-relaxed">
            You're viewing as {currentUser.role}. Webhook URLs are credentials — editing channel
            configuration requires a full-write role. You can still view settings and test enabled
            channels below.
          </p>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                {loading ? (
                  <div className="w-9 h-5 rounded-full skeleton-shimmer" />
                ) : (
                  <ToggleSwitch
                    on={config.enabled}
                    disabled={!canManage}
                    onChange={() => updateField(meta.id, "enabled", !config.enabled)}
                    label={`${config.enabled ? "Disable" : "Enable"} ${meta.name}`}
                  />
                )}
              </div>
...
export function AlertRouting({ rules, channels, alerts, onLog }) {
  ...
  function getRule(id) {
    return rows.find((r) => r.id === id) || { channels: [] };
  }

  async function toggleChannel(alertType, channelId) {
    const rule = getRule(alertType);
    const nextChannels = rule.channels.includes(channelId)
      ? rule.channels.filter((c) => c !== channelId)
      : [...rule.channels, channelId];
    setRows((prev) => {
      const exists = prev.find((r) => r.id === alertType);
      return exists
        ? prev.map((r) => (r.id === alertType ? { ...r, channels: nextChannels } : r))
        : [...prev, { id: alertType, channels: nextChannels }];
    });
    if (IS_CONFIGURED) {
      try {
        await sb("notification_rules")
  ...
  async function dispatchNow() {
    if (alerts.length === 0) {
      notify("No active alerts to dispatch right now.");
      return;
    }
    setDispatching(true);
    let sent = 0,
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Routes the same real alerts already shown in the topbar Notification Center — nothing here
          is a separate alert system. "Dispatch Now" sends the alerts currently active to whichever
          enabled channels are checked below.
        </p>
      </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="btn-primary text-white text-[13px] font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5 shadow-sm disabled:opacity-40"
        >
          <Send size={14} />{" "}
          {dispatching ? "Dispatching..." : `Dispatch Now (${alerts.length} active)`}
        </button>
      </div>
                ...
                <tr key={l.id} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <span
                      ...
                      style={{
                        backgroundColor: `${STATUS_COLOR[l.status]}14`,
                  ...
                  <td
                    className="px-4 py-3 text-slate-400 text-[11.5px] max-w-[240px] truncate"
                    title={l.note}
                  >
                    {l.note}
                  </td>
                  <td className="px-4 py-3 text-slate-400 font-mono text-[11.5px]">
              ...
              <tr>
                <td colSpan={5}>
                  <EmptyState
                    ...
                    hint="Test a channel or dispatch alerts to see the log fill in here."
                  />
                </td>
              </tr>
            )}
