...
export function PortalNoticeboard({ company }) {
  ...
  const filtered =
    filter === "All" ? ANNOUNCEMENTS_SEED : ANNOUNCEMENTS_SEED.filter((a) => a.category === filter);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            {co.name || "SMART MANAGER"} · Official Announcements
          </p>
        </div>
      ...
      {pinned.length > 0 && (
        <div className="space-y-3">
          ...
          {pinned.map((ann) => {
            const [bg, col, border] = ANN_CAT_COLORS[ann.category] || [
              "#F8FAFB",
              "#374151",
              "#E5E7EB",
            ];
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-[10.5px] font-bold text-[#EF4444] bg-[#FEF2F2] px-2 py-0.5 rounded-full">
                        {ann.priority}
                      </span>
                    </div>
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            {filtered.length} colleagues · {depts.length - 1} departments
          </p>
        </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {depts.map((d, i) => (
          <button
            ...
            style={
              deptFilter === d
                ? { background: i === 0 ? "#0D2214" : DEPT_COLORS[(i - 1) % DEPT_COLORS.length] }
                : {}
            }
          >
            {d}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map((emp, idx) => {
          ...
          const avatarBg =
            DEPT_COLORS[(deptIdx - 1 + DEPT_COLORS.length) % DEPT_COLORS.length] || "#0D2214";
          const isSelf = emp.id === self?.id || emp.name === empName;
          return (
            <div
              key={emp.id}
              className={`bg-white rounded-xl border shadow-sm p-4 transition-all hover:shadow-md ${isSelf ? "border-[#16A34A]/40 ring-1 ring-[#16A34A]/20" : ""}`}
            >
              <div className="flex items-start gap-3">
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {isSelf && (
                      <span className="text-[9.5px] font-black text-[#16A34A] bg-[#F0FDF4] px-1.5 py-0.5 rounded-full border border-[#BBF7D0]">
                        YOU
                      </span>
                    )}
                  </div>
              ...
              <div className="mt-3 space-y-1.5 border-t border-slate-100 pt-3">
                {emp.email && (
                  <a
                    ...
                    className="flex items-center gap-2 text-[11.5px] text-slate-500 hover:text-[#2563EB] transition-colors"
                  >
                ...
                {emp.phone && (
                  <a
                    href={`tel:${emp.phone}`}
                    className="flex items-center gap-2 text-[11.5px] text-slate-500 hover:text-[#16A34A] transition-colors"
                  >
                ...
                {emp.phone && (
                  <a
                    href={`https://wa.me/${emp.phone.replace(/[^0-9]/g, "")}?text=Hi ${emp.name.split(" ")[0]},`}
                    target="_blank"
                    rel="noopener"
...
export function PortalExpenses({ empName, employees }) {
  ...
  const [claims, setClaims] = useState([
    {
      id: "CLM-001",
      category: "Travel",
      description: "Taxi to client site — Uzuri Beauty",
      amount: 45,
      date: "2026-07-18",
      status: "Approved",
    ...
    },
    {
      id: "CLM-002",
      category: "Meals",
      description: "Team lunch — July strategy meeting",
      amount: 120,
      date: "2026-07-15",
      status: "Pending",
      approvedBy: null,
    },
    {
      id: "CLM-003",
      category: "Transport",
      description: "Fuel reimbursement — delivery run",
      amount: 85,
      date: "2026-07-10",
      status: "Rejected",
  ...
  function submitClaim() {
    if (!form.description.trim() || !form.amount) {
      notify("Description and amount are required", "error");
      return;
    }
    const claim = {
      id: docId("CLM"),
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
            ...
            <p className="text-[18px] font-black" style={{ color: col }}>
              {v}
            </p>
          </div>
        ))}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              ...
              >
                {EXPENSE_CATEGORIES_PERSONAL.map((cat) => (
            ...
            <div className="col-span-2 sm:col-span-3">
              ...
              <input
                ...
                placeholder="Receipt no., vendor name, etc."
              />
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            <button
              ...
              className="flex-1 text-[12.5px] font-medium border border-slate-200 rounded-xl py-2.5 text-slate-500"
            >
              Cancel
            </button>
            <button
              onClick={submitClaim}
              className="flex-1 text-[12.5px] font-bold text-white rounded-xl py-2.5 bg-[#16A34A]"
            >
              Submit Claim
            </button>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-[14px] font-black font-mono" style={{ color: sc.col }}>
                          TZS {money(claim.amount)}k
                        </p>
                        <span className="text-[10.5px] font-bold" style={{ color: sc.col }}>
...
export function PortalTraining({ empName }) {
  const training = useCompanyTable("hr_training", trainingSeed, {
    order: { col: "course", ascending: true },
    mapRow: (r) => r,
  });
  const myTraining = training.rows.filter(
    (t) =>
  ...
  async function markDone(id) {
    ...
    training.setRows((p) =>
      p.map((t) => (t.id === id ? { ...t, status: "Completed", completionDate: today } : t)),
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#0D2214] rounded-2xl p-5 text-white">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            ["Not Started", pending, "#94A3B8"],
          ].map(([l, v, col]) => (
            <div key={l} className="text-center">
              <p className="text-[22px] font-black" style={{ color: col }}>
                {v}
              </p>
      ...
      {allTraining.filter(
        (t) => t.employee !== empName && !myTraining.find((m) => m.course === t.course),
      ).length > 0 && (
        <div>
          <p className="text-[11.5px] font-bold text-slate-400 uppercase tracking-wide mb-2">
                ...
                return (
                  <div key={t.id} className="bg-white rounded-xl border border-slate-200/80 p-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12.5px] font-bold text-[#111827] truncate">
                          {t.course}
                        </p>
...
export function EmployeePortal({ currentUser, company, employees, leaveRequests, canManage }) {
  ...
  const self = useMemo(
    () =>
      ...
      employees.find((e) =>
        e.name.toLowerCase().includes(currentUser.name.split(" ")[0].toLowerCase()),
      ),
  ...
  const myAttendance = useMemo(
    () =>
      attendance.rows.filter(
        (a) =>
          (self?.name || currentUser.name)
            .toLowerCase()
            .includes(a.employee?.toLowerCase() || "") ||
          a.employee
            ?.toLowerCase()
            .includes((self?.name || currentUser.name).split(" ")[0].toLowerCase()),
      ),
  ...
  async function enrollBiometric() {
    ...
    try {
      ...
      notify(
        `🔒 Biometric registered for ${name} on this device — clock-ins are now biometrically verified`,
      );
  ...
  async function verifyBiometric() {
    ...
    try {
      ...
      return assertion
        ? { method: "biometric", verified: true }
  ...
  async function clockIn() {
    ...
    const draft = {
      id: docId("ATT"),
      employee: self?.name || currentUser.name,
      date: TODAY_STR,
      status,
      clockIn: now,
      clockOut: null,
    ...
    attendance.setRows((p) => [
      draft,
      ...p.filter((a) => !(a.date === TODAY_STR && a.employee === draft.employee)),
    ]);
    setClockedIn(true);

    const sigBadge = sigResult.verified ? "🔒 Biometrically verified" : "📝 Manual entry";
    notify(
      `✓ Clocked in at ${now}${isLate ? " (Late)" : ""} · ${sigBadge}${location ? " · 📍 " + location.lat + "," + location.lng : ""}`,
    );
    logAudit(
      "Clock In",
      "Employee Portal",
      currentUser.name,
      `${draft.employee} at ${now} (${status}) · ${sigResult.method}`,
    );
  ...
  async function clockOut() {
    ...
    attendance.setRows((p) =>
      p.map((a) =>
        a.date === TODAY_STR && a.employee === (self?.name || currentUser.name)
          ? { ...a, clockOut: now, sigMethod: sigResult.method, verified: sigResult.verified }
          : a,
      ),
    );
    setClockedIn(false);
    ...
    logAudit(
      ...
      "Employee Portal",
      currentUser.name,
      `clocked out at ${now} · ${sigResult.method}`,
    );
    if (IS_CONFIGURED) {
      try {
        await sb("hr_attendance")
          ...
          .update({ clock_out: now, sig_method: sigResult.method, verified: sigResult.verified })
          .run();
      } catch (_e) {}
    }
    setBioVerifying(false);
  ...
  const myDuties = useMemo(
    () =>
      duties.rows
        .filter((d) => d.assignee === (self?.name || currentUser.name) || d.assignee === "ALL")
  ...
  async function startDuty(duty) {
    ...
    if (bioEnrolled && bioAvailable) {
      ...
      setBioVerifying(false);
      if (result === null) {
        notify("Biometric cancelled — duty not started.", "error");
        return;
      }
    }
    const now = new Date().toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
    duties.setRows((p) =>
      p.map((d) => (d.id === duty.id ? { ...d, status: "In Progress", startedAt: now } : d)),
    );
    notify(
      `▶ "${duty.title}" started at ${now}${bioEnrolled ? " · 🔒 Biometrically confirmed" : ""}`,
    );
  ...
  async function completeDuty(duty) {
    ...
    if (bioEnrolled && bioAvailable) {
      ...
      setBioVerifying(false);
      if (result === null) {
        notify("Biometric cancelled — duty not marked complete.", "error");
        return;
      }
      sigResult = result;
    }
      ...
      p.map((d) =>
        d.id === duty.id
          ? {
              ...
              status: "Completed",
              completedAt: now,
              completionVerified: sigResult.verified,
              completionMethod: sigResult.method,
            }
          : d,
      ),
    );
      ...
      try {
        await sb("hr_duties")
          ...
          .update({
            status: "Completed",
            completed_at: now,
            completion_verified: sigResult.verified,
            completion_method: sigResult.method,
          })
          .run();
      } catch (_e) {}
    }
  ...
  async function submitLeave() {
    if (!leaveForm.startDate || !leaveForm.endDate) {
      notify("Please fill in dates", "error");
      return;
    }
    const draft = {
      id: docId("LV"),
      employeeName: self?.name || currentUser.name,
      employeeId: self?.id || "",
      type: leaveForm.type,
  ...
  function printMyPayslip() {
    if (!self) {
      notify("Payslip requires your employee profile in the system", "error");
      return;
    }
    ...
    win.document
      .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Payslip ${period}</title>
  ...
  function joinViaCode() {
    ...
    try {
      ...
      const inv = codes.find(
        (c) => c.code === code && !c.used && new Date(c.expires) >= new Date(),
      );
      if (!inv) {
      ...
      const updated = codes.map((c) =>
        c.code === code ? { ...c, used: true, usedBy: selfName } : c,
      );
    ...
    return (
      <div className="min-h-[600px] flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xl p-8 w-full max-w-md">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-[11.5px] font-bold text-slate-600 block mb-1.5">
                Invite Code from HR *
              </label>
              <input
                className={
                  inputClass +
                  " text-center font-mono text-[18px] font-black tracking-widest uppercase"
                }
                value={inviteInput}
                onChange={(e) => {
              ...
              <p className="text-[10.5px] text-slate-400 mt-1 text-center">
                Get this code from your HR manager
              </p>
            </div>
            <div>
              <label className="text-[11.5px] font-bold text-slate-600 block mb-1.5">
            ...
            {inviteError && (
              <p className="text-[12px] text-[#EF4444] font-semibold text-center">{inviteError}</p>
            )}
            <button
              onClick={joinViaCode}
          ...
          <div className="mt-5 p-3 bg-slate-50 rounded-xl text-center">
            <p className="text-[11.5px] text-slate-500">
              Don't have a code? Ask your HR department to generate one from
            </p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg,#0D2214 0%,#1a3a2a 60%,#16A34A 120%)" }}
      >
        <div
        ...
        <div className="relative px-6 py-5 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <p className="text-[rgba(255,255,255,.6)] text-[12px] mb-0.5">
              Employee Portal · {co.name || "SMART MANAGER"}
            </p>
            <h1 className="text-white text-[22px] font-black">
              {greeting}, {empName.split(" ")[0]} 👋
            </h1>
            {self && (
              <p className="text-[rgba(255,255,255,.6)] text-[12px] mt-1">
                {self.role} · {self.department}
              </p>
            )}
          </div>
          <div className="flex items-center gap-3">
            ...
            {bioVerifying ? (
              ...
              <div className="flex flex-col items-center gap-1.5">
                ...
                {bioAvailable === false && (
                  <p className="text-[10.5px] text-[rgba(255,255,255,.4)]">
                    📝 Manual sign-in (no sensor found)
                  </p>
                )}
              </div>
            ) : clockedIn ? (
              ...
              <div className="text-center">
                ...
                <p className="text-[rgba(255,255,255,.5)] text-[11px]">
        ...
        <div className="space-y-4">
          ...
          {(() => {
            const weekPresent = weekAtt.filter(
              (w) => w.status === "Present" || w.status === "Late",
            ).length;
            ...
            const approvedLeave = myLeave.filter(
              (l) => l.status === "Approved" && l.endDate >= TODAY_STR,
            );
            const totalAnnual = 21; // TZS standard
            const usedLeave = leaveRows
              .filter(
                (l) =>
                  (l.employeeName || l.employee || "")
                    .toLowerCase()
                    .includes(empName.split(" ")[0].toLowerCase()) &&
                  l.status === "Approved" &&
                  l.startDate?.startsWith(String(TODAY.getFullYear())),
              )
              .reduce(
                (s, l) =>
                  s +
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  [
                    "Week Attendance",
                    attRate + "%",
                    attRate >= 80 ? "#16A34A" : attRate >= 60 ? "#F59E0B" : "#EF4444",
                  ],
            ...
            <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl p-3.5 flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[13px] font-bold text-[#1D4ED8]">
                  {myDuties.filter((d) => d.status === "Completed" && !d.approvedBy).length} dut
                  {myDuties.filter((d) => d.status === "Completed" && !d.approvedBy).length === 1
                    ? "y"
                    : "ies"}{" "}
                  awaiting manager approval
                </p>
                <p className="text-[11.5px] text-[#2563EB]">
                  Your manager will approve completed duties in HR → Timetable
                </p>
              </div>
            </div>
          )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 font-mono">
                          {duty.startTime}–{duty.endTime} · {duty.type}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        {canStart && (
                          <button
                            onClick={() => startDuty(duty)}
                            className="text-[11px] font-bold text-white bg-[#F59E0B] px-2.5 py-1.5 rounded-lg disabled:opacity-50"
                          >
                        ...
                        {duty.status === "Approved" && (
                          <span className="text-[#16A34A] font-bold text-[13px]">✓</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] font-bold text-[#16A34A]">
                {weekAtt.filter((w) => w.status === "Present" || w.status === "Late").length}/
                {weekAtt.filter((_, i) => weekDays[i] <= TODAY_STR).length} days present
              </span>
            </div>
            <div className="flex gap-2 justify-between">
              {weekAtt.map((wa, i) => {
                ...
                return (
                  <div
                    key={wa.date}
                    className={`flex-1 flex flex-col items-center gap-1 px-1 py-2 rounded-xl ${isToday ? "bg-[#F0FDF4] border border-[#16A34A]/20" : ""}`}
                  >
                    <div
                      className="w-10 h-10 rounded-full flex items-center justify-center"
                      style={{ background: col + "20", border: `2px solid ${col}` }}
                    >
                      <span className="text-[9px] font-black" style={{ color: col }}>
                        {wa.clockIn ? wa.clockIn : "—"}
                      </span>
                    </div>
                    <span
                      className={`text-[10.5px] font-bold ${isToday ? "text-[#16A34A]" : "text-slate-500"}`}
                    >
                      {dow}
                    </span>
      ...
      {activeTab === "attendance" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 text-center">
            ...
            <div
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[12px] font-bold mb-3 ${
                bioEnrolled
            ...
            >
              <Fingerprint size={13} />
              {bioEnrolled
                ? "🔒 Biometric Active — clock-in will be cryptographically signed"
                : bioAvailable === false
            ...
            {!clockedIn && !todayAtt?.clockIn ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 justify-center">
                  <p className="text-[13px] font-semibold text-[#16A34A]">
                    ✓ Clocked in at <strong>{todayAtt?.clockIn}</strong>
                  </p>
                  {todayAtt?.verified && (
                    <span className="text-[10.5px] font-bold text-[#16A34A] bg-[#F0FDF4] border border-[#BBF7D0] px-2 py-0.5 rounded-full flex items-center gap-1">
                      <Fingerprint size={10} />
                      Verified
                    </span>
                  )}
              ...
              <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded-xl p-4">
                <p className="text-[#16A34A] font-black text-[15px] flex items-center justify-center gap-2">
                  {todayAtt?.verified ? <Fingerprint size={16} /> : null}✓ Attendance recorded for
                  today
                </p>
                ...
                <p
                  ...
                  style={{ color: todayAtt?.verified ? "#16A34A" : "#94A3B8" }}
                >
                  {todayAtt?.verified
                    ? "🔒 Biometrically verified — HR has tamper-proof record"
                    : "📝 Manual entry — not biometrically signed"}
                </p>
              </div>
            )}
          </div>
              ...
              return (
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  <p className="text-[13.5px] font-bold text-[#111827] mb-3">
                    This Month —{" "}
                    {new Date(thisMonth + "-01").toLocaleDateString("en", {
                      month: "long",
                      year: "numeric",
                    })}
                  </p>
                    ...
                    <div className="mt-3">
                      <div className="flex justify-between text-[11px] text-slate-400 mb-1">
                        ...
                        <span
                          ...
                          style={{
                            color:
                              (present + late) / (present + late + absent) >= 0.9
                                ? "#16A34A"
                                : "#F59E0B",
                          }}
                        >
                  ...
                  {clockInLocation && (
                    <p className="text-[11px] text-slate-400 mt-2 flex items-center gap-1">
                      📍 Last clock-in: {clockInLocation.lat}, {clockInLocation.lng} (±
                      {clockInLocation.acc}m)
                    </p>
                  )}
                </div>
              );
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-bold text-[#111827]">
                My Attendance History ({myAttendance.length} records)
              </p>
            </div>
            {myAttendance.length === 0 ? (
              <div className="py-10 text-center text-slate-400">
                ...
                <tbody>
                  {myAttendance.slice(0, 20).map((att, i) => {
                    ...
                    return (
                      <tr
                        key={att.id}
                        className={`border-b border-slate-50 last:border-0 ${att.date === TODAY_STR ? "bg-[#F0FDF4]/50" : ""}`}
                      >
                        <td className="px-4 py-3 font-mono text-[11.5px]">{att.date}</td>
                        <td className="px-4 py-3 text-slate-500">
                          {new Date(att.date).toLocaleDateString("en", { weekday: "short" })}
                        </td>
          ...
          {todayDuties.length === 0 && upcoming.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200/80 p-10 text-center">
              ...
              <p className="text-[12.5px] text-slate-400 mt-1">
                Your manager will assign duties here. Check back later.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {[
                ["Today", todayDuties],
                ["Upcoming", upcoming],
                ["Completed", doneDuties.slice(0, 5)],
              ].map(
                ([section, dts]) =>
                  dts.length > 0 && (
                    <div
                        ...
                        {dts.map((duty) => {
                          ...
                          return (
                            <div key={duty.id} className="px-4 py-3.5 flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p
                                  className={`text-[13px] font-bold text-[#111827] ${duty.status === "Approved" ? "line-through opacity-60" : ""}`}
                                >
                                  {duty.title}
                                </p>
                                ...
                                {duty.status === "Approved" && duty.approvedBy && (
                                  <p className="text-[10.5px] text-[#16A34A] mt-0.5">
                                    ✓ Approved by {duty.approvedBy.split("(")[0].trim()}
                                  </p>
                                )}
                                {duty.notes && (
                              ...
                              <div className="flex gap-1.5 shrink-0 mt-0.5">
                                {canStart && (
                                  <button
                                    onClick={() => startDuty(duty)}
                                    className="text-[11.5px] font-bold text-white bg-[#F59E0B] px-3 py-1.5 rounded-lg disabled:opacity-50"
                                  >
                                    {bioVerifying ? "⏳…" : bioEnrolled ? "🔒 Start" : "▶ Start"}
                                  </button>
                                )}
                                {canComplete && (
                                  <button
                                    onClick={() => completeDuty(duty)}
                                    className="text-[11.5px] font-bold text-white bg-[#2563EB] px-3 py-1.5 rounded-lg disabled:opacity-50"
                                  >
                                    {bioVerifying
                                      ? "⏳…"
                                      : bioEnrolled
      ...
      {activeTab === "leave" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            ...
            <button
              onClick={submitLeave}
              className="mt-4 flex items-center gap-2 text-[13px] font-bold text-white px-5 py-2.5 rounded-xl bg-[#16A34A]"
            >
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-bold text-[#111827]">
                My Leave History ({myLeave.length})
              </p>
            </div>
            {myLeave.length === 0 ? (
              <div className="py-10 text-center text-slate-400">
              ...
              <div className="divide-y divide-slate-50">
                {myLeave.map((lv) => {
                  const sc =
                    { Pending: "#F59E0B", Approved: "#16A34A", Rejected: "#EF4444" }[lv.status] ||
                    "#94A3B8";
                  const days =
                    lv.startDate && lv.endDate
                      ? Math.ceil((new Date(lv.endDate) - new Date(lv.startDate)) / 86400000) + 1
                      : 0;
                  return (
                    <div key={lv.id} className="px-4 py-3.5 flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[11.5px] text-slate-500 mt-0.5">
                          {lv.startDate} → {lv.endDate} · {days} day{days !== 1 ? "s" : ""}
                        </p>
                        {lv.reason && (
                          <p className="text-[11px] text-slate-400 mt-0.5">{lv.reason}</p>
                        )}
                      </div>
                    </div>
                  );
            ...
            {self ? (
              <>
                        ...
                        money(
                          Math.round(
                            self.salary <= 270
                              ? 0
                              : self.salary <= 520
                                ? (self.salary - 270) * 0.08
                                : self.salary <= 760
                                  ? 20 + (self.salary - 520) * 0.2
                                  : 68 + (self.salary - 760) * 0.25,
                          ),
                        ) +
                        "k",
                      "#EF4444",
                ...
                <p className="text-[12px] text-slate-500 mb-4">
                  {self.role} · {self.department} · Pay Period: {TODAY_STR.slice(0, 7)}
                </p>
                <button
                  onClick={printMyPayslip}
              ...
              <div className="py-4">
                ...
                <p className="text-slate-400 text-[12px] mt-1">
                  Ask HR to add you to the employee list.
                </p>
              </div>
            )}
          </div>
      ...
      {activeTab === "profile" && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div
              className="px-5 py-5 flex items-center gap-4 border-b border-slate-100"
              style={{ background: "linear-gradient(135deg,#0D2214,#1a3a2a)" }}
            >
              ...
              <div>
                ...
                <p className="text-[rgba(255,255,255,.6)] text-[12.5px] mt-0.5">
                  {self?.role || "Employee"} · {self?.department || "—"}
                </p>
              </div>
            </div>
            <div className="p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={l} className="border-b border-slate-100 pb-3">
                      <p className="text-[10.5px] font-bold text-slate-400 uppercase tracking-wide mb-0.5">
                        {l}
                      </p>
          ...
          <div
            className={`rounded-xl border p-4 ${bioEnrolled ? "bg-[#F0FDF4] border-[#BBF7D0]" : "bg-[#F8FAFB] border-slate-200"}`}
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-[20px] ${bioEnrolled ? "bg-[#16A34A]/10" : "bg-slate-100"}`}
                >
                  {bioEnrolled ? "🔒" : "👆"}
                </div>
                <div>
                  <p
                    className={`text-[13px] font-bold ${bioEnrolled ? "text-[#15803D]" : "text-[#111827]"}`}
                  >
                  ...
                  onClick={
                    bioEnrolled
                      ? () => {
                          ...
                          notify("Biometrics removed from this device");
                        }
                      : enrollBiometric
                  }
                  disabled={bioVerifying}
          ...
          {bioSetupOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="relative bg-white rounded-2xl shadow-2xl p-7 w-full max-w-sm mx-4 text-center">
                ...
                <h3 className="text-[18px] font-black text-[#111827] mb-2">
                  Set Up Biometric Sign-In
                </h3>
                <p className="text-[12.5px] text-slate-500 mb-4 leading-relaxed">
                  Register your <strong>fingerprint or Face ID</strong> on this device. Every
                  clock-in, clock-out, and duty completion will then be biometrically signed —
                  giving HR tamper-proof verified records.
                </p>
