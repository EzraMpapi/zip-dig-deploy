...
export function Finance({
  ...
  async function markInvoicePaid(id) {
    ...
    setAllInvoices((prev) =>
      prev.map((i) =>
        i.id === id ? { ...i, status: "Paid", amountPaid: lineTotal(i.items).total } : i,
      ),
    );
  ...
  async function addExpense(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("finance_expenses")
          .insert({
            vendor: form.vendor,
        ...
        if (header?.id) {
          setExpenses((prev) =>
            prev.map((e) => (e.id === draft.id ? { ...e, dbId: header.id } : e)),
          );
        }
      } catch (e) {
  ...
  async function setExpenseStatus(id, status) {
    ...
    if (IS_CONFIGURED && (exp?.dbId || exp?.id)) {
      try {
        await sb("finance_expenses")
          .eq("id", exp.dbId ?? exp.id)
          .update({ status })
          .run();
      } catch (e) {
  ...
  const FIN_KPIS = [
    {
      ...
      value: `TZS ${money(revenueCollected)}k`,
      delta: "MTD",
      up: true,
    ...
    },
    {
      ...
      value: `TZS ${money(Math.round(receivablesTotal))}k`,
    ...
    },
    {
      ...
      value: `TZS ${money(expensesTotal)}k`,
      delta: "MTD",
      up: false,
      icon: Wallet,
    },
    {
      ...
      value: `TZS ${money(netCash)}k`,
      delta: netCash >= 0 ? "Positive" : "Negative",
      up: netCash >= 0,
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Revenue, receivables, and operating expenses at a glance
        </p>
      </div>
      ...
      {tab === "receivables" && (
        <Receivables
          ...
          onRecordPayment={(id, payment) =>
            recordPayment(invoices, id, payment, `${currentUser.name} (${currentUser.role})`)
          }
      ...
      {tab === "ledger" && (
        <GeneralLedger
          ...
          expenses={expenses}
          posTransactions={posTransactionsHook.rows}
        />
      )}
...
export function FinanceOverview({ expenses }) {
  const catTotals = useMemo(() => {
    ...
    return Object.entries(map)
      .sort((a, b) => b[1] - a[1])
      .map(([category, amount]) => ({ category, amount, pct: (amount / max) * 100 }));
  }, [expenses]);
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3 text-[11px] text-slate-500">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#16A34A]" /> Inflow
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#F59E0B]" /> Outflow
            </span>
          </div>
        </div>
...
export function InlinePayForm({ onSubmit, max }) {
  ...
  return (
    <div className="flex flex-wrap gap-2 items-end">
      <div>
        <label className="text-[11.5px] text-slate-500 block mb-1">Amount (TZS k)</label>
        <input
          type="number"
          min="0"
          max={max}
      ...
      <div>
        <label className="text-[11.5px] text-slate-500 block mb-1">Method</label>
        <select className={inputClass} value={method} onChange={(e) => setMethod(e.target.value)}>
          {["Cash", "Mobile Money", "Bank Transfer", "Cheque", "Card"].map((m) => (
      ...
      <button
        onClick={() => {
          const a = Number(amt);
          if (a > 0) onSubmit({ amount: a, method, date: TODAY.toISOString().slice(0, 10) });
        }}
        disabled={!Number(amt) || Number(amt) <= 0}
        className="btn-primary text-white text-[12px] font-medium rounded-lg px-4 py-2.5 disabled:opacity-40"
      >
        Record payment
      </button>
    </div>
...
export function Receivables({ outstanding, onMarkPaid, onDelete, onRecordPayment, company }) {
  ...
  const bucketTotals = useMemo(() => {
    ...
    aged.forEach((inv) => {
      t[inv.bucket].count += 1;
      t[inv.bucket].total += inv.balance;
    });
    return t;
  }, [aged]);
  ...
  const criticalTotal =
    (bucketTotals["61–90 days"].total || 0) + (bucketTotals["90+ days"].total || 0);
  ...
  const byCustomer = useMemo(() => {
    ...
    filtered.forEach((inv) => {
      if (!m[inv.customer])
        m[inv.customer] = { customer: inv.customer, total: 0, count: 0, oldest: 0 };
  ...
  function exportAging() {
    exportExcel(
      `receivables-aging-${TODAY.toISOString().slice(0, 10)}.xlsx`,
      "Aging",
      ["Invoice", "Customer", "Due Date", "Days", "Bucket", "Balance (TZS 000)"],
      filtered.map((inv) => [
        inv.id,
        inv.customer,
        inv.dueDate || "—",
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          <div>
            ...
            <p className="text-[12px] text-slate-500">
              {aged.length} outstanding invoice(s) · TZS {money(Math.round(grandTotal))}k total due
              {criticalTotal > 0 && (
                <span className="text-[#EF4444] font-medium">
                  {" "}
                  · TZS {money(Math.round(criticalTotal))}k 61+ days overdue
                </span>
              )}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
              {["aging", "customer"].map((v) => (
                <button
                  key={v}
                  onClick={() => setView(v)}
                  className={`px-2.5 py-1.5 rounded-md text-[11.5px] font-medium transition-colors ${view === v ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
                >
            ...
            <button
              onClick={exportAging}
              className="text-[11.5px] font-medium text-[#16A34A] border border-[#16A34A]/30 rounded-lg px-3 py-1.5 hover:bg-[#16A34A]/5"
            >
              Export
            </button>
          </div>
          ...
          <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
            {BUCKETS.filter((b) => bucketTotals[b].total > 0).map((b) => (
              <div
                ...
                title={`${b}: TZS ${money(Math.round(bucketTotals[b].total))}k`}
                style={{
                  width: `${(bucketTotals[b].total / grandTotal) * 100}%`,
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {BUCKETS.filter((b) => bucketTotals[b].count > 0).map((b) => (
            <div
              ...
              style={{ backgroundColor: `${BUCKET_COLORS[b]}18`, color: BUCKET_COLORS[b] }}
            >
              <span className="font-semibold">{b}</span>
              <span className="opacity-70">
                {bucketTotals[b].count} inv · TZS {money(Math.round(bucketTotals[b].total))}k
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="relative">
        ...
        <input
          className="w-full border border-slate-200 rounded-xl pl-8 pr-4 py-2.5 text-[13px] outline-none focus:border-[#16A34A]"
          value={search}
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <span
                      ...
                      style={{
                        color: r.oldest > 60 ? "#EF4444" : r.oldest > 30 ? "#F59E0B" : "#16A34A",
                      }}
                    >
                      {r.oldest}d
                    </span>
              ...
              {byCustomer.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-[12px] text-slate-400">
                    No outstanding invoices matching this search.
                  </td>
                </tr>
              )}
            </tbody>
              ...
              {filtered
                ...
                .map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-[#111827] max-w-[140px] truncate">
                      {inv.customer}
                    </td>
                    <td className="px-4 py-2.5 text-slate-500 font-mono text-[11.5px]">
                      {inv.dueDate || "—"}
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      <span
                        className="font-mono font-medium text-[11.5px]"
                        style={{ color: BUCKET_COLORS[inv.bucket] }}
                      >
                        {inv.days > 0 ? `${inv.days}d` : "—"}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      <span
                        ...
                        style={{
                          backgroundColor: `${BUCKET_COLORS[inv.bucket]}18`,
                          color: BUCKET_COLORS[inv.bucket],
                        }}
                      >
                        {inv.bucket}
                      </span>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex gap-1.5 justify-end">
                        <button
                          onClick={() => {
                            ...
                            notify(
                              "Reminder for " +
                                inv.customer +
                                " copied to clipboard — paste into email or SMS",
                            );
                          }}
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (inv.customerEmail)
                              window.location.href = `mailto:${inv.customerEmail}?subject=${subj}&body=${body}`;
                            else {
                              emailBus.push({
                                subject: decodeURIComponent(subj),
                                body: decodeURIComponent(body),
                              });
                              notify("Open Collaboration → Email to send reminder");
                            }
                          }}
                        ...
                        <button
                          ...
                          className="text-[11px] font-medium text-white bg-[#16A34A] rounded-lg px-2 py-1 hover:bg-[#15803D]"
                        >
                          Pay
                        </button>
                      </div>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-[12px] text-slate-400">
                    No outstanding invoices{search ? " matching this search" : ""}.
                  </td>
                </tr>
              )}
            </tbody>
            {filtered.length > 0 && (
              <tfoot>
                <tr className="border-t border-slate-200 bg-slate-50">
                  <td colSpan={5} className="px-4 py-2.5 text-[12px] font-semibold text-[#111827]">
                    Total outstanding
                  </td>
                  <td className="px-4 py-2.5 text-right font-mono font-bold text-[#111827]">
                    {money(Math.round(filtered.reduce((s, i) => s + i.balance, 0)))}
                  </td>
                  <td />
                </tr>
              </tfoot>
        ...
        <div className="bg-white rounded-xl border border-[#16A34A]/30 shadow-md p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] font-semibold text-[#111827]">
              Record payment — {selected.id} ({selected.customer})
            </p>
            <button
          ...
          <p className="text-[12px] text-slate-500">
            Balance: TZS {money(Math.round(selected.balance))}k
          </p>
          <InlinePayForm
            onSubmit={(payment) => {
              onRecordPayment(selected.id, payment);
              setSelected(null);
            }}
...
export function Expenses({ expenses, onAdd, onSetStatus, onDelete, loading }) {
  ...
  const buckets = useMemo(() => {
    ...
    unpaid.forEach((e) => {
      b[agingBucket(e.dueDate)] += e.amount;
    });
    return b;
  }, [unpaid]);
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="text-[13px] text-slate-500">
          <span className="font-mono font-semibold text-[#111827]">
            TZS {money(Math.round(totalPayable))}k
          </span>{" "}
          owed to {unpaid.length} vendor{unpaid.length === 1 ? "" : "s"}
        </p>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {Object.entries(buckets).map(([bucket, amount]) => (
          ...
          >
            ...
            <p className="text-[16px] font-mono font-semibold text-[#111827]">
              {money(Math.round(amount))}k
            </p>
          </div>
        ))}
      </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {bucket ? (
                        <span
                          ...
                          style={{
                            backgroundColor: `${AGING_COLOR[bucket]}14`,
                            color: AGING_COLOR[bucket],
                          }}
                        >
                          <span
                            className="w-1.5 h-1.5 rounded-full"
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                        style={{
                          backgroundColor: `${EXPENSE_STATUS_COLOR[e.status]}14`,
                          color: EXPENSE_STATUS_COLOR[e.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: EXPENSE_STATUS_COLOR[e.status] }}
                        />
                        {e.status}
                      </span>
                    </td>
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${EXPENSE_STATUS_COLOR[expense.status]}14`,
              color: EXPENSE_STATUS_COLOR[expense.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: EXPENSE_STATUS_COLOR[expense.status] }}
            />
            {expense.status}
          </span>
        </div>
...
export function ExpenseFormPanel({ onClose, onSubmit }) {
  const [form, setForm] = useState({
    vendor: "",
    category: EXPENSE_CATEGORIES_LIST[0],
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Vendor" required>
            ...
            />
            {touched && !form.vendor.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Vendor is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Amount (TZS 000)" required>
              <input
                ...
                placeholder="0"
              />
              {touched && !(Number(form.amount) > 0) && (
                <p className="text-[11px] text-[#EF4444] mt-1">Enter an amount.</p>
              )}
            </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Payment method">
              ...
              >
                {["Bank Transfer", "Mobile Money", "Cash", "Card"].map((m) => (
          ...
          {form.status !== "Paid" && (
            <FormField label="Payment due date">
              ...
              <p className="text-[11px] text-slate-400 mt-1">
                Drives the Payables aging view — leave blank to default to the expense date.
              </p>
            </FormField>
          )}
        </div>
...
export function buildLedger(invoices, expenses, posTransactions) {
  ...
  invoices.forEach((inv) => {
    if (inv.payments && inv.payments.length > 0) {
      inv.payments.forEach((p) => {
        entries.push({
          date: p.date,
          description: `Payment received — ${inv.id} (${inv.customer})`,
          method: p.method,
          credit: p.amount,
          debit: 0,
        });
      ...
      entries.push({
        date: inv.date,
        description: `${inv.id} (${inv.customer}) — paid in full, no itemized payment on record`,
        method: "—",
        credit: total,
        debit: 0,
      });
  ...
  (posTransactions || []).forEach((t) => {
    ...
    if (net !== 0)
      entries.push({
        date: t.date,
        description: `POS sale — ${t.id}${refunded ? " (net of TZS " + money(refunded) + "k returned)" : ""}`,
        method: t.method,
        credit: net,
        debit: 0,
      });
...
export function GeneralLedger({ invoices, expenses, posTransactions }) {
  const ledger = useMemo(
    () => buildLedger(invoices, expenses, posTransactions),
    [invoices, expenses, posTransactions],
  );
  const totals = useMemo(
    () =>
      ledger.reduce((t, e) => ({ credit: t.credit + e.credit, debit: t.debit + e.debit }), {
        credit: 0,
        debit: 0,
      }),
    [ledger],
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Cash-basis ledger: recorded invoice payments, POS sales (net of returns), and paid
          expenses appear here, chronologically, with a running balance.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <p className="text-[16px] font-mono font-semibold text-[#16A34A]">
            +{money(Math.round(totals.credit))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11.5px] text-slate-400 mb-1">Total Debits</p>
          <p className="text-[16px] font-mono font-semibold text-[#EF4444]">
            −{money(Math.round(totals.debit))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11.5px] text-slate-400 mb-1">Ending Balance</p>
          <p className="text-[16px] font-mono font-semibold text-[#111827]">
            {money(Math.round(totals.credit - totals.debit))}k
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <AreaChart
              data={entries
                .slice(-30)
                .map((e) => ({ date: e.date.slice(5), balance: Math.round(e.balance / 1000) }))}
            ...
            <tbody>
              {ledger.map((e, i) => (
                <tr key={i} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-3 text-right font-mono font-medium text-[#111827]">
                    {money(Math.round(e.balance))}
                  </td>
                </tr>
              ))}
              {ledger.length === 0 && (
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      hint="Record a payment or a paid expense and it will appear here."
                    />
                  </td>
                </tr>
              )}
...
export function ChartOfAccountsView({ invoices, expenses, posTransactions, company }) {
  ...
  const rowsWithEquity = rows.map((r) =>
    r.code === "3000"
      ? { ...r, credit: Math.max(0, equityBalance), debit: Math.max(0, -equityBalance) }
      : r,
  );
  ...
  const summaryByType = ["Asset", "Liability", "Equity", "Revenue", "Expense"].map((type) => ({
    type,
    debit: rowsWithEquity.filter((r) => r.type === type).reduce((s, r) => s + r.debit, 0),
    credit: rowsWithEquity.filter((r) => r.type === type).reduce((s, r) => s + r.credit, 0),
  }));
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          A real, standard chart of accounts mapped to this system&apos;s own actual categories —
          every balance below aggregates real transactions, not a fabricated example. Honest scope:
          this reports real category-level totals as accounts, it doesn&apos;t mean every
          transaction was individually posted as a double-entry journal line the moment it was
          recorded — that&apos;s a larger, separate project.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit">
          <button
            ...
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-colors ${!detailed ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
          >
            Summary
          </button>
          <button
        ...
        {company?.businessScale && (
          <p className="text-[11px] text-slate-400">
            Defaulted to {detailed ? "full detail" : "summary"} for a {company.businessScale}{" "}
            business — switch anytime.
          </p>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="px-4 sm:px-5 py-3.5 border-b border-slate-100">
          ...
          <p className="text-[11.5px] text-slate-400">
            As of {TODAY.toISOString().slice(0, 10)} · TZS thousands
          </p>
        </div>
        <div className="overflow-x-auto">
              ...
              {(detailed ? rowsWithEquity : summaryByType).map((r, i) => (
                <tr key={i} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-right font-mono text-slate-600">
                    {r.debit ? money(Math.round(r.debit)) : "—"}
                  </td>
                  <td className="px-4 py-2.5 text-right font-mono text-slate-600">
                    {r.credit ? money(Math.round(r.credit)) : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="px-4 sm:px-5 py-3 border-t border-slate-100">
          <div
            className={`flex items-center gap-1.5 text-[11.5px] font-medium ${Math.abs(totalDebit - totalCredit) < 1 ? "text-[#16A34A]" : "text-[#EF4444]"}`}
          >
            ...
            {Math.abs(totalDebit - totalCredit) < 1
              ...
              : "Does not balance — check underlying data"}
          </div>
          <p className="text-[10.5px] text-slate-400 mt-1.5">
            Owner&apos;s Equity (3000) is computed as the exact residual needed to balance — the
            same honest reasoning as the Balance Sheet&apos;s equity line (Reports). This system has
            no separate paid-in-capital or retained-earnings ledger to draw an independent figure
            from.
          </p>
        </div>
      </div>
...
export function BudgetsView({ expenses }) {
  const budgets = useCompanyTable("expense_budgets", [], {
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      category: r.category,
    ...
    if (!budgetAlertFired.current && overCount > 0) {
      ...
      notify(
        overCount +
          " budget" +
          (overCount > 1 ? "s" : "") +
          " exceeded this month — review the Budgets tab.",
        "error",
      );
    }
  }, [overCount]);
  ...
  async function saveBudget(category) {
    ...
    if (existing) {
      budgets.setRows((prev) =>
        prev.map((b) => (b.category === category ? { ...b, monthlyLimit: limit } : b)),
      );
    } else {
      budgets.setRows((prev) => [
        ...prev,
        { id: `BUD-${category}`, category, monthlyLimit: limit },
      ]);
    }
    setEditing(null);
    setDraftLimit("");
    ...
    if (IS_CONFIGURED) {
      try {
        if (existing?.dbId) {
          await sb("expense_budgets")
            .eq("id", existing.dbId)
            .update({ monthly_limit: limit })
            .run();
        } else {
          const header = await sb("expense_budgets")
            .insert({ category, monthly_limit: limit })
            .single()
            .run();
          if (header?.id)
            budgets.setRows((prev) =>
              prev.map((b) => (b.category === category ? { ...b, dbId: header.id } : b)),
            );
        }
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          [
            "Spent This Month",
            "TZS " + money(Math.round(totalActual)) + "k",
            totalActual > totalBudget && totalBudget > 0 ? "#EF4444" : "#16A34A",
          ],
          [
            "Over-Budget",
            overBudgetCats + " categories",
            overBudgetCats > 0 ? "#EF4444" : "#16A34A",
          ],
        ].map(([l, v, col]) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={chartData} margin={{ left: -10, right: 4, top: 0, bottom: 40 }}>
              ...
              <Tooltip
                formatter={(v, n) => [
                  "TZS " + money(v) + "k",
                  n === "actual" ? "Actual Spend" : "Budget Limit",
                ]}
              />
              <Bar dataKey="actual" name="actual" radius={[4, 4, 0, 0]}>
                {chartData.map((d, i) => (
                  <Cell key={i} fill={d.over ? "#EF4444" : "#16A34A"} />
                ))}
              </Bar>
              <Line
                type="monotone"
          ...
          <div className="flex gap-4 mt-2 text-[11.5px]">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-500">Under Budget</span>
            </div>
            <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-0.5 bg-[#2563EB] border-dashed border-t-2 border-[#2563EB]" />
      ...
      {nearCount > 0 && overCount === 0 && (
        <div className="flex items-center gap-3 p-3.5 rounded-xl border border-[#FDE68A] bg-[#FFFBEB]">
          ...
          <p className="text-[12.5px] font-medium text-[#92400E]">
            <strong>
              {nearCount} categor{nearCount > 1 ? "ies" : "y"}
            </strong>{" "}
            at 80%+ of monthly budget.
          </p>
        </div>
      )}
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Real budget vs real actual — spend is computed live from this month&apos;s recorded
          expenses, never a separately-typed number that could drift from the books.
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11px] text-slate-400 mb-1">Total Budgeted</p>
          <p className="text-[16px] font-mono font-bold text-[#111827]">
            TZS {money(Math.round(totalBudget))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11px] text-slate-400 mb-1">Spent This Month</p>
          <p className="text-[16px] font-mono font-bold text-[#111827]">
            TZS {money(Math.round(totalActual))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <p
            className={`text-[16px] font-mono font-bold ${overCount > 0 ? "text-[#EF4444]" : "text-[#16A34A]"}`}
          >
                  ...
                  {editing === l.category ? (
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        autoFocus
                        onKeyDown={(e) => e.key === "Enter" && saveBudget(l.category)}
                        className="w-24 text-right text-[12px] bg-slate-50 border border-slate-200 rounded-md px-2 py-1"
                        placeholder="Limit"
                      />
                      <button
                        onClick={() => saveBudget(l.category)}
                        className="text-[11.5px] font-medium btn-primary text-white rounded-md px-2.5 py-1"
                      >
                        Save
                      </button>
                      <button
                        ...
                        className="text-[11.5px] text-slate-400 px-1"
                    ...
                    <button
                      onClick={() => {
                        setEditing(l.category);
                        setDraftLimit(l.budget ? String(l.budget.monthlyLimit) : "");
                      }}
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${l.pct === null ? 0 : Math.min(100, l.pct)}%`,
                        backgroundColor: barColor,
                      }}
                    />
                  </div>
                  <span className="text-[11.5px] font-mono text-slate-500 shrink-0 w-32 text-right">
                    {money(Math.round(l.actual))}k{" "}
                    {l.budget ? `/ ${money(l.budget.monthlyLimit)}k` : "· no budget"}
                  </span>
                </div>
                {l.pct !== null && l.pct > 100 && (
                  <p className="text-[10.5px] text-[#EF4444] mt-1">
                    Over budget by TZS {money(Math.round(l.actual - l.budget.monthlyLimit))}k this
                    month.
                  </p>
                )}
              </div>
...
export function FinancialRatiosView({ invoices, expenses, posTransactions, inventory }) {
  const loansHook = useCompanyTable("business_loans", [], {
    mapRow: (r) => ({
      id: r.id,
      principal: Number(r.principal) || 0,
      repayments: (r.loan_repayments || []).map((rp) => ({ amount: Number(rp.amount) || 0 })),
    }),
  ...
  const f = useMemo(() => {
    ...
    const ap = expenses.filter((e) => e.status !== "Paid").reduce((s, e) => s + e.amount, 0);
    const loans = loansHook.rows.reduce(
      (s, l) => s + Math.max(0, l.principal - l.repayments.reduce((rs, r) => rs + r.amount, 0)),
      0,
    );
    const liab = ap + loans;
    ...
    const revenue =
      ...
      (posTransactions || [])
        ...
        .reduce((s, t) => s + t.items.reduce((ts, it) => ts + it.qty * it.price, 0), 0);
    ...
    const dayOfYear = Math.max(
      1,
      Math.floor((TODAY - new Date(`${TODAY.getFullYear()}-01-01`)) / 86400000) + 1,
    );
  ...
  const ratios = [
    {
      label: "Current Ratio",
      short: "Liquidity",
      value: f.liab > 0 ? ((f.cash + f.ar + f.inv) / f.liab).toFixed(2) : "∞",
      formula: "(Cash + AR + Inv) ÷ Liabilities",
      target: 1.5,
      scale: 3,
      good: "↑",
    },
    {
      label: "Quick Ratio",
      short: "Quick",
      value: f.liab > 0 ? ((f.cash + f.ar) / f.liab).toFixed(2) : "∞",
      formula: "(Cash + AR) ÷ Liabilities",
      target: 1.0,
      scale: 2,
      good: "↑",
    },
    {
      ...
      short: "Margin",
      value: f.revenue > 0 ? ((f.profit / f.revenue) * 100).toFixed(1) + "%" : "—",
    ...
    },
    {
      ...
      short: "Leverage",
      value: f.equity > 0 ? (f.liab / f.equity).toFixed(2) : "—",
    ...
    },
    {
      label: "DSO (days)",
      short: "DSO",
      value: f.revenue > 0 ? Math.round(f.ar / (f.revenue / f.dayOfYear)) : "—",
      formula: "AR ÷ (Revenue ÷ Days Elapsed)",
      target: 30,
      scale: 90,
      good: "↓",
    },
    {
      label: "Cash Runway",
      short: "Runway",
      value: f.avgMonthlyExp > 0 ? (f.cash / f.avgMonthlyExp).toFixed(1) + " mo" : "—",
      formula: "Cash ÷ Avg Monthly Expenses",
      target: 3,
      scale: 12,
      good: "↑",
    },
  ...
  const radarData = ratios.map((r) => {
    ...
    let score;
    if (r.short === "Margin")
      score = Math.min(100, raw * 2); // 50% margin = 100 score
    else if (r.short === "DSO")
      score = Math.max(0, 100 - raw); // lower DSO = better
    else if (r.short === "Leverage")
      score = Math.max(0, 100 - raw * 25); // lower D/E = better
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Six core ratios computed from live data — RadarChart shows overall financial health at a
          glance.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-right">
              <p className="text-[22px] font-black" style={{ color: healthCol }}>
                {overallHealth}
              </p>
              <p className="text-[10.5px] font-semibold" style={{ color: healthCol }}>
                {overallHealth >= 70 ? "Healthy" : overallHealth >= 50 ? "Moderate" : "Needs Work"}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {ratios.map((r) => {
            const raw = parseFloat(r.value) || 0;
            const isGood =
              r.good === "↑" ? raw >= parseFloat(r.target) : raw <= parseFloat(r.target);
            ...
            return (
              <div
                ...
                className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4"
              >
                <p className="text-[10.5px] text-slate-400 uppercase tracking-wide leading-tight">
                  {r.label}
                </p>
                <p className="text-[22px] font-mono font-bold my-1" style={{ color: col }}>
                  {r.value}
                </p>
      ...
      <p className="text-[10.5px] text-slate-400">
        Inventory turnover and gross margin absent intentionally — both require cost-of-goods-sold
        matched to units sold, which this schema doesn&apos;t track per-unit.
      </p>
    </div>
  );
}
...
export function DocumentScannerView({ expensesHook }) {
  ...
  async function saveAsExpense() {
    ...
    const category = EXPENSE_CATEGORIES_LIST.includes(result.category)
      ? result.category
      : "Supplies";
    const method = ["Cash", "Mobile Money", "Bank Transfer", "Card"].includes(result.paymentMethod)
      ? result.paymentMethod
      : "Cash";
    const draft = {
      id: `EXP-SCAN-${Date.now()}`,
      vendor: result.issuer || "Scanned receipt",
      category,
      date: result.date || TODAY.toISOString().slice(0, 10),
      dueDate: result.date || TODAY.toISOString().slice(0, 10),
      amount,
      status: "Paid",
      method,
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("finance_expenses")
          ...
          .run();
        if (header?.id)
          expensesHook.setRows((prev) =>
            prev.map((x) => (x.id === draft.id ? { ...x, dbId: header.id } : x)),
          );
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      }
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Photograph a receipt, invoice, tax document, ID, business license, or TIN certificate —
          real AI vision classifies it and extracts the data. The photo is processed, not stored;
          QR/barcode decoding needs a dedicated decoder and is named as future work, not faked.
        </p>
      </div>
      <input
        ref={fileRef}
      ...
      <label
        ...
        className="btn-primary text-white text-[13px] font-medium px-4 py-2.5 rounded-lg inline-flex items-center gap-2 cursor-pointer"
      >
        <ScanText size={15} /> {busy ? "Reading document..." : "Scan with camera / upload"}
      </label>
      {busy && (
        <p className="text-[12px] text-slate-400">
          Real AI vision is reading the document — a few seconds.
        </p>
      )}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10.5px] text-slate-400">
              Fields not visible come back empty — never invented.
            </span>
          </div>
          {[
            ...
            [
              "Amount",
              result.totalAmount != null ? `${result.currency || ""} ${result.totalAmount}` : null,
            ],
            [
              "VAT (itemized)",
              result.vatAmount != null ? `${result.currency || ""} ${result.vatAmount}` : null,
            ],
            ["Category (AI suggestion)", result.category],
          ...
          {result.summary && (
            <p className="text-[11.5px] text-slate-400 pt-1 border-t border-slate-50">
              {result.summary}
            </p>
          )}
          {(result.docType === "Receipt" || result.docType === "Invoice") && !saved && (
            <button
              ...
              className="btn-primary text-white text-[12px] font-medium rounded-lg px-3.5 py-2 mt-1"
            >
              Save as Expense
            </button>
          )}
          {saved && (
            <p className="text-[11.5px] font-medium text-[#16A34A]">
              Saved to Payables with the extracted category and payment method — both editable there
              if the AI read them wrong.
            </p>
          )}
        </div>
      )}
...
export function LoansView() {
  const loans = useCompanyTable("business_loans", [], {
    order: { col: "borrowed_date", ascending: false },
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      lender: r.lender,
      ...
      status: r.status,
      notes: r.notes || "",
      repayments: (r.loan_repayments || []).map((rp) => ({
        id: rp.id,
  ...
  async function addLoan(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("business_loans")
          .insert({
            lender: draft.lender,
        ...
        if (header?.id)
          loans.setRows((prev) =>
            prev.map((l) => (l.id === draft.id ? { ...l, dbId: header.id } : l)),
          );
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      }
  ...
  async function recordRepayment(loan) {
    ...
    if (amt > balance + 1) {
      notify(
        `Overpayment blocked — balance is TZS ${money(Math.round(balance))}k. Enter at most ${money(Math.round(balance))}k.`,
        "error",
      );
      return;
    }
    ...
    loans.setRows((prev) =>
      prev.map((l) =>
        l.id === loan.id ? { ...l, repayments: [...l.repayments, draft], status: newStatus } : l,
      ),
    );
    ...
    const msg =
      newStatus === "Paid"
        ? `Loan fully repaid — TZS ${money(Math.round(newTotal))}k settled.`
        : `Repayment of TZS ${money(amt)}k recorded. Remaining balance: TZS ${money(Math.round(balance - amt))}k.`;
    notify(msg, newStatus === "Paid" ? "success" : "info");
    logAudit(
      `Loan repayment: ${loan.lender}`,
      "Finance",
      "User",
      `TZS ${money(amt)}k via ${repayMethod}. Balance: TZS ${money(Math.round(Math.max(0, balance - amt)))}k`,
    );
    if (IS_CONFIGURED && loan.dbId) {
      try {
        await sb("loan_repayments")
          .insert({
            loan_id: loan.dbId,
            amount: amt,
          ...
          .run();
        if (newStatus === "Paid")
          await sb("business_loans").eq("id", loan.dbId).update({ status: "Paid" }).run();
      } catch (_e) {
        notify("Saved locally — server update failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            Money the business has borrowed — the real ledger behind Financing Activities in the
            Cash Flow Statement.
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="btn-primary text-white text-[12.5px] font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5"
        >
      ...
      >
        ...
        <p className="text-[26px] font-mono font-bold text-white mb-4">
          TZS {money(Math.round(totals.outstanding))}k
        </p>
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-white/20">
          <div>
            <p className="text-[11px] text-white/70">Total Borrowed</p>
            <p className="text-[15px] font-mono font-semibold text-white">
              TZS {money(Math.round(totals.borrowed))}k
            </p>
          </div>
          <div>
            <p className="text-[11px] text-white/70">Total Repaid</p>
            <p className="text-[15px] font-mono font-semibold text-white">
              TZS {money(Math.round(totals.repaid))}k
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        {["All", "Active", "Paid", "Defaulted"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-colors ${filter === f ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
          >
            {f}
          </button>
        ))}
      ...
      <div className="space-y-3">
        {!loans.loading && filtered.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              icon={Landmark}
              title="No loans"
              hint="Real loans recorded here become real financing activity in the Cash Flow Statement."
        ...
        {loans.loading && (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-8 text-center text-[12.5px] text-slate-400">
            Loading...
          </div>
        )}
        {filtered.map((l) => (
          <div key={l.id} className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="text-[13.5px] font-semibold text-[#111827]">{l.lender}</p>
                <p className="text-[11px] text-slate-400">
                  {l.loanType} · borrowed {l.borrowedDate}
                  {l.dueDate ? ` · due ${l.dueDate}` : ""}
                </p>
              </div>
              <span
                className={`text-[10.5px] font-medium px-2 py-0.5 rounded-full ${l.status === "Paid" ? "bg-[#16A34A]/10 text-[#16A34A]" : l.status === "Defaulted" ? "bg-[#EF4444]/10 text-[#EF4444]" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
              >
                {l.status}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-[12px] mb-3">
              <div>
                ...
                <p className="font-mono font-medium text-[#111827]">
                  {money(Math.round(l.principal))}k
                </p>
              </div>
              <div>
                <p className="text-slate-400">Repaid</p>
                <p className="font-mono font-medium text-[#16A34A]">
                  {money(Math.round(totalRepaid(l)))}k
                </p>
              </div>
              <div>
                <p className="text-slate-400">Balance</p>
                <p className="font-mono font-medium text-[#EF4444]">
                  {money(Math.round(outstandingBalance(l)))}k
                </p>
              </div>
            </div>
            {l.status !== "Paid" &&
              (repayingLoan === l.id ? (
                <div className="space-y-2.5 pt-2 border-t border-slate-100">
                  ...
                  <div>
                    <div className="flex justify-between text-[10.5px] text-slate-400 mb-1">
                      ...
                      <span>
                        {Math.min(
                          100,
                          Math.round(
                            ((totalRepaid(l) + (Number(repayAmount) || 0)) / l.principal) * 100,
                          ),
                        )}
                        % of principal
                      </span>
                    ...
                    <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all"
                        style={{
                          width: `${Math.min(100, (totalRepaid(l) / l.principal) * 100)}%`,
                          backgroundColor: "#16A34A",
                        }}
                      />
                      {Number(repayAmount) > 0 && (
                        <div
                          className="h-full rounded-full -mt-2 transition-all"
                          style={{
                            width: `${Math.min(100, ((totalRepaid(l) + Number(repayAmount)) / l.principal) * 100)}%`,
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={() => recordRepayment(l)}
                      className="btn-primary text-white text-[12px] font-medium px-3.5 py-2 rounded-lg shrink-0"
                    >
                      Record
                    </button>
                    <button
                      ...
                      className="text-[12px] font-medium border border-slate-200 rounded-lg px-3 py-2 shrink-0"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                  ...
                  <span className="text-[10.5px] text-slate-400 shrink-0">
                    {Math.round((totalRepaid(l) / l.principal) * 100)}% repaid
                  </span>
                  <button
                    onClick={() => setRepayingLoan(l.id)}
                    className="btn-secondary text-[12px] font-medium rounded-lg py-2 px-3 shrink-0"
                  >
                    Record Repayment
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-40 flex justify-end">
          ...
          >
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
              ...
              <FormField label="Borrowed date">
                <input
                  type="date"
                  className={inputClass}
                  value={form.borrowedDate}
                  onChange={(e) => setForm((f) => ({ ...f, borrowedDate: e.target.value }))}
                />
              </FormField>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
              <button
                type="button"
                onClick={() => setShowForm(false)}
                className="flex-1 text-[12px] font-medium border border-slate-200 rounded-lg py-2.5"
              >
                Cancel
              </button>
              <button
...
export function Banking({ invoices, expenses, posTransactions }) {
  const channels = useMemo(() => {
    ...
    PAYMENT_METHODS.forEach((m) => {
      map[m] = { method: m, inflow: 0, outflow: 0 };
    });
  ...
  const totals = channels.reduce(
    (t, c) => ({ inflow: t.inflow + c.inflow, outflow: t.outflow + c.outflow }),
    { inflow: 0, outflow: 0 },
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Cash movement by channel, from recorded invoice payments, POS sales net of returns, and
          paid expenses. Legacy invoices marked Paid before an itemized payment method was recorded
          aren&apos;t attributed to a channel here — see the General Ledger for those.
        </p>
      </div>
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  ...
                  <p className="text-[13px] font-mono font-medium text-[#16A34A]">
                    +{money(c.inflow)}
                  </p>
                </div>
                <div>
                  <p className="text-[10.5px] text-slate-400 mb-0.5">Out</p>
                  <p className="text-[13px] font-mono font-medium text-[#EF4444]">
                    −{money(c.outflow)}
                  </p>
                </div>
                <div>
                  ...
                  <p
                    className={`text-[13px] font-mono font-semibold ${c.net >= 0 ? "text-[#111827]" : "text-[#EF4444]"}`}
                  >
                    {money(c.net)}
                  </p>
                </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center justify-between">
        ...
        <div className="flex gap-4 text-[13px] font-mono">
          ...
          <span className="font-semibold text-[#111827]">
            {money(totals.inflow - totals.outflow)}k
          </span>
        </div>
      </div>
    </div>
...
export function OtherDebtorsView() {
  const debtors = useCompanyTable("other_debtors", [], {
    order: { col: "loan_date", ascending: false },
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      debtorType: r.debtor_type,
      name: r.debtor_name,
      phone: r.phone || "",
      amountOwed: Number(r.amount_owed) || 0,
      amountCollected: Number(r.amount_collected) || 0,
  ...
  const totals = {
    outstanding: debtors.rows.reduce(
      (s, d) => s + Math.max(0, d.amountOwed - d.amountCollected),
      0,
    ),
  ...
  const filtered =
    filter === "All" ? debtors.rows : debtors.rows.filter((d) => d.status === filter);
  ...
  async function addDebtor(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("other_debtors")
          .insert({
            debtor_type: draft.debtorType,
            debtor_name: draft.name,
            phone: draft.phone,
            amount_owed: draft.amountOwed,
          ...
          .run();
        if (header?.id)
          debtors.setRows((prev) =>
            prev.map((d) => (d.id === draft.id ? { ...d, dbId: header.id } : d)),
          );
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            Money owed to the business outside a formal sale — an advance, a personal loan, an
            informal debt.
          </p>
        </div>
        <button
      ...
      <div
        className="rounded-xl p-5"
        style={{ background: "linear-gradient(135deg, #F59E0B, #C4622D)" }}
      >
        <p className="text-[12px] text-white/80 mb-1">Outstanding Balance</p>
        <p className="text-[26px] font-mono font-bold text-white mb-4">
          TZS {money(Math.round(totals.outstanding))}k
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-white/20">
          <div>
            <p className="text-[11px] text-white/70">Total Owed</p>
            <p className="text-[15px] font-mono font-semibold text-white">
              TZS {money(Math.round(totals.owed))}k
            </p>
          </div>
          <div>
            <p className="text-[11px] text-white/70">Total Collected</p>
            <p className="text-[15px] font-mono font-semibold text-white">
              TZS {money(Math.round(totals.collected))}k
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm divide-y divide-slate-50">
        {!debtors.loading && filtered.length === 0 && (
          <EmptyState
            icon={UserPlus}
            title="No debtors"
            hint="Money owed to the business outside a formal sale, tracked separately from Sales and CRM."
          ...
          <div key={d.id} className="flex items-center justify-between px-4 py-3.5">
            <div className="min-w-0">
              <p className="text-[13px] font-medium text-[#111827] truncate">
                {d.name}{" "}
                <span className="text-[10.5px] text-slate-400 font-normal">· {d.debtorType}</span>
              </p>
              <p className="text-[11px] text-slate-400">
                {d.phone || "No phone"} · since {d.loanDate}
              </p>
            </div>
            <div className="text-right shrink-0 ml-3">
              <p className="text-[13px] font-mono font-semibold text-[#F59E0B]">
                TZS {money(Math.round(d.amountOwed - d.amountCollected))}k
              </p>
              <span
                className={`text-[10px] font-medium ${d.status === "Paid" ? "text-[#16A34A]" : "text-slate-400"}`}
              >
                {d.status}
              </span>
            </div>
        ...
        <div className="fixed inset-0 z-40 flex justify-end">
          ...
          >
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {DEBTOR_TYPES.map((t) => (
                    <button
                      key={t}
                      type="button"
                      onClick={() => setForm((f) => ({ ...f, debtorType: t }))}
                      className={`text-[12px] font-medium px-3 py-1.5 rounded-full border transition-colors ${form.debtorType === t ? "border-[#F59E0B]/50 bg-[#F59E0B]/10 text-[#F59E0B]" : "border-slate-200 text-slate-500"}`}
                    >
                      {t}
                    </button>
                  ))}
              ...
              <FormField label="Debtor name" required>
                <input
                  className={inputClass}
                  value={form.name}
              ...
              <FormField label="Amount owed" required>
                <input
                  type="number"
                  min="0"
                  className={inputClass}
...
export function OtherIncomeView() {
  const income = useCompanyTable("other_income", [], {
    order: { col: "income_date", ascending: false },
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      title: r.title,
  ...
  async function addIncome(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("other_income")
          .insert({
            title: draft.title,
            amount: draft.amount,
            description: draft.description,
            payment_method: draft.paymentMethod,
            income_date: draft.date,
          })
          .single()
          .run();
        if (header?.id)
          income.setRows((prev) =>
            prev.map((i) => (i.id === draft.id ? { ...i, dbId: header.id } : i)),
          );
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            Real, genuine revenue outside a sale — interest, a grant, an asset sale — kept honestly
            separate from what the business actually sold.
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
      ...
      <div className="rounded-xl p-5 bg-[#DCFCE7]">
        ...
        <p className="text-[26px] font-mono font-bold text-[#15803D]">
          TZS {money(Math.round(total))}k
        </p>
      </div>

      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm divide-y divide-slate-50">
        {!income.loading && income.rows.length === 0 && (
          <EmptyState
            icon={Wallet}
            title="No other income recorded"
            hint="Real non-sales revenue this system's P&L would otherwise miss entirely."
          ...
          return (
            <div key={i.id} className="flex items-center gap-3 px-4 py-3.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-medium text-[#111827] truncate">{i.title}</p>
                <p className="text-[11px] text-slate-400">
                  {i.date} · {i.paymentMethod}
                </p>
              </div>
              <p className="text-[13px] font-mono font-semibold text-[#16A34A] shrink-0">
                +{money(Math.round(i.amount))}k
              </p>
            </div>
          );
        })}
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
              <FormField label="Title" required>
                <input
                  ...
                  placeholder="e.g. Interest earned"
                />
              </FormField>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(PAY_ICONS).map(([m, Icon]) => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 rounded-xl border py-3 transition-colors ${form.paymentMethod === m ? "border-[#16A34A]/50 bg-[#16A34A]/5" : "border-slate-200"}`}
                    >
                      <Icon
                        size={16}
                        className={form.paymentMethod === m ? "text-[#16A34A]" : "text-slate-400"}
                      />
                      <span
                        className={`text-[11px] font-medium ${form.paymentMethod === m ? "text-[#111827]" : "text-slate-500"}`}
                      >
                        {m}
                      </span>
                    </button>
...
export const TAX_COUNTRIES = {
  ...
  KE: { label: "Kenya (KRA)", active: false },
  UG: { label: "Uganda (URA)", active: false },
  RW: { label: "Rwanda (RRA)", active: false },
  ZM: { label: "Zambia (ZRA)", active: false },
};
...
export function TaxCenterView({ invoices, expenses, employeesHook, company }) {
  ...
  const figures = useMemo(() => {
    ...
    const outputVat = invoices.rows
      .filter((i) => i.date >= yearStart)
      .reduce((s, i) => s + lineTotal(i.items).tax, 0);
    const staff = employeesHook.rows.filter((e) => e.status === "Active");
    const payroll = staff.reduce((s, e) => s + e.salary, 0);
    const payeRows = staff.map((e) => ({
      name: e.name,
      salary: e.salary,
      paye: cfg.paye(e.salary),
    }));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            Real published rates against your real records — planning estimates, never a filing.
          </p>
        </div>
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
          {Object.entries(TAX_COUNTRIES).map(([k, v]) => (
            <button
              ...
              className={`text-[11.5px] font-medium px-2.5 py-1.5 rounded-md transition-colors ${country === k ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
            >
              {k}
            </button>
          ))}
      ...
      >
        ...
        <p className="text-[11.5px] text-[#92400E]">
          These are estimates computed from what this system has recorded, using published rates —
          accounting profit is not taxable profit, rates change, and input-VAT credits need
          per-expense VAT capture this build doesn&apos;t yet do. Verify every figure with TRA or
          your accountant before filing or paying anything.
        </p>
      </div>
      ...
      {!cfg.active && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-8 text-center">
          ...
          <p className="text-[11.5px] text-slate-400 mt-1">
            The engine is a per-country rates table — this jurisdiction needs its real brackets
            added and checked, not a rewrite. Tanzania is live today.
          </p>
        </div>
      )}
        ...
        <>
          ...
          >
            <p className="text-[12px] text-white/80 mb-1">
              Estimated Monthly Obligation (PAYE + SDL + WCF)
            </p>
            <p className="text-[26px] font-mono font-bold text-white mb-4">
              TZS {money(Math.round(figures.payeTotal + figures.sdl + figures.wcf))}k
            </p>
            <div className="grid grid-cols-2 gap-4 pt-3 border-t border-white/20">
              <div>
                <p className="text-[11px] text-white/70">Output VAT (YTD)</p>
                <p className="text-[15px] font-mono font-semibold text-white">
                  TZS {money(Math.round(figures.outputVat))}k
                </p>
              </div>
              <div>
                <p className="text-[11px] text-white/70">Corporate Tax (YTD est.)</p>
                <p className="text-[15px] font-mono font-semibold text-white">
                  TZS {money(Math.round(figures.corp))}k
                </p>
              </div>
            </div>
          </div>
            ...
            return (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {items.map((x) => (
                  <div
                    key={x.label}
                    className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-3.5 flex items-center gap-3"
                  >
                    <div
                      className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 font-mono font-bold text-[13px]"
                      style={{
                        backgroundColor: x.days <= 5 ? "#FEE2E2" : "#DCFCE7",
                        color: x.days <= 5 ? "#EF4444" : "#16A34A",
                      }}
                    >
                      {x.days}d
                    </div>
                    <div className="min-w-0">
                      <p className="text-[12px] font-medium text-[#111827] truncate">{x.label}</p>
                      <p className="text-[10.5px] text-slate-400">
                        due {x.due} — published TRA date
                      </p>
                    </div>
                  </div>
                ))}
          ...
          <div className="flex justify-end">
            <ExportMenu
              ...
              rows={[
                ["Output VAT (YTD)", Math.round(figures.outputVat), "Real invoice tax lines"],
                [
                  "PAYE (monthly)",
                  Math.round(figures.payeTotal),
                  `TRA brackets across ${figures.staff} salaries`,
                ],
                ["SDL (monthly)", Math.round(figures.sdl), "3.5% of gross payroll"],
                ["WCF (monthly)", Math.round(figures.wcf), "0.5% of gross payroll"],
                [
                  "Corporate Tax (YTD est.)",
                  Math.round(figures.corp),
                  "30% of YTD accounting profit",
                ],
              ]}
            />
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <Card
              ...
              note={`${company.taxRate || 18}% on real invoice tax lines. Input-VAT credit not yet netted — stated, not hidden.`}
            />
            <Card
              title="PAYE (monthly)"
              value={figures.payeTotal}
              note={`Real TRA brackets across ${figures.staff} active salaries.`}
            />
            <Card
              title="SDL (monthly)"
              value={figures.sdl}
              note="3.5% of gross payroll. Applies from 10+ employees — check your headcount."
            />
            <Card
              title="WCF (monthly)"
              value={figures.wcf}
              note="0.5% of gross payroll, private sector rate."
            />
            <Card
              title="Corporate Tax (YTD est.)"
              value={figures.corp}
              note={`30% on YTD accounting profit of TZS ${money(Math.round(figures.profit))}k — taxable profit differs after adjustments.`}
            />
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <p className="text-[10.5px] text-slate-400 mt-1">
                Excise is product-specific (fuel, beverages, airtime) — needs per-product excise
                rates this catalog doesn&apos;t carry. Named, not guessed.
              </p>
            </div>
          </div>

          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <p className="text-[12.5px] font-semibold text-[#111827] px-4 pt-3.5 pb-2">
              PAYE per employee — real TRA monthly brackets
            </p>
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 text-left text-[11px] text-slate-400 uppercase tracking-wide">
                  <th className="px-4 py-2">Employee</th>
                  <th className="px-4 py-2 text-right">Gross (TZS k)</th>
                  <th className="px-4 py-2 text-right">PAYE (TZS k)</th>
                </tr>
              </thead>
              <tbody>
                {figures.payeRows.map((r) => (
                  <tr key={r.name} className="border-b border-slate-50 last:border-0">
                    <td className="px-4 py-2 text-[#111827]">{r.name}</td>
                    <td className="px-4 py-2 text-right font-mono text-slate-600">
                      {money(r.salary)}
                    </td>
                    <td className="px-4 py-2 text-right font-mono text-[#111827]">
                      {money(Math.round(r.paye * 10) / 10)}
                    </td>
                  </tr>
                ))}
                {figures.payeRows.length === 0 && !employeesHook.loading && (
                  <tr>
                    <td colSpan={3} className="px-4 py-6 text-center text-slate-400 text-[12px]">
                      No active employees.
                    </td>
                  </tr>
                )}
        ...
        <div className="space-y-3 pt-2">
          <div className="flex items-center justify-between">
            <h3 className="text-[14px] font-semibold text-[#111827]">
              TRA Portal — Tax Return Templates
            </h3>
            <a
              href="https://taxpayerportal.tra.go.tz"
              target="_blank"
          ...
          <p className="text-[12px] text-slate-500">
            Pre-filled from your real records. Print each form to use as a filing reference at{" "}
            <strong>taxpayerportal.tra.go.tz</strong>. Deadlines: PAYE, SDL, WCF by the{" "}
            <strong>7th</strong> · VAT by the <strong>20th</strong> of the following month.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <TRAReturnCard
              title="PAYE Monthly Return"
              formCode="PAYE.1"
              deadline="7th of next month"
              fields={[
                { l: "Employer TIN", v: company.tin || "—" },
                { l: "No. of Employees", v: String(figures.staff) },
                { l: "Gross Payroll (TZS)", v: money(Math.round(figures.payroll * 1000)) },
                { l: "Total PAYE Due", v: `TZS ${money(Math.round(figures.payeTotal * 1000))}` },
              ]}
            ...
            <TRAReturnCard
              ...
              fields={[
                { l: "Taxpayer TIN", v: company.tin || "—" },
                {
                  l: "Output VAT (Sales)",
                  v: `TZS ${money(Math.round(figures.outputVat * 1000))}`,
                },
                { l: "Input VAT", v: "Enter from purchase invoices" },
                {
                  l: "Net VAT Payable",
                  v: `TZS ${money(Math.round(figures.outputVat * 1000))} (before input credit)`,
                },
              ]}
            ...
            <TRAReturnCard
              ...
              fields={[
                { l: "Employer TIN", v: company.tin || "—" },
                { l: "Gross Payroll", v: `TZS ${money(Math.round(figures.payroll * 1000))}` },
                { l: "SDL Rate", v: "3.5%" },
                { l: "SDL Due", v: `TZS ${money(Math.round(figures.sdl * 1000))}` },
              ]}
            ...
            <TRAReturnCard
              ...
              fields={[
                ...
                { l: "Gross Payroll", v: `TZS ${money(Math.round(figures.payroll * 1000))}` },
                { l: "WCF Rate", v: "0.5%" },
                { l: "WCF Due", v: `TZS ${money(Math.round(figures.wcf * 1000))}` },
              ]}
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div>
          ...
          <p className="text-[10.5px] text-[#F59E0B] flex items-center gap-1 mt-0.5">
            <Clock size={10} /> Due {deadline}
          </p>
        </div>
        <button
          onClick={onPrint}
          className="flex items-center gap-1 text-[11px] font-medium text-[#16A34A] border border-[#16A34A]/30 rounded-lg px-2.5 py-1.5 hover:bg-[#16A34A]/5 shrink-0"
        >
...
export function printTRAReturn(type, figures, company, employees) {
  ...
  const computedRows =
    {
      "PAYE Return": [
        ["No. of Employees", String(figures.staff)],
        ["Gross Payroll", `TZS ${fmt(figures.payroll * 1000)}`],
        ["Total PAYE Deducted", `TZS ${fmt(figures.payeTotal * 1000)}`],
      ],
      "VAT Return": [
        ["Output VAT (Sales)", `TZS ${fmt(figures.outputVat * 1000)}`],
        ["Input VAT (Purchases)", "[Enter from purchase records]"],
        ["Net VAT Payable", `TZS ${fmt(figures.outputVat * 1000)} (before input credit)`],
      ],
      "SDL Return": [
        ["Gross Payroll", `TZS ${fmt(figures.payroll * 1000)}`],
        ["SDL Rate", "3.5%"],
        ["SDL Due", `TZS ${fmt(figures.sdl * 1000)}`],
      ],
      "WCF Return": [
        ["Gross Payroll", `TZS ${fmt(figures.payroll * 1000)}`],
        ["WCF Rate", "0.5%"],
        ["WCF Due", `TZS ${fmt(figures.wcf * 1000)}`],
      ],
    }[type] || [];
  ...
  const rowsHtml = computedRows
    .map(function (pair, i) {
      const l = pair[0];
      const v = pair[1];
      const bg = i % 2 ? "" : ' style="background:#F8FAFC"';
      const color =
        l.includes("Due") || l.includes("Deducted") || l.includes("Payable")
          ? "#EF4444"
          : "#111827";
      return (
        "<tr" +
        ...
        l +
        '</td><td style="padding:6px 10px;font-size:12px;font-weight:700;text-align:right;color:' +
        color +
        '">' +
        v +
        "</td></tr>"
    ...
    .join("");
  const empSection = empRows
    ? '<p style="font-size:11px;font-weight:700;color:#111827;margin-bottom:6px;letter-spacing:.05em">EMPLOYEE BREAKDOWN</p><table style="width:100%;border-collapse:collapse"><thead><tr style="background:#F0FDF4"><th style="padding:5px 8px;font-size:9.5px;text-align:left;font-weight:600;color:#166534">Employee</th><th style="padding:5px 8px;font-size:9.5px;text-align:left;font-weight:600;color:#166534">Role</th><th style="padding:5px 8px;font-size:9.5px;text-align:right;font-weight:600;color:#166534">Gross Salary</th><th style="padding:5px 8px;font-size:9.5px;text-align:right;font-weight:600;color:#166534">PAYE</th></tr></thead><tbody>' +
      empRows +
      "</tbody></table>"
    : "";
  printAsPDF(
    "TRA — " + type,
    '<div style="font-family:Inter,sans-serif;max-width:640px;margin:0 auto;padding:32px"><div style="display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #16A34A;padding-bottom:16px;margin-bottom:20px"><div><div style="font-size:18px;font-weight:800;color:#16A34A">TANZANIA REVENUE AUTHORITY</div><div style="font-size:13px;font-weight:700;color:#111827;margin-top:2px">' +
      type +
      '</div><div style="font-size:11px;color:#6B7280">Period: ' +
      month +
      ...
      '</div><div style="font-size:11px;color:#6B7280">TIN: ' +
      (co.tin || "—") +
      '</div></div></div><table style="width:100%;border-collapse:collapse;margin-bottom:16px"><tr style="background:#F0FDF4"><td colspan="2" style="padding:8px 10px;font-weight:700;font-size:11px;color:#166534;letter-spacing:.05em">TAXPAYER DETAILS</td></tr><tr><td style="padding:6px 10px;font-size:11px;color:#6B7280;width:40%">Business Name</td><td style="padding:6px 10px;font-size:11.5px;font-weight:600">' +
      (co.name || "—") +
      '</td></tr><tr style="background:#F8FAFC"><td style="padding:6px 10px;font-size:11px;color:#6B7280">TIN Number</td><td style="padding:6px 10px;font-size:11.5px;font-weight:600">' +
      (co.tin || "—") +
      '</td></tr><tr><td style="padding:6px 10px;font-size:11px;color:#6B7280">Registration No.</td><td style="padding:6px 10px;font-size:11.5px">' +
      (co.regNumber || "—") +
      '</td></tr><tr style="background:#F8FAFC"><td style="padding:6px 10px;font-size:11px;color:#6B7280">Address</td><td style="padding:6px 10px;font-size:11px">' +
      [co.address, co.city, "Tanzania"].filter(Boolean).join(", ") +
      '</td></tr><tr><td style="padding:6px 10px;font-size:11px;color:#6B7280">Return Period</td><td style="padding:6px 10px;font-size:11.5px;font-weight:600">' +
      month +
      '</td></tr></table><table style="width:100%;border-collapse:collapse;margin-bottom:16px"><tr style="background:#F0FDF4"><td colspan="2" style="padding:8px 10px;font-weight:700;font-size:11px;color:#166534;letter-spacing:.05em">TAX COMPUTATION</td></tr>' +
      rowsHtml +
      "</table>" +
      empSection +
      '<div style="margin-top:20px;padding:10px;background:#FEF3C7;border-radius:6px;font-size:10px;color:#92400E">&#9888; Verify with your accountant before filing. Input VAT credits must be applied manually at taxpayerportal.tra.go.tz</div><div style="margin-top:10px;font-size:9.5px;color:#9CA3AF;text-align:center">Generated by Smart Manager &middot; ' +
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          VAT on an invoice basis (owed when invoiced, not when paid) — Tanzania&apos;s standard
          treatment. Input VAT from vendor bills isn&apos;t tracked yet since expenses don&apos;t
          carry a VAT breakdown, so this shows output VAT only, not a true net position.
        </p>
      </div>
...
export function Assets() {
  const assets = useCompanyTable("finance_assets", financeAssetsSeed, {
    order: { col: "acquisition_date", ascending: false },
    mapRow: mapAssetRow,
  });
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (t, a) => {
        ...
        return {
          cost: t.cost + a.cost,
          accumulated: t.accumulated + accumulated,
          bookValue: t.bookValue + bookValue,
        };
      },
  ...
  async function addAsset(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("finance_assets")
          .insert({
            name: draft.name,
            category: draft.category,
            acquisition_date: draft.acquisitionDate,
            cost: draft.cost,
            useful_life_years: draft.usefulLifeYears,
          })
          .single()
          .run();
        if (header?.id)
          setRows((prev) => prev.map((a) => (a.id === draft.id ? { ...a, dbId: header.id } : a)));
      } catch (_e) {
        notify("Asset added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteAsset(id) {
    ...
    if (IS_CONFIGURED && asset?.dbId) {
      try {
        await sb("finance_assets").eq("id", asset.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the asset on the server.", "error");
      }
    }
  }
  ...
  const ASSET_KPIS = [
    {
      ...
      value: `TZS ${money(Math.round(totals.cost))}k`,
      delta: `${rows.length} assets`,
      up: true,
      icon: Package,
    },
    {
      label: "Accumulated Depreciation",
      value: `TZS ${money(Math.round(totals.accumulated))}k`,
      delta: "To date",
      up: false,
      icon: TrendingDown,
    },
    {
      ...
      value: `TZS ${money(Math.round(totals.bookValue))}k`,
      delta: "Current",
      up: true,
      icon: CircleDollarSign,
    },
  ...
  return (
    <div className="space-y-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-mono font-medium">
                        {money(bookValue)}
                        {fullyDepreciated && (
                          <span className="ml-1.5 text-[10px] text-slate-400">fully deprec.</span>
                        )}
                      </td>
      ...
      {selected && (
        <AssetPanel asset={selected} onClose={() => setSelected(null)} onDelete={deleteAsset} />
      )}
      ...
      >
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <h2 className="text-[17px] font-semibold text-[#111827] mt-0.5 leading-snug">
              {asset.name}
            </h2>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-slate-50 rounded-lg p-3">
            ...
            <p className="text-[15px] font-mono font-semibold text-[#F59E0B]">
              TZS {money(accumulated)}k
            </p>
          </div>
          <div className="bg-slate-50 rounded-lg p-3">
            <p className="text-[11px] text-slate-400 mb-1">Net Book Value</p>
            <p className="text-[15px] font-mono font-semibold text-[#16A34A]">
              TZS {money(bookValue)}k
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 text-[13px] text-slate-600 mb-6">
          <TrendingDown size={14} className="text-slate-400" /> TZS {money(monthlyDep)}k
          depreciation per month{fullyDepreciated ? " (fully depreciated)" : ""}
        </div>
...
export function AssetFormPanel({ onClose, onSubmit }) {
  const [form, setForm] = useState({
    ...
    acquisitionDate: TODAY.toISOString().slice(0, 10),
    cost: "",
    usefulLifeYears: "5",
  });
  const [touched, setTouched] = useState(false);
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Asset name" required>
            <input
              ...
              placeholder="e.g. Toyota Hilux — Delivery Truck"
            />
            {touched && !form.name.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Asset name is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Cost (TZS 000)" required>
              ...
              {touched && !(Number(form.cost) > 0) && (
                <p className="text-[11px] text-[#EF4444] mt-1">Enter a cost.</p>
              )}
            </FormField>
          ...
          <p className="text-[11.5px] text-slate-400">
            Depreciation is calculated automatically using the straight-line method from this date.
          </p>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          ...
          <button
            type="submit"
            className="flex-1 btn-primary text-white text-[12px] font-medium rounded-lg py-2.5"
          >
            Create Asset
          </button>
        </div>
...
export function MobileMoneyReconciliation({ invoices, currentUser }) {
  ...
  function submit(e) {
    ...
    const patch = recordPayment(
      invoices,
      invoiceId,
      {
        amount: Math.min(Number(amount), balance),
        method: "Mobile Money",
        date: TODAY.toISOString().slice(0, 10),
    ...
    invoices.rows.forEach((inv) =>
      (inv.payments || []).forEach((p) => {
        if (p.method === "Mobile Money")
          all.push({ ...p, invoiceId: inv.id, customer: inv.customer });
      }),
    );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          No live M-Pesa, Airtel Money, Tigo Pesa, or HaloPesa API connection exists — each requires
          server-held credentials and a hosted callback URL. This records a payment you&apos;ve
          already confirmed (from the SMS notification on your phone) against the right invoice, the
          same real workflow most businesses already use.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        ...
        {outstanding.length === 0 ? (
          <p className="text-[12.5px] text-slate-400">
            No outstanding invoices to reconcile against right now.
          </p>
        ) : (
          <form onSubmit={submit} className="space-y-4">
            <FormField label="Invoice">
              ...
              {invoice && (
                <p className="text-[11px] text-slate-400 mt-1">
                  Balance due: TZS {money(Math.round(balance))}k
                </p>
              )}
            </FormField>
            <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Transaction reference" required>
              <input
                ...
                placeholder="e.g. QJJ4K7XLMN (from the confirmation SMS)"
              />
            </FormField>
            <button
              type="submit"
              className="w-full btn-primary text-white text-[13px] font-medium rounded-lg py-2.5"
            >
              Record Payment
            </button>
          </form>
...
export function BankStatementImport({ invoices, expenses }) {
  ...
  function findMatch(amount) {
    return outstanding.find(
      (inv) => Math.abs(lineTotal(inv.items).total - (inv.amountPaid || 0) - Math.abs(amount)) < 1,
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          No bank in this environment exposes a connectable API — real Open Banking access needs
          bank-specific, regulated credentials. Upload a real statement export (CSV or Excel)
          instead; this parses it and flags amounts that match an outstanding invoice.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-xl py-8 cursor-pointer hover:border-[#16A34A]/40 hover:bg-slate-50/50 transition-colors">
          ...
          <span className="text-[13px] font-medium text-slate-600">
            {fileName || "Upload bank statement (CSV or Excel)"}
          </span>
      ...
      {transactions.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 sm:px-5 py-3.5 border-b border-slate-100">
            <h3 className="text-[14px] font-semibold text-[#111827]">Parsed Transactions</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-[13px] min-w-[560px]">
              <thead>
    ...
    <div className="max-w-2xl space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        ...
        <p className="text-[13px] text-slate-500 leading-relaxed mb-4">
          Finance&apos;s Tax tab already computes output VAT live from real invoices, on an invoice
          basis — the number most VAT filings ask for. There&apos;s no direct submission from here;
          use it to prepare what you file manually.
        </p>
        {onNavigate && (
          <button
            onClick={() => onNavigate("finance")}
            className="btn-primary text-white text-[13px] font-medium rounded-lg py-2.5 px-4 flex items-center gap-1.5"
          >
