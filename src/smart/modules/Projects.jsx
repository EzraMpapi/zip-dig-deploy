...
export function Projects({ filesHook, expensesHook, company }) {
  const projects = useCompanyTable("projects", projectsSeed, {
    order: { col: "start_date", ascending: false },
    mapRow: mapProjectRow,
  });
  const tasks = useCompanyTable("project_tasks", projectTasksSeed, {
    order: { col: "due_date", ascending: true },
    mapRow: mapProjectTaskRow,
  });
  const milestones = useCompanyTable("project_milestones", projectMilestonesSeed, {
    order: { col: "due_date", ascending: true },
    mapRow: mapMilestoneRow,
  });
  ...
  const stats = useMemo(() => {
    ...
    const dueSoonTasks = tasks.rows.filter(
      (t) =>
        t.status !== "Done" &&
        t.dueDate &&
        t.dueDate >= todayStr &&
        daysBetween(new Date(t.dueDate), TODAY) <= 7,
    ).length;
    const overdueMilestones = milestones.rows.filter(
      (m) => milestoneStatus(m) === "Overdue",
    ).length;
  ...
  const PROJECT_KPIS = [
    {
      ...
      delta: `${projects.rows.length} total`,
      up: true,
      icon: Kanban,
    },
    {
      label: "Total Budget",
      value: `TZS ${money(stats.totalBudget)}k`,
      delta: "All projects",
      up: true,
    ...
    },
    {
      ...
      value: String(stats.dueSoonTasks),
      delta: "Next 7 days",
      up: false,
      icon: ListTodo,
    },
  ...
  async function addProject(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("projects")
          .insert({
            name: draft.name,
            client: draft.client,
            status: "Planning",
          ...
          .run();
        if (header?.id)
          projects.setRows((prev) =>
            prev.map((p) => (p.id === draft.id ? { ...p, dbId: header.id } : p)),
          );
      } catch (_e) {
        notify("Project created locally, but saving to the server failed.", "error");
      }
    }
  }
      ...
      <ProjectDetail
        ...
        onSetStatus={async (status) => {
          ...
          if (IS_CONFIGURED && selectedProject.dbId) {
            try {
              await sb("projects").eq("id", selectedProject.dbId).update({ status }).run();
            } catch (_e) {
              notify("Couldn't save the project status to the server.", "error");
            }
          }
        }}
        tasksHook={tasks}
  ...
  function exportProjectsCsv() {
    downloadCSV(
      ...
      [
        ...
        { key: "Budget_k", label: "Budget (TZS k)" },
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Tasks, timelines, milestones, files, and budgets per project
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={exportProjectsCsv}
            className="flex items-center gap-1.5 text-[12.5px] font-semibold text-[#16A34A] border border-[#16A34A]/25 bg-[#F0FDF4] px-3 py-2 rounded-lg"
          >
            <Download size={13} /> CSV
          </button>
          <button
            onClick={printProjects}
            className="flex items-center gap-1.5 text-[12.5px] font-semibold text-white bg-[#0D2214] px-3 py-2 rounded-lg"
          >
        ...
        <button
          ...
          className="btn-primary text-white text-[13px] font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5 shadow-sm shrink-0"
        >
      ...
      {projects.rows.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {(() => {
              const statusData = ["Planning", "Active", "On Hold", "Completed", "Cancelled"]
                .map((s) => ({
                  name: s,
                  value: projects.rows.filter((p) => p.status === s).length,
                  fill: {
                    Planning: "#94A3B8",
                    Active: "#16A34A",
                    "On Hold": "#F59E0B",
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {(() => {
              const budgetData = projects.rows
                .filter((p) => p.budget > 0)
                .slice(0, 6)
                .map((p) => {
              ...
              if (budgetData.length === 0)
                return <p className="text-slate-400 text-center py-6">No projects with budgets</p>;
              return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.loading &&
          Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-xl border border-slate-200/80 h-36 skeleton-shimmer"
            />
          ))}
        {!projects.loading &&
          projects.rows.map((p) => {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <p className="text-[14px] font-semibold text-[#111827] leading-snug pr-2">
                    {p.name}
                  </p>
                  <span
                    className="text-[10px] font-medium px-1.5 py-0.5 rounded shrink-0"
                    style={{
                      backgroundColor: `${PROJECT_STATUS_COLOR[p.status]}14`,
        ...
        {!projects.loading && projects.rows.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Create a project to track tasks, milestones, files, and budget in one place."
      ...
      {(() => {
        const dated = projects.rows.filter(
        ...
        return (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            ...
            <p className="text-[11.5px] text-slate-500 mb-4">
              {min} → {max} · green line marks today ({TODAY.toISOString().slice(0, 10)})
            </p>
            <div className="relative space-y-2.5">
              <div
                className="absolute top-0 bottom-0 w-px bg-[#16A34A] z-10"
                style={{ left: `${todayPct}%` }}
              />
              {dated.map((p) => {
                const left = pct(p.startDate),
                  width = Math.max(2, pct(p.endDate) - left);
                return (
                  <div key={p.id} className="relative h-7">
                    <div
                      ...
                      style={{
                        left: `${left}%`,
                        width: `${width}%`,
                        backgroundColor: `${color[p.status] || "#94A3B8"}22`,
                        borderLeft: `3px solid ${color[p.status] || "#94A3B8"}`,
                      }}
                    >
            ...
            {undated > 0 && (
              <p className="text-[10.5px] text-slate-400 mt-3">
                {undated} project(s) missing a start or end date — excluded from the timeline and
                stated, never drawn as guesses.
              </p>
            )}
          </div>
        );
        ...
        return (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            <h3 className="text-[14px] font-semibold text-[#111827] mb-3">
              Resource Allocation — open tasks per person
            </h3>
            <div className="space-y-2">
              {entries.map(([name, n]) => (
                <div key={name} className="flex items-center gap-3">
                  <span
                    className={`text-[12px] w-32 truncate shrink-0 ${name === "Unassigned" ? "text-[#F59E0B] font-medium" : "text-slate-600"}`}
                  >
                    {name}
                  </span>
                  <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(n / maxN) * 100}%`,
                        backgroundColor: name === "Unassigned" ? "#F59E0B" : "#16A34A",
                      }}
                    />
                  </div>
                  <span className="text-[11.5px] font-mono text-slate-500 shrink-0 w-6 text-right">
                    {n}
                  </span>
                </div>
              ))}
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Projects</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Project</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Project name" required>
            <input
              ...
              placeholder="e.g. Cold Chain Rollout"
            />
            {touched && !form.name.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Project name is required.</p>
            )}
          </FormField>
          <FormField label="Client" required>
            ...
            />
            {touched && !form.client.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Client is required.</p>
            )}
          </FormField>
          ...
          <FormField label="Budget (TZS 000)">
            <input
              type="number"
              min="0"
              className={inputClass}
...
export function ProjectDetail({
  ...
  return (
    <div className="space-y-5">
      <button
        onClick={onBack}
        className="text-[12.5px] font-medium text-slate-500 hover:text-[#111827] flex items-center gap-1"
      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            {project.client} · Managed by {project.manager || "Unassigned"}
          </p>
        </div>
        <select
          ...
          style={{
            borderColor: `${PROJECT_STATUS_COLOR[project.status]}40`,
            color: PROJECT_STATUS_COLOR[project.status],
            backgroundColor: `${PROJECT_STATUS_COLOR[project.status]}0a`,
          }}
        >
          {Object.keys(PROJECT_STATUS_COLOR).map((s) => (
      ...
      {tab === "timeline" && (
        <ProjectTimeline tasks={projectTasks} milestones={projectMilestones} />
      )}
      {tab === "milestones" && (
        <ProjectMilestones project={project} milestonesHook={milestonesHook} />
      )}
      ...
      {tab === "budget" && (
        <ProjectBudget
          ...
          financeExpensesHook={financeExpensesHook}
        />
      )}
...
export function ProjectTasks({ project, tasksHook }) {
  ...
  async function addTask(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("project_tasks")
          .insert({
        ...
        notify("Task added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function moveTask(id, status) {
    ...
    if (IS_CONFIGURED && t?.dbId) {
      try {
        await sb("project_tasks").eq("id", t.dbId).update({ status }).run();
      } catch (_e) {
        notify("Couldn't save the task status to the server.", "error");
      }
    }
  }
  ...
  async function deleteTask(id) {
    ...
    if (IS_CONFIGURED && t?.dbId) {
      try {
        await sb("project_tasks").eq("id", t.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the task on the server.", "error");
      }
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
          <button
            ...
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-colors ${view === "board" ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
          >
            Board
          </button>
          <button
      ...
      {tasks.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            icon={ListTodo}
            title="No tasks yet"
            hint="Break this project into tasks and track them on the board."
        ...
        <div className="flex sm:grid sm:grid-cols-4 gap-3 overflow-x-auto pb-2 -mx-1 px-1 snap-x">
          {TASK_STATUSES.map((status) => (
            <div
              key={status}
              className="bg-slate-50 rounded-xl p-2.5 min-h-[280px] w-[220px] sm:w-auto shrink-0 snap-start"
            >
              <div className="flex items-center justify-between px-1.5 py-1.5 mb-1">
                <span
                  ...
                  style={{
                    backgroundColor: `${TASK_STATUS_COLOR[status]}14`,
              ...
              <div className="space-y-2">
                {grouped[status].map((t) => (
                  ...
                  >
                    <p className="text-[13px] font-medium text-[#111827] leading-snug mb-1.5">
                      {t.title}
                    </p>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                        style={{
                          backgroundColor: `${TASK_STATUS_COLOR[t.status]}14`,
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${TASK_STATUS_COLOR[task.status]}14`,
        ...
        <div className="space-y-3 mb-6 text-[13px]">
          <div className="flex items-center gap-2.5 text-slate-600">
            <Users size={14} className="text-slate-400" /> {task.assignee || "Unassigned"}
          </div>
          <div className="flex items-center gap-2.5 text-slate-600">
            <Clock size={14} className="text-slate-400" /> Due {task.dueDate}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {TASK_STATUSES.map((s) => (
              <button
                ...
                className={`text-[11.5px] font-medium rounded-lg py-2 border transition-colors ${s === task.status ? "opacity-40 cursor-not-allowed border-slate-200" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
              >
                {s}
              </button>
            ))}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Task</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Task</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Title" required>
            <input
              ...
              placeholder="e.g. Install racking system"
            />
          </FormField>
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
      {items.length === 0 ? (
        <EmptyState
          ...
          hint="Tasks and milestones with due dates will appear here in order."
        />
      ) : (
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-[#111827] truncate">
                    {it.kind === "milestone" && <Flag size={11} className="inline mr-1 -mt-0.5" />}
                    {it.title}
                  </p>
                ...
                <div className="text-right shrink-0">
                  <p
                    className={`text-[12px] font-mono ${it.date < todayStr && it.label !== "Done" && it.label !== "Completed" ? "text-[#EF4444] font-medium" : "text-slate-500"}`}
                  >
                    {it.date}
                  </p>
                  <p className="text-[10.5px]" style={{ color: it.color }}>
                    {it.label}
                  </p>
                </div>
              </div>
...
export function ProjectMilestones({ project, milestonesHook }) {
  ...
  async function addMilestone(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function toggleComplete(id) {
    ...
    if (IS_CONFIGURED && m?.dbId) {
      try {
        await sb("project_milestones").eq("id", m.dbId).update({ completed: next }).run();
      } catch (_e) {
        notify("Couldn't save the milestone on the server.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {milestones.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Mark the key dates that matter for this project."
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <button
                    ...
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${m.completed ? "bg-[#16A34A] border-[#16A34A]" : "border-slate-300"}`}
                  ...
                  <div className="min-w-0">
                    <p
                      className={`text-[13.5px] font-medium truncate ${m.completed ? "text-slate-400 line-through" : "text-[#111827]"}`}
                    >
                      {m.title}
                    </p>
                ...
                <span
                  className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5 shrink-0"
                  style={{
                    backgroundColor: `${MILESTONE_STATUS_COLOR[status]}14`,
                    color: MILESTONE_STATUS_COLOR[status],
                  }}
                >
                  <span
                    className="w-1.5 h-1.5 rounded-full"
                    style={{ backgroundColor: MILESTONE_STATUS_COLOR[status] }}
                  />
                  {status}
                </span>
              </div>
      ...
      {showForm && (
        <MilestoneFormPanel onClose={() => setShowForm(false)} onSubmit={addMilestone} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Milestone</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Milestone</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          These are real files from the Documents module, linked to this project — not a separate
          store. Upload from Documents and link it to{" "}
          <span className="font-mono">{project.id}</span> to see it here.
        </p>
      </div>
      {linkedFiles.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Link a file to this project from the Documents module."
          />
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {linkedFiles.map((f) => (
            <div
              key={f.id}
              className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-3.5"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-3 text-center">
          ...
          <p className="text-[22px] font-bold text-[#16A34A]">
            TZS {money(Math.round(totalCost))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 p-3 text-center">
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
        ...
        {touched && (!form.hours || Number(form.hours) <= 0) && (
          <p className="text-[11.5px] text-[#EF4444]">Enter hours above zero.</p>
        )}
        {touched && !form.description.trim() && (
          <p className="text-[11.5px] text-[#EF4444]">Add a description.</p>
        )}
        <button
          onClick={addEntry}
      ...
      {entries.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm py-10 text-center">
          ...
          <p className="text-[11.5px] text-slate-400 mt-1">
            Add your first entry above to start tracking project hours.
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <tbody>
              {entries.map((e) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 font-mono font-bold text-[#111827]">
                    {e.hours.toFixed(1)}h
                  </td>
                  <td className="px-4 py-2.5 text-slate-600 max-w-[200px] truncate">
                    {e.description}
                  </td>
                  <td className="px-4 py-2.5 font-mono text-[#16A34A]">
                    TZS {money(Math.round(e.hours * HOURLY_RATE_DEFAULT))}k
                  </td>
              ...
              <tr className="bg-slate-50 border-t-2 border-slate-200">
                <td colSpan={2} className="px-4 py-2.5 text-[11.5px] font-semibold text-slate-500">
                  Total
                </td>
                <td className="px-4 py-2.5 font-mono font-bold text-[#111827]">
                ...
                <td />
                <td className="px-4 py-2.5 font-mono font-bold text-[#16A34A]">
                  TZS {money(Math.round(totalCost))}k
                </td>
                <td />
              </tr>
...
export function ProjectBudget({ project, expensesHook, financeExpensesHook }) {
  ...
  const pctUsed =
    project.budget > 0 ? Math.min(100, Math.round((totalSpent / project.budget) * 100)) : 0;
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        const header = await sb("finance_expenses")
          .insert({
            vendor: financeDraft.vendor,
            category: "Project Costs",
            expense_date: financeDraft.date,
            due_date: financeDraft.dueDate,
          ...
          .run();
        if (header?.id)
          financeExpensesHook.setRows((prev) =>
            prev.map((e) => (e.id === financeDraft.id ? { ...e, dbId: header.id } : e)),
          );
      } catch (_e) {
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="flex-1 space-y-1.5">
                {byCategory.slice(0, 4).map((d) => (
                  <div key={d.name} className="flex items-center justify-between">
                    ...
                    <span className="text-[11.5px] font-mono font-bold" style={{ color: d.fill }}>
                      TZS {money(d.value)}k
                    </span>
                  </div>
                ))}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Budget used:{" "}
              <strong
                style={{ color: pctUsed >= 90 ? "#EF4444" : pctUsed >= 70 ? "#F59E0B" : "#16A34A" }}
              >
                {pctUsed}%
              </strong>
            </p>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div>
            <p className="text-[11px] text-slate-400 mb-1">Budget</p>
            <p className="text-[16px] font-mono font-semibold text-[#111827]">
              TZS {money(project.budget)}k
            </p>
          </div>
          <div>
            <p className="text-[11px] text-slate-400 mb-1">Spent</p>
            <p className="text-[16px] font-mono font-semibold text-[#F59E0B]">
              TZS {money(Math.round(totalSpent))}k
            </p>
          </div>
          <div>
            <p className="text-[11px] text-slate-400 mb-1">Remaining</p>
            <p
              className={`text-[16px] font-mono font-semibold ${remaining >= 0 ? "text-[#16A34A]" : "text-[#EF4444]"}`}
            >
              TZS {money(Math.round(remaining))}k
            </p>
          </div>
        </div>
        <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${pctUsed}%`,
              backgroundColor: pctUsed >= 100 ? "#EF4444" : pctUsed >= 80 ? "#F59E0B" : "#16A34A",
            }}
          />
        </div>
                ...
                <tr>
                  <td colSpan={3}>
                    <EmptyState
                      ...
                      hint="Log project costs to track spend against budget."
      ...
      {showForm && (
        <ProjectExpenseFormPanel onClose={() => setShowForm(false)} onSubmit={logExpense} />
      )}
    </div>
  );
}
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Expense</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Description" required>
            <input
              ...
              placeholder="e.g. Racking materials"
            />
          </FormField>
          ...
          <p className="text-[11.5px] text-slate-400">
            This creates a real "Project Costs" expense in Finance automatically.
          </p>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          <button
