...
export function Documents({ filesHook, company }) {
  ...
  async function addFile(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("documents")
          .insert({
            name: draft.name,
            file_type: draft.type,
            folder: draft.folder,
            size_label: draft.size,
            linked_record: draft.linkedRecord,
          ...
          .run();
        if (header?.id)
          setFiles((prev) => prev.map((f) => (f.id === draft.id ? { ...f, dbId: header.id } : f)));
      } catch (_e) {
        notify("File added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function addVersion(fileId, versionForm) {
    ...
    const newVersion = {
      ...
      date: TODAY.toISOString().slice(0, 10),
      size: versionForm.size || file.size,
      note: versionForm.note || "",
    };
    const updatedVersions = [
      ...(file.versions || []),
      {
        version: file.versions?.length ? file.versions[file.versions.length - 1].version : 0,
        date: file.date,
        size: file.size,
    ...
    setFiles((prev) =>
      prev.map((f) =>
        f.id === fileId
          ? { ...f, date: newVersion.date, size: newVersion.size, versions: updatedVersions }
          : f,
      ),
    );
    setSelected((s) =>
      s && s.id === fileId
        ? { ...s, date: newVersion.date, size: newVersion.size, versions: updatedVersions }
        : s,
    );
    ...
    if (IS_CONFIGURED && file.dbId) {
      try {
        await sb("documents")
          .eq("id", file.dbId)
          .update({
            size_label: newVersion.size,
            versions: updatedVersions,
            created_at: newVersion.date,
          })
          .run();
      } catch (_e) {
        notify("Version saved locally, but the server update failed.", "error");
      }
    }
  }
  ...
  async function deleteFile(id) {
    ...
    if (IS_CONFIGURED && file?.dbId) {
      try {
        await sb("documents").eq("id", file.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the file on the server.", "error");
      }
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            {files.length} files · {totalSizeLabel} stored
          </p>
        </div>
        <button
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-2.5 h-fit lg:sticky lg:top-0">
          <button
            ...
            className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-[13px] mb-0.5 transition-colors ${
              folder === "all"
                ? "bg-[#16A34A]/8 text-[#111827] font-medium"
                : "text-slate-600 hover:bg-slate-50"
            }`}
          >
            <span className="flex items-center gap-2">
              <FolderOpen size={15} className="text-slate-400" /> All Files
            </span>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-0.5 bg-slate-100 rounded-lg p-1 shrink-0">
              <button
                ...
                className={`p-1.5 rounded-md ${view === "grid" ? "bg-white shadow-sm text-[#111827]" : "text-slate-400"}`}
              >
                <Grid3x3 size={15} />
              </button>
              <button
                ...
                className={`p-1.5 rounded-md ${view === "list" ? "bg-white shadow-sm text-[#111827]" : "text-slate-400"}`}
              >
                <List size={15} />
              </button>
            </div>
          ...
          {loading ? (
            ...
            >
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-white rounded-xl border border-slate-200/80 p-4 h-24 skeleton-shimmer"
                />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
              <EmptyState
                ...
                hint={
                  files.length === 0
                    ? "Upload contracts, statements, and compliance records to keep them alongside the records they relate to."
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[14.5px] font-semibold text-[#111827] leading-snug break-words">
                {file.name}
              </p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-slate-50 rounded-lg p-3">
            <p className="text-[11px] text-slate-400 mb-1">
              {file.versions?.length > 0 ? "Current version" : "Uploaded"}
            </p>
        ...
        {file.linkedRecord && (
          <div className="flex items-center gap-2.5 text-[13px] text-slate-600 mb-5">
            <Tag size={14} className="text-slate-400" /> Linked to record{" "}
            <span className="font-mono text-[#16A34A] font-medium">{file.linkedRecord}</span>
          </div>
        )}
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="text-[11px] font-medium text-[#16A34A] hover:text-[#15803D]"
            >
              + New Version
            </button>
          </div>
          {!file.versions || file.versions.length === 0 ? (
            ...
            <div className="border border-slate-100 rounded-lg overflow-hidden">
              ...
              {[...file.versions].reverse().map((v) => (
                <div
                  key={v.version}
                  className="flex items-center justify-between px-3 py-2 text-[12px] border-t border-slate-50"
                >
        ...
        <div className="mb-5">
          ...
          {!file.content?.trim() ? (
            <p className="text-[12px] text-slate-400">
              No text content on this document yet — scan it with OCR or add text on upload to
            ...
            <button
              ...
              className="btn-secondary text-[12px] font-medium rounded-lg py-2 px-3 flex items-center gap-1.5 disabled:opacity-50"
            >
              {summaryBusy ? (
                <>
                  <LoaderCircle size={12} className="animate-spin" /> Summarizing...
                </>
              ) : (
                <>
                  <Brain size={12} /> Generate Summary
                </>
              )}
            </button>
          )}
      ...
      {showSignaturePad && (
        <DocumentSignaturePad
          ...
          onSigned={(entry) => {
            signatures.setRows((prev) => [entry, ...prev]);
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Version History</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Version</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="What changed?">
            <input
              ...
              placeholder="e.g. Updated pricing terms"
            />
          </FormField>
          <FormField label="New file size">
            <input
              className={inputClass}
              value={size}
              onChange={(e) => setSize(e.target.value)}
              placeholder="e.g. 1.3 MB"
            />
          </FormField>
          <p className="text-[11.5px] text-slate-400">
            The previous version moves into history below; this becomes the current one.
          </p>
        </div>
  ...
  async function save() {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Signed locally, but saving to the server failed.", "error");
      }
    }
  }
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">{documentId}</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Sign Document</h2>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600"
        ...
        <p className="text-[11.5px] text-slate-400 mb-4">
          A lightweight signature capture for internal sign-off — not a certified e-signature
          platform (no identity verification or legal audit trail). See Integrations for the same
          disclosure.
        </p>
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button
            ...
            className="flex-1 btn-primary text-white text-[12px] font-medium rounded-lg py-2 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Save Signature
          </button>
        </div>
...
export function FileFormPanel({ onClose, onSubmit }) {
  ...
  async function handleScanFile(e) {
    ...
    if (result.ok) {
      ...
      notify(
        result.text
          ? "Text extracted from the scan."
          : "OCR ran, but found no readable text in this image.",
      );
    } else {
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <div className="border-2 border-dashed border-slate-200 rounded-xl p-5 flex flex-col items-center text-center">
            ...
            <p className="text-[12px] text-slate-400 mb-3">
              This is a demo register — raw files aren&apos;t stored, but scanning one below does
              run real OCR and keeps the extracted text.
            </p>
            ...
            {ocrBusy && (
              <p className="text-[11px] text-slate-400 mt-2 flex items-center gap-1.5">
                <LoaderCircle size={11} className="animate-spin" /> Reading text... {ocrProgress}%
              </p>
            )}
          ...
          <FormField label="File name" required>
            <input
              ...
              placeholder="e.g. Vendor Agreement — ABC Ltd.pdf"
            />
            {touched && !form.name.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">File name is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Type">
              ...
              >
                {Object.entries(FILE_TYPE_STYLE).map(([k, v]) => (
          ...
          <FormField label="Linked record (optional)">
            <input
              ...
              placeholder="e.g. INV-8801, SO-2117, EMP-104"
            />
          </FormField>
          ...
          <FormField label="Text content (for search & AI summary)">
            <textarea
              ...
              placeholder="Populated automatically by Scan (OCR) above, or type/paste it yourself."
            />
            <p className="text-[11px] text-slate-400 mt-1">
              Full-text search and AI summaries both work from this — a document with no text here
              can only be found by its file name.
            </p>
          </FormField>
        </div>
