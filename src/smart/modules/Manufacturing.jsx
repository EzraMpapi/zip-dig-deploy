...
export function bomUnitCost(bom, inventoryRows) {
  const materials = bom.components.reduce(
    (s, c) => s + bomComponentCost(c.sku, inventoryRows) * c.qty,
    0,
  );
...
export function Manufacturing({ inventory, workOrdersHook, expensesHook }) {
  ...
  const boms = useCompanyTable("manufacturing_boms", bomsSeed, {
    select: "*,manufacturing_bom_components(*)",
    order: { col: "product_name", ascending: true },
    mapRow: mapBomRow,
  });
  ...
  const MFG_KPIS = [
    ...
    {
      label: "WIP Value",
      value: `TZS ${money(Math.round(stats.wipValue))}k`,
      delta: "Active orders",
      up: true,
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Bills of materials and production work orders
        </p>
      </div>
      ...
      {tab === "workorders" &&
        workOrders.length > 0 &&
        (() => {
          const woStatusData = ["Planned", "In Progress", "Completed", "Cancelled", "On Hold"]
            .map((s, i) => ({
              name: s,
          ...
          const woByCat = Object.entries(
            workOrders.reduce((m, w) => {
              const bom = boms.rows.find((b) => b.id === w.bomId);
              const cat = bom?.product_name || w.productName || "Unknown";
              m[cat] = (m[cat] || 0) + 1;
              return m;
            }, {}),
          )
...
export function BOMs({ boms, inventory }) {
  ...
  async function addBom(form) {
    const draft = {
      ...
      outputUnit: form.outputUnit || "unit",
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("manufacturing_boms")
          .insert({
            product_name: draft.product,
            output_unit: draft.outputUnit,
            labor_cost: draft.laborCost,
          })
          .single()
          .run();
        if (header?.id) {
          await sb("manufacturing_bom_components")
            .insert(
        ...
        notify("BOM created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteBom(id) {
    ...
    if (IS_CONFIGURED && b?.dbId) {
      try {
        await sb("manufacturing_boms").eq("id", b.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the BOM on the server.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
            ...
            >
              ...
              <div className="flex items-center justify-between text-[12px] text-slate-500">
                ...
                <span className="font-mono font-medium text-[#111827]">
                  TZS {money(bomUnitCost(bom, inventory.rows))}k / {bom.outputUnit}
                </span>
              </div>
            </button>
          ))}
        {!loading && rows.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              icon={ClipboardList}
              title="No BOMs yet"
              hint="Define what goes into a manufactured product so work orders can check material sufficiency and cost it accurately."
              actionLabel="New BOM"
      ...
      {showForm && (
        <BOMFormPanel inventory={inventory} onClose={() => setShowForm(false)} onSubmit={addBom} />
      )}
    </div>
  );
}
...
export function BOMPanel({ bom, inventory, onClose, onDelete }) {
  const materialsCost = bom.components.reduce(
    (s, c) => s + bomComponentCost(c.sku, inventory.rows) * c.qty,
    0,
  );
        ...
        <div className="px-6 py-5 flex-1">
          ...
          <div className="border border-slate-100 rounded-lg overflow-hidden mb-5">
            {bom.components.map((c, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex items-center justify-between px-3 py-2.5 text-[13px] ${i !== bom.components.length - 1 ? "border-b border-slate-50" : ""}`}
                >
                  <div className="min-w-0 pr-2">
                    <p className="text-slate-700 truncate">{item?.name || c.sku}</p>
                    <p className="text-[11px] text-slate-400 font-mono">
                      {c.qty} × TZS {money(bomComponentCost(c.sku, inventory.rows))}k
                    </p>
                  </div>
                  <span className="font-mono text-[#111827] shrink-0">
                    {money(c.qty * bomComponentCost(c.sku, inventory.rows))}k
                  </span>
                </div>
              );
            })}
          ...
          <div className="space-y-1.5 text-[13px]">
            <div className="flex justify-between text-slate-500">
              <span>Materials</span>
              <span className="font-mono">TZS {money(materialsCost)}k</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Labor</span>
              <span className="font-mono">TZS {money(bom.laborCost)}k</span>
            </div>
            <div className="flex justify-between text-[#111827] font-semibold text-[14px] pt-1.5 border-t border-slate-100 mt-1.5">
              <span>Cost per {bom.outputUnit}</span>
              <span className="font-mono">TZS {money(totalCost)}k</span>
            </div>
          </div>
...
export function BOMFormPanel({ inventory, onClose, onSubmit }) {
  ...
  function addComponent() {
    setComponents((prev) => [...prev, { sku: inventory.rows[0]?.sku || "", qty: "" }]);
  }
  ...
  const validComponents = components
    .filter((c) => c.sku && Number(c.qty) > 0)
    .map((c) => ({ sku: c.sku, qty: Number(c.qty) }));
  const materialsCost = validComponents.reduce(
    (s, c) => s + bomComponentCost(c.sku, inventory.rows) * c.qty,
    0,
  );
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Product name" required>
            <input
              ...
              placeholder="e.g. Cold Chain Storage Unit"
            />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Labor cost (TZS 000)">
              <input
                type="number"
                min="0"
                className={inputClass}
          ...
          <div>
            ...
            <button
              ...
              className="text-[12px] font-medium text-[#16A34A] hover:text-[#15803D] mt-2 flex items-center gap-1"
            >
          ...
          {validComponents.length > 0 && (
            <div className="bg-slate-50 rounded-lg p-3 text-[13px] space-y-1">
              <div className="flex justify-between text-slate-500">
                <span>Materials</span>
                <span className="font-mono">TZS {money(Math.round(materialsCost))}k</span>
              </div>
              <div className="flex justify-between font-semibold text-[#111827] pt-1 border-t border-slate-200">
                <span>Cost per {outputUnit || "unit"}</span>
                <span className="font-mono">TZS {money(Math.round(totalCost))}k</span>
              </div>
            </div>
          )}
        </div>
...
export function WorkOrders({ workOrders, setWorkOrders, inventory, boms, loading }) {
  ...
  async function addWorkOrder(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("manufacturing_work_orders")
          .insert({
            bom_id: form.bomId,
  ...
  async function advanceOrder(id, next) {
    ...
    if (next === "Completed" && order) {
      ...
      if (bom) {
        ...
        if (shortages.length) {
          setNotice(
            `Completed, but stock went negative-clamped for: ${shortages.join(", ")}. Check Inventory.`,
          );
        } else {
    ...
    if (IS_CONFIGURED && order?.dbId) {
      try {
        await sb("manufacturing_work_orders").eq("id", order.dbId).update({ status: next }).run();
      } catch (e) {
        notify("Couldn't save the work order status to the server.", "error");
      }
    }
  }
  ...
  async function deleteOrder(id) {
    ...
    if (IS_CONFIGURED && order?.dbId) {
      try {
        await sb("manufacturing_work_orders").eq("id", order.dbId).delete().run();
      } catch (e) {
        notify("Couldn't delete the work order on the server.", "error");
      }
    }
  }
  ...
  const wipValue = workOrders
    .filter((w) => w.status !== "Cancelled")
    .reduce((s, w) => {
      const b = boms.rows.find((b) => b.id === w.bomId);
      return s + (b ? bomUnitCost(b, inventory.rows) * w.qty : 0);
    }, 0);
      ...
      {notice && (
        <div className="flex items-start justify-between gap-3 bg-[#F59E0B]/[0.07] border border-[#F59E0B]/20 text-[#8a670a] text-[12.5px] rounded-lg px-3.5 py-2.5">
          <span className="flex items-start gap-2">
            <AlertCircle size={14} className="shrink-0 mt-0.5" /> {notice}
          </span>
          <button
            onClick={() => setNotice(null)}
            className="text-[#8a670a]/60 hover:text-[#8a670a] shrink-0"
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              ...
              [
                "Completion %",
                completionRate + "%",
                completionRate >= 80 ? "#16A34A" : completionRate >= 50 ? "#F59E0B" : "#EF4444",
              ],
              ["WIP Value", "TZS " + money(Math.round(wipValue)) + "k", "#7C3AED"],
            ].map(([l, v, col]) => (
              <div
                key={l}
                className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-3 text-center"
              >
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      ...
                      style={{
                        backgroundColor: `${WO_STATUS_COLOR[w.status]}14`,
                        color: WO_STATUS_COLOR[w.status],
                      }}
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ backgroundColor: WO_STATUS_COLOR[w.status] }}
                      />
                      {w.status}
                    </span>
                  </td>
...
export function WorkOrderPanel({ order, onClose, onAdvance, onDelete, inventory, boms }) {
  ...
  const requirements = (bom?.components || []).map((c) => {
    ...
    return {
      sku: c.sku,
      name: item?.name || c.sku,
      unit: item?.unit || "",
      required,
      onHand,
      short: required > onHand,
    };
  });
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100">
          ...
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${WO_STATUS_COLOR[order.status]}14`,
              color: WO_STATUS_COLOR[order.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: WO_STATUS_COLOR[order.status] }}
            />
            {order.status}
          </span>
        </div>
        ...
        <div className="px-6 py-5 flex-1">
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="bg-slate-50 rounded-lg p-3">
              ...
              <p className="text-[15px] font-mono font-semibold text-[#111827]">
                {order.qty} {bom?.outputUnit || "unit"}
              </p>
            </div>
            <div className="bg-slate-50 rounded-lg p-3">
              <p className="text-[11px] text-slate-400 mb-1">Est. Cost</p>
              <p className="text-[15px] font-mono font-semibold text-[#111827]">
                TZS {money(Math.round(unitCost * order.qty))}k
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3 mb-6 text-[13px]">
            <div className="flex items-center gap-2.5 text-slate-600">
              <ClipboardList size={14} className="text-slate-400" /> {bom?.id} — {bom?.product}
            </div>
            <div className="flex items-center gap-2.5 text-slate-600">
              <Star size={14} className="text-slate-400" /> Assigned to {order.assignedTo}
            </div>
            <div className="flex items-center gap-2.5 text-slate-600">
              <Clock size={14} className="text-slate-400" /> {order.startDate} → {order.dueDate}
            </div>
          </div>
          ...
          {bom && (
            <div>
              <p className="text-[11px] text-slate-400 mb-2 uppercase tracking-wide">
                Material requirement (×{order.qty})
              </p>
              <div className="border border-slate-100 rounded-lg overflow-hidden">
                {requirements.map((r, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between px-3 py-2.5 text-[13px] ${i !== requirements.length - 1 ? "border-b border-slate-50" : ""}`}
                  >
                    <div className="min-w-0 pr-2">
                      <p className="text-slate-700 truncate">{r.name}</p>
                      <p
                        className={`text-[11px] font-mono ${r.short ? "text-[#EF4444]" : "text-slate-400"}`}
                      >
                        {r.onHand} {r.unit} on hand{r.short ? " — insufficient" : ""}
                      </p>
                    </div>
                    <span
                      className={`font-mono shrink-0 ${r.short ? "text-[#EF4444] font-medium" : "text-[#111827]"}`}
                    >
                      {r.required} {r.unit}
                    </span>
                  </div>
              ...
              {order.status !== "Completed" && hasShortage && (
                <div className="flex items-start gap-2.5 bg-[#EF4444]/5 border border-[#EF4444]/20 rounded-lg p-3 mt-3">
                  ...
                  <p className="text-[12.5px] text-[#EF4444] leading-snug">
                    Inventory can&apos;t fully cover this run. Restock the flagged components or
                    reduce the quantity before completing.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label={`Quantity (${bom?.outputUnit || "unit"})`} required>
            <input
              ...
              placeholder="0"
            />
            {touched && !(Number(form.qty) > 0) && (
              <p className="text-[11px] text-[#EF4444] mt-1">Enter a quantity.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Due date" required>
              ...
              />
              {touched && !form.dueDate && (
                <p className="text-[11px] text-[#EF4444] mt-1">Due date is required.</p>
              )}
            </FormField>
          </div>
          ...
          {bom && (
            <div className="bg-slate-50 rounded-lg p-3 text-[12.5px] text-slate-500">
              ...
              <span className="font-mono text-[#111827] font-medium">
                TZS {money(Math.round(bomUnitCost(bom, inventory.rows) * (Number(form.qty) || 0)))}k
              </span>{" "}
              for {form.qty || 0} {bom.outputUnit}(s)
            </div>
          )}
        </div>
...
export function Machines() {
  ...
  async function addMachine(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("manufacturing_machines")
          .insert({
            name: draft.name,
            machine_type: draft.type,
          ...
          .run();
        if (header?.id)
          setRows((prev) => prev.map((m) => (m.id === draft.id ? { ...m, dbId: header.id } : m)));
      } catch (_e) {
        notify("Machine added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function setStatus(id, status) {
    ...
    if (IS_CONFIGURED && m?.dbId) {
      try {
        await sb("manufacturing_machines").eq("id", m.dbId).update({ status }).run();
      } catch (_e) {
        notify("Couldn't save the machine status to the server.", "error");
      }
    }
  }
  ...
  async function deleteMachine(id) {
    ...
    if (IS_CONFIGURED && m?.dbId) {
      try {
        await sb("manufacturing_machines").eq("id", m.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the machine on the server.", "error");
      }
    }
  }
              ...
              >
                <div className="flex items-center gap-2.5 mb-3">
                  <div className="w-9 h-9 rounded-lg bg-[#111827]/5 flex items-center justify-center">
                    <Cog size={16} className="text-[#111827]" />
                  </div>
                  <div>
                ...
                <span
                  className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                  style={{
                    backgroundColor: `${MACHINE_STATUS_COLOR[m.status]}14`,
                    color: MACHINE_STATUS_COLOR[m.status],
                  }}
                >
                  <span
                    className="w-1.5 h-1.5 rounded-full"
                    style={{ backgroundColor: MACHINE_STATUS_COLOR[m.status] }}
                  />
                  {m.status}
                </span>
              </button>
        ...
        {!loading && rows.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Track production equipment status and maintenance history here."
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-[13px] text-slate-500">{selected.type}</p>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="text-slate-400 hover:text-slate-600"
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {Object.keys(MACHINE_STATUS_COLOR).map((s) => (
                  <button
                    ...
                    className={`text-[11.5px] font-medium rounded-lg py-2 border transition-colors ${selected.status === s ? "border-[#16A34A] bg-[#16A34A]/8 text-[#111827]" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`}
                  >
                    {s}
                  </button>
                ))}
            ...
            <div className="flex items-center gap-2.5 text-[13px] text-slate-600 mb-6">
              <Clock size={14} className="text-slate-400" /> Purchased {selected.purchaseDate}
            </div>
            <div className="flex-1" />
            <ConfirmDeleteButton
              label="Remove machine"
              onConfirm={() => deleteMachine(selected.id)}
            />
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Manufacturing</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Machine</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Machine name" required>
            <input
              ...
              placeholder="e.g. CNC Panel Cutter #2"
            />
          </FormField>
...
export function QualityControl({ workOrders }) {
  const inspections = useCompanyTable("manufacturing_qc_inspections", qcInspectionsSeed, {
    order: { col: "inspection_date", ascending: false },
    mapRow: mapQcInspectionRow,
  });
  const { rows, setRows, loading } = inspections;
  const [showForm, setShowForm] = useState(false);

  const passRate = rows.length
    ? Math.round((rows.filter((r) => r.result === "Pass").length / rows.length) * 100)
    : null;
  const passColor =
    passRate === null
      ? "#6B7280"
  ...
  const defects = useMemo(() => {
    const map = {};
    rows
      .filter((r) => r.result === "Fail" && r.defects)
      .forEach((r) => {
        (r.defects || "")
          .split(",")
          .map((d) => d.trim())
  ...
  async function addInspection(form) {
    const draft = {
      ...
      notes: form.notes,
      inspection_date: TODAY.toISOString().slice(0, 10),
    };
    setRows((p) => [draft, ...p]);
    setShowForm(false);
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("manufacturing_qc_inspections")
          .insert({
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <ComposedChart data={trend} margin={{ left: -10, right: 4, top: 0, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(v, n) => [
                  n === "rate" ? v + "%" : v + " inspections",
                  n === "rate" ? "Pass Rate" : "Total",
                ]}
              />
              <Bar
                dataKey="total"
          ...
          {passRate !== null && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-[12px] font-bold shrink-0" style={{ color: passColor }}>
                {passRate}%
              </span>
              <span className="text-[11px] text-slate-400 shrink-0">
                {passRate >= 90 ? "Excellent" : passRate >= 70 ? "Acceptable" : "Needs Attention"}
              </span>
            </div>
          )}
        </div>
      ...
      {failedWOs.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          <p className="text-[13px] font-semibold text-red-700 mb-2">
            ⚠ {failedWOs.length} Failed Inspection{failedWOs.length > 1 ? "s" : ""} — Requires
            Action
          </p>
          <div className="flex flex-wrap gap-2">
            {failedWOs.slice(0, 5).map((f) => (
              <span
                key={f.id}
                className="text-[11.5px] bg-white border border-red-200 text-red-600 px-2.5 py-1 rounded-lg font-medium"
              >
          ...
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["Work Order", "Inspector", "Date", "Result", "Defects", "Notes"].map((h) => (
                  <th
                    key={h}
              ...
              {rows.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-400 text-[11.5px] max-w-[140px] truncate">
                      {r.defects || "—"}
                    </td>
                    <td className="px-4 py-3 text-slate-400 text-[11.5px] max-w-[120px] truncate">
                      {r.notes || "—"}
                    </td>
                  </tr>
                );
...
export function QcFormPanel({ workOrders, onClose, onSubmit }) {
  ...
  const [form, setForm] = useState({
    workOrderId: completedOrders[0]?.id || "",
    inspector: "",
    result: "Pass",
    defectsFound: "0",
    notes: "",
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.workOrderId || !form.inspector.trim()) return;
    onSubmit(form);
  }
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Quality Control</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Inspection</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Work order">
            {completedOrders.length === 0 ? (
              <p className="text-[12.5px] text-slate-400">
                No completed work orders yet to inspect.
              </p>
            ) : (
              <select
                className={inputClass}
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Result">
              ...
              >
                {Object.keys(QC_RESULT_COLOR).map((r) => (
            ...
            <FormField label="Defects found">
              <input
                ...
                onChange={(e) => set("defectsFound", e.target.value)}
              />
            </FormField>
          </div>
...
export function Maintenance({ expensesHook }) {
  const records = useCompanyTable("manufacturing_maintenance", maintenanceSeed, {
    order: { col: "maintenance_date", ascending: false },
  ...
  async function addRecord(form) {
    ...
    if (draft.cost > 0) {
      const expenseDraft = {
        id: docId("EX"),
        vendor: `Maintenance — ${draft.machine}`,
      ...
      if (IS_CONFIGURED) {
        try {
          ...
          /* the maintenance record itself still saves below; a failed expense sync is reported there */
        }
      }
    }
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("manufacturing_maintenance")
          .insert({
            machine_name: draft.machine,
            maintenance_type: draft.type,
            technician: draft.technician,
            maintenance_date: draft.date,
            cost: draft.cost,
            notes: draft.notes,
      ...
      {overdue.length > 0 && (
        <div className="flex items-start gap-2.5 bg-[#EF4444]/5 border border-[#EF4444]/20 rounded-lg p-3">
          ...
          <p className="text-[12px] text-[#8a1a15] leading-relaxed">
            {overdue.length} machine{overdue.length > 1 ? "s are" : " is"} past due for maintenance:{" "}
            {overdue.map((r) => r.machine).join(", ")}.
          </p>
        </div>
      )}
              ...
              return (
                <div>
                  <p className="text-[12px] text-slate-400 mb-2">
                    YTD total:{" "}
                    <strong className="text-[#111827]">TZS {money(Math.round(totalCost))}k</strong>
                  </p>
                  <ResponsiveContainer width="100%" height={120}>
                ...
                return upcoming.map((r) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ borderColor: col + "30", background: col + "05" }}
                    >
                      <div
                        className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-white font-bold text-[11px] text-center leading-tight"
          ...
          <table className="w-full text-[13px] min-w-[720px]">
            <thead>
              <tr className="border-b border-slate-100 text-left text-[11px] text-slate-400 uppercase tracking-wide">
                ...
                <th className="px-4 py-3 font-medium text-right">Cost (TZS 000)</th>
              </tr>
            </thead>
            <tbody>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-slate-50 last:border-0">
                      ...
                      <td
                        className={`px-4 py-3 font-mono ${isOverdue ? "text-[#EF4444] font-medium" : "text-slate-500"}`}
                      >
                        {r.nextDueDate || "—"}
                        {isOverdue && " (overdue)"}
                      </td>
                ...
                <tr>
                  <td colSpan={5}>
                    <EmptyState
                      ...
                      hint="Track preventive and corrective maintenance per machine here."
...
export function MaintenanceFormPanel({ onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.machine.trim() || !form.technician.trim()) return;
    onSubmit(form);
  }
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Maintenance</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Maintenance</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Machine" required>
            <input
              ...
              placeholder="e.g. Powder Coat Booth"
            />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Type">
            ...
            <FormField label="Technician" required>
              <input
                ...
                placeholder="e.g. S. Kileo"
              />
            </FormField>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Cost (TZS 000)">
              <input
                type="number"
                min="0"
                className={inputClass}
          ...
          <p className="text-[11.5px] text-slate-400">
            A non-zero cost creates a real "Maintenance" expense in Finance automatically.
          </p>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          <button
