...
export function CustomerSupport({ company }) {
  ...
  const tickets = useCompanyTable("support_tickets", supportTicketsSeed, {
    ...
    order: { col: "created_date", ascending: false },
    mapRow: mapTicketRow,
  });

  const openCount = tickets.rows.filter((t) => t.status === "Open").length;
  const urgentCount = tickets.rows.filter(
    (t) => t.status !== "Closed" && t.status !== "Resolved" && t.priority === "Urgent",
  ).length;
  const resolvedCount = tickets.rows.filter(
    (t) => t.status === "Resolved" || t.status === "Closed",
  ).length;
  const resolutionRate = tickets.rows.length
    ? Math.round((resolvedCount / tickets.rows.length) * 100)
    : null;
  ...
  const SUPPORT_KPIS = [
    {
      ...
      delta: `${tickets.rows.length} total`,
      up: false,
      icon: Ticket,
    },
    {
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Tickets, live chat, knowledge base, call log, and AI-drafted replies
        </p>
      </div>
...
export function Tickets({ tickets }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (t) => t.subject.toLowerCase().includes(q) || t.customer.toLowerCase().includes(q),
    );
  }, [rows, query]);
  ...
  async function addTicket(form) {
    const draft = {
      ...
      messages: form.description
        ? [{ from: "Customer", text: form.description, date: TODAY.toISOString().slice(0, 10) }]
        : [],
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("support_tickets")
          .insert({
            doc_number: draft.id,
        ...
        if (header?.id) {
          if (form.description) {
            await sb("support_ticket_messages")
              .insert({ ticket_id: header.id, sender: "Customer", body: form.description })
              .run();
          }
          setRows((prev) => prev.map((t) => (t.id === draft.id ? { ...t, dbId: header.id } : t)));
        }
      } catch (_e) {
        notify("Ticket created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function setStatus(id, status) {
    ...
    if (IS_CONFIGURED && t?.dbId) {
      try {
        await sb("support_tickets").eq("id", t.dbId).update({ status }).run();
      } catch (_e) {
        notify("Couldn't save the ticket status to the server.", "error");
      }
    }
  }
  ...
  async function reply(id, text) {
    ...
    setRows((prev) =>
      prev.map((x) => (x.id === id ? { ...x, messages: [...x.messages, message] } : x)),
    );
    setSelected((s) => (s && s.id === id ? { ...s, messages: [...s.messages, message] } : s));
    if (IS_CONFIGURED && t?.dbId) {
      try {
        await sb("support_ticket_messages")
          .insert({ ticket_id: t.dbId, sender: "Agent", body: text })
          .run();
      } catch (_e) {
        notify("Reply saved locally, but the server update failed.", "error");
      }
    }
  }
  ...
  async function deleteTicket(id) {
    ...
    if (IS_CONFIGURED && t?.dbId) {
      try {
        await sb("support_tickets").eq("id", t.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the ticket on the server.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {rows.length > 0 &&
        (() => {
          const statusData = ["Open", "In Progress", "Waiting", "Resolved", "Closed"]
            .map((s, i) => ({
              name: s,
          ...
          const resolvedToday = rows.filter(
            (r) =>
              r.status === "Resolved" && r.updatedAt?.startsWith(TODAY.toISOString().slice(0, 10)),
          ).length;
          return (
                ...
                filtered.map((t) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <p className="font-medium text-[#111827]">{t.subject}</p>
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${TICKET_PRIORITY_COLOR[t.priority]}14`,
                          color: TICKET_PRIORITY_COLOR[t.priority],
                        }}
                      >
                        {t.priority}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${TICKET_STATUS_COLOR[t.status]}14`,
                          color: TICKET_STATUS_COLOR[t.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: TICKET_STATUS_COLOR[t.status] }}
                        />
                        {t.status}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={5}>
                    <EmptyState
                      ...
                      hint="Customer issues will appear here for tracking and resolution."
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100">
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="text-[11px] font-mono text-slate-400">{ticket.id}</p>
              <h2 className="text-[16px] font-semibold text-[#111827] mt-0.5 leading-snug">
                {ticket.subject}
              </h2>
            </div>
            <button
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
              style={{
                backgroundColor: `${TICKET_STATUS_COLOR[ticket.status]}14`,
                color: TICKET_STATUS_COLOR[ticket.status],
              }}
            >
              <span
                className="w-1.5 h-1.5 rounded-full"
                style={{ backgroundColor: TICKET_STATUS_COLOR[ticket.status] }}
              />
              {ticket.status}
            </span>
            <span
              className="text-[11px] font-medium px-1.5 py-0.5 rounded"
              style={{
                backgroundColor: `${TICKET_PRIORITY_COLOR[ticket.priority]}14`,
                color: TICKET_PRIORITY_COLOR[ticket.priority],
              }}
            >
              {ticket.priority}
            </span>
        ...
        <div className="px-6 py-5 flex-1 space-y-3">
          {ticket.messages.map((m, i) => (
            <div key={i} className={`flex ${m.from === "Agent" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[85%] rounded-xl px-3.5 py-2.5 text-[13px] ${m.from === "Agent" ? "btn-primary text-white rounded-br-sm" : "bg-slate-50 text-slate-700 border border-slate-100 rounded-bl-sm"}`}
              >
                <p className="whitespace-pre-wrap leading-relaxed">{m.text}</p>
                <p
                  className={`text-[10.5px] mt-1 ${m.from === "Agent" ? "text-white/70" : "text-slate-400"}`}
                >
                  {m.from} · {m.date}
                </p>
              </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 space-y-3">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="btn-primary text-white px-4 rounded-lg shrink-0"
            >
              <Send size={15} />
            </button>
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {TICKET_STATUSES.map((s) => (
              <button
                key={s}
                onClick={() => onSetStatus(ticket.id, s)}
                disabled={s === ticket.status}
                className={`text-[11px] font-medium rounded-md px-2 py-1 border transition-colors ${s === ticket.status ? "opacity-40 cursor-not-allowed border-slate-200" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
              >
                {s}
              </button>
            ))}
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Customer Support</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Ticket</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Subject" required>
            ...
            />
            {touched && !form.subject.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Subject is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Priority">
              ...
              >
                {Object.keys(TICKET_PRIORITY_COLOR).map((p) => (
...
export function LiveChat() {
  ...
  async function sendReply() {
    ...
    if (IS_CONFIGURED && selected.dbId) {
      try {
        await sb("support_chat_messages")
          .insert({ conversation_id: selected.dbId, sender: "Agent", body: message.text })
          .run();
      } catch (_e) {
        notify("Reply saved locally, but the server update failed.", "error");
      }
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
        <EmptyState
          ...
          hint="Customer chat conversations will appear here."
        />
      </div>
    );
  ...
  return (
    <div
      className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden"
        ...
        {rows.map((c) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left px-4 py-3 border-b border-slate-50 transition-colors ${selected?.id === c.id ? "bg-slate-50" : "hover:bg-slate-50/60"}`}
            >
              <div className="flex items-center justify-between mb-1">
                <p className="text-[13px] font-medium text-[#111827] truncate">{c.customer}</p>
                <span
                  className="w-1.5 h-1.5 rounded-full shrink-0"
                  style={{ backgroundColor: CHAT_STATUS_COLOR[c.status] }}
                />
              </div>
        ...
        <div className="flex flex-col">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <span
                ...
                style={{ color: CHAT_STATUS_COLOR[selected.status] }}
              >
                {selected.status}
              </span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {selected.messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[75%] rounded-xl px-3.5 py-2.5 text-[13px] ${m.from === "Agent" ? "btn-primary text-white rounded-br-sm" : "bg-slate-50 text-slate-700 border border-slate-100 rounded-bl-sm"}`}
                >
                  <p className="whitespace-pre-wrap leading-relaxed">{m.text}</p>
                  <p
                    className={`text-[10.5px] mt-1 ${m.from === "Agent" ? "text-white/70" : "text-slate-400"}`}
...
export function KnowledgeBase() {
  const articles = useCompanyTable("kb_articles", kbArticlesSeed, {
    order: { col: "updated_at", ascending: false },
    mapRow: mapKbArticleRow,
  });
  ...
  async function addArticle(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("kb_articles")
          .insert({
            title: draft.title,
            category: draft.category,
        ...
        notify("Article created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function openArticle(article) {
    ...
    if (IS_CONFIGURED && article.dbId) {
      try {
        await sb("kb_articles").eq("id", article.dbId).update({ views: updated.views }).run();
      } catch (_e) {
        /* view-count sync failure isn't worth surfacing to the reader */
      }
    }
  }
  ...
  async function deleteArticle(id) {
    ...
    if (IS_CONFIGURED && a?.dbId) {
      try {
        await sb("kb_articles").eq("id", a.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the article on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
          ...
          filtered.map((a) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <span className="text-[10px] font-medium text-[#16A34A] bg-[#16A34A]/8 px-1.5 py-0.5 rounded">
                  {a.category}
                </span>
                {!a.published && (
              ...
              <p className="text-[13.5px] font-semibold text-[#111827] leading-snug mb-2 line-clamp-2">
                {a.title}
              </p>
              <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
                <Eye size={11} /> {a.views} views
              </div>
            </button>
          ))}
        {!loading && filtered.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Write help articles your team and customers can reference."
      ...
      {selected && (
        <div className="fixed inset-0 z-30 flex justify-end">
          ...
          />
          <div
            className="relative w-full sm:w-[440px] bg-white h-full shadow-2xl p-6 overflow-y-auto flex flex-col"
            style={{ animation: "slideIn .15s ease-out" }}
          >
            ...
            <p className="text-[13px] text-slate-600 leading-relaxed whitespace-pre-wrap mb-6">
              {selected.content}
            </p>
            ...
            <ConfirmDeleteButton
              ...
              onConfirm={() => deleteArticle(selected.id)}
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Knowledge Base</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Article</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Title" required>
            <input
              ...
              placeholder="e.g. How to request a bulk quote"
            />
          </FormField>
          <FormField label="Category">
            <select
...
export function CallCenter() {
  const calls = useCompanyTable("support_call_log", callLogSeed, {
    order: { col: "call_date", ascending: false },
    mapRow: mapCallLogRow,
  });
  const { rows, setRows, loading } = calls;
  const [showForm, setShowForm] = useState(false);
  const avgDuration = rows.length
    ? Math.round(rows.reduce((s, c) => s + c.duration, 0) / rows.length)
    : 0;
                  ...
                  <tr key={c.id} className="border-b border-slate-50 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-1.5 py-0.5 rounded"
                        style={{
                          backgroundColor: `${CALL_DIRECTION_COLOR[c.direction]}14`,
                          color: CALL_DIRECTION_COLOR[c.direction],
                        }}
                      >
                        {c.direction}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                        style={{
                          backgroundColor: `${CALL_OUTCOME_COLOR[c.outcome]}14`,
                          color: CALL_OUTCOME_COLOR[c.outcome],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: CALL_OUTCOME_COLOR[c.outcome] }}
                        />
                        {c.outcome}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      hint="Track inbound and outbound support calls here."
...
export function CallFormPanel({ onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.customer.trim() || !form.agent.trim()) return;
    onSubmit(form);
  }
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Call Center</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Call</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Direction">
              ...
              >
                {Object.keys(CALL_DIRECTION_COLOR).map((d) => (
          ...
          <FormField label="Outcome">
            ...
            >
              {Object.keys(CALL_OUTCOME_COLOR).map((o) => (
    ...
    <div className="max-w-2xl space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Drafts a suggested reply for a ticket using a real Claude API call — review before
          sending, this doesn&apos;t reply automatically. A fully autonomous customer-facing bot
          needs a server-side proxy for the API key, the same prerequisite documented for the main
          AI Assistant.
        </p>
      </div>
      ...
      {tickets.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            icon={Brain}
            title="No tickets to draft replies for"
            hint="Create a ticket first."
          />
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
          ...
          {ticket && (
            <div className="border border-slate-100 rounded-lg p-3 max-h-[160px] overflow-y-auto space-y-1.5">
              {ticket.messages.map((m, i) => (
                <p key={i} className="text-[12.5px] text-slate-600">
                  <span className="font-medium text-[#111827]">{m.from}:</span> {m.text}
                </p>
              ))}
              {ticket.messages.length === 0 && (
                <p className="text-[12.5px] text-slate-400">No messages on this ticket yet.</p>
              )}
            </div>
          )}
          ...
          >
            {busy ? (
              <>
                <LoaderCircle size={14} className="animate-spin" /> Drafting reply...
              </>
            ) : (
              <>
                <Brain size={14} /> Generate AI Reply
              </>
            )}
          </button>
