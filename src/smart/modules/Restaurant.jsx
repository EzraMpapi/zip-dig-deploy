...
export function RestaurantModule({ currentUser, company }) {
  ...
  const reservations = useCompanyTable("rst_reservations", RST_RESERVATIONS_SEED, {
    mapRow: (r) => r,
  });
  ...
  const occupancy =
    tables.rows.length > 0 ? ((occupiedTbls / tables.rows.length) * 100).toFixed(0) : 0;
  ...
  async function placeOrder() {
    if (!activeTable || cart.length === 0) {
      notify("Select a table and add items", "error");
      return;
    }
    const tbl = tables.rows.find((t) => t.id === activeTable);
    const row = {
      id: docId("ORD"),
      table: tbl?.number || activeTable,
    ...
    tables.setRows((p) =>
      p.map((t) =>
        t.id === activeTable
          ? { ...t, status: "Occupied", waiter: selectedWaiter, currentOrder: row.id }
          : t,
      ),
    );
    setCart([]);
    ...
    logAudit(
      ...
      "Table " + tbl?.number + ", TZS " + money(cartTotal) + "k",
    );
  }
      ...
      p.map((o) =>
        o.id === orderId
          ? {
              ...o,
              status,
              kitchen: status === "Ready" ? "Ready" : status === "Paid" ? "Served" : "In Progress",
            }
          : o,
      ),
    );
    if (status === "Paid") {
      ...
      if (ord) {
        ...
        tables.setRows((p) =>
          p.map((t) =>
            t.number === ord.table
              ? { ...t, status: "Cleaning", waiter: "", currentOrder: null }
              : t,
          ),
        );
      }
  ...
  async function addReservation() {
    ...
    const row = {
      ...resvForm,
      id: docId("RES"),
      covers: Number(resvForm.covers) || 2,
      status: "Pending",
    };
    ...
    if (resvForm.table) {
      ...
      if (tblId)
        tables.setRows((p) => p.map((t) => (t.id === tblId ? { ...t, status: "Reserved" } : t)));
    }
  ...
  const filteredMenu =
    menuCat === "All" ? menuItems.rows : menuItems.rows.filter((m) => m.category === menuCat);
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg,#7F1D1D 0%,#B91C1C 40%,#C2410C 100%)" }}
      >
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.6)" }}>
              Tables · Orders · Kitchen · Menu · Reservations · Billing
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            {[
              ["Tables", tables.rows.length],
              ["Occupied", occupiedTbls + " (" + occupancy + "%)"],
              ["Active Orders", pendingOrders.length],
              ["Revenue", TZS_FMT(todayRevenue)],
            ].map(([l, v]) => (
              <div
                key={l}
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={{ background: tab === t.id ? RST_RED : "transparent" }}
            >
              <I size={12} />
              {t.label}
              {t.id === "kitchen" && pendingOrders.length > 0 && (
                <span className="ml-0.5 bg-yellow-400 text-yellow-900 text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
            ...
            {Object.entries({
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11.5px] text-slate-400 mt-0.5">{s}</p>
                </div>
              );
            })}
          </div>
            ...
            {TABLE_ZONES.map((zone) => {
              ...
              return (
                <div key={zone} className="mb-5 last:mb-0">
                  <p className="text-[11.5px] font-semibold text-slate-400 uppercase tracking-wide mb-2.5">
                    {zone}
                  </p>
                  <div className="flex flex-wrap gap-3">
                    {zoneTables.map((t) => {
                      ...
                      const ord = t.currentOrder
                        ? orders.rows.find((o) => o.id === t.currentOrder)
                        : null;
                      return (
                        <div
                          key={t.id}
                          ...
                          className="w-36 rounded-2xl p-3 cursor-pointer hover:shadow-lg transition-all border-2"
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-1.5">
                              ...
                              />
                              <span className="text-[14px] font-black" style={{ color: s.text }}>
                                {t.number}
                              </span>
                            </div>
                          ...
                          {t.waiter && (
                            <p className="text-[10px] text-slate-500 truncate mt-0.5">{t.waiter}</p>
                          )}
                          {ord && (
                            <p className="text-[11px] font-bold mt-1.5" style={{ color: s.text }}>
                              TZS {money(ord.total)}k
                            </p>
                          )}
                          {t.status === "Cleaning" && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                tables.setRows((p) =>
                                  p.map((x) => (x.id === t.id ? { ...x, status: "Available" } : x)),
                                );
                                notify("Table " + t.number + " ready");
                              }}
                              className="mt-2 w-full text-[10px] font-bold py-1 rounded-lg bg-white border"
                ...
                {["All", ...MENU_CATEGORIES].map((cat) => (
                  <button
                    ...
                    className={
                      ...
                      (menuCat === cat
                        ? "text-white"
                        : "text-slate-500 bg-white border border-slate-200 hover:border-red-300")
                    }
                    style={{ background: menuCat === cat ? RST_RED : "white" }}
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {filteredMenu.map((item) => (
                <div
                  ...
                  className={
                    "bg-white rounded-xl border shadow-sm p-3 cursor-pointer transition-all " +
                    (item.available
                      ? "hover:border-red-300 hover:shadow-md"
                      : "opacity-50 cursor-not-allowed")
                  }
                  style={{
                    borderColor: cart.find((c) => c.id === item.id) ? "#EF4444" : "#E5E7EB",
                  }}
                >
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-[13px] font-bold" style={{ color: RST_RED }}>
                      {TZS_FMT(item.price)}
                    </p>
                    {item.popular && (
                      <span className="text-[9.5px] font-bold px-1.5 py-0.5 rounded-full bg-yellow-50 text-yellow-700">
                        ★ Popular
                      </span>
                    )}
                  </div>
                  {!item.available && (
                    <p className="text-[10px] text-red-400 font-semibold mt-1">Unavailable</p>
                  )}
                  {cart.find((c) => c.id === item.id) && (
                    <div className="mt-1.5 bg-red-600 text-white text-[10px] font-bold text-center py-0.5 rounded-lg">
                      In Order ({cart.find((c) => c.id === item.id).qty})
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-slate-100" style={{ background: RST_RED }}>
                ...
                <div className="flex gap-2 mt-2">
                  <select
                    className="flex-1 bg-white/20 text-white border border-white/30 rounded-lg px-2 py-1.5 text-[11.5px]"
                    value={activeTable || ""}
                    onChange={(e) => setActiveTable(e.target.value)}
                  >
                    <option value="">Select Table</option>
                    {tables.rows
                      .filter((t) => t.status === "Available" || t.status === activeTable)
                      .map((t) => (
                        <option key={t.id} value={t.id}>
                          {t.number} — {t.seats} seats ({t.zone})
                        </option>
                      ))}
                  </select>
                  <select
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400">{TZS_FMT(item.price)}</p>
                        </div>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => updateQty(item.id, -1)}
                            className="w-6 h-6 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-[14px] font-bold hover:bg-red-100 hover:text-red-600"
                          >
                            −
                          </button>
                          <span className="text-[12.5px] font-bold w-5 text-center">
                            {item.qty}
                          </span>
                          <button
                            onClick={() => updateQty(item.id, 1)}
                            className="w-6 h-6 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-[14px] font-bold hover:bg-green-100 hover:text-green-600"
                          >
                            +
                          </button>
                        </div>
                        <span className="text-[12px] font-bold text-[#111827] w-16 text-right">
                          {TZS_FMT(item.price * item.qty)}
                        </span>
                        <button
                ...
                <div className="border-t border-slate-100 p-3 space-y-2">
                  ...
                  <div className="space-y-1 text-[12.5px]">
                    <div className="flex justify-between">
                      ...
                      <span>{TZS_FMT(cartSubtotal)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Tax (10%)</span>
                      <span>{TZS_FMT(cartTax)}</span>
                    </div>
                    <div className="flex justify-between font-bold text-[14px] border-t border-slate-100 pt-1.5">
                      <span>Total</span>
                      <span style={{ color: RST_RED }}>{TZS_FMT(cartTotal)}</span>
                    </div>
                  </div>
                  <button
                    onClick={placeOrder}
                    className="w-full py-3 rounded-xl text-[13.5px] font-bold text-white"
                    style={{ background: RST_RED }}
                  >
                    🍽️ Send to Kitchen
                  </button>
                  <button
                    onClick={() => setCart([])}
                    className="w-full py-2 rounded-xl text-[12px] text-slate-500 border border-slate-200"
                  >
                    Clear Order
                  </button>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {pendingOrders.length === 0 ? (
                  <p className="text-slate-400 text-[12px] text-center py-2">No active orders</p>
                ) : (
                  pendingOrders.map((o) => (
                    <div key={o.id} className="p-2.5 rounded-lg border border-slate-100">
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="text-[12.5px] font-bold text-[#111827]">Table {o.table}</p>
                          <p className="text-[11px] text-slate-400">
                            {o.items.length} items · {o.timeIn}
                          </p>
                        </div>
                        <div className="flex gap-1">
                          {o.status === "Preparing" && (
                            <button
                              ...
                              className="text-[10.5px] font-bold text-white px-2 py-1 rounded-lg bg-yellow-500"
                            >
                              Ready
                            </button>
                          )}
                          {o.status === "Ready" && (
                            <button
                              ...
                              className="text-[10.5px] font-bold text-white px-2 py-1 rounded-lg bg-green-600"
                            >
                              Bill & Pay
                            </button>
                          )}
      ...
      {tab === "kitchen" && (
        <div className="space-y-4">
          <div className="flex gap-2">
            {[
              ["active", "Active Orders", "#B91C1C"],
              ["ready", "Ready to Serve", "#16A34A"],
              ["all", "All Today", "#2563EB"],
            ].map(([id, label, col]) => (
              <button
                key={id}
                ...
                style={{
                  background: kitchenTab === id ? col : "white",
                  color: kitchenTab === id ? "white" : col,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              .map((o) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      borderColor: isReady ? "#16A34A" : isPreparing ? "#F59E0B" : "#E5E7EB",
                    }}
                  >
                    <div
                      className="px-4 py-3 flex items-center justify-between"
                      style={{
                        background: isReady ? "#16A34A" : isPreparing ? "#F59E0B" : "#F3F4F6",
                      }}
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-[22px] font-black text-white">{o.table}</span>
                        <div>
                          <p className="text-[11px] text-white/80">{o.waiter}</p>
                          <p className="text-[11px] text-white/70">{o.timeIn}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-[12px] font-bold text-white">
                          {isReady ? "✓ READY" : isPreparing ? "⏳ COOKING" : "✅ DONE"}
                        </p>
                        <p className="text-[10px] text-white/70">{o.id}</p>
                      </div>
                    </div>
                    <div className="p-4">
                        ...
                        {o.items.map((item, i) => (
                          <div key={i} className="flex items-center gap-3">
                            <span className="text-[22px]">
                              {menuItems.rows.find((m) => m.id === item.id)?.image || "🍽️"}
                            </span>
                            ...
                            <span
                              ...
                              style={{ color: isPreparing ? "#F59E0B" : "#16A34A" }}
                            >
                              ×{item.qty}
                            </span>
                          </div>
                      ...
                      <div className="flex gap-2">
                        {isPreparing && (
                          <button
                            onClick={() => updateOrderStatus(o.id, "Ready")}
                            className="flex-1 py-2.5 rounded-xl text-[12.5px] font-bold text-white bg-green-600"
                          >
                            ✓ Mark Ready
                          </button>
                        )}
                        {isReady && (
                          <button
                            onClick={() => updateOrderStatus(o.id, "Paid")}
                            className="flex-1 py-2.5 rounded-xl text-[12.5px] font-bold text-white"
                            style={{ background: RST_RED }}
                          >
                            Bill & Pay
                          </button>
            ...
            {orders.rows.filter((o) =>
              ...
              <div className="col-span-full text-center py-16 text-slate-300">
                <ChefHat size={40} className="mx-auto mb-3" />
      ...
      {tab === "menu" && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ["Menu Items", menuItems.rows.length, "#B91C1C"],
              ["Available", menuItems.rows.filter((m) => m.available).length, "#16A34A"],
              ["Popular", menuItems.rows.filter((m) => m.popular).length, "#F59E0B"],
              ["Categories", MENU_CATEGORIES.length, "#7C3AED"],
            ].map(([l, v, col]) => (
              <div
                key={l}
                className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-semibold text-[#111827]">Menu Items</p>
              <button
                onClick={() => notify("Add menu item")}
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <span
                          ...
                          style={{ background: RST_RED + "15", color: RST_RED }}
                        >
                          {item.category}
                        </span>
                      </td>
                      <td className="px-3 py-3 font-bold" style={{ color: RST_RED }}>
                        {TZS_FMT(item.price)}
                      </td>
                      ...
                      <td
                        ...
                        style={{
                          color: margin > 50 ? "#16A34A" : margin > 30 ? "#F59E0B" : "#EF4444",
                        }}
                      >
                        {margin}%
                      </td>
        ...
        <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <FormField label="Guest Name *">
                  <input
                    ...
                    value={resvForm.name}
                    onChange={(e) => setResvForm({ ...resvForm, name: e.target.value })}
                  />
                </FormField>
                ...
                <FormField label="Date *">
                  <input
                    ...
                    className={inputClass}
                    value={resvForm.date}
                    onChange={(e) => setResvForm({ ...resvForm, date: e.target.value })}
                  />
                </FormField>
                <FormField label="Time *">
                  <input
                    ...
                    className={inputClass}
                    value={resvForm.time}
                    onChange={(e) => setResvForm({ ...resvForm, time: e.target.value })}
                  />
                </FormField>
                <FormField label="Covers (guests)">
                  <input
                    type="number"
                    min="1"
                    className={inputClass}
                    value={resvForm.covers}
                    onChange={(e) => setResvForm({ ...resvForm, covers: e.target.value })}
                  />
                </FormField>
                <FormField label="Table">
                  <select
                    className={inputClass}
                    value={resvForm.table}
                    onChange={(e) => setResvForm({ ...resvForm, table: e.target.value })}
                  >
                    <option value="">Select table...</option>
                    {tables.rows
                ...
                <FormField label="Special Note" cls="col-span-2">
                  <input
                    className={inputClass}
                    value={resvForm.note}
                    onChange={(e) => setResvForm({ ...resvForm, note: e.target.value })}
          ...
          <div className="space-y-3">
            {reservations.rows.map((r) => {
              ...
              }[r.status] || ["#F3F4F6", "#6B7280"];
              return (
                <div
                  key={r.id}
                  className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 flex items-center gap-4 flex-wrap"
                >
                  <div
                    className="w-16 h-16 rounded-2xl flex flex-col items-center justify-center shrink-0"
                    style={{ background: RST_RED + "15" }}
                  >
                    <p className="text-[10.5px] font-bold text-red-700">
                      {r.date?.slice(5, 7)}/{r.date?.slice(8, 10)}
                    </p>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] text-slate-400">
                      {r.covers} covers · Table {r.table || "TBA"} · {r.phone}
                    </p>
                    {r.note && (
                      <p className="text-[12px] text-orange-500 font-medium mt-0.5">📝 {r.note}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full"
                      style={{ background: sty[0], color: sty[1] }}
                    >
                      {r.status}
                    </span>
                    {r.status === "Pending" && (
                      <button
                        onClick={() =>
                          reservations.setRows((p) =>
                            p.map((x) => (x.id === r.id ? { ...x, status: "Confirmed" } : x)),
                          )
                        }
      ...
      {tab === "reports" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ["Total Orders", todayOrders.length, "#B91C1C"],
              ["Revenue", TZS_FMT(todayRevenue), "#16A34A"],
              [
                "Avg Order Value",
                TZS_FMT(
                  todayOrders.length > 0
                    ? todayRevenue /
                        Math.max(todayOrders.filter((o) => o.status === "Paid").length, 1)
                    : 0,
                ),
                "#2563EB",
              ],
              ["Tables Served", new Set(todayOrders.map((o) => o.table)).size, "#7C3AED"],
            ].map(([l, v, col]) => (
              <div
                key={l}
                className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {menuItems.rows
              .map((m) => {
                const sold = orders.rows
                  .flatMap((o) => o.items)
                  .filter((i) => i.id === m.id)
                  .reduce((s, i) => s + i.qty, 0);
                const rev = orders.rows
                  .flatMap((o) => o.items)
                  .filter((i) => i.id === m.id)
                  .reduce((s, i) => s + i.price * i.qty, 0);
                if (!sold) return null;
                const maxSold = Math.max(
                  ...menuItems.rows.map((mi) =>
                    orders.rows
                      .flatMap((o) => o.items)
                      .filter((i) => i.id === mi.id)
                      .reduce((s, i) => s + i.qty, 0),
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 mb-2.5">
                    ...
                    <span className="text-[12.5px] text-slate-700 w-40 shrink-0 truncate">
                      {m.name}
                    </span>
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full"
                        style={{ width: (sold / maxSold) * 100 + "%", background: RST_RED }}
                      />
                    </div>
                    ...
                    <span
                      className="text-[12px] font-mono font-bold w-20 text-right"
                      style={{ color: RST_RED }}
                    >
                      {TZS_FMT(rev)}
                    </span>
          ...
          {activeTable &&
            (() => {
              const tblOrder = orders.rows.find(
                (o) => o.tableId === activeTable && o.status !== "Paid" && o.status !== "Cancelled",
              );
                ...
                <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-3.5 flex items-center justify-between">
                  <div>
                    <p className="text-[13px] font-bold text-[#B91C1C]">
                      Active order on Table {tables.rows.find((t) => t.id === activeTable)?.number}
                    </p>
                    <p className="text-[11.5px] text-[#991B1B]">
                      TZS {TZS_FMT(tblOrder.total)} · {tblOrder.items?.length} items
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      if (!win) return;
                      win.document
                        .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Bill</title><style>
                    ...
                    className="flex items-center gap-1.5 text-[12.5px] font-bold text-white px-4 py-2 rounded-xl"
...
export function InviteCodeForm({ onGenerate }) {
  ...
  return (
    <>
      <div>
        <label className="text-[11px] font-bold text-[#5B21B6] uppercase tracking-wide block mb-1">
          Department
        </label>
      ...
      <div className="col-span-2">
        <button
          onClick={() => onGenerate(dept, role || "Employee")}
...
export const ANNOUNCEMENTS_SEED = [
  {
    id: "ANN-001",
    title: "Q3 Performance Reviews — Reminder",
    body: "All employees should complete their self-assessment by July 31. Log into the Employee Portal under Profile to access your review form.",
    category: "HR",
    priority: "High",
    date: "2026-07-20",
  ...
  {
    id: "ANN-002",
    title: "Office Closure — 7th August",
    body: "The office will be closed on 7th August 2026 for the public holiday. All employees should ensure pending tasks are completed by 6th August.",
    category: "General",
    priority: "Medium",
    date: "2026-07-18",
  ...
  },
  {
    ...
    body: "We are pleased to announce upgraded health insurance coverage for all permanent employees, effective 1st August 2026. Dental and optical cover are now included. Details will be shared by HR shortly.",
    category: "Benefits",
    priority: "High",
    date: "2026-07-15",
    author: "HR Department",
  ...
  {
    id: "ANN-004",
    title: "Monthly Town Hall — Friday 3pm",
    body: "Join us this Friday at 3pm in the Main Conference Room (or via Zoom link shared by email) for our monthly company update. Attendance is strongly encouraged.",
    category: "Events",
    priority: "Medium",
    date: "2026-07-12",
    author: "Management",
  ...
  {
    id: "ANN-005",
    title: "Safety Drill — Next Tuesday 10am",
    body: "A scheduled fire safety drill will take place next Tuesday 10am. Please cooperate with the safety officer's instructions. Estimated duration: 20 minutes.",
    category: "Safety",
    priority: "Medium",
    date: "2026-07-10",
