...
import { BarChart3, CircleDollarSign, Heart, Plus, Users } from "lucide-react";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
export function CommunityGroupsModule({ currentUser }) {
  ...
  const groups = useCompanyTable("community_groups", COMM_GROUPS_SEED, {
    mapRow: (r) => ({ ...r, fund: r.fund || 0 }),
  });
  const contributions = useCompanyTable("community_contributions", COMM_CONTRIBUTIONS_SEED, {
    mapRow: (r) => r,
  });
  const [groupForm, setGroupForm] = useState({
    name: "",
    type: COMMUNITY_GROUP_TYPES[0],
    cycle: "Monthly",
    startDate: TODAY.toISOString().slice(0, 10),
  });
  const [ctbForm, setCtbForm] = useState({
    groupId: "",
    member: "",
    amount: "",
    type: "Contribution",
  ...
  const thisMonthCtbs = contributions.rows.filter(
    (c) => c.date >= TODAY.toISOString().slice(0, 8) + "01",
  );
  ...
  async function addContribution() {
    ...
    groups.setRows((prev) =>
      prev.map((g) =>
        g.id === ctbForm.groupId ? { ...g, fund: g.fund + Number(ctbForm.amount) } : g,
      ),
    );
    ...
    logAudit(
      "Contribution: " + row.member,
      "Community",
      currentUser?.name || "System",
      "TZS " + money(row.amount) + "k",
    );
  }
  ...
  const WELFARE_EVENTS = [
    {
      ...
      member: "Alice Ng&apos;endo",
      amount: 200,
      date: "2026-06-15",
      status: "Paid",
    },
    {
      event: "Funeral Support",
      member: "Bob Otieno",
      amount: 150,
      date: "2026-05-20",
      status: "Paid",
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[12px] mt-0.5" style={{ color: "rgba(255,255,255,.65)" }}>
              Table Banking &middot; Investment Clubs &middot; VICOBA &middot; Welfare &middot;
              SUCCESS &middot; Chamas
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-center">
              <p className="text-[10px] text-purple-300">Total Funds</p>
              <p className="text-[16px] font-bold text-white">TZS {money(totalFunds)}k</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] text-purple-300">Members</p>
              <p className="text-[16px] font-bold text-white">{totalMembers}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <div className="grid grid-cols-3 gap-3 flex-1 mr-4">
              {[
                ["Groups", groups.rows.length, "#7C3AED"],
                ["Total Members", totalMembers, "#2563EB"],
                ["Total Funds", "TZS " + money(totalFunds) + "k", "#16A34A"],
              ].map(([l, v, col]) => (
                <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-3">
                  <p className="text-[11px] text-slate-400">{l}</p>
                  <p className="text-[18px] font-bold mt-0.5" style={{ color: col }}>
                    {v}
                  </p>
                </div>
              ))}
            ...
            <button
              ...
              className="flex items-center gap-1.5 btn-primary text-white text-[12.5px] rounded-xl px-4 py-2.5 shrink-0"
            >
              <Plus size={13} />
              New Group
            </button>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Contribution Cycle">
                  ...
                  >
                    {["Weekly", "Bi-weekly", "Monthly", "Quarterly"].map((c) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {groups.rows.map((g) => {
              const typeColor =
                {
                  "Table Banking": "#16A34A",
                  "Investment Club": "#2563EB",
                  Chama: "#7C3AED",
                  "Welfare Fund": "#EF4444",
                  "SUCCESS Group": "#F59E0B",
                  "Church Fund": "#EC4899",
                  "NGO / CBO": "#0891B2",
                  Cooperative: "#D97706",
                }[g.type] || "#6B7280";
              return (
                <div
                  key={g.id}
                  className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 cursor-pointer hover:border-[#7C3AED] transition-colors"
                  onClick={() => {
                    setSelGroup(g);
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-[14px] font-semibold text-[#111827]">{g.name}</p>
                      <span
                    ...
                    <span className="text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-[#DCFCE7] text-[#16A34A]">
                      {g.status}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-50 rounded-lg p-2 text-center">
                      ...
                      <p className="text-[15px] font-bold text-[#111827]">{g.members}</p>
                    </div>
                    <div
                      className="rounded-lg p-2 text-center"
                      style={{ background: typeColor + "10" }}
                    >
                      <p className="text-[10px] text-slate-400">Fund</p>
                      <p className="text-[15px] font-bold" style={{ color: typeColor }}>
                        TZS {money(g.fund)}k
                      </p>
                    </div>
                  </div>
                  <p className="text-[10.5px] text-slate-400 mt-2">
                    {g.cycle} contributions &middot; Since {g.startDate}
                  </p>
                </div>
              );
            })}
      ...
      {tab === "contributions" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[13.5px] font-semibold text-[#111827]">
                Contributions {selGroup ? "— " + selGroup.name : "(All Groups)"}
              </p>
              <p className="text-[12px] text-slate-400">
                {pendingCtbs.length} pending &middot; TZS{" "}
                {money(thisMonthCtbs.reduce((s, c) => s + c.amount, 0))}k collected this month
              </p>
            </div>
            <button
              onClick={() => setShowCtbForm(true)}
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <FormField label="Group">
                  <select
                    className={inputClass}
                    value={ctbForm.groupId}
                    onChange={(e) => setCtbForm({ ...ctbForm, groupId: e.target.value })}
                  >
                ...
                <FormField label="Member Name">
                  <input
                    className={inputClass}
                    value={ctbForm.member}
                    onChange={(e) => setCtbForm({ ...ctbForm, member: e.target.value })}
                ...
                <FormField label="Amount (TZS k)">
                  <input
                    type="number"
                    className={inputClass}
                    value={ctbForm.amount}
                    onChange={(e) => setCtbForm({ ...ctbForm, amount: e.target.value })}
                  />
                </FormField>
                <FormField label="Type">
                  <select
                    className={inputClass}
                    value={ctbForm.type}
                    onChange={(e) => setCtbForm({ ...ctbForm, type: e.target.value })}
                  >
                    {["Contribution", "Share Purchase", "Fine", "Registration", "Special Levy"].map(
                      (t) => (
                ...
                <FormField label="Date">
                  <input
                    ...
                    className={inputClass}
                    value={ctbForm.date}
                    onChange={(e) => setCtbForm({ ...ctbForm, date: e.target.value })}
                  />
                </FormField>
              </div>
              <div className="flex gap-2">
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Date", "Member", "Group", "Type", "Amount", "Status"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {(selGroup
                  ? contributions.rows.filter((c) => c.groupId === selGroup.id)
                  : contributions.rows
                ).map((ct) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 font-mono font-semibold text-[#7C3AED]">
                        TZS {money(ct.amount)}k
                      </td>
                      <td className="px-4 py-2.5">
                        <span
                          className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full"
                          style={{
                            background: ct.status === "Paid" ? "#DCFCE7" : "#FEF3C7",
                            color: ct.status === "Paid" ? "#16A34A" : "#92400E",
                          }}
                        >
                          {ct.status}
                        </span>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-slate-200/80 p-4">
              ...
              <p className="text-[22px] font-bold text-[#7C3AED]">TZS {money(1200)}k</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200/80 p-4">
              <p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">
                Disbursed YTD
              </p>
              <p className="text-[22px] font-bold text-[#EF4444]">TZS 350k</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200/80 p-4">
              <p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">
                Claims Pending
              </p>
              <p className="text-[22px] font-bold text-[#F59E0B]">1</p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-semibold text-[#111827]">Welfare Claims</p>
            </div>
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          ...
                          color: w.status === "Paid" ? "#16A34A" : "#92400E",
                        }}
                      >
                        {w.status}
                      </span>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              [
                "Funds Collected",
                `TZS ${money(contributions.rows.filter((c) => c.status === "Paid").reduce((s, c) => s + c.amount, 0))}k`,
                "#16A34A",
              ],
              [
                "Pending",
                `TZS ${money(contributions.rows.filter((c) => c.status === "Pending").reduce((s, c) => s + c.amount, 0))}k`,
                "#F59E0B",
              ],
            ].map(([l, v, col]) => (
              <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <h3 className="text-[13.5px] font-semibold text-[#111827] mb-3">
                Funds Collected by Group
              </h3>
              {(() => {
                const grpData = groups.rows
                  .map((g, i) => ({
                    name: g.name.length > 14 ? g.name.slice(0, 12) + "…" : g.name,
                    value: contributions.rows
                      .filter((c) => c.groupId === g.id && c.status === "Paid")
                      .reduce((s, c) => s + c.amount, 0),
                    fill: ["#7C3AED", "#2563EB", "#16A34A", "#D97706", "#EF4444"][i % 5],
                  }))
                  .filter((d) => d.value > 0);
                return grpData.length === 0 ? (
                  <p className="text-slate-400 text-center py-6">No contributions</p>
                ) : (
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const ctbStatus = [
                  {
                    name: "Paid",
                    value: contributions.rows.filter((c) => c.status === "Paid").length,
                    fill: "#16A34A",
                  },
                  {
                    name: "Pending",
                    value: contributions.rows.filter((c) => c.status === "Pending").length,
                    fill: "#F59E0B",
                  },
                  {
                    name: "Waived",
                    value: contributions.rows.filter((c) => c.status === "Waived").length,
                    fill: "#94A3B8",
                  },
                ].filter((d) => d.value > 0);
                return ctbStatus.length === 0 ? (
                  <p className="text-slate-400 text-center py-6">No records</p>
                ) : (
...
export const HC_PATIENTS_SEED = [
  ...
  },
  {
    ...
    status: "Stable",
    phone: "5522336699",
    email: "admin@studies.com",
    nationalId: "121545",
    nationality: "Philipian",
    occupation: "",
    allergies: "None",
    chronicDiseases: "None",
    notes: "",
...
export const HC_REPORTS_SEED = [
  {
    ...
    description:
      "Patient presents with controlled hypertension. BP reading 130/85. Continue current medications and dietary modifications. Follow-up in 3 months.",
    date: "2026-07-03",
    status: "Signed",
  },
];
...
export const VITAL_SEED = [
  ...
  },
  {
    ...
    pain: 5,
    nurse: "Nurse Hana",
    notes: "Elevated BP, referred to doctor",
  },
];
