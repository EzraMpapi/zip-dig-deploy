...
export function ResourceSchedulerPanel({ currentUser }) {
  ...
  const bookings = useCompanyTable("resource_bookings", [], {
    order: { col: "booking_date", ascending: true },
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
  ...
  async function book() {
    if (!draft.resource.trim() || !draft.date || draft.end <= draft.start) {
      notify("Check the resource name and that the end time is after the start.", "error");
      return;
    }
    const clash = bookings.rows.find(
      (b) =>
        b.resource.toLowerCase() === draft.resource.trim().toLowerCase() &&
        b.date === draft.date &&
        draft.start < b.end &&
        b.start < draft.end,
    );
    if (clash) {
      notify(
        `Conflict: ${clash.resource} is already booked ${clash.start}–${clash.end} by ${clash.by}. Pick another slot.`,
        "error",
      );
      return;
    }
    ...
    bookings.setRows((prev) =>
      [...prev, row].sort((a, b) => (a.date + a.start < b.date + b.start ? -1 : 1)),
    );
    setDraft({ ...draft, resource: "", purpose: "" });
    setOpen(false);
    notify(
      `${row.resource} booked ${row.date} ${row.start}–${row.end} — conflict-checked against every existing booking.`,
    );
    if (IS_CONFIGURED) {
      try {
        const header = await sb("resource_bookings")
          .insert({
        ...
        notify("Booked locally, but the server update failed.", "error");
      }
    }
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 mb-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400">
            Rooms, equipment, vehicles, field visits, installations, maintenance — every booking
            conflict-checked, never silently double-booked.
          </p>
        </div>
        <button
          onClick={() => setOpen((s) => !s)}
          className="text-[11.5px] font-medium text-[#16A34A] hover:underline shrink-0"
        >
          {open ? "Cancel" : "New booking"}
        </button>
      </div>
      {open && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-3 pt-3 border-t border-slate-100">
          ...
          <input
            ...
            placeholder="Resource — e.g. Boardroom, Hilux T123"
          />
          <input
            type="date"
            className={inputClass}
          ...
          <button
            onClick={book}
            className="btn-primary text-white text-[12px] font-medium rounded-lg px-3.5 py-2 col-span-2 sm:col-span-1"
          >
            Book — conflict-checked
          </button>
        </div>
      )}
      {upcoming.length > 0 && (
          ...
          {upcoming.map((b) => (
            <div key={b.id} className="flex items-center justify-between text-[12px]">
              <span className="text-slate-600 truncate">
                <span className="font-medium text-[#111827]">{b.resource}</span> · {b.type}
                {b.purpose ? ` · ${b.purpose}` : ""} · {b.by}
              </span>
      ...
      {!bookings.loading && upcoming.length === 0 && (
        <p className="text-[11.5px] text-slate-400 mt-3">No upcoming bookings.</p>
      )}
    </div>
  );
}
...
export function VideoMeetingBar({ currentUser }) {
  function startMeeting() {
    ...
    notify(
      `Video meeting started by ${currentUser.name} — link copied; paste it into Team Chat so others can join.`,
    );
  }
  return (
    <div className="flex justify-end mb-3">
      <button
        ...
        className="btn-primary text-white text-[12px] font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5"
      >
...
export function WhatsAppCenter({ currentUser, crm, employees, invoices, company }) {
  ...
  const contacts = useMemo(() => {
    const wonLeads = (crm?.rows || [])
      .filter((l) => l.phone || l.email)
      .map((l) => ({
        id: "lead-" + l.id,
        name: l.company || l.contact || "Unknown",
        phone: l.phone || "",
        email: l.email || "",
  ...
  const tmpl =
    WA_TEMPLATES.find((t) => t.id === templateId) || WA_TEMPLATES[WA_TEMPLATES.length - 1];
  function applyTemplate(t) {
    setTmplId(t.id);
    if (t.id === "custom") return;
    // Pull latest relevant vars from context
    const relInvs = (invoices?.rows || []).filter(
      (inv) => inv.customer?.toLowerCase() === (contact?.name || "").toLowerCase(),
    );
    ...
    const merged = t.body
      ...
      .replace(
        /\{amount\}/g,
        latestInv
          ? money(Math.round(lineTotal(latestInv.items || []).total))
          : vars.amount || "{amount}",
      )
  ...
  async function sendMessage() {
    ...
    if (apiToken && apiPhone && contact.phone) {
      ...
      try {
        ...
        setLocalMsgs((m) => ({
          ...m,
          [contact.id]: m[contact.id].map((ms) =>
            ms.id === newMsg.id ? { ...ms, status: waId ? "delivered" : "failed", waId } : ms,
          ),
        }));
        ...
        setLocalMsgs((m) => ({
          ...m,
          [contact.id]: m[contact.id].map((ms) =>
            ms.id === newMsg.id ? { ...ms, status: "via-link" } : ms,
          ),
        }));
      }
    } else {
  ...
  function openWaLink(text, phone) {
    ...
    if (!num) {
      notify("No phone number for this contact", "error");
      return;
    }
      ...
      <div className="w-72 shrink-0 border-r border-slate-100 flex flex-col">
        ...
        {showApi && (
          <div className="px-3 py-3 bg-[#DCF8C6]/30 border-b border-green-200 space-y-2">
            ...
            <input
              className="w-full text-[11.5px] border border-slate-200 rounded-lg px-2.5 py-1.5 outline-none"
            ...
            <input
              ...
              value={apiPhone}
              onChange={(e) => {
                setApiPhone(e.target.value);
                localStorage.setItem("wa_api_phone", e.target.value);
              }}
            />
            <div
              className={`text-[10.5px] font-semibold px-2 py-1 rounded ${apiToken && apiPhone ? "text-[#16A34A] bg-[#F0FDF4]" : "text-[#F59E0B] bg-[#FFFBEB]"}`}
            >
              {apiToken && apiPhone
                ? "✓ API configured — messages send directly"
                : "⚠ No API — messages open WhatsApp Web"}
            </div>
            <p className="text-[9.5px] text-slate-400">
              Get credentials at business.facebook.com/wa/manage
            </p>
          </div>
        )}
                  ...
                  <div className="flex justify-between items-baseline">
                    ...
                    {last && (
                      <span className="text-[10px] text-slate-400 shrink-0 ml-1">
                        {last.sentAt?.slice(11, 16)}
                      </span>
                    )}
                  </div>
                  <div className="flex justify-between items-center">
                    <p className="text-[11.5px] text-slate-400 truncate">
                      {last ? last.body.slice(0, 30) + "…" : ct.phone || ct.email || ct.role || ""}
                    </p>
                    {unread > 0 && (
      ...
      {!contact ? (
        ...
        >
          ...
          <p className="text-[13px] text-slate-500 text-center max-w-xs">
            Select a contact to start messaging. Messages open WhatsApp Web pre-filled or send
            directly via the Business API.
          </p>
        </div>
      ) : (
        <div className="flex-1 flex flex-col" style={{ background: "#ECE5DD" }}>
          ...
          >
            ...
            <div className="flex gap-2">
              {contact.phone && (
                <button
                  onClick={() => openWaLink("", contact.phone)}
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[10.5px] font-semibold whitespace-nowrap border transition-all ${
                  templateId === t.id
                    ...
                    : "bg-white text-slate-600 border-slate-200 hover:border-[#075E54]/40"
                }`}
              >
                {t.label}
              </button>
            ...
            {msgs.map((msg) => {
              ...
              const statusIcon =
                msg.status === "delivered" || msg.status === "read" ? (
                  <CheckCheck
                    size={12}
                    className={msg.status === "read" ? "text-[#53bdeb]" : "text-white/60"}
                  />
                ) : msg.status === "via-link" ? (
              ...
              return (
                <div key={msg.id} className={`flex ${isOut ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[75%] px-3 py-2 rounded-xl shadow-sm relative ${
                  ...
                  >
                    ...
                    />
                    <div
                      className={`flex items-center justify-end gap-1 mt-1 ${isOut ? "text-white/60" : "text-slate-400"}`}
                    >
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
              ...
              {compose && (
                <div className="px-4 pb-2">
                  ...
                  <div
                    className="text-[11.5px] text-[#111827] leading-relaxed"
...
export function EmailCenter({ currentUser, crm, employees, invoices, company }) {
  ...
  const [smtpCfg, setSmtp] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("smtp_cfg") || "{}");
    } catch {
      return {};
    }
  });
  ...
  function mergeTemplate(tmpl) {
    const relInvs = (invoices?.rows || []).filter(
      (inv) => inv.customer?.toLowerCase() === (to.split("@")[0] || "").toLowerCase(),
    );
    const lat = relInvs[0];
    return tmpl
      .replace(/\{company\}/g, co.name || "SMART MANAGER")
      ...
      .replace(
        /\{amount\}/g,
        lat ? money(Math.round(lineTotal(lat.items || []).total)) : "{amount}",
      )
  ...
  async function sendEmail() {
    if (!to.trim() || !subject.trim()) {
      notify("To and Subject are required", "error");
      return;
    }
    setBusy(true);
    const sent = {
      ...
      from: co.email || currentUser.name,
      starred: false,
    };
      ...
      try {
        await sb("emails")
          .insert({
            ...
            direction: "out",
            sent_at: sent.sentAt,
            sender: co.email || currentUser.name,
          })
          .run();
      } catch (_) {}
    }
  ...
  function toggleStar(email) {
    ...
    setStarred((s) =>
      star ? [...s, { ...email, starred: true }] : s.filter((e) => e.id !== email.id),
    );
  }
  ...
  const filteredContacts = contacts.filter(
    (c) =>
      ...
      c.email.toLowerCase().includes(contactQ.toLowerCase()),
  );
        ...
        <div className="px-4 pt-4 pb-3 border-b border-slate-100">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-[11px] font-bold text-white bg-[#2563EB] px-2 py-1 rounded-lg"
            >
              + New
            </button>
          </div>
            ...
            {[
              ...
              <div key={k}>
                ...
                <input
                  ...
                  value={smtpCfg[k] || ""}
                  onChange={(e) => {
                    const n = { ...smtpCfg, [k]: e.target.value };
                    setSmtp(n);
                    localStorage.setItem("smtp_cfg", JSON.stringify(n));
                  }}
                />
              </div>
            ))}
            <p className="text-[9.5px] text-slate-400">
              SMTP sending requires a backend proxy for CORS. Credentials saved locally.
            </p>
          </div>
        )}
      </div>
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-full text-[10.5px] font-semibold whitespace-nowrap border transition-all ${
                    tmplId === t.id
                      ...
                      : "bg-white text-slate-600 border-slate-200 hover:border-[#2563EB]/40"
                  }`}
                >
                  {t.label}
                </button>
            ...
            <div className="border-b border-slate-100 divide-y divide-slate-100">
              ...
              {showCC && (
                <>
                  ...
                  <div className="flex items-center gap-2 px-4 py-2">
                    <span className="text-[11.5px] font-bold text-slate-400 w-10 shrink-0">
                      BCC
                    </span>
                    <input
                      value={bcc}
        ...
        {folder !== "compose" && !selectedEmail && (
          <div className="flex-1 flex flex-col">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-bold text-[#111827] capitalize">
              ...
              {folder === "drafts" && (
                <button
                  ...
                  className="text-[11.5px] text-[#2563EB] font-bold"
                >
                  + Compose
                </button>
              )}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between gap-2">
                        <p className="text-[13px] font-semibold text-[#111827] truncate">
                          {folder === "sent" ? email.to : email.from || email.to}
                        </p>
            ...
            <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  setBody(
                    "\n\n---\nOn " +
                      selectedEmail.sentAt?.slice(0, 10) +
                      ":\n" +
                      selectedEmail.body,
                  );
                  setFolder("compose");
                ...
                className="text-[11.5px] font-bold text-[#2563EB] border border-[#2563EB]/30 px-2.5 py-1.5 rounded-lg"
              >
                ↩ Reply
              </button>
              <button onClick={() => toggleStar(selectedEmail)} className="p-1">
                <Star
                  size={15}
                  className={
                    selectedEmail.starred ? "text-[#F59E0B] fill-[#F59E0B]" : "text-slate-300"
                  }
                />
              </button>
            </div>
            <div className="px-5 py-4 border-b border-slate-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {selectedEmail.cc && (
                    <p className="text-[11.5px] text-slate-400">CC: {selectedEmail.cc}</p>
                  )}
                  <p className="text-[11.5px] text-slate-400">
                    From: {selectedEmail.from || co.email || "You"} ·{" "}
                    {selectedEmail.sentAt?.slice(0, 16).replace("T", " ")}
                  </p>
                </div>
              </div>
            </div>
            <div className="px-6 py-5 flex-1">
              <pre className="text-[13px] text-[#374151] leading-relaxed whitespace-pre-wrap font-sans">
                {selectedEmail.body}
              </pre>
            </div>
          </div>
    ...
    <div className="space-y-5 h-full flex flex-col">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Team chat, an enterprise calendar pulling from every module, and team workspaces — all
          real. Voice and video calls coordinate through a real meeting link rather than being
          hosted in-app; see the note on the Calendar tab.
        </p>
      </div>
        ...
        {COLLAB_TABS.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`text-[12px] font-medium px-3 py-1.5 rounded-md flex items-center gap-1.5 whitespace-nowrap transition-colors ${isActive ? "bg-white text-[#111827] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
      ...
      <div className="flex-1 min-h-0">
        ...
        {tab === "whatsapp" && (
          <WhatsAppCenter
            ...
            employees={employees}
            invoices={invoices}
            company={window.__smartManagerCompany || {}}
          />
        )}
        {tab === "email" && (
          <EmailCenter
...
export function ChannelsView({ currentUser, employees }) {
  const channels = useCompanyTable("collab_channels", collabChannelsSeed, {
    mapRow: mapCollabChannelRow,
  });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const rows = await sb("collab_messages")
          .select("*")
          .eq("channel_ref", activeChannelId)
          .order("created_at", { ascending: true })
          .run();
        setMessages((prev) => {
        ...
        /* a missed poll just tries again in 4s; nothing to surface to the user for one skipped refresh */
      }
    }, 4000);
  ...
  async function sendMessage() {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("collab_messages")
          .insert({ channel_ref: activeChannelId, sender: currentUser.name, body: entry.text })
          .run();
      } catch (_e) {
        notify("Message shown locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function addChannel(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("collab_channels")
          .insert({
            name: draftChannel.name,
            scope: draftChannel.scope,
            description: draftChannel.description,
          })
          .single()
          .run();
        if (header?.id)
          channels.setRows((prev) =>
            prev.map((c) => (c.id === draftChannel.id ? { ...c, dbId: header.id } : c)),
          );
      } catch (_e) {
        notify("Channel created locally, but saving to the server failed.", "error");
      }
    }
  }
        ...
        <div className="flex-1 overflow-y-auto py-1.5">
          {channels.rows.map((c) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 flex items-center gap-2 text-[12.5px] ${activeChannelId === c.id ? "bg-[#16A34A]/8 text-[#111827] font-medium" : "text-slate-600 hover:bg-slate-50"}`}
            >
          ...
          <>
            <div className="px-4 py-3 border-b border-slate-100 shrink-0">
              <p className="text-[13.5px] font-semibold text-[#111827] flex items-center gap-1.5">
                <Hash size={13} className="text-slate-400" /> {activeChannel.name}
              </p>
              <p className="text-[11px] text-slate-400">
                {activeChannel.scope} · {activeChannel.description}
              </p>
            </div>
            ...
            <div className="p-3 border-t border-slate-100 shrink-0 flex gap-2">
              ...
              <button
                onClick={sendMessage}
                disabled={!draft.trim()}
                className="btn-primary text-white rounded-lg px-3 disabled:opacity-40"
                aria-label="Send"
              >
                <Send size={15} />
              </button>
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      <form
        ...
        className="relative w-full sm:w-[360px] bg-white h-full shadow-2xl overflow-y-auto flex flex-col"
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Team Chat</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Channel</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
...
export function computeTaxDeadlines(monthsAhead = 3) {
  ...
  for (let i = -1; i <= monthsAhead; i++) {
    ...
    deadlines.push({
      ...
      label: `VAT return due (period: ${new Date(TODAY.getFullYear(), TODAY.getMonth() + i, 1).toLocaleDateString(undefined, { month: "long" })})`,
    });
  }
  return deadlines;
}
...
export function SharedCalendar({ invoices, crm, workOrders, leaveRequests }) {
  const events = useCompanyTable("calendar_events", calendarEventsSeed, {
    mapRow: mapCalendarEventRow,
  });
  ...
  const entriesByDate = useMemo(() => {
    ...
    function add(dateStr, entry) {
      (map[dateStr] = map[dateStr] || []).push(entry);
    }

    events.rows.forEach((e) =>
      add(e.date, { id: e.id, title: e.title, category: "meetings", color: "#16A34A" }),
    );
    ...
    workOrders.rows
      ...
      .forEach((w) => {
        add(w.dueDate, {
          id: w.id,
          title: `${w.product} — production due`,
          category: "production",
          color: "#5B6472",
        });
      });
    ...
    crm.rows
      .filter((l) => l.expectedCloseDate && l.stage !== "Won" && l.stage !== "Lost")
      .forEach((l) => {
        add(l.expectedCloseDate, {
          id: l.id,
          title: `${l.company} — expected close`,
          category: "sales",
          color: "#22C55E",
        });
      });
    ...
    invoices.rows
      ...
      .forEach((inv) => {
        add(inv.dueDate, {
          id: inv.id,
          title: `${inv.customer} — payment due (${inv.id})`,
          category: "payments",
          color: "#EF4444",
        });
      });
    ...
    taxDeadlines.forEach((t) =>
      add(t.date, { id: `tax-${t.date}`, title: t.label, category: "tax", color: "#0EA5E9" }),
    );
  ...
  async function addEvent(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("calendar_events")
          .insert({
            title: draft.title,
        ...
        notify("Event saved locally, but the server update failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          There&apos;s no in-app calling here — real voice and video calls need signaling and media
          infrastructure this build doesn&apos;t run. Scheduling a Voice or Video Call creates a
          real calendar entry with a real link field for wherever the actual call happens (Zoom,
          Google Meet, Teams). Tax deadlines below follow Tanzania&apos;s published VAT filing rule
          (due the 20th of the following month) — confirm against actual TRA guidance, since an
          authority can move a deadline this app has no way to know about.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[14px] font-semibold text-[#111827] w-36 text-center">
              {monthLabel}
            </p>
            <button
              onClick={() => setMonthOffset((m) => m + 1)}
          ...
          <button
            ...
            className="btn-primary text-white text-[12px] font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5"
          >
        ...
        <div className="grid grid-cols-7 gap-1.5 mb-1.5">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div key={d} className="text-center text-[10.5px] font-medium text-slate-400 py-1">
              {d}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-1.5">
          {cells.map((d, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-[11px] font-medium ${isToday ? "text-[#16A34A]" : "text-slate-600"}`}
                >
                  {d}
                </span>
                <div className="flex-1 flex flex-col gap-0.5 mt-1 overflow-hidden">
                  {dayEntries.slice(0, 2).map((e) => (
                    <span
                      ...
                      style={{ backgroundColor: `${e.color}14`, color: e.color }}
                    >
                      {e.title}
                    </span>
                  ))}
                  {dayEntries.length > 2 && (
                    <span className="text-[9px] text-slate-400">+{dayEntries.length - 2} more</span>
                  )}
                </div>
              </button>
            );
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        <h3 className="text-[14px] font-semibold text-[#111827] mb-3">
          Upcoming — Everything Integrated
        </h3>
        <div className="space-y-2">
          {Object.entries(entriesByDate)
            ...
            .flatMap(([date, entries]) =>
              entries.filter((e) => visibleCategories.has(e.category)).map((e) => ({ ...e, date })),
            )
            .slice(0, 8)
            .map((e) => {
              const linkedEvent =
                e.category === "meetings" ? events.rows.find((ev) => ev.id === e.id) : null;
              return (
                <div
                  key={`${e.category}-${e.id}`}
                  className="flex items-center justify-between border border-slate-100 rounded-lg px-3.5 py-2.5"
                >
                  <div className="min-w-0 flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-400">
                        {e.date} · {CALENDAR_CATEGORIES.find((c) => c.id === e.category)?.label}
                      </p>
                    </div>
                  </div>
                  {linkedEvent?.meetingLink && (
                    <a
                      href={linkedEvent.meetingLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="btn-secondary text-[11px] font-medium px-2.5 py-1.5 rounded-lg shrink-0 ml-2"
                    >
                      Join
                    </a>
                  )}
          ...
          {Object.entries(entriesByDate).filter(
            ([date]) => date >= TODAY.toISOString().slice(0, 10),
          ).length === 0 && (
            <p className="text-[12.5px] text-slate-400 text-center py-4">Nothing scheduled.</p>
          )}
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Shared Calendar</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Event</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          {needsLink && (
            <FormField label="Meeting link">
              <input
                ...
                placeholder="Paste your Zoom / Google Meet / Teams link"
              />
              <p className="text-[11px] text-slate-400 mt-1">
                Generate this in whichever video platform your business uses — this app coordinates
                the meeting, not the call itself.
              </p>
            </FormField>
          )}
          <FormField label="Attendees">
            <input
              ...
              placeholder="e.g. Sales team, or specific names"
            />
          </FormField>
          <FormField label="Description">
            <textarea
...
export function TeamWorkspaces({ employees }) {
  ...
  async function addWorkspace(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("workspaces")
          .insert({
            name: draft.name,
          ...
          .run();
        if (header?.id)
          workspaces.setRows((prev) =>
            prev.map((w) => (w.id === draft.id ? { ...w, dbId: header.id } : w)),
          );
      } catch (_e) {
        notify("Workspace created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteWorkspace(id) {
    ...
    if (IS_CONFIGURED && w?.dbId) {
      try {
        await sb("workspaces").eq("id", w.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the workspace on the server.", "error");
      }
    }
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {workspaces.rows.map((w) => (
          ...
          >
            ...
            {w.department && (
              <span className="text-[10.5px] font-medium text-[#16A34A] bg-[#16A34A]/10 px-2 py-0.5 rounded-full">
                {w.department}
              </span>
            )}
            <p className="text-[12px] text-slate-500 mt-2 leading-relaxed">{w.description}</p>
            <div className="flex items-center gap-1.5 mt-3 text-[11.5px] text-slate-400">
              <Users size={12} /> {w.members || "No members listed"}
            </div>
          </div>
        ))}
        {!workspaces.loading && workspaces.rows.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Group a cross-functional team around a project or initiative."
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Team Workspaces</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Workspace</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Workspace name" required>
            <input
              ...
              value={form.name}
              onChange={(e) => set("name", e.target.value)}
              placeholder="e.g. Cold Chain Rollout Team"
            />
          </FormField>
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          ...
          className="btn-secondary text-[11.5px] font-medium px-3 py-1.5 rounded-lg"
        >
          Open Document Center
        </button>
      </div>
      <p className="text-[11.5px] text-slate-400 mb-4">
        The same real files as the Document Center — not a separate store. OCR, version history, AI
        summaries, and e-signatures all live there.
      </p>
      <div className="space-y-1">
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[12.5px] font-medium text-[#111827] truncate">{f.name}</p>
                <p className="text-[11px] text-slate-400">
                  {f.folder} · {f.date}
                </p>
              </div>
        ...
        {recent.length === 0 && (
          <p className="text-[12.5px] text-slate-400 text-center py-6">
            No files yet — upload one in the Document Center.
          </p>
        )}
      </div>
    </div>
...
export function NotebookView({ currentUser }) {
  const notes = useCompanyTable("notebook_notes", [], {
  ...
  const visible = notes.rows.filter(
    (n) => n.visibility === "Team" || n.createdBy === currentUser.name,
  );
  ...
  async function addNote(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("notebook_notes")
          .insert({
            title: draft.title,
        ...
        if (header?.id)
          notes.setRows((prev) =>
            prev.map((n) => (n.id === draft.id ? { ...n, dbId: header.id } : n)),
          );
      } catch (_e) {
  ...
  async function toggleStatus(note) {
    ...
    if (IS_CONFIGURED && note.dbId) {
      try {
        await sb("notebook_notes").eq("id", note.dbId).update({ status: newStatus }).run();
      } catch (_e) {
        notify("Couldn't update the server.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {notes.loading && (
          <p className="col-span-full text-[12.5px] text-slate-400 text-center py-8">Loading...</p>
        )}
        {filtered.map((n) => (
          <div key={n.id} className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-start justify-between mb-1.5">
              <p
                className={`text-[13.5px] font-medium ${n.status === "Completed" ? "text-slate-400 line-through" : "text-[#111827]"}`}
              >
                {n.title}
              </p>
              <button
                onClick={() => toggleStatus(n)}
                aria-label={n.status === "Active" ? "Mark completed" : "Mark active"}
              >
                {n.status === "Completed" ? (
            ...
            {n.content && (
              <p className="text-[12px] text-slate-500 mb-2 line-clamp-2">{n.content}</p>
            )}
            <div className="flex items-center gap-1.5">
              <span
                className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${n.visibility === "Private" ? "bg-[#F59E0B]/10 text-[#F59E0B]" : "bg-slate-100 text-slate-500"}`}
              >
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {["Team", "Private"].map((v) => (
                    <button
                      ...
                      className={`flex-1 text-[12.5px] font-medium py-2 rounded-lg border transition-colors ${form.visibility === v ? "border-[#16A34A]/50 bg-[#16A34A]/5 text-[#111827]" : "border-slate-200 text-slate-500"}`}
                    >
                      {v}
                    </button>
                  ))}
                ...
                <p className="text-[10.5px] text-slate-400 mt-1.5">
                  Private notes are only ever visible to you — real filtering, not a cosmetic label.
                </p>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
