...
import { AlertCircle, CheckCircle2, Circle, GitBranch, Plus, Store, Trash2, X } from "lucide-react";
...
export async function executeWorkflowStep(step, context) {
  ...
  if (step.type === "notify_slack" || step.type === "notify_teams") {
    ...
    if (!channel?.enabled || !channel?.webhookUrl)
      return {
        ok: false,
        note: `${stepType.label.replace("Notify via ", "")} isn't configured — set it up in Notifications > Channels first.`,
      };
    const result = await sendWebhookNotification(
      channel.webhookUrl,
      `[Workflow: ${context.workflowName}] ${step.config.message || ""}`,
    );
  ...
  if (step.type === "log_audit") {
    logAudit(
      step.config.note || "Workflow step",
      "Workflow Studio",
  ...
  if (step.type === "draft_email") {
    if (!step.config.recipient?.trim())
      return { ok: false, note: "No recipient email configured for this step." };
    const subject = `A note from ${context.company.name}`;
    const body = step.config.context || "";
    window.open(
      `mailto:${encodeURIComponent(step.config.recipient.trim())}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,
      "_blank",
    );
...
export function WorkflowStudio({ company, invoices, expenses, inventory }) {
  ...
  const marketplace = useCompanyTable(
    "workflow_marketplace_templates",
    OFFICIAL_MARKETPLACE_TEMPLATES,
    { mapRow: mapMarketplaceTemplateRow },
  );
  ...
  const channels = useCompanyTable(
    ...
    { mapRow: mapIntegrationConnectionRow },
  );
  ...
  async function saveWorkflow(workflow) {
    ...
    if (IS_CONFIGURED) {
      try {
        if (isNew) {
          const header = await sb("workflows")
            .insert({
              name: workflow.name,
              trigger_type: workflow.trigger,
              enabled: workflow.enabled,
              steps: workflow.steps,
              condition: workflow.condition || null,
            })
            .single()
            .run();
          if (header?.id)
            workflows.setRows((prev) =>
              prev.map((w) => (w.id === workflow.id ? { ...w, dbId: header.id } : w)),
            );
        } else {
          await sb("workflows")
            .eq("id", workflow.dbId)
  ...
  async function deleteWorkflow(id) {
    ...
    if (IS_CONFIGURED && w?.dbId) {
      try {
        await sb("workflows").eq("id", w.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the workflow on the server.", "error");
      }
    }
  }
  ...
  async function installTemplate(template) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        if (template.dbId)
          await sb("workflow_marketplace_templates")
            .eq("id", template.dbId)
            .update({ install_count: template.installCount + 1 })
            .run();
      } catch (_e) {
        notify("Installed locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function publishTemplate({ workflow, category, description }) {
    const sanitizedSteps = workflow.steps.map((s) =>
      s.type === "draft_email" ? { ...s, config: { ...s.config, recipient: "" } } : s,
    );
    const draft = {
      id: `TPL-${Date.now()}`,
      name: workflow.name,
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("workflow_marketplace_templates")
          .insert({
            name: draft.name,
            ...
            trigger_type: draft.trigger,
            steps: draft.steps,
            published_by_company_id: company.id,
            published_by_company_name: company.name,
          })
          .single()
          .run();
        if (header?.id)
          marketplace.setRows((prev) =>
            prev.map((t) => (t.id === draft.id ? { ...t, dbId: header.id } : t)),
          );
      } catch (_e) {
        notify("Published locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function runWorkflow(workflow) {
    ...
    if (workflow.condition && workflow.condition.type && workflow.condition.type !== "none") {
      ...
      const verdict = condDef
        ? condDef.evaluate({ invoices, expenses, inventory }, workflow.condition.value)
        : { met: true, detail: "Unknown condition — ran anyway" };
      if (!verdict.met) {
        setRunTrace((prev) => ({
          ...prev,
          results: [
            {
              stepId: "condition",
              ok: false,
              detail: `Condition not met — skipped. ${verdict.detail}`,
            },
          ],
        }));
        logAudit(
          "Workflow skipped — condition not met",
          "Workflow Studio",
          verdict.detail,
          workflow.name,
        );
      ...
      setRunTrace((prev) => ({
        ...prev,
        results: [{ stepId: "condition", ok: true, detail: `Condition met — ${verdict.detail}` }],
      }));
    }
    for (const step of workflow.steps) {
      ...
      setRunTrace((prev) => ({
        ...prev,
        results: [...prev.results, { stepId: step.id, ...result }],
      }));
    }
    const today = TODAY.toISOString().slice(0, 10);
    workflows.setRows((prev) =>
      prev.map((w) => (w.id === workflow.id ? { ...w, lastRun: today } : w)),
    );
    ...
    if (IS_CONFIGURED && workflow.dbId) {
      try {
        await sb("workflows").eq("id", workflow.dbId).update({ last_run: today }).run();
      } catch (_e) {
        /* the run itself already completed; a log-sync miss isn't worth a second toast */
      }
    }
  }

  const readyToRun = workflows.rows.filter(
    (w) => w.enabled && w.trigger !== "manual" && alerts.some((a) => a.id === w.trigger),
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Drag steps into a sequence, then run it — every step is a real action, nothing simulated
          </p>
        </div>
        {view === "my-workflows" && (
          <button
      ...
      <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        <button
          onClick={() => setView("my-workflows")}
          className={`text-[12px] font-medium px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-colors ${view === "my-workflows" ? "bg-white text-[#111827] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          <GitBranch size={13} /> My Workflows
        </button>
        <button
          onClick={() => setView("marketplace")}
          className={`text-[12px] font-medium px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-colors ${view === "marketplace" ? "bg-white text-[#111827] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
        ...
        <>
          <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
            ...
            <p className="text-[12px] text-slate-500 leading-relaxed">
              There&apos;s no server here to watch for events while this tab is closed, so a
              workflow either runs when you click Run Now, or — for workflows set to trigger on a
              real alert — gets surfaced below as ready to run the moment that alert is genuinely
              active in your current session. Neither is silent background automation; both need a
              person to see it happen.
            </p>
          </div>

          {readyToRun.length > 0 && (
            <div className="bg-[#16A34A]/5 border border-[#16A34A]/20 rounded-xl p-4">
              <h3 className="text-[13px] font-semibold text-[#111827] mb-2 flex items-center gap-1.5">
                <AlertCircle size={14} className="text-[#16A34A]" /> Ready to run
              </h3>
              <div className="space-y-2">
                {readyToRun.map((w) => (
                  <div
                    key={w.id}
                    className="flex items-center justify-between bg-white rounded-lg px-3.5 py-2.5 border border-slate-100"
                  >
                    <div>
                      <p className="text-[12.5px] font-medium text-[#111827]">{w.name}</p>
                      <p className="text-[11px] text-slate-400">
                        {WORKFLOW_TRIGGERS.find((t) => t.id === w.trigger)?.label}
                      </p>
                    </div>
                    <button
                      onClick={() => runWorkflow(w)}
                      disabled={runningId === w.id}
                      className="btn-primary text-white text-[11.5px] font-medium px-3 py-1.5 rounded-lg disabled:opacity-40"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {workflows.loading &&
              Array.from({ length: 2 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-white rounded-xl border border-slate-200/80 h-40 skeleton-shimmer"
                />
              ))}
            {!workflows.loading &&
              workflows.rows.map((w) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${w.enabled ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-slate-100 text-slate-400"}`}
                    >
                  ...
                  <p className="text-[11px] text-slate-400 mb-3">
                    {w.lastRun ? `Last ran ${w.lastRun}` : "Never run"}
                  </p>
                  <div className="flex gap-2 mb-2">
                    <button
                      onClick={() => setBuilderOpen(w)}
                      className="btn-secondary flex-1 text-[12px] font-medium rounded-lg py-2"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => runWorkflow(w)}
                      disabled={runningId === w.id}
                      className="btn-primary text-white flex-1 text-[12px] font-medium rounded-lg py-2 disabled:opacity-40"
                    >
                      {runningId === w.id ? "Running..." : "Run Now"}
                    </button>
                    <button
                      onClick={() => deleteWorkflow(w.id)}
                      className="text-slate-300 hover:text-[#EF4444] px-2"
                  ...
                  <button
                    onClick={() => setPublishingWorkflow(w)}
                    className="w-full text-[11.5px] font-medium text-slate-500 hover:text-[#16A34A] py-1.5 flex items-center justify-center gap-1.5"
                  >
            ...
            {!workflows.loading && workflows.rows.length === 0 && (
              <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
                <EmptyState
                  ...
                  hint="Build one by dragging steps into a sequence, or install one from the Automation Marketplace."
                  actionLabel="New Workflow"
                  onAction={() => setBuilderOpen("new")}
                />
              </div>
            )}
          </div>
      ...
      {builderOpen && (
        <WorkflowBuilder
          workflow={builderOpen === "new" ? null : builderOpen}
...
export function WorkflowBuilder({ workflow, onClose, onSave }) {
  ...
  function handleSave() {
    if (!valid) return;
    onSave({
      id: workflow?.id || `WF-${Math.floor(10 + Math.random() * 89)}`,
      dbId: workflow?.dbId,
      name: name.trim(),
      trigger,
      enabled,
      ...
      <div className="px-5 sm:px-8 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <input
            ...
            className="text-[16px] font-semibold text-[#111827] outline-none border-b border-transparent focus:border-[#16A34A] bg-transparent min-w-0"
          />
        </div>
        <button
          onClick={handleSave}
          disabled={!valid}
          className="btn-primary text-white text-[13px] font-medium px-4 py-2 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
        >
          Save Workflow
        </button>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 border-r border-slate-100 p-4 overflow-y-auto shrink-0 hidden sm:block">
            ...
            {WORKFLOW_STEP_TYPES.map((t) => {
              ...
              return (
                <div
                  ...
                  draggable
                  onDragStart={() => {
                    dragState.current = { kind: "new", stepTypeId: t.id };
                  }}
          ...
          <p className="text-[11px] text-slate-400 mt-4 leading-relaxed">
            Or tap a step below to add it on mobile, where dragging isn&apos;t available the same
            way.
          </p>
          <div className="sm:hidden space-y-1.5 mt-2">
            {WORKFLOW_STEP_TYPES.map((t) => (
              <button
                key={t.id}
                onClick={() => addStepAt(t.id, steps.length)}
                className="w-full text-left text-[12px] font-medium text-[#16A34A] py-1.5"
              >
                + {t.label}
              </button>
            ))}
            ...
            <FormField label="Condition — the gate between When and Actions">
              <select
                ...
                onChange={(e) =>
                  setCondition(
                    e.target.value === "none"
                      ? null
                      : { type: e.target.value, value: condition?.value || "0" },
                  )
                }
              >
                {WORKFLOW_CONDITIONS.map((c) => (
              ...
              {condition && condition.type !== "none" && (
                <input
                  ...
                  placeholder={`Threshold (${WORKFLOW_CONDITIONS.find((c) => c.id === condition.type)?.unit || ""})`}
                />
              )}
              <p className="text-[10.5px] text-slate-400 mt-1.5">
                Evaluated against your live records the moment the workflow runs — a skipped run
                tells you exactly why, in real numbers.
              </p>
            </FormField>
            <div className="flex justify-center text-slate-300 text-[14px] leading-none">↓</div>
                ...
                return (
                  <div key={step.id}>
                    <div
                      draggable
                      onDragStart={() => {
                        dragState.current = { kind: "reorder", index: i };
                      }}
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <div
                          ...
                          style={{ backgroundColor: `${stepType?.color}14` }}
                        >
                          <Icon size={14} style={{ color: stepType?.color }} />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400 truncate">
                            {Object.values(step.config).filter(Boolean).join(" · ") ||
                              "Tap to configure"}
                          </p>
                        </div>
                        <button
                          ...
                          className="text-slate-300 hover:text-[#EF4444] shrink-0"
                            ...
                            <div key={f.key}>
                              ...
                              {f.options ? (
                                <select
                                  ...
                                  onChange={(e) => updateStepConfig(step.id, f.key, e.target.value)}
                                >
                                  {f.options.map((o) => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Every template here — official or published by another organization using this software —
          is built entirely from the same five real step types in your own builder. Installing one
          just copies its steps into My Workflows; nothing about an installed template behaves
          differently from one you&apos;d built by hand.
        </p>
      </div>
      ...
      <div className="flex items-center gap-1 bg-white border border-slate-200/80 rounded-lg p-1 w-fit overflow-x-auto max-w-full">
        {MARKETPLACE_CATEGORIES.map((c) => (
          <button
            ...
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md whitespace-nowrap transition-colors ${category === c ? "bg-[#16A34A] text-white" : "text-slate-500 hover:text-slate-700"}`}
          >
            {c}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading &&
          Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-xl border border-slate-200/80 h-44 skeleton-shimmer"
            />
          ))}
        {!loading &&
          filtered.map((t) => (
            <div
              key={t.id}
              className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex flex-col"
            >
              ...
              <p className="text-[12px] text-slate-500 leading-relaxed mb-3 flex-1">
                {t.description}
              </p>
              <div className="flex items-center gap-1 mb-3 flex-wrap">
                {t.steps.map((s, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={stepType?.label}
                    >
                      <Icon size={11} style={{ color: stepType?.color }} />
                    </div>
                  );
                })}
              </div>
              <p className="text-[10.5px] text-slate-400 mb-3">
                By {t.publisherName} · {t.installCount} install{t.installCount === 1 ? "" : "s"}
              </p>
              <button
                onClick={() => onInstall(t)}
                className="btn-primary text-white text-[12.5px] font-medium rounded-lg py-2.5 w-full"
              >
                Install
              </button>
            </div>
        ...
        {!loading && filtered.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Publish one of your own workflows to get this category started."
            />
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Description for other organizations" required>
            <textarea
              ...
              placeholder="What does this automation do, and when should someone install it?"
            />
          </FormField>
          {hasEmailStep && (
            <div className="flex items-start gap-2 bg-[#F59E0B]/10 rounded-lg p-3">
              ...
              <p className="text-[11.5px] text-[#8a670a] leading-relaxed">
                This workflow includes an email step with a recipient address — that&apos;s cleared
                automatically before publishing, since it&apos;s your own contact data, not
                something that belongs in a shared template.
              </p>
            </div>
          )}
          <p className="text-[11px] text-slate-400">
            Published under your company&apos;s name — visible to any organization using this
            software.
          </p>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          ...
          <button
            ...
            className="flex-1 text-[12px] font-medium btn-primary text-white rounded-lg py-2.5 disabled:opacity-40"
          >
            Publish
          </button>
        </div>
