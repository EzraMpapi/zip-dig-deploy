...
export function PharmacyManagementModule({ currentUser, company, onStockLoad }) {
  ...
  useEffect(() => {
    if (onStockLoad) onStockLoad(stock.rows);
  }, [stock.rows, onStockLoad]);
  ...
  const todayRevenue = dispense.rows
  ...
  const expiryColor = (days) =>
    days < 0 ? "#EF4444" : days < 30 ? "#EF4444" : days < 60 ? "#F59E0B" : "#16A34A";
  ...
  async function addDrug() {
    ...
    const row = {
      ...
      unitCost: Number(drugForm.unitCost),
    };
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("phm_drugs").insert(row).run();
      } catch (_e) {}
    }
  }
  ...
  async function dispenseDrug() {
    ...
    const row = {
      ...
      status: "Dispensed",
      prescriber: disForm.prescriber || currentUser?.name || "Pharmacist",
    };
    dispense.setRows((p) => [row, ...p]);
    // Reduce stock
    stock.setRows((p) =>
      p.map((s) =>
        s.drugId === disForm.drugId ? { ...s, qty: Math.max(0, s.qty - Number(disForm.qty)) } : s,
      ),
    );
    ...
    logAudit(
      ...
      disForm.patient + " × " + disForm.qty,
    );
  }

  const filteredDrugs = drugs.rows.filter(
    (d) =>
      !searchDrug ||
      d.name.toLowerCase().includes(searchDrug.toLowerCase()) ||
      d.genericName?.toLowerCase().includes(searchDrug.toLowerCase()) ||
      d.category?.toLowerCase().includes(searchDrug.toLowerCase()),
  );

  const StatusPill = ({ s, mini }) => {
    const cfg = {
      Active: ["#DCFCE7", "#16A34A"],
      Dispensed: ["#DCFCE7", "#16A34A"],
      Pending: ["#FEF3C7", "#D97706"],
      Expired: ["#FEE2E2", "#EF4444"],
    ...
    return (
      <span
        className={
          ...
          (mini ? "text-[9.5px] px-1.5 py-0.5" : "text-[10.5px] px-2 py-0.5")
        }
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Tablets size={22} className="text-white" />
              <h1 className="text-[20px] font-bold text-white">Pharmacy Management System</h1>
            </div>
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.65)" }}>
              Drug Catalog · Stock Control · Dispensing · Expiry Alerts · Billing
            </p>
          </div>
          <div className="flex gap-2">
            {lowStock.length > 0 && (
              <div className="flex items-center gap-1.5 bg-[#FEF3C7] text-[#92400E] px-3 py-2 rounded-xl text-[12px] font-semibold">
                <AlertCircle size={13} />
                {lowStock.length} Low Stock
              </div>
            )}
            {expiringItems.length > 0 && (
              <div className="flex items-center gap-1.5 bg-[#FEE2E2] text-[#991B1B] px-3 py-2 rounded-xl text-[12px] font-semibold">
                <AlertCircle size={13} />
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={{ background: tab === t.id ? PHM_GREEN : "transparent" }}
            >
              <I size={12} />
              {t.label}
              {t.id === "expiry" && expiringItems.length > 0 && (
                <span className="ml-0.5 bg-[#EF4444] text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
        ...
        <div className="space-y-4">
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p
                      className="text-[11.5px] mt-0.5 truncate max-w-[140px]"
                      style={{ color: k.c }}
                    >
                      {k.sub}
                    </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {["Antibiotic", "Analgesic", "Antidiabetic", "Antihypertensive", "Insulin"]
                .map((cat) => {
                  const catDrugs = drugs.rows.filter((d) => d.category === cat);
                  const catStock = stock.rows
                    .filter((s) => catDrugs.some((d) => d.id === s.drugId))
                    ...
                    <div key={cat} className="flex items-center gap-2 mb-2.5">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: pct + "%", background: PHM_GREEN }}
                        />
                      </div>
                      <span className="text-[11.5px] font-mono font-bold text-slate-700 w-10 text-right">
                        {catStock}
                      </span>
                    </div>
                  );
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {dispense.rows.slice(0, 5).map((d) => (
                  ...
                  >
                    <div>
                      <p className="text-[12.5px] font-medium text-[#111827]">{d.patient}</p>
                      <p className="text-[11px] text-slate-400">
                        {d.drug} · {d.date}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-[13px] font-bold text-[#059669]">TZS {money(d.price)}k</p>
                      <StatusPill s={d.status} mini />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {tab === "catalog" && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() =>
                downloadCSV("pharmacy-drugs", drugs.rows, [
                  { key: "name", label: "Drug" },
              ...
              className="flex items-center gap-1 text-[12px] font-medium text-slate-500 border border-slate-200 px-3 py-2.5 rounded-xl hover:border-green-500 hover:text-green-600 transition-colors"
            >
              <Download size={13} />
              Export
            </button>
          ...
          {showDrug && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Selling Price (TZS)">
                  <input
                    type="number"
                    step="0.01"
                    className={inputClass}
                ...
                <FormField label="Unit Cost (TZS)">
                  <input
                    type="number"
                    step="0.01"
                    className={inputClass}
                    value={drugForm.unitCost}
                    onChange={(e) => setDrugForm({ ...drugForm, unitCost: e.target.value })}
                  />
                </FormField>
              </div>
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={drugForm.requiresRx}
                    onChange={(e) => setDrugForm({ ...drugForm, requiresRx: e.target.checked })}
                    className="rounded"
                  />
                  <span className="text-[12.5px] text-slate-600">Requires Prescription (Rx)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={drugForm.controlled}
                    onChange={(e) => setDrugForm({ ...drugForm, controlled: e.target.checked })}
                    className="rounded"
                  />
                  <span className="text-[12.5px] text-slate-600">Controlled Substance</span>
                </label>
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <span
                        ...
                        style={{ background: PHM_GREEN + "18", color: PHM_GREEN }}
                      >
                        {d.category}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-3 font-mono font-semibold" style={{ color: PHM_GREEN }}>
                      TZS {d.price}
                    </td>
      ...
      {tab === "stock" && (
        <div className="space-y-3">
          <div className="grid grid-cols-4 gap-3">
            {[
              ["Total Units", stock.rows.reduce((s, r) => s + r.qty, 0), PHM_GREEN],
              ["Stock Value", "TZS " + money(stockValue) + "k", "#2563EB"],
              ["Low Stock", lowStock.length, "#F59E0B"],
              ["Expired", expiredItems.length, "#EF4444"],
            ].map(([l, v, col]) => (
              <div
                key={l}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-semibold text-[#111827]">Stock Levels</p>
              <button
                onClick={() => notify("Add stock receipt form")}
                  ...
                  return (
                    <tr
                      ...
                      className={
                        ...
                        (isLow || isExp ? "bg-red-50/30" : "hover:bg-slate-50/50")
                      }
                    >
                      ...
                      <td className="px-3 py-3 font-bold" style={{ color: expiryColor(days) }}>
                        {days < 0 ? "EXPIRED" : days + "d"}
                      </td>
                      <td className="px-3 py-3 text-slate-500 text-[11.5px]">{s.supplier}</td>
                      <td
                        className="px-3 py-3 font-mono font-semibold"
                        style={{ color: PHM_GREEN }}
                      >
                        TZS {money(s.qty * s.unitCost)}k
                      </td>
                      <td className="px-3 py-3">
                        <StatusPill s={rowStatus} />
                      </td>
                    </tr>
                  );
                })}
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <FormField label="Patient Name *">
                  <input
                    className={inputClass}
                    value={disForm.patient}
                    onChange={(e) => setDisForm({ ...disForm, patient: e.target.value })}
                ...
                <FormField label="Drug *">
                  <select
                    className={inputClass}
                    value={disForm.drugId}
                    onChange={(e) => setDisForm({ ...disForm, drugId: e.target.value })}
                  >
                ...
                <FormField label="Quantity">
                  <input
                    ...
                    className={inputClass}
                    value={disForm.qty}
                    onChange={(e) => setDisForm({ ...disForm, qty: e.target.value })}
                  />
                </FormField>
                <FormField label="Dosage Instructions">
                  <input
                    className={inputClass}
                    value={disForm.dosage}
                    onChange={(e) => setDisForm({ ...disForm, dosage: e.target.value })}
                    placeholder="e.g. 1 TID × 7 days"
                  />
                </FormField>
                <FormField label="Prescriber">
                  <input
                    className={inputClass}
                    value={disForm.prescriber}
                    onChange={(e) => setDisForm({ ...disForm, prescriber: e.target.value })}
                ...
                <FormField label="Prescription #">
                  <input
                    className={inputClass}
                    value={disForm.rxNo}
                    onChange={(e) => setDisForm({ ...disForm, rxNo: e.target.value })}
                    placeholder="RX-001"
                  />
                </FormField>
              </div>
                  ...
                  return (
                    <div className="p-3 rounded-xl bg-green-50 border border-green-100">
                      <p className="text-[13px] font-semibold text-green-800">
                        Total: <strong>TZS {money(total)}k</strong> · {d?.name} × {disForm.qty}{" "}
                        units @ TZS {d?.price} each
                      </p>
                    </div>
                  );
                })()}
              ...
              <tbody>
                {dispense.rows.map((d, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 font-mono font-bold" style={{ color: PHM_GREEN }}>
                      TZS {money(d.price)}k
                    </td>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-[12px] text-slate-400">{s.contact}</p>
                  </div>
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={l} className="flex justify-between">
                      ...
                      <span className="text-[11.5px] font-medium text-[#111827] truncate max-w-[160px]">
                        {v}
                      </span>
                    </div>
                  ))}
                ...
                <button
                  onClick={() => notify("Purchase order for " + s.name)}
                  className="mt-3 w-full text-[11.5px] font-semibold py-2 rounded-xl border border-slate-200 text-slate-600 hover:border-green-500 hover:text-green-600 transition-colors"
                >
      ...
      {tab === "expiry" && (
        <div className="space-y-4">
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[13.5px] font-semibold text-red-700">
                  {expiredItems.length} Expired Item{expiredItems.length > 1 ? "s" : ""} — Remove
                  Immediately
                </p>
              </div>
              {expiredItems.map((s) => (
                <p key={s.id} className="text-[12px] text-red-600 ml-5">
                  • {s.drug} (Batch: {s.batchNo}) — Expired {s.expiry}
                </p>
              ))}
            </div>
          )}
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Expired", expiredItems.filter((s) => daysToExpiry(s.expiry) < 0).length, "#EF4444"],
              [
                "Expiring < 30 days",
                stock.rows.filter((s) => {
              ...
              [
                "Expiring < 90 days",
                expiringItems.filter((s) => daysToExpiry(s.expiry) >= 0).length,
                "#D97706",
              ],
            ].map(([l, v, col]) => (
              <div
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Drug", "Batch", "Qty", "Expiry Date", "Days Left", "Action"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {expiringItems.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-10 text-center text-green-600 font-medium">
                      ✓ No items expiring within 90 days — stock is healthy!
                    </td>
                  </tr>
                ) : (
                  expiringItems.map((s) => {
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          background: days < 0 ? "#FEF2F2" : days < 30 ? "#FFFBEB" : "#F0FFF4",
                        }}
                      >
                        ...
                        <td className="px-4 py-3 font-mono" style={{ color: expiryColor(days) }}>
                          {s.expiry}
                        </td>
                        <td
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => notify("Return/quarantine " + s.drug)}
                            className="text-[11px] font-semibold px-3 py-1.5 rounded-lg"
                            style={{ background: days < 0 ? "#EF4444" : "#F59E0B", color: "#fff" }}
                          >
                            {days < 0 ? "Remove" : "Prioritise"}
                          </button>
                        </td>
                      </tr>
                    );
      ...
      {tab === "billing" && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Today Revenue", "TZS " + money(todayRevenue) + "k", "#059669"],
              [
                "This Month Revenue",
                "TZS " + money(dispense.rows.reduce((s, d) => s + d.price, 0)) + "k",
                "#2563EB",
              ],
              [
                "Avg Sale",
                "TZS " +
                  money(
                    dispense.rows.length > 0
                      ? dispense.rows.reduce((s, d) => s + d.price, 0) / dispense.rows.length
                      : 0,
                  ) +
                  "k",
                "#7C3AED",
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[13.5px] font-semibold text-[#111827]">Revenue by Drug</p>
            </div>
            {drugs.rows
              .map((d) => {
                const drugSales = dispense.rows
                  .filter((dp) => dp.drug === d.name)
                  .reduce((s, dp) => s + dp.price, 0);
                ...
                return (
                  <div key={d.id} className="flex items-center gap-3 mb-2.5">
                    <span className="text-[12px] text-slate-600 w-44 shrink-0 truncate">
                      {d.name}
                    </span>
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div
                    ...
                    <span className="text-[12px] font-mono font-bold text-slate-700 w-20 text-right">
                      TZS {money(drugSales)}k
                    </span>
                  </div>
                );
              })
      ...
      {tab === "analytics" &&
        (() => {
          const catData = DRUG_CATEGORIES.map((cat, i) => ({
            ...
            value: stock.rows
              .filter((s) => drugs.rows.find((d) => d.id === s.drugId)?.category === cat)
          ...
          const expiringData = stock.rows
            ...
            .slice(0, 6)
            .map((s) => ({
              name: (drugs.rows.find((d) => d.id === s.drugId)?.name || s.name || "Drug").slice(
                0,
                14,
              ),
          ...
          const dispensedData = drugs.rows
            .slice(0, 6)
            .map((d) => ({
              name: d.name.slice(0, 14),
              value: dispense.rows
                .filter((r) => r.drugId === d.id)
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ...
                  [
                    "Low Stock",
                    String(stock.rows.filter((s) => s.stock <= s.reorder && s.reorder > 0).length),
                    "#EF4444",
                  ],
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  {catData.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No stock data</p>
                  ) : (
                    <div className="flex items-center gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  {dispensedData.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No dispensing records</p>
                  ) : (
              ...
              {expiringData.length > 0 && (
                <div className="bg-[#FFFBEB] border border-[#FDE68A] rounded-xl p-4">
                  <h3 className="text-[13.5px] font-bold text-[#92400E] mb-3">
                    ⚠ Drugs Expiring Within 90 Days
                  </h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-[12px]">
                      <thead>
                        <tr className="bg-[#FCD34D]/30">
                          {["Drug", "Expiry Date", "Days Left", "Stock"].map((h) => (
                            <th
                              key={h}
                              className="px-3 py-2 text-left text-[10px] font-bold text-[#92400E]"
                            >
                              {h}
                            </th>
                          ))}
                          ...
                          <tr key={i} className="border-b border-[#FDE68A]/50">
                            ...
                            <td className="px-3 py-2 font-mono text-[11.5px]">
                              {stock.rows.find(
                                (s) =>
                                  drugs.rows.find(
                                    (dr) => dr.id === s.drugId && dr.name?.slice(0, 14) === d.name,
                                  )?.id === s.drugId,
                              )?.expiryDate || "—"}
                            </td>
                            <td
                              className="px-3 py-2 font-bold"
                              style={{ color: d.days <= 30 ? "#EF4444" : "#F59E0B" }}
                            >
                              {d.days}d
                            </td>
...
export const HTL_ROOMS_SEED = [
  ...
  },
  {
    ...
    price: 220,
    status: "Occupied",
    amenities: ["AC", "WiFi", "TV", "Minibar", "Balcony", "Jacuzzi"],
  },
  {
    id: "RM-302",
    number: "302",
