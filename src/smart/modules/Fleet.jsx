...
import { Bus, Car, Gauge, LayoutDashboard, MapPin, Plus, Wrench } from "lucide-react";
import {
  Area,
  AreaChart,
  Bar,
...
const FLT_MAINTENANCE_SEED = [
  {
    ...
    type: "Service",
    date: "2026-05-18",
    workshop: "Kariakoo Auto Works",
    mileageAtService: 178400,
    cost: 420,
    status: "Completed",
  },
  {
    ...
    type: "Tyres",
    date: "2026-06-11",
    workshop: "Mikocheni Tyre Hub",
    mileageAtService: 92100,
    cost: 1180,
    status: "Completed",
  },
  {
...
export function FleetManagementModule({ currentUser, company, onVehiclesLoad }) {
  ...
  useEffect(() => {
    if (onVehiclesLoad) onVehiclesLoad(vehicles.rows);
  }, [vehicles.rows, onVehiclesLoad]);
  const trips = useCompanyTable("flt_trips", FLT_TRIPS_SEED, { mapRow: (r) => r });
  const maintenance = useCompanyTable("flt_maintenance", FLT_MAINTENANCE_SEED, {
    mapRow: (r) => r,
  });
  ...
  const expIns = vehicles.rows.filter(
    (v) => new Date(v.insurance) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000),
  );
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Bus size={22} className="text-[#EAB308]" />
              <h1 className="text-[20px] font-bold text-white">Fleet Management</h1>
            </div>
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.55)" }}>
              Vehicles · Trip Logs · Fuel Tracking · Maintenance · Insurance
            </p>
          </div>
          <div className="flex gap-2">
            {dueService.length > 0 && (
              <div className="bg-yellow-500 text-yellow-900 px-3 py-2 rounded-xl text-[12px] font-bold">
                {dueService.length} Service Due
              </div>
            )}
            {expIns.length > 0 && (
              <div className="bg-red-500 text-white px-3 py-2 rounded-xl text-[12px] font-bold">
                {expIns.length} Insurance Expiring
              </div>
            )}
          </div>
        </div>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => setTab(t.id)}
              className={
                "flex items-center gap-1 px-4 py-2 rounded-lg text-[12px] font-medium transition-all " +
                (tab === t.id ? "text-white shadow-sm" : "text-slate-500 hover:bg-slate-50")
              }
              style={{ background: tab === t.id ? FLT_BLUE : "transparent" }}
            >
              <I size={13} />
              {t.label}
            </button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {
                l: "Fleet Size",
                v: vehicles.rows.length,
                sub: activeVeh + " active",
                c: "#0F172A",
                I: Car,
              },
              {
              ...
              {
                l: "Fuel Cost",
                v: "TZS " + money(fuelCost) + "k",
                sub: totalFuel + "L used",
                c: FLT_GOLD,
                I: Gauge,
              },
              {
                l: "Maintenance",
                v: "TZS " + money(maintCost) + "k",
                sub: "YTD spend",
                c: "#EF4444",
                I: Wrench,
              },
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const months = Array.from({ length: 6 }, (_, i) => {
                  ...
                  const dist = trips.rows
                    .filter((t) => (t.date || "").startsWith(key))
                    .reduce((s, t) => s + t.distance, 0);
                  const fuel = trips.rows
                    .filter((t) => (t.date || "").startsWith(key))
                    .reduce((s, t) => s + t.fuelUsed, 0);
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                ...
                const statusData = Object.entries(
                  vehicles.rows.reduce(
                    (m, v) => ({ ...m, [v.status]: (m[v.status] || 0) + 1 }),
                    {},
                  ),
          ...
          {dueService.length > 0 && (
            <div className="bg-yellow-50er-yellow-200 rounded-xl p-4">
              <p className="text-[13px] font-semibold text-yellow-800 mb-2">⚠ Service Due Soon</p>
              {dueService.map((v) => (
                <p key={v.id} className="text-[12px] text-yellow-700">
                  • {v.reg} ({v.make} {v.model}) — {v.mileage.toLocaleString()}km /{" "}
                  {v.nextService.toLocaleString()}km service
                </p>
              ))}
            </div>
          )}
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {vehicles.rows.map((v) => {
              ...
              const insExp =
                new Date(v.insurance) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000);
              return (
                <div
                  key={v.id}
                  className={
                    "bg-white rounded-xl border shadow-sm p-4 " +
                    (v.status === "Service" ? "border-yellow-200" : "border-slate-200/80")
                  }
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-[16px] font-bold text-[#111827]">{v.reg}</p>
                      ...
                      <div key={l} className="flex justify-between">
                        ...
                        <span
                          className={
                            ...
                            (l === "Insurance" && insExp ? "text-red-500" : "text-[#111827]")
                          }
                        >
                          {val}
                        </span>
                  ...
                  <div>
                    <p className="text-[10.5px] text-slate-400 mb-1">
                      Next service: {v.nextService.toLocaleString()}km (
                      {kmLeft > 0 ? kmLeft.toLocaleString() + "km left" : "OVERDUE"})
                    </p>
                    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: pct + "%",
                          background: pct > 95 ? "#EF4444" : pct > 80 ? "#F59E0B" : "#16A34A",
                        }}
                      />
                    </div>
                  </div>
      ...
      {tab === "trips" && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            <p className="text-[13.5px] font-semibold text-[#111827]">Trip Log</p>
            <button
              onClick={() => notify("Log new trip")}
              className="flex items-center gap-1 text-[12px] font-semibold text-white px-3 py-2 rounded-xl"
              style={{ background: FLT_BLUE }}
            >
              <Plus size={12} />
              Log Trip
            </button>
          ...
          <table className="w-full text-[12.5px]">
            ...
            <tbody>
              {trips.rows.map((t) => (
                ...
                >
                  <td
                    className="px-3 py-3 font-mono text-[11.5px] font-semibold"
                    style={{ color: FLT_BLUE }}
                  >
                    {t.vehicle}
                  </td>
                  ...
                  <td className="px-3 py-3 font-mono font-bold" style={{ color: FLT_GOLD }}>
                    TZS {money(t.cost)}k
                  </td>
                  <td className="px-3 py-3">
                    <VStatusChip s={t.status} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {tab === "maintenance" && (
        <div className="space-y-3">
          <div className="flex justify-end">
            <button
              onClick={() => notify("Log maintenance record")}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Vehicle", "Type", "Workshop", "Mileage", "Date", "Cost", "Status"].map((h) => (
                    <th
                      key={h}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-4">
            <p className="text-[11.5px] text-slate-400">
              Maintenance costs recorded here feed the cost-per-kilometre figures in the Analytics
              tab.
            </p>
          </div>
        </div>
          ...
          return (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  <h3 className="text-[13.5px] font-semibold text-[#111827] mb-3">
                    Operating Cost by Vehicle (TZS)
                  </h3>
                  {perVeh.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No trip data yet</p>
                  ) : (
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  {maintTrend.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No records yet</p>
                  ) : (
                    <ResponsiveContainer width="100%" height={155}>
                      <AreaChart
                        data={maintTrend}
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100 bg-[#0F172A]">
                    <p className="text-[12.5px] font-bold text-white">
                      Efficiency — Cost per KM by Vehicle
                    </p>
                  </div>
                  <table className="w-full text-[12.5px]">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50">
                        {["Vehicle", "KM Driven", "Fuel (L)", "Trip Cost", "Cost/KM"].map((h) => (
                          <th
                            key={h}
                            className="px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-wide text-slate-400"
                          >
                            {h}
                          </th>
                        ))}
                      ...
                      {perVeh.map((v, i) => (
                        <tr key={v.name} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                          ...
                          <td
                            ...
                            style={{
                              color: v.cpk > 50 ? "#EF4444" : v.cpk > 30 ? "#F59E0B" : "#16A34A",
                            }}
                          >
                            {v.cpk} TZS/km
                          </td>
...
export const BANK_TRANSACTIONS_SEED = [
  {
    id: "TXN-0001",
    accountNo: "1000000001",
    account: "Amina Hassan",
    type: "Deposit",
    amount: 500.0,
    balance: 2450.0,
    date: "2026-07-16 09:14",
    channel: "Teller",
    reference: "DEP-20260716-001",
    narration: "Cash deposit",
    teller: "Alice Njoroge",
    status: "Completed",
  },
  {
    id: "TXN-0002",
    ...
    amount: 2000.0,
    balance: 15800.0,
    date: "2026-07-16 10:02",
    channel: "Teller",
    reference: "WDR-20260716-001",
  ...
  },
  {
    id: "TXN-0003",
    accountNo: "2000000001",
    account: "Baraka Enterprise Ltd",
    type: "Transfer Out",
    amount: 5000.0,
    balance: 87500.0,
    date: "2026-07-16 11:30",
    channel: "Online",
    reference: "TRF-20260716-001",
  ...
  },
  {
    id: "TXN-0004",
    accountNo: "3000000001",
    account: "Grace Mwenda",
    type: "Interest",
    amount: 395.83,
    balance: 50000.0,
    date: "2026-07-01 00:00",
    channel: "System",
    reference: "INT-202607-001",
  ...
  },
  {
    ...
    amount: 1200.0,
    balance: 1950.0,
    date: "2026-07-14 15:45",
    channel: "Mobile",
    reference: "TRF-20260714-002",
    narration: "Received from sister",
    teller: "System",
    status: "Completed",
  },
];
...
export const BANK_LOANS_SEED = [
  ...
  },
  {
    ...
    loanNo: "LN20240002",
    clientId: "ACC-0003",
    client: "Baraka Enterprise Ltd",
    type: "Business",
    principal: 50000,
    rate: 15,
    months: 36,
    disbursed: "2024-03-15",
    installment: 1733.51,
    balance: 38200.0,
    arrears: 0,
    nextDue: "2026-08-15",
  ...
  },
  {
    ...
    loanNo: "LN20230001",
    clientId: "ACC-0006",
    client: "Uzuri Beauty Chain",
    type: "Business",
    principal: 100000,
    rate: 14,
    months: 60,
    disbursed: "2023-09-01",
    installment: 2327.43,
    balance: 0,
    arrears: 0,
    nextDue: "N/A",
...
export const BANK_STANDING_ORDERS_SEED = [
  {
    ...
    frequency: "Monthly",
    nextRun: "2026-08-01",
    beneficiary: "LUKU Prepaid",
    status: "Active",
  },
  {
    id: "SO-002",
    accountNo: "2000000001",
    debtor: "Baraka Enterprise",
    amount: 2000,
    frequency: "Monthly",
    nextRun: "2026-08-05",
    beneficiary: "NHIF Premium",
    status: "Active",
  },
];
