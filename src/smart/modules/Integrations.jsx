    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Real capabilities where a browser genuinely can — honest limits where it can't.{" "}
          {functionalCount} of {INTEGRATION_DIRECTORY.length} are genuinely working today, not
          configuration for a backend this app doesn't have.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
          {INTEGRATION_DIRECTORY.map((item) => (
            ...
            >
              ...
              <span
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${item.functional ? "bg-[#16A34A]" : "bg-slate-300"}`}
                title={item.functional ? "Real and working" : "Needs backend infrastructure"}
              />
            </button>
          ))}
        </div>
      ...
      {tab === "connections" && (
        <IntegrationConnections canManage={canManage} currentUser={currentUser} />
      )}
      {tab === "mobile-money" && (
        <MobileMoneyReconciliation invoices={invoices} currentUser={currentUser} />
      )}
...
export function IntegrationConnections({ canManage, currentUser }) {
  ...
  async function updateField(id, key, value) {
    ...
    if (IS_CONFIGURED) {
      ...
      try {
        await sb("integration_connections")
  ...
  function openLink(url) {
    if (!url || !url.trim()) {
      notify("No link configured yet.", "error");
      return;
    }
  ...
  async function testSlackWebhook(url) {
    if (!url || !url.trim()) {
      notify("No webhook URL configured yet.", "error");
      return;
    }
    const result = await sendWebhookNotification(
      url,
      "Test message from Smart Manager — your Slack connection works.",
    );
  ...
  function openWhatsApp(number) {
    if (!number || !number.trim()) {
      notify("No WhatsApp number configured yet.", "error");
      return;
    }
    ...
    <div className="space-y-4">
      {!canManage && (
        <div className="flex items-start gap-2.5 bg-[#F59E0B]/5 border border-[#F59E0B]/20 rounded-lg p-3">
          ...
          <p className="text-[12px] text-[#8a670a] leading-relaxed">
            You're viewing as {currentUser.role}. Editing connection configuration requires a
            full-write role.
          </p>
        </div>
      )}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {INTEGRATION_CONNECTIONS.map((meta) => {
          ...
          const linkField = meta.fields.find(
            (f) => f.key === "paymentLink" || f.key === "paypalMeLink",
          );
          return (
            <div
              key={meta.id}
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span
                      className={`text-[10px] font-medium ${meta.functional ? "text-[#16A34A]" : "text-slate-400"}`}
                    >
                      {meta.functional ? "Real, working today" : "Needs backend OAuth"}
                    </span>
                  </div>
                </div>
                {loading ? (
              ...
              {meta.functional && linkField && (
                <button
                  onClick={() => openLink(config[linkField.key])}
                  disabled={!config.enabled}
                  className="w-full text-[12px] font-medium btn-primary text-white rounded-lg py-2 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
              ...
              {meta.functional && meta.id === "slack" && (
                <button
                  onClick={() => testSlackWebhook(config.webhookUrl)}
                  disabled={!config.enabled}
                  className="w-full text-[12px] font-medium btn-primary text-white rounded-lg py-2 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
              ...
              {meta.functional && meta.id === "whatsapp-business" && (
                <button
                  onClick={() => openWhatsApp(config.businessNumber)}
                  disabled={!config.enabled}
                  className="w-full text-[12px] font-medium btn-primary text-white rounded-lg py-2 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
