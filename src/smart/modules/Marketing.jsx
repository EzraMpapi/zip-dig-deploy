...
export function Marketing({ crm }) {
  ...
  const campaigns = useCompanyTable("marketing_campaigns", campaignsSeed, {
    order: { col: "sent_date", ascending: false },
  ...
  const stats = useMemo(() => {
    ...
    const avgOpen = sent.length
      ? Math.round(sent.reduce((s, c) => s + (c.openRate || 0), 0) / sent.length)
      : 0;
    const avgClick = sent.length
      ? Math.round(sent.reduce((s, c) => s + (c.clickRate || 0), 0) / sent.length)
      : 0;
  ...
  const MKT_KPIS = [
    ...
    },
    {
      label: "Avg Open Rate",
      value: `${stats.avgOpen}%`,
      delta: "Sent campaigns",
      up: stats.avgOpen >= 40,
      icon: Eye,
    },
    {
      label: "Avg Click Rate",
      value: `${stats.avgClick}%`,
      delta: "Sent campaigns",
      up: stats.avgClick >= 10,
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Campaigns targeted at real segments of your CRM pipeline
        </p>
      </div>
...
export function Campaigns({ campaigns, segments }) {
  ...
  async function addCampaign(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("marketing_campaigns")
          .insert({
            name: draft.name,
        ...
        notify("Campaign created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function advanceCampaign(id, next) {
    ...
    if (IS_CONFIGURED && campaign?.dbId) {
      try {
        ...
        notify("Couldn't save the campaign status to the server.", "error");
      }
    }
  }
  ...
  async function deleteCampaign(id) {
    ...
    if (IS_CONFIGURED && campaign?.dbId) {
      try {
        await sb("marketing_campaigns").eq("id", campaign.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the campaign on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      ...
      {rows.length > 0 &&
        (() => {
          ...
          const avgOpen =
            sent.length > 0
              ? Math.round(sent.reduce((s, r) => s + (r.openRate || 0), 0) / sent.length)
              : 0;
          const avgClick =
            sent.length > 0
              ? Math.round(sent.reduce((s, r) => s + (r.clickRate || 0), 0) / sent.length)
              : 0;
          const byType = ["Email", "SMS", "WhatsApp", "Social"]
            .map((t, i) => ({
              name: t,
          ...
          const topCampaigns = [...rows]
            .filter((r) => r.openRate > 0)
            .sort((a, b) => (b.openRate || 0) - (a.openRate || 0))
            .slice(0, 6)
            .map((r) => ({
              name: r.name.slice(0, 18) + (r.name.length > 18 ? "…" : ""),
              open: r.openRate || 0,
              click: r.clickRate || 0,
            }));
          return (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  {topCampaigns.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No sent campaigns yet</p>
                  ) : (
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  {byType.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No campaigns</p>
                  ) : (
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div
                            ...
                            style={{ backgroundColor: `${typeMeta.color}14` }}
                          >
                      ...
                      <td className="px-4 py-3 text-slate-500">
                        {c.segment}{" "}
                        <span className="text-slate-400 font-mono text-[11px]">({audience})</span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                          style={{
                            backgroundColor: `${CAMPAIGN_STATUS_COLOR[c.status]}14`,
                            color: CAMPAIGN_STATUS_COLOR[c.status],
                          }}
                        >
                          <span
                            className="w-1.5 h-1.5 rounded-full"
                            style={{ backgroundColor: CAMPAIGN_STATUS_COLOR[c.status] }}
                          />
                          {c.status}
                        </span>
                      </td>
      ...
      >
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <h2 className="text-[16px] font-semibold text-[#111827] leading-snug break-words">
                {campaign.name}
              </h2>
            </div>
          </div>
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${CAMPAIGN_STATUS_COLOR[campaign.status]}14`,
              color: CAMPAIGN_STATUS_COLOR[campaign.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: CAMPAIGN_STATUS_COLOR[campaign.status] }}
            />
            {campaign.status}
          </span>
        </div>
        ...
        {campaign.status === "Sent" ? (
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-slate-50 rounded-lg p-3">
              ...
              <p className="text-[18px] font-mono font-semibold text-[#16A34A]">
                {campaign.openRate}%
              </p>
            </div>
            <div className="bg-slate-50 rounded-lg p-3">
          ...
          <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3 mb-6">
            ...
            <p className="text-[12px] text-slate-500 leading-snug">
              Performance metrics appear once this campaign is sent.
            </p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 pt-4 flex flex-col gap-2">
          {nextStatus && (
            <button
              onClick={() => onAdvance(campaign.id, nextStatus)}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Campaign name" required>
            <input
              ...
              placeholder="e.g. Q3 Wholesale Promo"
            />
            {touched && !form.name.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Campaign name is required.</p>
            )}
          </FormField>
            ...
            >
              {segments.map((s) => (
                <option key={s.industry} value={s.industry}>
                  {s.industry} ({s.count} leads)
                </option>
              ))}
            </select>
            <p className="text-[11px] text-slate-400 mt-1">
              Audience is computed live from your CRM pipeline by industry.
            </p>
          </FormField>
        </div>
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
        <EmptyState
          ...
          hint="Segments are computed from CRM leads by industry — add leads in CRM and they'll group here automatically."
        />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
      ...
      <p className="text-[11.5px] text-slate-400 mb-5">
        Live from CRM — every lead is counted exactly once, grouped by its industry
      </p>
      <div className="space-y-4">
        {segments.map((s) => (
          <div key={s.industry}>
            <div className="flex items-center justify-between text-[13px] mb-1.5">
              ...
              <span className="text-slate-500">
                <span className="font-mono">{s.count}</span> leads ·{" "}
                <span className="font-mono">TZS {money(s.value)}k</span> pipeline · avg score{" "}
...
export function BulkSmsView({ crm }) {
  ...
  const templates = useCompanyTable("sms_templates", [], {
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      name: r.name,
  ...
  const groups = useCompanyTable("sms_groups", [], {
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      name: r.name,
      members: (r.sms_group_members || []).map((m) => ({ name: m.name, phone: m.phone })),
    }),
    select: "*,sms_group_members(*)",
  });
  ...
  async function saveTemplate(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("sms_templates")
          .insert({ name: draft.name, category: draft.category, message: draft.message })
          .single()
          .run();
        if (header?.id)
          templates.setRows((prev) =>
  ...
  async function saveGroup(e) {
    ...
    const members = crm.rows
      .filter((l) => selectedLeads.has(l.id))
      .map((l) => ({ name: l.company, phone: l.phone }));
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        if (header?.id) {
          groups.setRows((prev) =>
            prev.map((g) => (g.id === draft.id ? { ...g, dbId: header.id } : g)),
          );
          if (members.length > 0)
            await sb("sms_group_members")
              .insert(members.map((m) => ({ group_id: header.id, name: m.name, phone: m.phone })))
              .run();
        }
      } catch (_e) {
        notify("Saved locally, but the server update failed.", "error");
      ...
      >
        <div className="flex items-start gap-2.5">
          ...
          <p className="text-[12px] text-white/90 leading-relaxed">
            Templates and groups below are real. Actually sending a message needs a real SMS gateway
            connected — this build has no SMS provider credentials configured, so "Send" is disabled
            rather than pretending to succeed. Connect a provider like Africa&apos;s Talking or
            Twilio to enable real sending.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        {[
          ...
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-colors ${tab === t.id ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
          >
            {t.label}
          </button>
        ))}
      ...
      {tab === "groups" && (
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm divide-y divide-slate-50">
            {!groups.loading && groups.rows.length === 0 && (
              <EmptyState
                icon={Users}
                title="No Customer Groups"
                hint="Create groups to organize customers for future bulk messaging."
                actionLabel="Create Group"
                onAction={() => setShowGroupForm(true)}
              />
            )}
            {groups.loading && (
            ...
            {groups.rows.map((g) => (
              <div key={g.id} className="flex items-center justify-between px-4 py-3.5">
                <div>
                  <p className="text-[13px] font-medium text-[#111827]">{g.name}</p>
                  <p className="text-[11px] text-slate-400">{g.members.length} members</p>
                </div>
                <button
                  disabled
                  title="Requires a connected SMS gateway"
                  className="text-[12px] font-medium text-slate-300 cursor-not-allowed flex items-center gap-1"
                >
                  <Send size={13} /> Send
                </button>
              </div>
      ...
      {tab === "templates" && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {!templates.loading && templates.rows.length === 0 && (
              <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
                <EmptyState
                  icon={FileText}
                  title="No templates"
                  hint="Save a reusable message template with variable placeholders."
            ...
            {templates.loading && (
              <p className="col-span-full text-[12.5px] text-slate-400 text-center py-8">
                Loading...
              </p>
            )}
            {templates.rows.map((t) => (
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <p className="text-[13px] font-medium text-[#111827]">{t.name}</p>
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
              ...
              <div>
                <label className="text-[12px] font-medium text-slate-600 block mb-1.5">
                  Select real customers from CRM ({selectedLeads.size} selected)
                </label>
                  ...
                  {crm.rows
                    ...
                    .map((l) => (
                      ...
                      >
                        ...
                        />
                        <div className="min-w-0">
                          <p className="text-[12.5px] text-[#111827] truncate">{l.company}</p>
                          <p className="text-[10.5px] text-slate-400">{l.phone}</p>
                        </div>
                      </label>
                    ))}
                  {crm.rows.filter((l) => l.phone).length === 0 && (
                    <p className="text-[11.5px] text-slate-400 text-center py-4">
                      No CRM leads with a phone number yet.
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-40 flex justify-end">
          ...
          >
            ...
            <div className="px-6 py-5 flex-1 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {SMS_CATEGORIES.map((c) => (
                    <button
                      key={c}
                      type="button"
                      onClick={() => setTemplateForm((f) => ({ ...f, category: c }))}
                      className={`text-[12px] font-medium px-3 py-1.5 rounded-full border transition-colors ${templateForm.category === c ? "border-[#2563EB]/50 bg-[#2563EB]/10 text-[#2563EB]" : "border-slate-200 text-slate-500"}`}
                    >
                      {c}
                    </button>
                  ))}
              ...
              <div className="flex flex-wrap gap-2">
                {SMS_VARIABLES.map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => setTemplateForm((f) => ({ ...f, message: f.message + v }))}
                    className="text-[11px] font-mono px-2 py-1 rounded-md bg-slate-100 text-slate-500 hover:bg-slate-200"
                  >
                    {v}
                  </button>
                ))}
              ...
              <p className="text-[11px] text-slate-400">
                {templateForm.message.length} characters · {smsCount}/{smsCount} SMS
              </p>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
              <button
