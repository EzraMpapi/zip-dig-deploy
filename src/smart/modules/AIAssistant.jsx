...
export function buildBusinessSnapshot(
  { company, invoices, inventory, crm, expenses, employees, leaveRequests, suppliers },
  scope,
) {
  const snapshot = {
    company: {
    ...
    const outstanding = invoices.rows
      ...
      .map((inv) => {
        ...
        return {
          ...
          dueDate: inv.dueDate,
          status: inv.status,
          balance_tzs_k: Math.round(total - (inv.amountPaid || 0)),
        };
      });
  ...
  if (scope.includes("inventory")) {
    snapshot.inventory = inventory.rows.map((it) => ({
      ...
      status: stockStatus(it.qty, it.reorder),
      unitCost_tzs_k: it.unitCost,
      warehouse: it.warehouse,
    }));
    snapshot.inventory_totals = {
      stock_value_tzs_k: Math.round(inventory.rows.reduce((s, it) => s + it.qty * it.unitCost, 0)),
    };
  }

  if (scope.includes("suppliers")) {
    snapshot.suppliers = suppliers.rows.map((s) => ({
      name: s.name,
      category: s.category,
  ...
  if (scope.includes("hr")) {
    snapshot.employees = employees.rows.map((e) => ({
      name: e.name,
      role: e.role,
      department: e.department,
      status: e.status,
      salary_tzs_k: e.salary,
    }));
    snapshot.leave_requests = leaveRequests.rows.map((l) => ({
      employee: l.employee,
      type: l.type,
    ...
    snapshot.hr_totals = {
      monthly_payroll_tzs_k: employees.rows
...
export const AI_PERSONAS = [
  {
    ...
    icon: Brain,
    color: "#16A34A",
    tagline: "Cross-functional strategic guidance",
    scope: ["finance", "sales", "inventory", "hr"],
    tools: [
      "create_lead",
      "adjust_stock",
      "mark_invoice_paid",
    ...
    suggestions: [
      "What are the three biggest risks to this business right now?",
      "Show me expenses by category as a chart",
      "Give me a plain-language summary of overall business health",
    ],
  },
  {
    id: "accountant",
    ...
    color: "#111827",
    tagline: "Bookkeeping, reconciliation, and expense review",
    scope: ["finance"],
    tools: ["mark_invoice_paid", "record_expense", "create_invoice", "show_chart"],
    suggestions: [
      "Which invoices are overdue and by how much?",
      "Are there any unusual expenses I should look at?",
      "Create an invoice for Kilimo Fresh Distributors for 50 bags of cement",
    ],
  },
  {
    ...
    icon: TrendingUp,
    color: "#16A34A",
    tagline: "Cash flow, profitability, and financial strategy",
    scope: ["finance"],
    tools: ["mark_invoice_paid", "show_chart"],
    suggestions: [
      "Is our cash position healthy right now?",
      "Show me outstanding receivables as a chart",
      "Should we be concerned about our receivables?",
    ],
  },
  {
    id: "hr-officer",
    ...
    icon: Users,
    color: "#F59E0B",
    tagline: "Workforce, leave, and payroll guidance",
    scope: ["hr"],
    tools: ["approve_leave"],
    suggestions: [
      "Who has pending leave requests right now?",
      "What's our current monthly payroll cost?",
      "Approve the oldest pending leave request",
    ],
  },
  {
    id: "sales-coach",
    ...
    icon: Award,
    color: "#F59E0B",
    tagline: "Pipeline strategy and deal coaching",
    scope: ["sales"],
    tools: ["create_lead", "create_quotation", "show_chart"],
    suggestions: [
      "Show me my pipeline by stage as a chart",
      "Draft a quotation for 10 salon styling chairs for Uzuri Beauty Chain",
      "Coach me on how to advance our biggest open deal",
    ],
  },
  {
    ...
    icon: ClipboardCheck,
    color: "#5B6472",
    tagline: "Purchasing decisions and supplier insight",
    scope: ["inventory", "suppliers"],
    tools: ["adjust_stock", "show_chart"],
    suggestions: [
      "What should I reorder this week?",
      "Which suppliers have the longest lead times?",
      "Estimate the cost of restocking everything below its reorder level",
    ],
  },
  {
    ...
    icon: Package,
    color: "#16A34A",
    tagline: "Stock levels, valuation, and reorder planning",
    scope: ["inventory"],
    tools: ["adjust_stock", "show_chart"],
    suggestions: [
      "Show me stock value by category as a chart",
      "What's out of stock right now?",
      "Restock the fleet GPS tracking units by 20",
    ],
  },
  {
    id: "legal-assistant",
    ...
    icon: FileCheck,
    color: "#111827",
    tagline: "Contract and policy drafting support",
    scope: ["suppliers"],
    tools: [],
    disclaimer:
      "Not a substitute for a licensed attorney. This is informational drafting and review support only — have a qualified lawyer review anything before it's signed or relied upon.",
    suggestions: [
      "Draft a simple vendor confidentiality clause",
      "What should a standard supply contract include?",
      "Explain what a force majeure clause protects against",
    ],
  },
  {
    id: "forecasting",
    ...
    icon: Gauge,
    color: "#F59E0B",
    tagline: "Forward-looking analysis from current data",
    scope: ["finance", "sales", "inventory"],
    tools: ["show_chart"],
    mode: "forecast",
    suggestions: [
      "Project our cash position risk over the next quarter",
      "What sales trend should we expect from the current pipeline?",
      "What are the biggest financial risks over the next 90 days?",
    ],
  },
  {
    ...
    icon: FileText,
    color: "#16A34A",
    tagline: "Draft letters, emails, memos, and policies you can download or send",
    scope: [],
    tools: [],
    mode: "docgen",
    suggestions: [],
  },
  {
    ...
    icon: Mic,
    color: "#5B6472",
    tagline: "Live transcription, structured minutes, and real follow-ups on your Shared Calendar",
    scope: [],
    tools: [],
    mode: "meeting",
    suggestions: [],
  },
  {
    ...
    icon: Mic,
    color: "#F59E0B",
    tagline: "Speak instead of typing, using the mic button below",
    scope: ["finance", "sales", "inventory"],
    tools: ["create_lead", "adjust_stock", "mark_invoice_paid", "create_invoice", "show_chart"],
    suggestions: [
      "What should I restock, and roughly what will it cost?",
      "Show me my sales pipeline as a chart",
      "Which invoices need my attention most urgently?",
    ],
  },
];
...
export const AI_TOOLS_ALL = [
  {
    name: "create_lead",
    description:
      "Create a new sales lead in the CRM pipeline at the 'New' stage. Use when the owner asks to add a lead, prospect, or potential customer.",
    input_schema: {
      type: "object",
      properties: {
        ...
        value_tzs_k: {
          type: "number",
          description: "Estimated deal value in thousands of TZS (0 if unknown)",
        },
  ...
  },
  {
    name: "adjust_stock",
    description:
      "Adjust the on-hand quantity of an inventory item by a signed delta (positive = restock, negative = remove). Use the exact SKU from the business snapshot.",
    input_schema: {
      type: "object",
      properties: {
        sku: {
          type: "string",
          description: "Exact SKU from the inventory snapshot, e.g. HDW-2204",
        },
        delta: {
          type: "number",
          description: "Signed quantity change; positive adds stock, negative removes",
        },
  ...
  },
  {
    ...
    description:
      "Mark an outstanding invoice as fully paid, settling its balance. Use the exact invoice ID from the business snapshot.",
    input_schema: {
      type: "object",
      properties: {
        invoice_id: {
          type: "string",
          description: "Exact invoice ID from the snapshot, e.g. INV-8798",
        },
      },
      required: ["invoice_id"],
    },
  },
  {
    name: "record_expense",
    description:
      "Record a new business expense. Use when the owner describes a cost that needs to be logged.",
    input_schema: {
      type: "object",
      properties: {
        ...
        category: {
          type: "string",
          description: "Expense category, e.g. Supplies, Rent & Utilities, Marketing",
        },
        amount_tzs_k: { type: "number", description: "Amount in thousands of TZS" },
        method: {
          type: "string",
          description: "Payment method: Cash, Card, Mobile Money, or Bank Transfer",
        },
      },
  ...
  },
  {
    name: "approve_leave",
    description:
      "Approve a pending leave request. Use the exact employee name and leave type from the snapshot.",
    input_schema: {
      type: "object",
      properties: {
        employee: {
          type: "string",
          description: "Exact employee name from the leave_requests snapshot",
        },
        type: {
          type: "string",
          description:
            "Leave type from the snapshot, e.g. Annual, Sick, Unpaid — disambiguates if an employee has more than one pending request",
        },
      },
      required: ["employee"],
    },
  },
  {
    name: "create_invoice",
    description:
      "Create a new invoice for a customer with one or more line items. Use exact SKUs from the inventory snapshot for pricing; if the owner names an item without a SKU, match it by name from the snapshot.",
    input_schema: {
      type: "object",
      properties: {
        ...
        due_in_days: {
          type: "number",
          description: "Payment terms in days from today; defaults to 14 if not specified",
        },
      },
  ...
  },
  {
    ...
    description:
      "Draft a new price quotation for a prospect or customer, before any commitment to buy. Use exact SKUs from the inventory snapshot for pricing.",
    input_schema: {
      type: "object",
      properties: {
        ...
        valid_days: {
          type: "number",
          description: "How many days the quote stays valid; defaults to 14",
        },
      },
      required: ["customer", "items"],
    },
  },
  {
    ...
    description:
      "Set up a recurring automated workflow: generate a specific business report on a schedule, in a chosen format. This is the closest real automation this system supports — it does not create arbitrary if-this-then-that rules.",
    input_schema: {
      type: "object",
      properties: {
        report_type: {
          type: "string",
          description: "One of: Sales & Revenue, Inventory Valuation, Profit & Loss",
        },
        ...
        recipient_email: {
          type: "string",
          description:
            "Who should receive it (informational — see the tool's own caveat about delivery not being automatic yet)",
        },
      },
  ...
  },
  {
    name: "show_chart",
    description:
      "Render a real chart alongside your written answer, built from the exact data you're citing. Use whenever a comparison, breakdown, or trend would be clearer as a chart than as prose — e.g. revenue by customer, expenses by category, pipeline by stage, stock value by category.",
    input_schema: {
      type: "object",
      properties: {
        ...
        data: {
          type: "array",
          description:
            "Data points to plot, using values already present in the business snapshot — never invented numbers",
          items: {
            type: "object",
            properties: {
              ...
              value: {
                type: "number",
                description:
                  "The value in thousands of TZS or count, matching what's being charted",
              },
            },
...
export function actionLabel(name, input) {
  ...
  if (name === "adjust_stock")
    return `Adjusted ${input.sku} by ${input.delta > 0 ? "+" : ""}${input.delta}`;
  if (name === "mark_invoice_paid") return `Marked ${input.invoice_id} as paid`;
  if (name === "record_expense")
    return `Recorded expense: ${input.vendor} — TZS ${input.amount_tzs_k}k`;
  ...
  if (name === "create_workflow")
    return `Scheduled ${input.report_type} (${input.frequency}, ${input.format})`;
  return name;
}
...
export function AIAssistant({
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            One assistant, twelve roles — each grounded in the live data relevant to it
          </p>
        </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${p.color}, #111827)` }}
                >
  ...
  return (
    <div className="flex flex-col h-full min-h-[75vh] max-w-3xl mx-auto">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${persona.color}, #111827)` }}
          >
            <persona.icon size={18} className="text-white" />
          </div>
          <div>
            <h1 className="text-[18px] sm:text-[20px] font-semibold text-[#111827] tracking-tight">
              {persona.name}
            </h1>
        ...
        <button
          ...
          className="btn-secondary text-[12px] font-medium rounded-lg px-3 py-1.5 shrink-0"
        >
          Switch Assistant
        </button>
      </div>
      ...
      {persona.mode !== "docgen" && persona.mode !== "meeting" && (
        <ChatInterface persona={persona} data={data} />
      )}
    </div>
  );
}
...
export function ChatInterface({ persona, data }) {
  ...
  async function executeTool(name, toolInput) {
    try {
      if (name === "create_lead") {
        const draft = {
          id: docId("L"),
          name: toolInput.contact_name,
          company: toolInput.company_name,
          stage: "New",
          value: Number(toolInput.value_tzs_k) || 0,
          currency: "TZS000",
          owner: `AI ${persona.name}`,
      ...
      if (name === "adjust_stock") {
        ...
        if (!item)
          return `Error: no inventory item with SKU ${toolInput.sku}. Check the snapshot for valid SKUs.`;
        const delta = Number(toolInput.delta) || 0;
        const newQty = Math.max(0, item.qty + delta);
        data.inventory.setRows((prev) =>
          prev.map((it) => (it.sku === item.sku ? { ...it, qty: newQty } : it)),
        );
        notify(`Stock adjusted: ${item.name} ${delta > 0 ? "+" : ""}${delta}`);
        logAudit(
          "Stock adjusted via AI Assistant",
          "Inventory",
          `AI ${persona.name}`,
          `${item.name} (${item.sku}) ${delta > 0 ? "+" : ""}${delta} → ${newQty} ${item.unit}`,
        );
        if (IS_CONFIGURED) {
          try {
            ...
            notify("Adjustment saved locally, but the server update failed.", "error");
          }
        }
      ...
      if (name === "mark_invoice_paid") {
        ...
        if (!inv)
          return `Error: no invoice with ID ${toolInput.invoice_id}. Check the snapshot for valid invoice IDs.`;
        if (inv.status === "Paid") return `No change: ${inv.id} is already fully paid.`;
        const total = lineTotal(inv.items).total;
        data.invoices.setRows((prev) =>
          prev.map((i) => (i.id === inv.id ? { ...i, status: "Paid", amountPaid: total } : i)),
        );
        ...
        if (IS_CONFIGURED && inv.dbId) {
          try {
            ...
            notify("Payment saved locally, but the server update failed.", "error");
          }
        }
      ...
      if (name === "record_expense") {
        const draft = {
          id: docId("EX"),
          vendor: toolInput.vendor,
          category: toolInput.category || "Supplies",
          date: TODAY.toISOString().slice(0, 10),
          dueDate: TODAY.toISOString().slice(0, 10),
          amount: Number(toolInput.amount_tzs_k) || 0,
          status: "Paid",
          method: toolInput.method || "Cash",
        };
      ...
      if (name === "approve_leave") {
        const candidates = data.leaveRequests.rows.filter(
          (l) =>
            l.employee === toolInput.employee &&
            l.status === "Pending" &&
            (!toolInput.type || l.type === toolInput.type),
        );
        if (candidates.length === 0)
          return (
            "Error: no pending leave request found for " +
            toolInput.employee +
            (toolInput.type ? " (" + toolInput.type + ")" : "") +
            ". Check the snapshot."
          );
        const target = candidates[0];
        data.leaveRequests.setRows((prev) =>
          prev.map((l) => (l.id === target.id ? { ...l, status: "Approved" } : l)),
        );
        notify(`Leave approved for ${target.employee}`);
        logAudit(
          "Leave approved via AI Assistant",
          "HR",
          `AI ${persona.name}`,
          `${target.employee} — ${target.type}, ${target.startDate} to ${target.endDate}`,
        );
        if (IS_CONFIGURED && target.dbId) {
          try {
            await sb("hr_leave_requests")
              .eq("id", target.dbId)
              .update({ status: "Approved" })
              .run();
          } catch (_e) {
            notify("Approved locally, but the server update failed.", "error");
          }
        }
      ...
      if (name === "create_invoice" || name === "create_quotation") {
        ...
        const lineItems = (toolInput.items || [])
          .map((li) => {
            ...
            return {
              ...
              name: invItem.name,
              qty: Number(li.qty) || 1,
              rate: invItem.unitCost,
            };
          })
          .filter(Boolean);
        if (lineItems.length === 0)
          return `Error: none of the requested SKUs were found in the inventory snapshot. Check the exact SKUs and try again.`;

        const docId = isInvoice
          ? `INV-${Math.floor(8000 + Math.random() * 1900)}`
          : `QT-${Math.floor(1000 + Math.random() * 900)}`;
        ...
        const draft = isInvoice
          ? {
              ...
              date: orderDate,
              dueDate: new Date(TODAY.getTime() + (Number(toolInput.due_in_days) || 14) * 86400000)
                .toISOString()
                .slice(0, 10),
              orderRef: "—",
              status: "Unpaid",
          ...
          : {
              ...
              date: orderDate,
              validUntil: new Date(
                TODAY.getTime() + (Number(toolInput.valid_days) || 14) * 86400000,
              )
                .toISOString()
                .slice(0, 10),
              status: "Draft",
        ...
        notify(
          `${isInvoice ? "Invoice" : "Quotation"} ${docId} created for ${toolInput.customer} — TZS ${money(Math.round(total))}k`,
        );
        logAudit(
          `${isInvoice ? "Invoice" : "Quotation"} created via AI Assistant`,
          "Sales",
          `AI ${persona.name}`,
          `${docId} — ${toolInput.customer}, TZS ${money(Math.round(total))}k`,
        );
          ...
          try {
            ...
            if (header?.id) {
              await sb(itemsTable)
                .insert(
                  lineItems.map((li) => ({
                    [isInvoice ? "invoice_id" : "quotation_id"]: header.id,
                    item_sku: li.sku,
                    item_name: li.name,
                    qty: li.qty,
                    rate: li.rate,
                ...
                .run();
              hook.setRows((prev) =>
                prev.map((d) => (d.id === docId ? { ...d, dbId: header.id } : d)),
              );
            }
          } catch (_e) {
            notify(
              `${isInvoice ? "Invoice" : "Quotation"} created locally, but saving to the server failed.`,
              "error",
            );
          }
        }
      ...
      if (name === "create_workflow") {
        const draft = {
          ...
          recipientEmail: toolInput.recipient_email || "",
          status: "Active",
          lastRun: null,
        };
        ...
        logAudit(
          ...
          "Reports",
          `AI ${persona.name}`,
          `${draft.reportType}, ${draft.frequency}, ${draft.format}`,
        );
        if (IS_CONFIGURED) {
          try {
            const header = await sb("scheduled_reports")
              ...
              .run();
            if (header?.id)
              data.scheduledWorkflows.setRows((prev) =>
                prev.map((s) => (s.id === draft.id ? { ...s, dbId: header.id } : s)),
              );
          } catch (_e) {
            notify("Workflow created locally, but saving to the server failed.", "error");
          }
        }
  ...
  async function callModel(convo) {
    ...
    const roleFraming =
      persona.id === "consultant" || persona.id === "voice-commands"
        ...
        {messages.length === 0 && !busy && (
          <div className="h-full flex flex-col items-center justify-center text-center px-4">
            <div
              ...
              style={{ backgroundColor: `${persona.color}14` }}
            >
            ...
            <p className="text-[12.5px] text-slate-500 mt-1 max-w-[320px] leading-relaxed mb-5">
              ...
              {tools.length > 0 ? " — and able to act on it when you ask." : "."} Try one of these:
            </p>
            <div className="flex flex-col gap-2 w-full max-w-[400px]">
              {persona.suggestions.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="text-left text-[12.5px] text-slate-600 bg-slate-50 hover:bg-[#16A34A]/8 hover:text-[#111827] border border-slate-100 rounded-lg px-3.5 py-2.5 transition-colors"
                >
                  {s}
                </button>
              ))}
        ...
        {messages.map((m, i) => {
          ...
          return (m.content || []).map((block, j) => {
                      ...
                      {i === messages.length - 1 && (
                        <button
                          onClick={() => {
                            ...
                            setMessages(prev);
                            send(messages[messages.length - 2]?.content || "");
                          }}
                          className="flex items-center gap-1 text-[10.5px] text-slate-400 hover:text-slate-600 bg-white border border-slate-200 rounded-md px-2 py-0.5 shadow-sm"
            ...
            if (block.type === "tool_use" && block.name === "show_chart") {
              const chartData = (block.input.data || []).map((d) => ({
                label: d.label,
              ...
              return (
                <div key={`${i}-${j}`} className="flex justify-start w-full">
                  <div className="max-w-[92%] w-full bg-white border border-slate-200/80 rounded-xl rounded-bl-sm shadow-sm p-4">
                    <p className="text-[12.5px] font-semibold text-[#111827] mb-3">
                      {block.input.title}
                    </p>
      ...
      <div className="border-t border-slate-100 p-3 sm:p-4">
        {persona.mode === "forecast" && (
          <button
            onClick={() =>
              send(
                "Generate a full forward-looking forecast: project cash position, sales trend, and the biggest risks over the next quarter based on the current data. Structure it as Cash Outlook, Sales Outlook, and Key Risks.",
              )
            }
            disabled={busy}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="btn-primary text-white rounded-lg px-4 flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
            aria-label="Send"
          >
            <Send size={15} />
          </button>
        ...
        <p className="text-[10.5px] text-slate-400 mt-2 ml-0.5">
          Powered by Claude · reads and acts on this session's data
          {IS_CONFIGURED ? "" : " (demo dataset)"}
        </p>
      </div>
    </div>
  );
  ...
  async function generate() {
    ...
    try {
      ...
      if (isEmail) {
        ...
        if (match) {
          setEmailSubject(match[1].trim());
          setResult(match[2].trim());
        } else {
          setEmailSubject(`Message from ${company.name}`);
          setResult(text);
        }
      } else {
        setResult(text);
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
      ...
      {isEmail && (
        <FormField label="Recipient email">
          <input
            ...
            placeholder="customer@company.tz"
          />
        </FormField>
      )}
      <FormField label="What should it say?">
        <textarea
          ...
          placeholder="e.g. A letter to Kilimo Fresh Distributors apologizing for a late delivery and offering a 5% discount on their next order."
        />
      </FormField>
      <button
        onClick={generate}
        disabled={busy || !brief.trim()}
        className="btn-primary text-white text-[13px] font-medium rounded-lg py-2.5 w-full disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {busy ? (
          <>
            <LoaderCircle size={14} className="animate-spin" /> Drafting...
          </>
        ) : (
          <>
            <FileText size={14} /> Generate {isEmail ? "Email" : "Document"}
          </>
        )}
      </button>
          ...
          <div className="flex gap-2">
            {isEmail && (
              <button
                ...
                className="flex-1 btn-primary text-white text-[12.5px] font-medium rounded-lg py-2.5 flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            <button
              onClick={downloadTxt}
              className="flex-1 text-[12.5px] font-medium border border-slate-200 rounded-lg py-2.5 hover:bg-slate-50 flex items-center justify-center gap-2"
            >
...
export function AIMeetingAssistant({ company }) {
  ...
  function startRecording() {
    if (!speechSupported) {
      notify(
        "Live transcription isn't supported in this browser — try Chrome or Edge. You can still paste a transcript below.",
        "error",
      );
      return;
    }
    ...
    recognition.onerror = () => {
      notify(
        "Lost the microphone connection — check permissions for this page. What was captured so far is kept.",
        "error",
      );
      setListening(false);
    };
  ...
  async function generateMinutes() {
    ...
    try {
      ...
      setResult({
        ...
        actionItems: (parsed.actionItems || []).map((a, i) => ({ ...a, id: i, include: true })),
      });
    } catch (e) {
      setError(
        "Couldn't generate minutes — the AI service didn't return a response we could read. Try again in a moment.",
      );
    } finally {
      setBusy(false);
    }
  ...
  function updateActionItem(id, key, value) {
    setResult((r) => ({
      ...r,
      actionItems: r.actionItems.map((a) => (a.id === id ? { ...a, [key]: value } : a)),
    }));
  }
  ...
  async function createFollowUps() {
    ...
    if (toSchedule.length === 0) {
      notify(
        "No action items have both a due date and are checked — nothing to schedule.",
        "error",
      );
      return;
    }
    ...
    for (const item of toSchedule) {
      const draft = {
        ...
        description: `Follow-up from meeting: ${item.task}`,
      };
      created.push(draft);
      calendarEvents.setRows((prev) => [...prev, draft]);
      if (IS_CONFIGURED) {
        try {
          await sb("calendar_events")
            .insert({
              title: draft.title,
          ...
          /* one failed sync among several shouldn't block the rest; the toast below reflects local success either way */
        }
      }
    }
    logAudit(
      "Meeting follow-ups scheduled",
      "AI Meeting Assistant",
      `AI ${company.name} meeting`,
      `${created.length} follow-up(s) added to the Shared Calendar`,
    );
    notify(
      `${created.length} follow-up${created.length > 1 ? "s" : ""} added to the Shared Calendar.`,
    );
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Recording captures real speech from this device&apos;s microphone via your browser — it
          can&apos;t capture audio from a call happening on Zoom, Meet, or Teams elsewhere, the same
          honest limit already stated for video calls in the Collaboration Hub. For those, paste the
          transcript your call platform generates instead.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          >
            {listening ? (
              <>
                <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" /> Stop Recording
              </>
            ) : (
              <>
        ...
        <FormField label="">
          <textarea
            ...
            placeholder="Click Start Recording to capture live speech, or paste an existing transcript / notes here..."
          />
        </FormField>
        <button
          onClick={generateMinutes}
          disabled={busy || !transcript.trim() || listening}
          className="btn-primary text-white text-[13px] font-medium rounded-lg py-2.5 w-full disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {busy ? (
            <>
              <LoaderCircle size={14} className="animate-spin" /> Generating minutes...
            </>
          ) : (
            <>
              <FileText size={14} /> Generate Minutes &amp; Action Items
            </>
          )}
        </button>
        {error && <p className="text-[12.5px] text-[#EF4444]">{error}</p>}
        ...
        <>
                ...
                <ul className="space-y-1 mb-1">
                  {result.decisions.map((d, i) => (
                    <li key={i} className="text-[13px] text-slate-600 flex gap-2">
                      <CheckCircle2 size={14} className="text-[#16A34A] shrink-0 mt-0.5" /> {d}
                    </li>
                  ))}
                </ul>
              </>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
            ...
            <p className="text-[11.5px] text-slate-400 mb-3">
              Edit the owner or due date below, then schedule follow-ups — each checked item with a
              due date becomes a real event on the Shared Calendar.
            </p>
            {result.actionItems.length === 0 ? (
              ...
              <div className="space-y-2 mb-4">
                {result.actionItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-start gap-2.5 border border-slate-100 rounded-lg p-3"
                  >
                    <input
                      type="checkbox"
                      checked={item.include}
                      onChange={(e) => updateActionItem(item.id, "include", e.target.checked)}
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2">
                      <input
                        ...
                        className="text-[12.5px] bg-transparent outline-none text-[#111827] font-medium"
                      />
                      <input
                        value={item.owner}
                        onChange={(e) => updateActionItem(item.id, "owner", e.target.value)}
                        placeholder="Owner"
                        className="text-[12px] bg-slate-50 border border-slate-200 rounded-md px-2 py-1 w-full sm:w-32"
                      />
                      <input
                        type="date"
                        value={item.dueDate}
                        onChange={(e) => updateActionItem(item.id, "dueDate", e.target.value)}
                        className="text-[12px] bg-slate-50 border border-slate-200 rounded-md px-2 py-1 w-full sm:w-36"
                      />
                    </div>
                  </div>
                ))}
            ...
            {result.actionItems.length > 0 && (
              <button
                ...
                className="btn-primary text-white text-[12.5px] font-medium rounded-lg py-2.5 w-full flex items-center justify-center gap-2"
              >
