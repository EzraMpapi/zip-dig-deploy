...
export function BankingMFIModule({ currentUser, company, onLoansLoad }) {
  ...
  useEffect(() => {
    if (onLoansLoad) onLoansLoad(loans.rows);
  }, [loans.rows, onLoansLoad]);
  const fds = useCompanyTable("bank_fixed_deposits", BANK_FIXED_DEPOSITS_SEED, {
    mapRow: (r) => r,
  });
  const sos = useCompanyTable("bank_standing_orders", BANK_STANDING_ORDERS_SEED, {
    mapRow: (r) => r,
  });
  ...
  const loanPortfolio = loans.rows
  ...
  const StatusBadge = ({ s }) => {
    const c = {
      ...
      Closed: ["#FEE2E2", "#EF4444"],
      Frozen: ["#FEF3C7", "#D97706"],
      Matured: ["#DBEAFE", "#1E40AF"],
    }[s] || ["#F3F4F6", "#6B7280"];
    return (
      <span
        className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full"
  ...
  async function doTellerOp(opType) {
    ...
    if (!accountNo || !amount) {
      notify("Enter account number and amount", "error");
      return;
    }
    ...
    if ((opType === "Withdrawal" || opType === "Transfer Out") && acc.balance < amt) {
      notify("Insufficient balance", "error");
      return;
    }

    const newBal =
      opType === "Deposit" || opType === "Transfer In" ? acc.balance + amt : acc.balance - amt;
    accounts.setRows((p) =>
      p.map((a) => (a.accountNo === accountNo ? { ...a, balance: newBal } : a)),
    );
    ...
    if (opType === "Transfer Out" && toAccountNo) {
      ...
      if (toAcc) {
        accounts.setRows((p) =>
          p.map((a) => (a.accountNo === toAccountNo ? { ...a, balance: a.balance + amt } : a)),
        );
        const inTxn = {
          id: docId("TXN"),
          ...
          narration: "Transfer from " + acc.name,
          teller: currentUser?.name || "Teller",
          status: "Completed",
        };
    ...
    const txn = {
      ...
      accountNo,
      account: acc.name,
      type: opType === "Transfer" ? "Transfer Out" : opType,
      amount: amt,
      balance: newBal,
      date: new Date().toISOString().slice(0, 16).replace("T", " "),
      channel: "Teller",
      reference: (opType === "Deposit" ? "DEP" : "WDR") + "-" + Date.now(),
      narration: narration || opType,
      teller: currentUser?.name || "Teller",
      status: "Completed",
    };
    txns.setRows((p) => [txn, ...p]);
    logAudit(
      `Teller: ${opType} TZS ${money(amt)}k`,
      "Banking",
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("bank_transactions").insert(txn).run();
      } catch (_e) {}
    }
  }
  ...
  async function openAccount() {
    ...
    const row = {
      ...
      id: docId("ACC"),
      accountNo: nextAccNo(),
      balance: Number(accForm.openingBalance) || 0,
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("bank_accounts").insert(row).run();
      } catch (_e) {}
    }
  }
  ...
  async function disburseLoan() {
    ...
    if (!acc) {
      notify("Client account not found", "error");
      return;
    }
    const P = Number(loanForm.principal),
      r = loanForm.rate / 100 / 12,
      n = Number(loanForm.months);
    const installment = r > 0 ? (P * (r * Math.pow(1 + r, n))) / (Math.pow(1 + r, n) - 1) : P / n;
    const row = {
      id: docId("LN"),
      loanNo: "LN" + Date.now().toString().slice(-8),
      clientId: acc.id,
      client: acc.name,
      type: loanForm.type,
      principal: P,
      ...
      balance: P,
      arrears: 0,
      nextDue: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
      status: "Active",
    ...
    accounts.setRows((p) =>
      p.map((a) =>
        a.accountNo === loanForm.clientAccountNo ? { ...a, balance: a.balance + P } : a,
      ),
    );
    const txn = {
      id: docId("TXN"),
      accountNo: loanForm.clientAccountNo,
      account: acc.name,
      type: "Loan Disbursement",
      amount: P,
      balance: acc.balance + P,
      date: TODAY.toISOString().slice(0, 10) + " 00:00",
      channel: "System",
      reference: row.loanNo,
      narration: "Loan disbursement — " + loanForm.type,
      teller: "Credit",
      status: "Completed",
    };
    txns.setRows((p) => [txn, ...p]);
    ...
    logAudit(
      "Loan disbursed: " + row.loanNo,
      "Banking",
      currentUser?.name || "System",
      acc.name + " TZS " + money(P) + "k",
    );
  }

  const filteredAccounts = accounts.rows.filter(
    (a) =>
      !searchAcc ||
      a.name.toLowerCase().includes(searchAcc.toLowerCase()) ||
      a.accountNo.includes(searchAcc) ||
      a.type.toLowerCase().includes(searchAcc.toLowerCase()),
  );
  ...
  const AccStatement = ({ acc }) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setSelectedAcc(null)}>
        ...
        <div
          className="relative w-full sm:w-[520px] bg-white h-full shadow-2xl overflow-y-auto"
        ...
        >
          ...
          <div className="p-4">
            <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-wide mb-3">
              Transaction History ({accTxns.length})
            </p>
            {accTxns.length === 0 ? (
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg,${BANK_BLUE} 0%,#2D5F8A 50%,#1B4F72 100%)` }}
      >
        <div
          className="absolute inset-0"
          style={{
            backgroundImage:
              "repeating-linear-gradient(0deg,transparent,transparent 40px,rgba(255,255,255,.025) 40px,rgba(255,255,255,.025) 41px),repeating-linear-gradient(90deg,transparent,transparent 40px,rgba(255,255,255,.025) 40px,rgba(255,255,255,.025) 41px)",
          }}
        />
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-[20px] font-bold text-white">
                {company?.name || "Banking"} & Financial Institution
              </h1>
            </div>
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.55)" }}>
              Accounts · Teller · Loans · Fixed Deposits · Standing Orders · NPL Management
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            >
              <p className="text-[20px] font-black text-white">
                {accounts.rows.filter((a) => a.status === "Active").length}
              </p>
            ...
            <div
              className="rounded-xl px-4 py-2.5 text-center"
              style={{ background: nplRatio > 5 ? "rgba(239,68,68,.2)" : "rgba(255,255,255,.1)" }}
            >
              <p
                className="text-[20px] font-black"
                style={{ color: nplRatio > 5 ? "#FCA5A5" : "#fff" }}
              >
                {nplRatio}%
              </p>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-[12.5px] font-semibold text-white"
              style={{
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => setTab(t.id)}
              className={
                "flex items-center gap-1 px-3 py-2 rounded-lg text-[11.5px] font-medium transition-all whitespace-nowrap " +
                (tab === t.id ? "text-white shadow-sm" : "text-slate-500 hover:bg-slate-50")
              }
              style={{ background: tab === t.id ? BANK_BLUE : "transparent" }}
            >
              <I size={12} />
              {t.label}
              {t.id === "loans" && npls.length > 0 && (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {
                l: "Total Deposits",
                v: "TZS " + money(totalDeposits) + "k",
                sub: accounts.rows.filter((a) => a.status === "Active").length + " active accounts",
                c: BANK_BLUE,
                I: PiggyBank,
              },
              {
                l: "Loan Portfolio",
                v: "TZS " + money(loanPortfolio) + "k",
                sub: loans.rows.filter((l) => l.status === "Active").length + " active loans",
                c: "#059669",
                I: Landmark,
              },
              {
                l: "Fixed Deposits",
                v: "TZS " + money(fdTotal) + "k",
                sub: fds.rows.filter((f) => f.status === "Active").length + " accounts",
                c: BANK_GOLD,
                I: Archive,
              },
              {
                l: "Est. Monthly Income",
                v: "TZS " + money(interestIncome) + "k",
                sub: "From loan interest",
                c: "#7C3AED",
                I: TrendingUp,
              },
            ].map((k) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-[11.5px] mt-0.5" style={{ color: k.c }}>
                      {k.sub}
                    </p>
                  </div>
                  <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {ACCOUNT_TYPES.slice(0, 5)
                .map((type) => {
                  const typeTotal = accounts.rows
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[10.5px] text-slate-400 ml-1.5">TZS {money(amt)}k</span>
                    </div>
                  </div>
                );
              })}
              <div className="mt-3 pt-2 border-t border-slate-100">
                <p className="text-[11.5px] text-slate-500">
                  NPL Ratio: <strong className="text-[#EF4444]">{nplRatio}%</strong>{" "}
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {todayTxns.length === 0 ? (
                  <p className="text-slate-400 text-[12.5px] text-center py-4">
                    No transactions today yet
                  </p>
                ) : (
                  todayTxns.slice(0, 5).map((t) => {
                    const isCredit =
                      t.type === "Deposit" || t.type === "Transfer In" || t.type === "Interest";
                    return (
                      <div
                        key={t.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10.5px] text-slate-400">{t.type}</p>
                        </div>
                        <p
      ...
      {tab === "accounts" && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() =>
                downloadCSV("accounts", accounts.rows, [
              ...
              className="flex items-center gap-1 text-[12px] font-medium text-slate-500 border border-slate-200 px-3 py-2.5 rounded-xl hover:border-[#16A34A] hover:text-[#16A34A] transition-colors"
            >
              <Download size={13} />
              Export
            </button>
          ...
          {showAccForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="ID / Passport No">
                  <input
                    className={inputClass}
                    value={accForm.idNo}
                    onChange={(e) => setAccForm({ ...accForm, idNo: e.target.value })}
                  />
                </FormField>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={openAccount}
                  className="text-[12.5px] font-semibold text-white px-5 py-2.5 rounded-xl"
                  style={{ background: BANK_BLUE }}
                >
                  Open Account
                </button>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <table className="w-full text-[12.5px]">
              ...
              <tbody>
                {filteredAccounts.map((a) => (
                  ...
                  >
                    <td
                      className="px-4 py-3 font-mono text-[11.5px] font-semibold"
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ background: accTypeColor[a.type] || "#6B7280" }}
                        >
                          {a.name.charAt(0)}
                        </div>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full"
                        style={{
                          background: (accTypeColor[a.type] || "#6B7280") + "18",
                          color: accTypeColor[a.type] || "#6B7280",
                        }}
                      >
                        {a.type}
                      </span>
                    ...
                    <td className="px-4 py-3 font-mono font-bold text-[#111827]">
                      TZS {money(a.balance)}k
                    </td>
                    <td className="px-4 py-3 font-semibold" style={{ color: BANK_GOLD }}>
                      {a.interest}%
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setTellerForm((f) => ({ ...f, accountNo: a.accountNo }));
                            setTab("teller");
                            setTellerTab("deposit");
                          }}
                          className="text-[10.5px] font-semibold text-white px-2 py-1 rounded-lg"
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            setTellerForm((f) => ({ ...f, accountNo: a.accountNo }));
                            setTab("teller");
                            setTellerTab("withdraw");
                          }}
                          className="text-[10.5px] font-semibold text-white px-2 py-1 rounded-lg bg-red-500"
                        >
                          Withdraw
                        </button>
                      </div>
            ...
            <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 text-[11.5px] text-slate-400">
              {filteredAccounts.length} of {accounts.rows.length} accounts · Total: TZS{" "}
              {money(totalDeposits)}k
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="flex border-b border-slate-100">
              {[
                ...
                ["withdraw", "Withdrawal", "#EF4444"],
                ["transfer", "Transfer", "#2563EB"],
                ["balance", "Balance Enquiry", "#7C3AED"],
              ].map(([id, label, col]) => (
                <button
                  key={id}
                  onClick={() => setTellerTab(id)}
                  className="flex-1 py-3.5 text-[12.5px] font-semibold transition-all"
                  style={{
                    background: tellerTab === id ? col : "transparent",
                    color: tellerTab === id ? "#fff" : col,
                    borderBottom: tellerTab === id ? "none" : "1px solid transparent",
                  }}
                >
                  {label}
                </button>
              ...
              <div className="max-w-lg mx-auto space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="Account Number">
                    <input
                      className={inputClass}
                      value={tellerForm.accountNo}
                      onChange={(e) => setTellerForm({ ...tellerForm, accountNo: e.target.value })}
                  ...
                  {tellerTab !== "balance" && (
                    <FormField label="Amount (TZS thousands)">
                      <input
                        type="number"
                        className={inputClass}
                        value={tellerForm.amount}
                        onChange={(e) => setTellerForm({ ...tellerForm, amount: e.target.value })}
                        placeholder="0.00"
                      />
                    </FormField>
                  )}
                  {tellerTab === "transfer" && (
                    <FormField label="To Account Number">
                      <input
                        className={inputClass}
                        value={tellerForm.toAccountNo}
                        onChange={(e) =>
                          setTellerForm({ ...tellerForm, toAccountNo: e.target.value })
                        }
                  ...
                  {tellerTab !== "balance" && (
                    <FormField label="Narration" cls="col-span-2">
                      <input
                        className={inputClass}
                        value={tellerForm.narration}
                        onChange={(e) =>
                          setTellerForm({ ...tellerForm, narration: e.target.value })
                        }
                    ...
                    return (
                      <div className="p-4 rounded-xl border-2 border-blue-100 bg-blue-50">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-[13.5px] font-bold text-[#111827]">{acc.name}</p>
                            <p className="text-[11.5px] text-slate-500">
                              {acc.type} · {acc.branch}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="text-[20px] font-black" style={{ color: BANK_BLUE }}>
                              TZS {money(acc.balance)}k
                            </p>
                          </div>
                        </div>
                        <StatusBadge s={acc.status} />
                ...
                {tellerTab !== "balance" && (
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl text-[14px] font-bold text-white transition-all"
                    style={{
                      background:
                        tellerTab === "deposit"
                          ? "#16A34A"
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-semibold text-[#111827]">
                Today&apos;s Teller Journal ({txns.rows.length} transactions)
              </p>
            </div>
            <table className="w-full text-[12.5px]">
              <thead>
                ...
                {txns.rows.slice(0, 15).map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className="px-3 py-2.5 font-mono text-[11.5px] font-semibold"
                        style={{ color: BANK_BLUE }}
                      >
                        {t.accountNo}
                      </td>
      ...
      {tab === "loans" && (
        <div className="space-y-3">
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold text-red-700">
                  {npls.length} Non-Performing Loan{npls.length > 1 ? "s" : ""} — Total: TZS{" "}
                  {money(nplAmount)}k | NPL Ratio: {nplRatio}%
                </p>
                <p className="text-[11.5px] text-red-500 mt-0.5">
                  {npls.map((l) => l.client).join(", ")}
                </p>
              </div>
            </div>
          )}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ["Active Loans", loans.rows.filter((l) => l.status === "Active").length, "#2563EB"],
              ["Portfolio", "TZS " + money(loanPortfolio) + "k", "#059669"],
              ["NPLs", npls.length, "#EF4444"],
              [
                "Collected Est.",
                "TZS " +
                  money(
          ...
          {showLoanForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <FormField label="Client Account No *">
                  <input
                    className={inputClass}
                    value={loanForm.clientAccountNo}
                    onChange={(e) => setLoanForm({ ...loanForm, clientAccountNo: e.target.value })}
                    placeholder="Account number"
                  />
                </FormField>
                {loanForm.clientAccountNo &&
                  (() => {
                    const a = accounts.rows.find((x) => x.accountNo === loanForm.clientAccountNo);
                    return a ? (
                      <div className="col-span-2 py-2 px-3 rounded-lg bg-blue-50 text-[12.5px] font-semibold text-blue-800">
                        {a.name} · {a.type}
                      </div>
                    ) : null;
                  })()}
                ...
                <FormField label="Interest Rate (% p.a.)">
                  <input
                    ...
                    className={inputClass}
                    value={loanForm.rate}
                    onChange={(e) => setLoanForm({ ...loanForm, rate: Number(e.target.value) })}
                  />
                </FormField>
                ...
                <FormField label="Collateral">
                  <input
                    ...
                    placeholder="Property, Guarantor, None"
                  />
                </FormField>
                ...
                (() => {
                  ...
                  return (
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ["Monthly Installment", "TZS " + money(inst) + "k", BANK_BLUE],
                        ["Total Repayable", "TZS " + money(total) + "k", "#111827"],
                        ["Interest Income", "TZS " + money(total - P) + "k", "#16A34A"],
                      ].map(([l, v, col]) => (
                        <div key={l} className="bg-slate-50 rounded-xl p-3 text-center">
                          <p className="text-[10.5px] text-slate-400">{l}</p>
                          <p className="text-[15px] font-bold mt-0.5" style={{ color: col }}>
                            {v}
                          </p>
                        </div>
                      ))}
                  ...
                  return (
                    <tr
                      ...
                      className={
                        ...
                        (l.status === "Overdue" || l.status === "Defaulted" ? "bg-red-50/20" : "")
                      }
                    >
                      <td
                      ...
                      <td className="px-3 py-3">
                        <span className="text-[10.5px] font-semibold px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-600">
                          {l.type}
                        </span>
                      </td>
                      ...
                      <td
                        ...
                        style={{ color: BANK_BLUE }}
                      >
                        TZS {money(l.installment)}k
                      </td>
                      <td
                        className="px-3 py-3 font-mono font-bold"
                        style={{ color: l.balance > 0 ? "#111827" : "#16A34A" }}
                      >
                        TZS {money(l.balance)}k
                      </td>
                      <td
                        className="px-3 py-3 font-mono font-bold"
                        style={{ color: l.arrears > 0 ? "#EF4444" : "#16A34A" }}
                      >
                        {l.arrears > 0 ? "TZS " + money(l.arrears) + "k" : "None"}
                      </td>
                      <td className="px-3 py-3 font-mono text-[11.5px] text-slate-400">
                        {l.nextDue}
                      </td>
                      <td className="px-3 py-3">
                        <span
      ...
      {tab === "deposits" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Total FD Value", "TZS " + money(fdTotal) + "k", BANK_GOLD],
              ["Active FDs", fds.rows.filter((f) => f.status === "Active").length, "#2563EB"],
              [
                "Total Interest Earned",
                "TZS " + money(fds.rows.reduce((s, f) => s + f.interestEarned, 0)) + "k",
                "#16A34A",
              ],
            ].map(([l, v, col]) => (
              <div
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-semibold text-[#111827]">Fixed Deposit Accounts</p>
              <button
                onClick={() => notify("Create FD — form coming")}
                className="flex items-center gap-1 text-[12px] font-semibold text-white px-3 py-2 rounded-xl"
                style={{ background: BANK_BLUE }}
              >
                <Plus size={12} />
                New FD
                ...
                {fds.rows.map((f) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono font-bold text-green-600">
                      TZS {money(f.interestEarned)}k
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={
                          "text-[10.5px] font-semibold px-2 py-0.5 rounded-full " +
                          (f.autoRenew ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-500")
                        }
                      >
                        {f.autoRenew ? "Yes" : "No"}
                      </span>
      ...
      {tab === "standing" && (
        <div className="space-y-3">
          <div className="flex justify-end">
            <button
              onClick={() => notify("Set up standing order — form")}
              className="flex items-center gap-1.5 text-[12.5px] font-semibold text-white px-4 py-2.5 rounded-xl"
              style={{ background: BANK_BLUE }}
            >
              <Plus size={13} />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              [
                "Active Accounts",
                String(accounts.rows.filter((a) => a.status === "Active").length),
                "#7C3AED",
              ],
            ].map(([l, v, col]) => (
              <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const statusData = ["Active", "Overdue", "Closed", "Written Off"]
                  .map((s, i) => ({
                    name: s,
                    value: loans.rows.filter((l) => l.status === s).length,
                    fill: ["#16A34A", "#EF4444", "#94A3B8", "#374151"][i],
                  }))
                  .filter((d) => d.value > 0);
                return statusData.length === 0 ? (
                  <p className="text-slate-400 text-center py-6">No loans</p>
                ) : (
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <h3 className="text-[13.5px] font-semibold text-[#111827] mb-3">
                Deposit by Account Type (TZS k)
              </h3>
              {(() => {
                const typeData = ACCOUNT_TYPES.slice(0, 6)
                  .map((type, i) => ({
                    name: type.replace(" ", ""),
                    value: Math.round(
                      accounts.rows
                        .filter((a) => a.type === type)
                        .reduce((s, a) => s + (a.balance || 0), 0) / 1000,
                    ),
                    fill: ["#2563EB", "#16A34A", "#D97706", "#7C3AED", "#EF4444", "#0891B2"][i],
                  }))
                  .filter((d) => d.value > 0);
                return typeData.length === 0 ? (
                  <p className="text-slate-400 text-center py-6">No accounts</p>
                ) : (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-[13px] font-semibold text-slate-700">ASSETS</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[12.5px] text-slate-500">Loans Receivable</span>
                  <span className="text-[12.5px] font-semibold text-[#111827]">
                    TZS {money(loanPortfolio)}k
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[12.5px] text-slate-500">Fixed Deposits Invested</span>
                  <span className="text-[12.5px] font-semibold text-[#111827]">
                    TZS {money(fdTotal)}k
                  </span>
                </div>
                <div className="flex justify-between border-t border-slate-100 pt-2">
                  <span className="text-[13px] font-semibold text-slate-700">LIABILITIES</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[12.5px] text-slate-500">Customer Deposits</span>
                  <span className="text-[12.5px] font-semibold text-[#111827]">
                    TZS {money(totalDeposits)}k
                  </span>
                </div>
                <div className="flex justify-between border-t border-slate-200 pt-2 mt-2">
                  ...
                  <span
                    ...
                    style={{ color: loanPortfolio - totalDeposits >= 0 ? "#16A34A" : "#EF4444" }}
                  >
                    TZS {money(Math.abs(loanPortfolio - totalDeposits))}k
                  </span>
                </div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {[
                  ["Interest on Loans (Monthly)", money(interestIncome) + "k", "#16A34A"],
                  [
                    "FD Interest Expense",
                    money(fds.rows.reduce((s, f) => s + (f.amount * f.rate) / 100 / 12, 0)) + "k",
                    "#EF4444",
                  ],
                  [
                    "Net Interest Margin",
                    money(
                      interestIncome -
                        fds.rows.reduce((s, f) => s + (f.amount * f.rate) / 100 / 12, 0),
                    ) + "k",
                    "#2563EB",
                  ],
                ].map(([l, v, col]) => (
                  ...
                  >
                    <span className="text-[12.5px] text-slate-600">{l}</span>
                    <span className="text-[13.5px] font-bold font-mono" style={{ color: col }}>
                      TZS {v}
                    </span>
                  </div>
                ))}
...
export const RST_MENU_SEED = [
  // Starters
  {
    ...
    prepTime: 8,
    available: true,
    description: "Crispy pastry with spiced beef filling",
    image: "🥟",
    popular: true,
  },
  {
    ...
    prepTime: 5,
    available: true,
    description: "Chef&apos;s daily soup with bread",
    image: "🍲",
    popular: false,
  },
  {
    ...
    prepTime: 10,
    available: true,
    description: "Tiger prawns with Marie Rose sauce",
    image: "🍤",
    popular: true,
  },
  // Mains
  ...
  },
  {
    ...
    prepTime: 20,
    available: true,
    description: "Whole tilapia with coconut rice and salad",
    image: "🐟",
    popular: true,
  },
  {
    id: "M006",
    name: "Zanzibar Biryani",
    category: "Main Course",
    price: 14000,
    cost: 5500,
    prepTime: 30,
  ...
  },
  {
    ...
    prepTime: 20,
    available: true,
    description: "Beef sirloin with fries and pepper sauce",
    image: "🥩",
    popular: false,
  },
  {
  ...
  // Grills
  {
    ...
    prepTime: 30,
    available: true,
    description: "Chicken, beef & sausage with chips and salad",
    image: "🍖",
    popular: true,
  },
  // Desserts
  {
    ...
    prepTime: 10,
    available: true,
    description: "Sweet fried dumplings with honey",
    image: "🍡",
    popular: true,
  },
  {
  ...
  // Drinks
  {
    ...
    prepTime: 5,
    available: true,
    description: "Mango, passion, orange or watermelon",
    image: "🧃",
    popular: true,
  },
  {
    ...
    prepTime: 1,
    available: true,
    description: "Coca-Cola, Sprite, Fanta, Water",
    image: "🥤",
    popular: false,
  },
  {
  ...
  },
  {
    ...
    prepTime: 2,
    available: true,
    description: "Red or white, Cape Town",
    image: "🍷",
    popular: false,
  },
];
...
export const RST_ORDERS_SEED = [
  {
    ...
    waiter: "Ali Hassan",
    items: [
      { id: "M004", name: "Nyama Choma (500g)", qty: 2, price: 18000 },
      { id: "M012", name: "Fresh Juice", qty: 2, price: 4500 },
    ],
    subtotal: 45000,
    tax: 4500,
    total: 49500,
  ...
  },
  {
    ...
    waiter: "Sara Mwenda",
    items: [
      { id: "M006", name: "Zanzibar Biryani", qty: 1, price: 14000 },
      { id: "M003", name: "Prawn Cocktail", qty: 1, price: 12000 },
      { id: "M013", name: "Softdrinks", qty: 2, price: 2500 },
    ],
    subtotal: 31000,
    tax: 3100,
    total: 34100,
  ...
  },
  {
    ...
    waiter: "John Kamau",
    items: [
      { id: "M005", name: "Grilled Tilapia", qty: 2, price: 16000 },
      { id: "M014", name: "Tusker Lager (500ml)", qty: 3, price: 5500 },
    ],
    subtotal: 48500,
    tax: 4850,
    total: 53350,
...
export const RST_RESERVATIONS_SEED = [
  {
    ...
    table: "T07",
    status: "Confirmed",
    note: "Anniversary — arrange flowers",
  },
  {
    id: "RES-002",
    name: "Baraka Enterprises",
    phone: "0756-002-002",
    date: "2026-07-21",
    time: "13:00",
    covers: 8,
  ...
  },
  {
    ...
    table: "T03",
    status: "Pending",
    note: "Birthday cake needed",
  },
];

export const RST_WAITERS = [
  "Ali Hassan",
  "Sara Mwenda",
  "John Kamau",
  "Amina Juma",
