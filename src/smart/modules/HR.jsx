...
export function LmsInsightsPanel({ employees }) {
  ...
  const overdue = training.rows.filter(
    (r) => r.mandatory && r.status !== "Completed" && r.dueDate && r.dueDate < t,
  );
  const completed = training.rows.filter((r) => r.status === "Completed").slice(0, 6);

  function printCertificate(row) {
    printAsPDF(
      `Certificate — ${row.course}`,
      `
    ...
    <div className="space-y-4 mt-5">
      {overdue.length > 0 && (
        ...
        >
          ...
          <p className="text-[11.5px] text-[#991B1B]">
            <strong>{overdue.length} overdue mandatory training(s):</strong>{" "}
            {overdue
              .slice(0, 4)
              .map((r) => `${r.employee} — ${r.course}`)
              .join("; ")}
            {overdue.length > 4 ? "…" : ""}
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
        ...
        <p className="text-[11.5px] text-slate-500 mb-3">
          Real completion rates from real assignments — video is linked, not hosted, and exams need
          a question model (named future work, not a quiz that grades nothing).
        </p>
        <div className="space-y-2">
                ...
                <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: s.done === s.total ? "#16A34A" : "#F59E0B",
                    }}
                  />
                </div>
          ...
          {!training.loading && training.rows.length === 0 && (
      ...
      {completed.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm divide-y divide-slate-50">
          <p className="text-[13px] font-semibold text-[#111827] px-4 pt-3.5 pb-2">
            ...
            <div key={row.id} className="flex items-center justify-between px-4 py-2.5">
              <div className="min-w-0">
                <p className="text-[12.5px] font-medium text-[#111827] truncate">
                  {row.employee} — {row.course}
                </p>
                <p className="text-[10.5px] text-slate-400">
                  {row.compliance ? "Compliance · " : ""}completed {row.completionDate || "—"}
                </p>
              </div>
              <button
                onClick={() => printCertificate(row)}
                className="text-[11px] font-medium text-[#16A34A] hover:underline shrink-0 ml-3 flex items-center gap-1"
              >
...
export const dutiesSeed = [
  {
    ...
    priority: "High",
    status: "Approved",
    completedAt: "2026-07-21 09:05",
    approvedBy: "Sarah Kileo",
    approvedAt: "2026-07-21 09:15",
    notes: "",
  },
  {
    id: "DTY-002",
    title: "Client meeting — Uzuri",
    assignee: "Sarah Kileo",
    dept: "Sales",
    date: "2026-07-21",
    startTime: "10:00",
    endTime: "11:30",
    ...
    priority: "High",
    status: "Completed",
    completedAt: "2026-07-21 11:35",
    approvedBy: null,
    approvedAt: null,
    notes: "Went well, follow-up needed",
  },
  {
    id: "DTY-003",
    title: "Inventory audit — Aisle C",
    assignee: "Grace Mmbaga",
    dept: "Warehouse",
    date: "2026-07-21",
    startTime: "08:00",
    endTime: "12:00",
  ...
  {
    id: "DTY-005",
    title: "Delivery run — Mwanza Rd",
    assignee: "Elias Rugambwa",
    dept: "Logistics",
    date: "2026-07-22",
    startTime: "07:00",
    endTime: "13:00",
...
export function mapDutyRow(r) {
  return {
    ...
    type: r.duty_type || r.type || "Operations",
...
export function WorkingTimetable({ employees, currentUser, canManage }) {
  ...
  const analytics = useMemo(() => {
    ...
    const overdue = weekDuties.filter(
      (d) => d.status === "Pending" && d.date < TODAY.toISOString().slice(0, 10),
    ).length;
    ...
    const byDay = weekDays.map((wd) => {
      ...
      return {
        ...
        pct: dayDs.length > 0 ? Math.round((dayDone / dayDs.length) * 100) : 0,
      };
    });
  ...
  async function addDuty(form) {
    ...
    const conflict = rows.find(
      (d) =>
        d.assignee === form.assignee &&
        d.date === form.date &&
        d.assignee !== "ALL" &&
        d.startTime &&
        form.startTime &&
        d.startTime < form.endTime &&
        form.startTime < d.endTime,
    );
    if (conflict) {
      notify(
        `⚠ Schedule conflict — ${form.assignee} already has "${conflict.title}" at ${conflict.startTime}–${conflict.endTime} on ${form.date}`,
        "error",
      );
      return;
    }
    ...
    logAudit(
      ...
      "Timetable",
      currentUser.name,
      `${draft.title} to ${draft.assignee} on ${draft.date}`,
    );
    if (IS_CONFIGURED) {
      try {
        await sb("hr_duties")
        ...
        notify("Duty saved locally, server sync failed.", "error");
      }
    }
  }
  ...
  async function markComplete(duty) {
    ...
    setRows((p) =>
      p.map((d) => (d.id === duty.id ? { ...d, status: "Completed", completedAt: now } : d)),
    );
  ...
  async function approveDuty(duty, approve) {
    if (!canManage && !isApprover(currentUser)) {
      notify("Only managers and higher-ranked staff can approve duties.", "error");
      return;
    }
    // Biometric gate (same pattern as Approvals)
    const method = await signWithBiometric(currentUser.name);
    if (approve && method === null) {
      notify("Signature cancelled — duty not approved.", "error");
      return;
    }
    ...
    setRows((p) =>
      p.map((d) => (d.id === duty.id ? { ...d, status: next, approvedBy, approvedAt } : d)),
    );
    ...
    logAudit(
      next === "Approved" ? "Duty approved" : "Duty rejected",
      "Timetable",
      currentUser.name,
      `${duty.title} by ${duty.assignee}`,
    );
    if (IS_CONFIGURED && duty.dbId) {
      try {
        await sb("hr_duties")
          .eq("id", duty.dbId)
          .update({ status: next, approved_by: approvedBy, approved_at: approvedAt })
          .run();
      } catch (_e) {}
    }
  }
  ...
  function printWeekSchedule() {
    ...
    const dayBlocks = weekDays
      .map((wd) => {
        const dayDs = filtered
          .filter((d) => d.date === wd.date)
        ...
        ${
          dayDs.length === 0
            ? `<div style="padding:12px 14px;font-size:11px;color:#9CA3AF;background:#F8FAFB;border-radius:0 0 8px 8px">No duties scheduled</div>`
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[12.5px] text-slate-500 mt-0.5">
            Assign duties · Track progress · Manager sign-off
          </p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 text-center">
          <span className="text-[13px] font-semibold text-[#111827]">
            Week of{" "}
            {new Date(weekStart).toLocaleDateString("en", { month: "long", day: "numeric" })}–
            {weekDays[6].num}, {weekStart.getFullYear()}
          </span>
          {weekOffset === 0 && (
            <span className="ml-2 text-[11px] text-[#16A34A] font-bold">Current Week</span>
          )}
        </div>
        <button
          onClick={() => setWeek((w) => w + 1)}
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <select
          className="text-[12px] border border-slate-200 rounded-lg px-3 py-2 bg-white text-slate-600 font-medium"
          value={deptFilter}
        ...
        {(deptFilter !== "all" || typeFilter !== "All Types") && (
          <button
            ...
            className="text-[11.5px] text-[#EF4444] font-semibold"
          >
            ✕ Clear
          </button>
        )}
        ...
        <div className="flex gap-2 justify-between">
          {analytics.byDay.map((wd) => {
            ...
            return (
              <div
                key={wd.date}
                className={`flex flex-col items-center gap-1 flex-1 px-1 py-2 rounded-xl transition-all ${wd.isToday ? "bg-[#F0FDF4] border border-[#16A34A]/20" : ""}`}
              >
                ...
                <span
                  className={`text-[10.5px] font-bold ${wd.isToday ? "text-[#16A34A]" : "text-slate-500"}`}
                >
                  {wd.label}
                </span>
        ...
        <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-3.5 flex items-start gap-2.5">
          ...
          <div>
            <p className="text-[13px] font-bold text-[#991B1B]">
              ⚠ {analytics.conflicts.length} scheduling conflict
              {analytics.conflicts.length > 1 ? "s" : ""} detected
            </p>
            <p className="text-[11.5px] text-[#B91C1C] mt-0.5">
              Same assignee is scheduled for overlapping duties. Review and reassign.
            </p>
          </div>
        </div>
      )}
            ...
            return (
              <div key={wd.date}>
                <div className={`flex items-center gap-3 mb-2 ${wd.isToday ? "" : "opacity-80"}`}>
                  <div
                    className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center text-white shrink-0 ${wd.isToday ? "bg-[#16A34A]" : "bg-[#374151]"}`}
                  >
                  ...
                  {dayDs.length > 0 && (
                    <span className="text-[11px] text-slate-400 shrink-0">
                      {dayDs.filter((d) => ["Completed", "Approved"].includes(d.status)).length}/
                      {dayDs.length} done
                    </span>
                  )}
                </div>
                {dayDs.length === 0 ? (
                  ...
                  <div className="ml-5 pl-4 border-l-2 border-slate-200 space-y-2 pb-2">
                    {dayDs.map((duty) => {
                      ...
                      const canComplete =
                        duty.status === "Pending" || duty.status === "In Progress";
                      const canApprove =
                        duty.status === "Completed" && (canManage || isApprover(currentUser));
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            onClick={() => setSel(isExpanded ? null : duty)}
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex items-start gap-2.5 flex-1 min-w-0">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (canComplete) markComplete(duty);
                                    else if (canApprove) setCfm(duty);
                                  }}
                                ...
                                <div className="flex-1 min-w-0">
                                  <p
                                    className={`text-[13px] font-bold text-[#111827] leading-tight ${duty.status === "Approved" ? "line-through opacity-60" : ""}`}
                                  >
                                    {duty.title}
                                    {isConflict && (
                                      <span className="ml-1.5 text-[10px] text-[#EF4444] font-black">
                                        ⚠ CONFLICT
                                      </span>
                                    )}
                                  </p>
                                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                                    ...
                                    <span
                                      ...
                                      style={{ background: pc.bg, color: pc.col }}
                                    >
                                      {duty.priority}
                                    </span>
                                    <span
                                  ...
                                  {duty.status === "Approved" && duty.approvedBy && (
                                    <p className="text-[10.5px] text-[#16A34A] mt-1">
                                      ✓ Approved by {duty.approvedBy} at {duty.approvedAt}
                                    </p>
                                  )}
                                  {duty.status === "Completed" && !duty.approvedBy && (
                                    <p className="text-[10.5px] text-[#2563EB] mt-1">
                                      ⏳ Completed {duty.completedAt} · Awaiting manager approval
                                    </p>
                                  )}
                                </div>
                              </div>
                          ...
                          {isExpanded && (
                            <div className="border-t border-slate-100 px-3 py-3 space-y-2">
                              {duty.notes && (
                                <p className="text-[12px] text-slate-600">📝 {duty.notes}</p>
                              )}
                              {/* Biometric verification panel */}
                              {(duty.completionVerified || duty.completionMethod) && (
                                <div
                                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border ${duty.completionVerified ? "bg-[#F0FDF4] border-[#BBF7D0]" : "bg-slate-50 border-slate-200"}`}
                                >
                                  <div
                                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-[16px] shrink-0 ${duty.completionVerified ? "bg-[#16A34A]/10" : "bg-slate-200"}`}
                                  >
                                    {duty.completionVerified ? "🔒" : "📝"}
                                  </div>
                                  <div>
                                    <p
                                      className={`text-[12px] font-bold ${duty.completionVerified ? "text-[#15803D]" : "text-slate-500"}`}
                                    >
                                      {duty.completionVerified
                                        ? "Biometrically Verified Completion"
                                        : "Manual Completion (unverified)"}
                                    </p>
                                    <p className="text-[10.5px] text-slate-400">
                                      {duty.completionVerified
                                    ...
                                    {duty.completionMethod && (
                                      <p className="text-[10px] font-mono text-slate-300 mt-0.5">
                                        sig_method: {duty.completionMethod}
                                      </p>
                                    )}
                                  </div>
                                </div>
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                                <div>
                                  ...
                                  <strong>{duty.dept || "—"}</strong>
                                </div>
                                <div>
                                  <span className="text-slate-400">ID:</span>{" "}
                                  <strong className="font-mono">{duty.id}</strong>
                                </div>
                                {duty.completedAt && (
                                  <div>
                                    <span className="text-slate-400">Completed:</span>{" "}
                                    <strong>{duty.completedAt}</strong>
                                  </div>
                                )}
                                {duty.approvedAt && (
                                  <div>
                                    <span className="text-slate-400">Approved:</span>{" "}
                                    <strong>{duty.approvedAt}</strong>
                                  </div>
                                )}
                              </div>
                              {/* Duty status flow */}
                              <div className="flex items-center gap-1.5 mt-2">
                                {["Pending", "In Progress", "Completed", "Approved"].map(
                                  (s, i, arr) => {
                                    const isCurrent = duty.status === s;
                                    const isDone =
                                      ["Completed", "Approved"].includes(duty.status) &&
                                      (s === "Pending" ||
                                        s === "In Progress" ||
                                        (s === "Completed" && duty.status === "Approved"));
                                    const sc2 = DUTY_STATUS_CFG[s];
                                    return (
                                      <React.Fragment key={s}>
                                        <div
                                          className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10.5px] font-bold transition-all ${isCurrent ? "text-white" : "opacity-40"}`}
                                          style={{
                                            background: isCurrent ? sc2.col : "#F1F5F9",
                                            color: isCurrent ? undefined : sc2.col,
                                          }}
                                        >
                                          {sc2.icon} {s}
                                        </div>
            ...
            <table className="w-full min-w-[700px] text-[12px]">
              <thead>
                <tr className="bg-[#0D2214]">
                  <th className="px-3 py-3 text-left text-[10px] font-bold uppercase tracking-wide text-[rgba(255,255,255,.5)] w-28">
                    Time / Staff
                  </th>
                  {weekDays.map((wd) => (
                    <th
                      key={wd.date}
                      className={`px-2 py-3 text-center font-bold text-[11px] ${wd.isToday ? "text-[#16A34A]" : "text-[rgba(255,255,255,.7)]"}`}
                    >
                      <div>{wd.label}</div>
                      <div
                        className={`w-6 h-6 rounded-full mx-auto mt-1 flex items-center justify-center text-[12px] font-black ${wd.isToday ? "bg-[#16A34A] text-white" : "text-[rgba(255,255,255,.6)]"}`}
                      >
                        {wd.num}
                      </div>
                    </th>
                    ...
                    <tr key={person} className={pi % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                      ...
                      {weekDays.map((wd) => {
                        const cell = filtered.filter(
                          (d) =>
                            d.date === wd.date && (d.assignee === person || d.assignee === "ALL"),
                        );
                        return (
                          <td
                            key={wd.date}
                            className={`px-1.5 py-2 align-top min-w-[90px] ${wd.isToday ? "bg-[#F0FDF4]/30" : ""}`}
                          >
                            {cell.length === 0 ? (
                              <div className="h-8" />
                            ) : (
                              cell.map((duty) => {
                                const sc =
                                  DUTY_STATUS_CFG[duty.status] || DUTY_STATUS_CFG["Pending"];
                                return (
                                  <div
                                    key={duty.id}
                                    onClick={() => {
                                  ...
                                  >
                                    ...
                                    {conflictIds.has(duty.id) && (
                                      <div className="absolute top-0.5 right-0.5 w-3 h-3 bg-[#EF4444] rounded-full text-white text-[8px] flex items-center justify-center">
                                        !
                                      </div>
                                    )}
                                  </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            <p className="text-[13.5px] font-bold text-[#111827]">
              All Duties — {filtered.length} for this week
            </p>
          </div>
          {filtered.length === 0 ? (
                ...
                {filtered
                  ...
                  .map((duty) => {
                    ...
                    return (
                      <tr
                        key={duty.id}
                        className={`border-b border-slate-50 last:border-0 hover:bg-slate-50/50 cursor-pointer ${isConflict ? "bg-red-50/40" : ""}`}
                        onClick={() => {
                          setView("timeline");
                          setSel(duty);
                        }}
                      >
                        <td className="px-3 py-3">
                          <span className="text-[13px]" style={{ color: sc.col }}>
                            {sc.icon}
                          </span>
                        </td>
                        <td className="px-3 py-3 font-semibold text-[#111827]">
                          {duty.title}
                          {isConflict && <span className="ml-1 text-[10px] text-[#EF4444]">⚠</span>}
                        </td>
                        <td className="px-3 py-3 text-slate-600">{duty.assignee}</td>
                        <td className="px-3 py-3 text-slate-500">{duty.dept || "—"}</td>
                        <td className="px-3 py-3 font-mono text-slate-500 text-[11.5px]">
                          {duty.date}
                        </td>
                        <td className="px-3 py-3 font-mono text-slate-500 text-[11.5px]">
                          {duty.startTime}–{duty.endTime}
                        ...
                        <td className="px-3 py-3 text-[11px] text-[#16A34A]">
                          {duty.approvedBy ? `✓ ${duty.approvedBy.split("(")[0].trim()}` : "—"}
                        </td>
                        <td className="px-3 py-3">
                          {duty.completionVerified ? (
      ...
      {confirmApprove && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4">
            <div className="w-14 h-14 rounded-2xl bg-[#F0FDF4] flex items-center justify-center text-[28px] mx-auto mb-4">
              ✓
            </div>
            ...
            <p className="text-[11.5px] text-slate-400 text-center mb-5">
              Completed by <strong>{confirmApprove.assignee}</strong> at{" "}
              {confirmApprove.completedAt}
            </p>
            <p className="text-[11.5px] text-slate-500 text-center mb-4">
              Your biometric signature or PIN will be requested to confirm approval.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => approveDuty(confirmApprove, false)}
                className="flex-1 py-2.5 rounded-xl border border-[#EF4444]/30 text-[#EF4444] text-[12.5px] font-bold hover:bg-[#FEF2F2]"
              >
                ✗ Reject
              </button>
              <button
                onClick={() => approveDuty(confirmApprove, true)}
...
export function DutyFormPanel({ employees, onClose, onSubmit }) {
  const [form, setForm] = useState({
    title: "",
    assignee: employees.filter((e) => e.status === "Active")[0]?.name || "ALL",
    dept: "",
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      <div
        className="relative w-full sm:w-[440px] bg-white h-full shadow-2xl overflow-y-auto flex flex-col"
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4 overflow-y-auto">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Morning stock count"
            />
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 text-[12.5px] font-medium border border-slate-200 rounded-xl text-slate-500 hover:bg-slate-50"
          >
            Cancel
          </button>
          <button
...
export function HR({
  ...
  const HR_KPIS = [
    ...
    },
    {
      ...
      value: `TZS ${money(stats.payroll)}k`,
      delta: "Active staff",
      up: true,
    ...
    {
      label: "Pending Leave",
      value: String(leaveRequests.filter((l) => l.status === "Pending").length),
      delta: "Needs review",
      up: false,
      icon: AlertCircle,
    },
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Team roster, recruitment, attendance, and payroll in one place
        </p>
      </div>
      ...
      {tab === "employees" && (
        <Employees employees={employees} setEmployees={setEmployees} loading={empLoading} />
      )}
      {tab === "timetable" && (
        <WorkingTimetable employees={employees} currentUser={currentUser} canManage={canManage} />
      )}
      ...
      {tab === "training" && (
        <>
          ...
          <LmsInsightsPanel employees={employees} />
        </>
      )}
      {tab === "leave" && (
        <LeaveRequests
...
export function Employees({ employees, setEmployees, loading }) {
  ...
  async function addEmployee(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("hr_employees")
          .insert({
            full_name: form.name,
            role: form.role,
  ...
  async function setStatus(id, status) {
    ...
    if (IS_CONFIGURED && emp?.dbId) {
      try {
        await sb("hr_employees").eq("id", emp.dbId).update({ status }).run();
      } catch (e) {
        notify("Couldn't update the employee status on the server.", "error");
      }
    }
  }
  ...
  async function deleteEmployee(id) {
    ...
    if (IS_CONFIGURED && emp?.dbId) {
      try {
        await sb("hr_employees").eq("id", emp.dbId).delete().run();
      } catch (e) {
        notify("Couldn't remove the employee on the server.", "error");
      }
    }
  }
        ...
        <div className="bg-[#F5F3FF] border border-[#C4B5FD] rounded-xl p-4 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px] text-[#7C3AED]">
                Employee enters this code in the Employee Portal to join the company system
              </p>
            </div>
            <button
              onClick={() => {
          ...
          {generatedCode && (
            <div className="bg-white rounded-xl border-2 border-[#7C3AED] p-4">
              <p className="text-[11px] font-bold text-[#7C3AED] uppercase tracking-wider mb-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => {
                    if (navigator.clipboard) navigator.clipboard.writeText(generatedCode.code);
                    notify("Code copied!");
                  }}
              ...
              <p className="text-[11px] text-slate-500 mt-2.5 text-center">
                Share this code with the employee. Valid for 7 days. One-time use.
              </p>
            </div>
          )}
          {/* Past codes */}
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {codes.map((inv, i) => (
                      ...
                      >
                        <span className="font-mono font-bold text-[14px] tracking-widest text-[#111827]">
                          {inv.code}
                        </span>
                        <div className="flex gap-3 text-[11.5px] text-slate-500">
                          ...
                          <span>{inv.role}</span>
                          <span
                            className={`font-semibold ${inv.used ? "text-[#16A34A]" : new Date(inv.expires) < new Date() ? "text-[#EF4444]" : "text-[#F59E0B]"}`}
                          >
                            {inv.used
                              ? "✓ Used"
                              : new Date(inv.expires) < new Date()
                                ? "Expired"
                                : "Valid until " + inv.expires}
                          </span>
                        </div>
                      </div>
                    ))}
          ...
          actions={[
            {
              ...
              onClick: () =>
                downloadCSV("employees-selected", bulk.selectedRows, [
                  { key: "name", label: "Name" },
            ...
            },
            {
              label: "Mark On Leave",
              onClick: () => {
                bulk.selectedRows.forEach((e) => {
                  setEmployees((p) =>
                    p.map((x) => (x.id === e.id ? { ...x, status: "On Leave" } : x)),
                  );
                });
                bulk.clearAll();
                notify(bulk.count + " employees marked On Leave");
              },
            },
            {
              label: "Deactivate",
              ...
              onClick: () => {
                bulk.selectedRows.forEach((e) => {
                  setEmployees((p) =>
                    p.map((x) => (x.id === e.id ? { ...x, status: "Inactive" } : x)),
                  );
                });
                bulk.clearAll();
                notify(bulk.count + " employees deactivated");
              },
            },
          ]}
        />
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          ...
                          style={{
                            backgroundColor: `${EMPLOYMENT_STATUS_COLOR[e.status]}14`,
                            color: EMPLOYMENT_STATUS_COLOR[e.status],
                          }}
                        >
                          <span
                            className="w-1.5 h-1.5 rounded-full"
                            style={{ backgroundColor: EMPLOYMENT_STATUS_COLOR[e.status] }}
                          />
                          {e.status}
                        </span>
                      </td>
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${EMPLOYMENT_STATUS_COLOR[employee.status]}14`,
              color: EMPLOYMENT_STATUS_COLOR[employee.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: EMPLOYMENT_STATUS_COLOR[employee.status] }}
            />
            {employee.status}
          </span>
        </div>
        ...
        <div className="bg-slate-50 rounded-lg p-3 mb-6">
          ...
          <p className="text-[13px] font-medium text-[#111827]">
            ...
            {employee.contractEndDate && (
              <span className="text-slate-500 font-normal"> · ends {employee.contractEndDate}</span>
            )}
          </p>
        </div>
        ...
        <div className="border-t border-slate-100 pt-4 flex flex-col gap-2">
          <div className="flex gap-2">
            {employee.status !== "Active" && (
              <button
                onClick={() => onSetStatus(employee.id, "Active")}
            ...
            {employee.status !== "On Leave" && (
              <button
                onClick={() => onSetStatus(employee.id, "On Leave")}
                className="flex-1 text-[12px] font-medium border border-slate-200 rounded-lg py-2.5 hover:bg-slate-50 transition-colors"
              >
                Mark On Leave
              </button>
            ...
            {employee.status !== "Inactive" && (
              <button
                onClick={() => onSetStatus(employee.id, "Inactive")}
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Role" required>
            ...
            />
            {touched && !form.role.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Role is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Contract type">
              ...
              >
                {EMPLOYMENT_CONTRACT_TYPES.map((c) => (
...
export function LeaveRequests({ requests, setRequests, loading, employees }) {
  async function setStatus(id, status) {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("hr_leave_requests").eq("id", id).update({ status }).run();
      } catch (e) {
        notify("Couldn't update the leave request on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        ...
        <p className="text-[11.5px] text-slate-400 mb-4">
          {ANNUAL_LEAVE_ALLOCATION} days/year allocation, minus approved Annual leave taken
        </p>
            ...
            <div key={b.name} className="border border-slate-100 rounded-lg p-3">
              ...
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden mb-1.5">
                <div
                  className="h-full rounded-full"
                  style={{
                    width: `${(b.used / ANNUAL_LEAVE_ALLOCATION) * 100}%`,
                    backgroundColor: b.remaining <= 3 ? "#EF4444" : "#16A34A",
                  }}
                />
              </div>
              <p className="text-[11px] font-mono text-slate-500">
                {b.remaining}d left · {b.used}d used
              </p>
            </div>
          ))}
        </div>
                ...
                <tr key={r.id} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                      style={{
                        backgroundColor: `${LEAVE_STATUS_COLOR[r.status]}14`,
                        color: LEAVE_STATUS_COLOR[r.status],
                      }}
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ backgroundColor: LEAVE_STATUS_COLOR[r.status] }}
                      />
                      {r.status}
                    </span>
                  </td>
                    ...
                    {r.status === "Pending" ? (
                      <div className="flex gap-1.5 justify-end">
                        <button
                          ...
                          className="text-[11.5px] font-medium text-[#EF4444] border border-[#EF4444]/25 rounded-md px-2.5 py-1 hover:bg-[#EF4444]/5"
                        >
                          Reject
                        </button>
                        <button
...
export function Recruitment() {
  const candidates = useCompanyTable("hr_candidates", candidatesSeed, {
    order: { col: "applied_date", ascending: false },
    mapRow: mapCandidateRow,
  });
  ...
  async function addCandidate(form) {
    const draft = {
      ...
      stage: "Applied",
      email: form.email,
      appliedDate: TODAY.toISOString().slice(0, 10),
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("hr_candidates")
          .insert({
            name: draft.name,
            role: draft.role,
        ...
        notify("Candidate added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function moveStage(id, dir) {
    ...
    const next =
      RECRUITMENT_STAGES[Math.min(Math.max(idx + dir, 0), RECRUITMENT_STAGES.length - 1)];
    setRows((prev) => prev.map((c) => (c.id === id ? { ...c, stage: next } : c)));
    setSelected((s) => (s && s.id === id ? { ...s, stage: next } : s));
    if (IS_CONFIGURED && cur.dbId) {
      try {
        await sb("hr_candidates").eq("id", cur.dbId).update({ stage: next }).run();
      } catch (_e) {
        notify("Couldn't save the candidate stage to the server.", "error");
      }
    }
  }
  ...
  async function deleteCandidate(id) {
    ...
    if (IS_CONFIGURED && cur?.dbId) {
      try {
        await sb("hr_candidates").eq("id", cur.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the candidate on the server.", "error");
      }
    }
  }
  ...
  const offerRate =
    total > 0
      ? Math.round((rows.filter((r) => ["Offer", "Hired"].includes(r.stage)).length / total) * 100)
      : 0;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
            ...
            <p className="text-[22px] font-bold" style={{ color: col }}>
              {v}
            </p>
          </div>
        ))}
      ...
      {total > 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {funnelData.map((d, i) => {
              ...
              return (
                <div key={d.stage} className="flex items-center gap-3">
                  ...
                  <div
                    className="flex-1 h-7 rounded-lg overflow-hidden bg-slate-100"
                    style={{ paddingLeft: i * 20 }}
                  >
                    <div
                      className="h-full rounded-lg flex items-center px-3 transition-all"
                      style={{ width: Math.max(pct, 8) + "%", background: d.fill }}
                    >
                      {d.count > 0 && (
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100 text-[11.5px]">
            ...
            <span className="text-slate-500">
              Rejected: <strong style={{ color: "#EF4444" }}>{rejected}</strong>
            </span>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Add candidates as they apply and move them through the hiring pipeline."
        ...
        <div className="flex sm:grid sm:grid-cols-3 lg:grid-cols-6 gap-3 overflow-x-auto pb-2 -mx-1 px-1 snap-x">
          {RECRUITMENT_STAGES.map((stage) => (
            <div
              key={stage}
              className="bg-slate-50 rounded-xl p-2.5 min-h-[320px] w-[200px] sm:w-auto shrink-0 snap-start"
            >
              <div className="flex items-center justify-between px-1.5 py-1.5 mb-1">
                <span
                  className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                  style={{
                    backgroundColor: `${RECRUITMENT_STAGE_COLOR[stage]}14`,
                    color: RECRUITMENT_STAGE_COLOR[stage],
                  }}
                >
                  <span
                    className="w-1.5 h-1.5 rounded-full"
                    style={{ backgroundColor: RECRUITMENT_STAGE_COLOR[stage] }}
                  />
                  {stage}
                </span>
                <span className="text-[11px] font-mono text-slate-400">
      ...
      {showForm && (
        <CandidateFormPanel onClose={() => setShowForm(false)} onSubmit={addCandidate} />
      )}
    </div>
  );
}
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${RECRUITMENT_STAGE_COLOR[candidate.stage]}14`,
              color: RECRUITMENT_STAGE_COLOR[candidate.stage],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: RECRUITMENT_STAGE_COLOR[candidate.stage] }}
            />
            {candidate.stage}
          </span>
        </div>
        <div className="space-y-3 mb-6">
          ...
          <div className="flex items-center gap-2.5 text-[13px] text-slate-600">
            <Clock size={14} className="text-slate-400" /> Applied {candidate.appliedDate}
          </div>
        </div>
        <div className="flex-1" />
        <div className="border-t border-slate-100 pt-4 flex flex-col gap-2">
          <div className="flex gap-2">
            ...
            <button
              disabled={idx === RECRUITMENT_STAGES.length - 1}
              onClick={() => onMove(candidate.id, 1)}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Recruitment</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Candidate</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
...
export async function signWithBiometric(approverName) {
  ...
  try {
    ...
    if (!cred) {
      const created = await navigator.credentials.create({
        publicKey: {
          ...
          user: {
            id: new TextEncoder().encode("approvals"),
            name: approverName || "Approver",
            displayName: approverName || "Approver",
          },
          pubKeyCredParams: [
    ...
    const assertion = await navigator.credentials.get({
      publicKey: {
        challenge: crypto.getRandomValues(new Uint8Array(32)),
        allowCredentials: [{ type: "public-key", id: b64ToBuf(cred) }],
        userVerification: "required",
        timeout: 60000,
      },
    });
...
export function BiometricClockPanel({ employees, attendance }) {
  ...
  const todayRow = (emp) =>
    attendance.rows.find((a) => a.employee === emp.name && a.date === todayStr && a.clockIn);
    ...
    try {
      const cred = await navigator.credentials.create({
        publicKey: {
          ...
          user: {
            id: new TextEncoder().encode(String(emp.id)),
            name: emp.name,
  ...
  async function clockIn(emp) {
    ...
    try {
      ...
      notify(
        `${emp.name} clocked in at ${now.toTimeString().slice(0, 5)}${late ? " (Late)" : ""} — fingerprint verified.`,
      );
      if (IS_CONFIGURED) {
        try {
          const header = await sb("hr_attendance")
            .insert({
              employee_id: emp.dbId || null,
            ...
            .run();
          if (header?.id)
            attendance.setRows((prev) =>
              prev.map((a) => (a.id === draft.id ? { ...a, dbId: header.id } : a)),
            );
        } catch (_e) {
          notify("Clocked in locally, but the server update failed.", "error");
        }
      }
    } catch (_e) {
  ...
  async function clockOut(emp) {
    ...
    try {
      ...
      attendance.setRows((prev) =>
        prev.map((a) => (a.id === row.id ? { ...a, clockOut: now.toISOString() } : a)),
      );
      notify(
        `${emp.name} clocked out at ${now.toTimeString().slice(0, 5)} — ${hours}h worked today.`,
      );
      if (IS_CONFIGURED && row.dbId) {
        try {
          await sb("hr_attendance")
            .eq("id", row.dbId)
            .update({ clock_out: now.toISOString() })
            .run();
        } catch (_e) {
          notify("Clocked out locally, but the server update failed.", "error");
        }
      }
    } catch (_e) {
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 flex items-start gap-2.5">
        ...
        <p className="text-[12px] text-slate-500">
          Biometric clock-in needs a device with a real fingerprint sensor or equivalent (Android
          fingerprint, Windows Hello, Touch ID) — none was detected in this browser. Manual
          attendance logging below still works normally.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
      <div className="px-4 sm:px-5 pt-4 pb-3 border-b border-slate-100 flex items-start gap-2.5">
        <div className="w-9 h-9 rounded-lg bg-[#DCFCE7] flex items-center justify-center shrink-0">
          <Fingerprint size={17} className="text-[#16A34A]" />
        </div>
        <div>
          <h3 className="text-[14px] font-semibold text-[#111827]">Biometric Clock-In</h3>
          <p className="text-[11.5px] text-slate-500">
            Real device fingerprint via WebAuthn — the print never leaves the employee&apos;s device
            by design; the sensor verifies locally and only "verified" comes back. Enroll each
            employee once per device.
          </p>
        </div>
      </div>
            ...
            <div key={emp.id} className="flex items-center justify-between px-4 sm:px-5 py-3">
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-slate-400">
                  {row?.clockOut
                    ? `In ${row.clockIn.slice(11, 16)} · Out ${row.clockOut.slice(11, 16)} · ${((new Date(row.clockOut) - new Date(row.clockIn)) / 3600000).toFixed(1)}h`
                    : row
                      ? `Clocked in at ${row.clockIn.slice(11, 16)}`
                      : enrolled
                        ? "Enrolled · not clocked in"
              ...
              <div className="shrink-0 ml-3">
                {!enrolled && (
                  <button
                    onClick={() => enroll(emp)}
                    disabled={busy}
                    className="text-[11.5px] font-medium border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-slate-50 disabled:opacity-50 flex items-center gap-1"
                  >
                    <Fingerprint size={12} /> {busy ? "Waiting for sensor..." : "Enroll"}
                  </button>
                )}
                {enrolled && !row && (
                  <button
                    onClick={() => clockIn(emp)}
                    disabled={busy}
                    className="text-[11.5px] font-medium btn-primary text-white rounded-lg px-3 py-1.5 disabled:opacity-50 flex items-center gap-1"
                  >
                    <Fingerprint size={12} /> {busy ? "Verifying..." : "Clock In"}
                  </button>
                )}
                {enrolled && row && !row.clockOut && (
                  <button
                    onClick={() => clockOut(emp)}
                    disabled={busy}
                    className="text-[11.5px] font-medium bg-[#F59E0B] text-white rounded-lg px-3 py-1.5 disabled:opacity-50 flex items-center gap-1"
                  >
                    <Fingerprint size={12} /> {busy ? "Verifying..." : "Clock Out"}
                  </button>
                )}
                {enrolled && row?.clockOut && (
                  <span className="text-[11px] font-medium text-[#16A34A]">Day complete</span>
                )}
              </div>
            </div>
          );
...
export function Attendance({ employees }) {
  const attendance = useCompanyTable("hr_attendance", attendanceSeed, {
    order: { col: "attendance_date", ascending: false },
  ...
  async function addAttendance(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("hr_attendance")
          .insert({
            employee_name: draft.employee,
            attendance_date: draft.date,
            status: draft.status,
          })
          .run();
      } catch (_e) {
        notify("Logged locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11.5px] text-slate-400 mb-1">Present Today</p>
          <p className="text-[18px] font-mono font-semibold text-[#16A34A]">{todayStats.present}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11.5px] text-slate-400 mb-1">Late Today</p>
          <p className="text-[18px] font-mono font-semibold text-[#F59E0B]">{todayStats.late}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11.5px] text-slate-400 mb-1">Absent Today</p>
          <p className="text-[18px] font-mono font-semibold text-[#EF4444]">{todayStats.absent}</p>
        </div>
      </div>
            ...
            {employees
              ...
              .map((e) => {
                const todayRecord = rows.find(
                  (a) =>
                    a.date === TODAY.toISOString().slice(0, 10) &&
                    (a.employee === e.name || a.employee === e.fullName),
                );
                const status = todayRecord?.status || "No Record";
                const col =
                  { Present: "#16A34A", Late: "#F59E0B", Absent: "#EF4444" }[status] || "#94A3B8";
                return (
                  <div key={e.id} className="flex items-center gap-2.5 py-1">
                    ...
                    <span className="text-[12.5px] font-medium text-[#111827] flex-1 truncate">
                      {e.fullName || e.name}
                    </span>
                    <span
      ...
      {(() => {
        ...
        const pct =
          todayRows.length > 0 ? Math.round((verified.length / todayRows.length) * 100) : 0;
        return (
          <div className="bg-[#0D2214] rounded-2xl p-4 text-white">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-[11px] text-[rgba(255,255,255,.5)]">
                  {todayStr} · {todayRows.length} records total
                </p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  <p className="text-[20px] font-black" style={{ color: col }}>
                    {n}
                  </p>
            ...
            {verified.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[rgba(255,255,255,.08)]">
                <p className="text-[10.5px] text-[rgba(255,255,255,.4)] mb-1.5">
                  Biometrically verified today:
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {verified.map((r) => (
                    <span
                      key={r.id}
                      className="inline-flex items-center gap-1 text-[10.5px] font-bold text-[#16A34A] bg-[rgba(22,163,74,0.15)] border border-[rgba(22,163,74,0.3)] px-2 py-0.5 rounded-full"
                    >
                      <Fingerprint size={9} />
                      {r.employee} · {r.clockIn}
                    </span>
                  ))}
                </div>
              </div>
              ...
              {!loading &&
                rows.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1"
                        style={{
                          background: ATTENDANCE_STATUS_COLOR[a.status] + "18",
                          color: ATTENDANCE_STATUS_COLOR[a.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: ATTENDANCE_STATUS_COLOR[a.status] }}
                        />
                        {a.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {a.verified ? (
                        ...
                        <span className="text-[10.5px] text-[#F59E0B] font-medium">
                          📝 Signed (unsigned)
                        </span>
                      ) : (
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      hint="Log attendance or clock in with the biometric panel above."
                    />
                  </td>
                </tr>
              )}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Attendance</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Attendance</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
...
export function Performance({ employees }) {
  const reviews = useCompanyTable("hr_performance_reviews", performanceReviewsSeed, {
    order: { col: "review_date", ascending: false },
    mapRow: mapPerformanceRow,
  });
  ...
  const OKR_SEED = [
    ...
    {
      id: "OKR-002",
      objective: "Expand market reach in Dar es Salaam",
      owner: "Marketing",
      period: "Q3 2026",
      keyResults: [
  ...
  async function submitReview() {
    ...
    const row = {
      ...
      score:
        Number(form.score) ||
        (form.rating === "Outstanding"
          ? 5
          : form.rating === "Exceeds Expectations"
            ? 4
            : form.rating === "Meets Expectations"
              ? 3
              : form.rating === "Needs Improvement"
                ? 2
                : 1),
    };
    setRows((p) => [row, ...p]);
  ...
  const avgScore =
    rows.length > 0 ? (rows.reduce((s, r) => s + (r.score || 3), 0) / rows.length).toFixed(1) : 0;
  const topPerformers = [...rows].sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 3);

  const RatingBadge = ({ rating }) => (
    <span
      className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full text-white"
      style={{ background: RATING_COLOR[rating] || "#6B7280" }}
    >
      {rating}
    </span>
  );
            ...
            {[
              ...
              <button
                ...
                className={
                  ...
                  (tab === id ? "bg-white text-[#111827] shadow-sm" : "text-slate-500")
                }
              >
                {label}
              </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          [
            "Avg Score",
            avgScore + "/5",
            rows.length > 0 && avgScore >= 4
              ? "#16A34A"
              : rows.length > 0 && avgScore >= 3
                ? "#2563EB"
                : "#EF4444",
          ],
          ...
          [
            "Needs Attention",
            rows.filter((r) => ["Needs Improvement", "Unsatisfactory"].includes(r.rating)).length,
            "#EF4444",
          ],
        ].map(([l, v, col]) => (
          <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <FormField label="Goals Achieved">
              <input
                className={inputClass}
                value={form.goals}
                onChange={(e) => setForm({ ...form, goals: e.target.value })}
                placeholder="Key goals met"
              />
            </FormField>
            <FormField label="Areas for Growth">
              <input
                className={inputClass}
                value={form.areas}
                  ...
                  {rows.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <span
                              ...
                              style={{ color: RATING_COLOR[r.rating] || "#6B7280" }}
                            >
                              {scoreVal}
                            </span>
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map((i) => (
                                <div
                                  ...
                                  style={{ background: i <= scoreVal ? "#7C3AED" : "#E5E7EB" }}
                                />
                              ))}
                            </div>
                          </div>
                        ...
                        <td className="px-4 py-3 text-slate-500 max-w-[140px] truncate text-[11.5px]">
                          {r.goals || "—"}
                        </td>
                        <td className="px-4 py-3 text-slate-400 max-w-[140px] truncate text-[11.5px]">
                          {r.areas || "—"}
                        </td>
      ...
      {tab === "okr" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => notify("Add OKR form")}
              className="flex items-center gap-1 text-[12px] font-semibold text-white px-3 py-2 rounded-xl bg-[#7C3AED]"
            >
              <Plus size={12} />
              Add OKR
            </button>
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right shrink-0 ml-4">
                    <p
                      ...
                      style={{
                        color:
                          avgProgress >= 70 ? "#16A34A" : avgProgress >= 40 ? "#F59E0B" : "#EF4444",
                      }}
                    >
                    ...
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span
                            className="text-[12px] font-mono font-bold ml-3 shrink-0"
                            style={{ color: col }}
                          >
      ...
      {tab === "matrix" && (
        <div className="space-y-4">
          <p className="text-[12.5px] text-slate-500">
            9-Box Grid: Performance (horizontal) vs Potential (vertical). Positions staff into 9
            talent segments to guide succession and development.
          </p>
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            {/* Y-axis label */}
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                {[
                  ...
                  {
                    row: "Low",
                    cells: ["Risk / Poor Fit", "Effective But...", "Strong Professional"],
                  },
                ].map(({ row, cells }) => (
                  <div key={row} className="grid grid-cols-3 gap-2 mb-2">
                    <div className="text-[10px] font-semibold text-slate-400 flex items-center justify-end pr-2 shrink-0 w-12">
                      {row}
                    </div>
                    {cells.map((label, ci) => {
                      const bg =
                      ...
                      const empsHere = employees
                        .filter((_, idx) => {
                          const colMap = ["Low", "Medium", "High"];
                          const perfMap = ["Low", "Medium", "High"];
                          return (
                            perfMap.indexOf("Medium") === ci &&
                            colMap.indexOf("Medium") ===
                              (row === "High" ? 2 : row === "Medium" ? 1 : 0)
                          );
                        })
                        .slice(0, 2);
                      return (
                        <div
                          key={ci}
                          className="rounded-xl border p-2.5 min-h-[72px]"
                          style={{ background: bg, borderColor: border }}
                        >
                          <p className="text-[10.5px] font-semibold text-slate-600 mb-1.5">
                            {label}
                          </p>
                          {empsHere.map((e) => (
                            <div key={e.id} className="flex items-center gap-1 mb-1">
                              <div className="w-5 h-5 rounded-full bg-[#7C3AED] flex items-center justify-center text-white text-[8px] font-bold">
                                {(e.fullName || e.name || "?").charAt(0)}
                              </div>
                              <span className="text-[10px] text-slate-600 truncate">
                                {(e.fullName || e.name || "—").split(" ")[0]}
                              </span>
                            </div>
                          ))}
                        </div>
...
export function PerformanceFormPanel({ employees, onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.employee || !form.period.trim()) return;
    onSubmit(form);
  }
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Review</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
...
export function Training({ employees }) {
  ...
  async function addTraining(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("hr_training")
          .insert({ employee_name: draft.employee, course: draft.course, status: "Not Started" })
          .run();
      } catch (_e) {
        notify("Assigned locally, server save failed.", "error");
      }
    }
  }
  ...
  async function markComplete(id) {
    setRows((p) =>
      p.map((r) =>
        r.id === id
          ? { ...r, status: "Completed", completionDate: TODAY.toISOString().slice(0, 10) }
          : r,
      ),
    );
    if (IS_CONFIGURED) {
      try {
        await sb("hr_training")
          .eq("id", id)
          .update({ status: "Completed", completion_date: TODAY.toISOString().slice(0, 10) })
          .run();
      } catch (_e) {}
    }
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          [
            "Completion Rate",
            compRate + "%",
            compRate >= 70 ? "#16A34A" : compRate >= 40 ? "#F59E0B" : "#EF4444",
          ],
        ].map(([l, v, col]) => (
          <div
            key={l}
            ...
            {courses.map((course) => {
              ...
              return (
                <div key={course.course} className="flex items-center gap-3">
                  <span className="text-[12.5px] text-slate-600 w-36 shrink-0 truncate">
                    {course.course}
                  </span>
                  <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5">
              {["all", "Not Started", "In Progress", "Completed"].map((s) => (
                <button
                  ...
                  onClick={() => setFilter(s)}
                  className={
                    "px-2.5 py-1 rounded-md text-[11.5px] font-medium transition-all " +
                    (filter === s ? "bg-white text-[#111827] shadow-sm" : "text-slate-500")
                  }
                >
                  {s === "all" ? "All" : s}
                </button>
            ...
            <button
              onClick={() => setShowForm(true)}
              className="flex items-center gap-1 text-[12px] font-semibold text-white px-3 py-2 rounded-xl bg-[#16A34A]"
            >
              <Plus size={12} />
              Assign
            </button>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-400">
            No {filter === "all" ? "training records" : filter.toLowerCase() + " trainings"} yet.
          </div>
        ) : (
          <table className="w-full text-[12.5px]">
            <thead>
              ...
              {filtered.map((r) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-[#16A34A] flex items-center justify-center text-white text-[10px] font-bold">
                          {r.employee?.charAt(0) || "?"}
                        </div>
                        <span className="font-medium text-[#111827]">{r.employee}</span>
                      </div>
                    ...
                    <td className="px-4 py-3">
                      {r.status !== "Completed" && (
                        <button
                          ...
                          className="text-[11px] font-semibold text-white bg-[#16A34A] px-2.5 py-1 rounded-lg"
                        >
                          ✓ Complete
                        </button>
                      )}
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Training</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Assign Training</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Course" required>
            <input
              ...
              placeholder="e.g. Fleet Safety Certification"
            />
          </FormField>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
...
export function Benefits({ employees }) {
  const benefits = useCompanyTable("hr_benefits", benefitsSeed, {
    order: { col: "enrollment_date", ascending: false },
    mapRow: mapBenefitRow,
  });
  ...
  async function addBenefit(form) {
    const draft = {
      ...
      status: "Active",
      enrollmentDate: TODAY.toISOString().slice(0, 10),
    };
    setRows((p) => [draft, ...p]);
    setShowForm(false);
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("hr_benefits")
          .insert({
            employee_name: draft.employee,
            benefit_type: draft.type,
        ...
        notify("Enrolled locally, server save failed.", "error");
      }
    }
  }
  ...
  const byType = useMemo(() => {
    ...
    rows
      .filter((b) => b.status === "Active")
      .forEach((b) => {
        map[b.type] = (map[b.type] || 0) + b.monthlyValue;
      });
    return Object.entries(map)
      ...
      .map(([name, value], i) => ({
        name,
        value,
        fill: ["#2563EB", "#16A34A", "#F59E0B", "#7C3AED", "#EF4444", "#0891B2"][i % 6],
      }));
  }, [rows]);
    ...
    <div className="space-y-4">
              ...
              <div className="flex-1 space-y-2">
                {byType.map((d) => (
                  <div key={d.name} className="flex items-center justify-between">
                    ...
                    <span className="text-[12px] font-bold" style={{ color: d.fill }}>
                      TZS {money(d.value)}k
                    </span>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1.5">
                  ...
                  <span
                    ...
                    style={{ color: coveragePct >= 80 ? "#16A34A" : "#F59E0B" }}
                  >
                    {covered}/{empCount}
                  </span>
                </div>
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: coveragePct + "%",
                      background: coveragePct >= 80 ? "#16A34A" : "#F59E0B",
                    }}
                  />
                </div>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100">
                ...
                <p className="text-[22px] font-bold text-[#2563EB]">
                  TZS {money(Math.round(totalMonthly / Math.max(covered, 1)))}k
                  <span className="text-[13px] font-normal text-slate-400">/mo</span>
                </p>
                <p className="text-[11px] text-slate-400">
                  Annual: TZS {money(Math.round((totalMonthly * 12) / Math.max(covered, 1)))}k per
                  covered employee
                </p>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          ...
          <button
            onClick={() => setShowForm(true)}
            className="flex items-center gap-1 text-[12px] font-semibold text-white px-3 py-2 rounded-xl bg-[#2563EB]"
          >
            <Plus size={12} />
            Enroll
          </button>
              ...
              {rows.map((b) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-[#2563EB] flex items-center justify-center text-white text-[10px] font-bold">
                          {b.employee?.charAt(0) || "?"}
                        </div>
...
export function BenefitFormPanel({ employees, onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!(Number(form.monthlyValue) > 0)) return;
    onSubmit(form);
  }
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Benefits</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Enroll Benefit</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
...
export function Payroll({ employees, expensesHook }) {
  const runs = useCompanyTable("hr_payroll_runs", payrollRunsSeed, {
    order: { col: "processed_date", ascending: false },
    mapRow: mapPayrollRunRow,
  });
  ...
  function generatePayslips(run, emps) {
        ...
        : s <= 520
          ...
          : s <= 760
            ...
            : s <= 1000
              ...
              : 128 + (s - 1000) * 0.3;
    if (!emps.length) {
      notify("No active employees to generate payslips for.", "error");
      return;
    }
    ...
    if (!win) {
      notify("Pop-up blocked — allow pop-ups to print payslips.", "error");
      return;
    }
    win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
  ...
  async function processPayroll() {
    const draft = {
      ...
      status: "Processed",
      processedDate: TODAY.toISOString().slice(0, 10),
    };
    setRows((prev) => [draft, ...prev]);
    ...
    const expenseDraft = {
      id: docId("EX"),
      vendor: `Payroll — ${currentPeriod}`,
      category: "Salaries",
      date: draft.processedDate,
      dueDate: draft.processedDate,
      amount: currentTotal,
      status: "Paid",
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("hr_payroll_runs")
          .insert({
            period: draft.period,
            employee_count: draft.employeeCount,
            total_amount: draft.totalAmount,
            status: "Processed",
            processed_date: draft.processedDate,
          })
          .single()
          .run();
        if (header?.id)
          setRows((prev) => prev.map((r) => (r.id === draft.id ? { ...r, dbId: header.id } : r)));
        const expHeader = await sb("finance_expenses")
          .insert({
            vendor: expenseDraft.vendor,
            category: "Salaries",
            expense_date: expenseDraft.date,
            due_date: expenseDraft.dueDate,
        ...
        if (expHeader?.id)
          expensesHook.setRows((prev) =>
            prev.map((e) => (e.id === expenseDraft.id ? { ...e, dbId: expHeader.id } : e)),
          );
      } catch (_e) {
        notify("Payroll processed locally, but saving to the server failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-[20px] font-mono font-semibold text-[#111827]">
            TZS {money(currentTotal)}k
          </p>
        </div>
        <button
        ...
        >
          {alreadyProcessed
            ? `${currentPeriod} already processed`
            : `Process Payroll — TZS ${money(currentTotal)}k`}
        </button>
        {!alreadyProcessed && (
          <p className="text-[11.5px] text-slate-400 mt-2.5">
            Creates a real "Salaries" expense in Finance for the total shown above.
          </p>
        )}
      </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {(() => {
              ...
              activeEmployees.forEach((e) => {
                ...
                dm[d] = (dm[d] || 0) + e.salary;
              });
              const data = Object.entries(dm)
                .sort((a, b) => b[1] - a[1])
              ...
              {activeEmployees.slice(0, 8).map((e) => {
                ...
                return (
                  <div key={e.id} className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[#16A34A] flex items-center justify-center text-white text-[9px] font-bold shrink-0">
                      {(e.fullName || e.name || "?").charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span className="text-[11px] font-mono font-bold text-[#16A34A] shrink-0 ml-2">
                          TZS {money(e.salary)}k
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                ...
                <tr>
                  <td colSpan={4}>
                    <EmptyState
                      ...
                      hint="Processed payroll periods will appear here."
                    />
                  </td>
                </tr>
              )}
