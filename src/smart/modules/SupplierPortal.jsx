...
import { ClipboardList, FileCheck, FileText, Lock, Truck, UploadCloud, Wallet } from "lucide-react";
...
export function ExternalSupplierPortal({ currentUser, onSignOut }) {
  ...
  const effectiveSupplier =
    currentUser.customerRef || purchaseOrders.rows[0]?.supplier || "Demo Supplier";
  ...
  return (
    ...
    >
      ...
      {!currentUser.customerRef && (
        <div className="bg-[#F59E0B]/10 border-b border-[#F59E0B]/20 px-4 sm:px-6 py-2">
          <p className="text-[11.5px] text-[#8a670a]">
            Demo mode — showing sample data for "{effectiveSupplier}" since there's no real
            signed-in supplier identity yet. In live mode, this portal shows exactly one supplier's
            own records, enforced by the database itself.
          </p>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-5">
        ...
        {tab === "deliveries" && (
          <SupplierDeliveriesTab myOrders={myOrders} purchaseOrdersHook={purchaseOrders} />
        )}
      </div>
    </div>
  );
      ...
      {myOrders.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Purchase orders issued to you will appear here as soon as they're raised."
          />
        </div>
      )}
      {myOrders.map((po) => {
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                className={`text-[10.5px] font-medium px-2 py-0.5 rounded-full ${po.status === "Paid" ? "bg-[#16A34A]/10 text-[#16A34A]" : po.status === "Cancelled" ? "bg-[#EF4444]/10 text-[#EF4444]" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
              >
                {po.status}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              ...
              <span className="text-[15px] font-mono font-bold text-[#111827]">
...
export function SupplierInvoiceUploadTab({ myOrders, filesHook }) {
  ...
  const uploadedForPO = filesHook.rows.filter(
    (f) => f.linkedRecord === selectedPO && f.folder === "Purchase Orders",
  );
  ...
  async function upload(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Submitted locally, but sending to the server failed.", "error");
      }
    }
    setBusy(false);
  }
  ...
  if (myOrders.length === 0)
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
        <EmptyState
          icon={UploadCloud}
          title="No purchase orders to invoice against"
          hint="You'll be able to upload an invoice once a purchase order is issued to you."
        />
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <FormField label="Invoice file name" required>
          <input
            ...
            placeholder="e.g. Invoice-4471.pdf"
          />
          <p className="text-[11px] text-slate-400 mt-1">
            This demo register doesn&apos;t store raw file bytes (see the Document Center&apos;s own
            note on this) — it records the invoice as a real, trackable entry the business can see
            and act on.
          </p>
        </FormField>
        <button
          ...
          className="btn-primary text-white text-[12.5px] font-medium rounded-lg py-2.5 w-full disabled:opacity-40"
        >
          Submit Invoice
        </button>
      </form>
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
        {uploadedForPO.length === 0 ? (
          <EmptyState
            icon={FileText}
            title="No invoices submitted for this PO yet"
            hint="Invoices you submit for the selected purchase order will appear here."
          />
        ) : (
            ...
            {uploadedForPO.map((f) => (
              <div key={f.id} className="flex items-center gap-2.5 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-400">Submitted {f.date}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11px] text-slate-400 mb-1">Received to date</p>
          <p className="text-[17px] font-mono font-bold text-[#16A34A]">
            TZS {money(Math.round(totalPaid))}k
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          <p className="text-[11px] text-slate-400 mb-1">Pending</p>
          <p className="text-[17px] font-mono font-bold text-[#F59E0B]">
            TZS {money(Math.round(totalPending))}k
          </p>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
        {myPayments.length === 0 ? (
          <EmptyState
            ...
            hint="Payments made to you will appear here."
          />
        ) : (
          <div className="divide-y divide-slate-50">
            {myPayments.map((e) => (
              <div key={e.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-[12.5px] font-medium text-[#111827]">{e.category}</p>
                  <p className="text-[11px] text-slate-400">
                    {e.date}
                    {e.method ? ` · ${e.method}` : ""}
                  </p>
                ...
                <div className="text-right">
                  <p className="text-[13px] font-mono font-semibold text-[#111827]">
                    TZS {money(Math.round(e.amount))}k
                  </p>
                  <span
                    className={`text-[10px] font-medium ${e.status === "Paid" ? "text-[#16A34A]" : "text-[#F59E0B]"}`}
                  >
                    {e.status}
                  </span>
                </div>
      ...
      {myContracts.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Supply agreements with your company will appear here."
          />
        </div>
      )}
      {myContracts.map((c) => (
...
export function SupplierDeliveriesTab({ myOrders, purchaseOrdersHook }) {
  ...
  async function saveDate(po) {
    ...
    if (IS_CONFIGURED) {
      try {
        await callRpc(
          "supplier_update_delivery_date",
          { p_po_doc_number: po.id, p_new_expected_date: newDate },
          (typeof window !== "undefined" && window.localStorage.getItem("bs_access_token")) || "",
        );
      } catch (_e) {
        notify("Couldn't update the delivery date on the server.", "error");
      }
    }
    purchaseOrdersHook.setRows((prev) =>
      prev.map((p) => (p.id === po.id ? { ...p, expectedDate: newDate } : p)),
    );
      ...
      {pending.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Purchase orders awaiting delivery will appear here."
          />
        </div>
      )}
      {pending.map((po) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <p className="text-[13.5px] font-semibold text-[#111827]">{po.id}</p>
            <span className="text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-[#F59E0B]/10 text-[#F59E0B]">
              {po.status}
            </span>
          </div>
          <p className="text-[12px] text-slate-500 mb-3">
            Currently committed: {po.expectedDate || "No date set"}
          </p>
          {editingId === po.id ? (
            <div className="flex gap-2">
              ...
              <button
                ...
                className="text-[12px] font-medium border border-slate-200 rounded-lg px-3 shrink-0"
              >
                Cancel
              </button>
            </div>
            ...
            <button
              onClick={() => {
                setEditingId(po.id);
                setNewDate(po.expectedDate || "");
              }}
              className="btn-secondary text-[12px] font-medium rounded-lg py-2 px-3 flex items-center gap-1.5"
            >
  ...
  return (
    <div className="h-[70vh] flex flex-col items-center justify-center text-center">
      ...
      <p className="text-[13px] text-slate-500 mt-1.5 max-w-xs">
        Next up in the build sequence. Dashboard and CRM are live — this module follows the same
        architecture.
      </p>
    </div>
  );
