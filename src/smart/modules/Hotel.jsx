...
export function HotelManagementModule({ currentUser, company }) {
  ...
  const revenue = bookings.rows
    .filter((b) => b.status === "Checked Out" || b.status === "Active")
    .reduce((s, b) => s + b.paid, 0);
  const adr = occupied > 0 ? (revenue / Math.max(occupied, 1)).toFixed(0) : 0; // Average Daily Rate
  const revPAR =
    rooms.rows.length > 0 ? ((Number(adr) * occupied) / rooms.rows.length).toFixed(0) : 0; // Revenue Per Available Room
  ...
  const bkgStatus = {
    ...
    "Checked Out": ["#F3F4F6", "#6B7280"],
    Upcoming: ["#DCFCE7", "#16A34A"],
    Cancelled: ["#FEE2E2", "#EF4444"],
    NoShow: ["#FEF3C7", "#D97706"],
  };
  const Chip = ({ s }) => {
    const [bg, col] = bkgStatus[s] || ["#F3F4F6", "#6B7280"];
    return (
      <span
  ...
  async function doCheckIn() {
    ...
    const nights = Math.max(
      1,
      Math.round((new Date(checkInForm.checkOut) - new Date()) / 86400000),
    );
    ...
    rooms.setRows((p) =>
      p.map((r) =>
        r.id === checkInForm.roomId
          ? { ...r, status: "Occupied", currentGuest: checkInForm.guestName }
          : r,
      ),
    );
    setCheckInForm({
    ...
    logAudit(
      "Hotel check-in: " + room?.number,
      "Hotel",
      currentUser?.name || "Front Desk",
  ...
  function doCheckOut(bookingId) {
    ...
    if (!bkg) return;
    bookings.setRows((p) =>
      p.map((b) => (b.id === bookingId ? { ...b, status: "Checked Out", paid: b.total } : b)),
    );
    rooms.setRows((p) =>
      p.map((r) => (r.number === bkg.room ? { ...r, status: "Cleaning", currentGuest: "" } : r)),
    );
    notify("Check-out: " + bkg.guest + " from Room " + bkg.room + " · Total: USD " + bkg.total);
    logAudit(
      "Hotel check-out: " + bkg.room,
      "Hotel",
      currentUser?.name || "Front Desk",
      bkg.guest + " USD " + bkg.total,
    );
  }

  const houseKeepingTasks = rooms.rows.filter(
    (r) => r.status === "Cleaning" || r.status === "Occupied",
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg,#0F172A 0%,${HTL_BLUE} 45%,#1e40af 100%)` }}
      >
        <div
          className="absolute inset-0 opacity-5"
          style={{
            backgroundImage:
              "url('data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2260%22 height=%2260%22><path d=%22M0 30h60M30 0v60%22 stroke=%22white%22 stroke-width=%221%22 fill=%22none%22/></svg>')",
          }}
        />
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-[20px] font-bold text-white">
                {company?.name || "Hotel"} Property Management
              </h1>
            </div>
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.55)" }}>
              Front Desk · Rooms · Check-In/Out · Housekeeping · Revenue Analytics
            </p>
          </div>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={{ background: tab === t.id ? HTL_BLUE : "transparent" }}
            >
              <I size={12} />
              {t.label}
              {t.id === "housekeeping" && cleaning > 0 && (
        ...
        <div className="space-y-4">
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-[22px] font-bold mt-1 text-[#111827]">{k.v}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div key={type} className="flex items-center gap-2 mb-2.5">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: pct + "%",
                            background: pct > 80 ? "#EF4444" : pct > 60 ? "#F59E0B" : HTL_BLUE,
                          }}
                        />
                      </div>
                      <span className="text-[11.5px] text-slate-500 w-16 text-right">
                        {typeOccupied}/{typeRooms.length} rooms
                      </span>
                    </div>
                  );
                })
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {[
                ["Average Daily Rate (ADR)", "USD " + adr, HTL_BLUE],
                ["RevPAR", "USD " + revPAR, "#16A34A"],
                ["Total Revenue", "USD " + revenue, HTL_GOLD],
                ["Occupancy Rate", occupancy + "%", "#7C3AED"],
              ].map(([l, v, col]) => (
                <div
                  key={l}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11.5px] text-slate-400">
                          Room {b.room} · Check out: {b.checkOut}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[12.5px] font-bold" style={{ color: HTL_BLUE }}>
                          USD {b.total}
                        </p>
                        <button
                          onClick={() => doCheckOut(b.id)}
                          className="text-[10px] font-bold text-white px-2 py-0.5 rounded-lg mt-0.5"
              ...
              <div key={type}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {typeRooms.map((r) => {
                    ...
                    const bkg = bookings.rows.find(
                      (b) => b.room === r.number && b.status === "Active",
                    );
                    return (
                      <div
                        key={r.id}
                        className="rounded-2xl border-2 p-3 cursor-pointer hover:shadow-lg transition-all"
                        style={{ background: sc.bg, borderColor: sc.border }}
                        onClick={() => setSelectedRoom(r)}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[20px] font-black" style={{ color: sc.text }}>
                            #{r.number}
                          </span>
                          <div
                        ...
                        <p className="text-[12px] font-bold mt-1.5" style={{ color: HTL_BLUE }}>
                          USD {r.price}
                        ...
                        {r.status === "Available" && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setCheckInForm((f) => ({ ...f, roomId: r.id }));
                              setShowCheckIn(true);
                            }}
                            className="mt-2 w-full text-[10.5px] font-bold py-1.5 rounded-lg text-white"
                            style={{ background: HTL_BLUE }}
                          >
                            Check In
                          </button>
                        ...
                        {r.status === "Cleaning" && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              rooms.setRows((p) =>
                                p.map((x) => (x.id === r.id ? { ...x, status: "Available" } : x)),
                              );
                              notify("Room " + r.number + " is now available");
                            }}
                            className="mt-2 w-full text-[10.5px] font-bold py-1.5 rounded-lg border"
                            style={{ color: sc.text, borderColor: sc.border, background: "white" }}
                          >
                            Mark Ready ✓
                          </button>
                        )}
                        {r.status === "Occupied" && bkg && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              doCheckOut(bkg.id);
                            }}
                            className="mt-2 w-full text-[10.5px] font-bold py-1.5 rounded-lg text-white bg-red-500"
                          >
                            Check Out
                          </button>
                        )}
          ...
          {showCheckIn && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-[15px] font-bold text-[#111827]">Guest Check-In</h3>
                <button onClick={() => setShowCheckIn(false)} className="text-slate-400">
                  <X size={16} />
                </button>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <FormField label="Guest Full Name *">
                  <input
                ...
                <FormField label="Nationality">
                  <input
                    ...
                    onChange={(e) =>
                      setCheckInForm({ ...checkInForm, nationality: e.target.value })
                    }
                    ...
                    {rooms.rows
                      ...
                      .map((r) => (
                        <option key={r.id} value={r.id}>
                          Room {r.number} — {r.type} (USD {r.price}/night)
                        </option>
                      ))}
                  </select>
                </FormField>
                ...
                <FormField label="Adults">
                  <input
                    ...
                    onChange={(e) =>
                      setCheckInForm({ ...checkInForm, adults: Number(e.target.value) })
                    }
                  />
                </FormField>
                <FormField label="Children">
                  <input
                    ...
                    onChange={(e) =>
                      setCheckInForm({ ...checkInForm, children: Number(e.target.value) })
                    }
                  />
                </FormField>
                <FormField label="Purpose">
                  ...
                  >
                    {["Leisure", "Business", "Transit", "Event", "Other"].map((p) => (
                ...
                <FormField label="Payment Method">
                  <select
                    ...
                    onChange={(e) =>
                      setCheckInForm({ ...checkInForm, paymentMethod: e.target.value })
                    }
                  >
                    {["Card", "Cash", "Mobile Money", "Bank Transfer", "Corporate Account"].map(
                      (m) => (
                ...
                <FormField label="Special Requests" cls="col-span-2">
                  <input
                    className={inputClass}
                    value={checkInForm.specialRequests}
                    onChange={(e) =>
                      setCheckInForm({ ...checkInForm, specialRequests: e.target.value })
                    }
                    placeholder="Late check-out, extra pillow, dietary..."
                  />
                </FormField>
              </div>
              {checkInForm.roomId &&
                ...
                (() => {
                  ...
                  return (
                    <div
                      ...
                      style={{ borderColor: HTL_BLUE + "40", background: HTL_BLUE + "06" }}
                    >
                      <p className="text-[12px] text-slate-500 mb-1">Stay Summary</p>
                      <p className="text-[22px] font-black" style={{ color: HTL_BLUE }}>
                        {nights} Night{nights > 1 ? "s" : ""} — USD {(room?.price || 0) * nights}
                      </p>
                      <p className="text-[12px] text-slate-400">
                        {room?.type} Room {room?.number} · USD {room?.price}/night ·{" "}
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-xl text-[13px] text-slate-500 border border-slate-200"
                >
                  Cancel
                </button>
              </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-semibold text-[#111827]">
                Currently Checked-In ({bookings.rows.filter((b) => b.status === "Active").length}{" "}
                guests)
              </p>
            </div>
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-[10px] font-medium uppercase tracking-wide text-slate-400"
                    >
                      {h}
                    </th>
                  ))}
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-[#111827]">{b.guest}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 font-bold" style={{ color: HTL_BLUE }}>
                        #{b.room}
                      </td>
                      ...
                      <td className="px-4 py-3 font-mono font-bold" style={{ color: HTL_BLUE }}>
                        USD {b.total}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => doCheckOut(b.id)}
                          className="text-[11px] font-bold text-white px-3 py-1.5 rounded-lg bg-red-500"
                        >
                          Check Out
                        </button>
                      </td>
            ...
            {bookings.rows.filter((b) => b.status === "Active").length === 0 && (
              <p className="text-center text-slate-400 py-8">No guests currently checked in</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-semibold text-[#111827]">All Bookings</p>
              <button
                onClick={() =>
                  downloadCSV("hotel-bookings", bookings.rows, [
                ...
                className="flex items-center gap-1 text-[12px] text-slate-500 border border-slate-200 px-3 py-2 rounded-xl hover:border-blue-400 hover:text-blue-600"
              >
                <Download size={13} />
                Export
              </button>
              ...
              <tbody>
                {bookings.rows.map((b) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 font-bold" style={{ color: HTL_BLUE }}>
                      #{b.room}
                    </td>
                    ...
                    <td className="px-3 py-3 font-mono font-bold" style={{ color: HTL_BLUE }}>
                      USD {b.total}
                    </td>
      ...
      {tab === "housekeeping" && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Rooms to Clean", cleaning, "#F59E0B"],
              ["Currently Occupied", occupied, "#2563EB"],
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[13.5px] font-semibold text-[#111827]">Housekeeping Tasks</p>
            </div>
            <div className="divide-y divide-slate-50">
                ...
                .map((r) => {
                  ...
                  return (
                    <div key={r.id} className="flex items-center gap-4 px-4 py-3.5">
                      <div
                        className="w-12 h-12 rounded-xl flex items-center justify-center text-[14px] font-black shrink-0"
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[11.5px] text-slate-400">
                          {r.type} · Floor {r.floor} · {r.beds} bed{r.beds > 1 ? "s" : ""}
                        </p>
                        {r.amenities && (
                          <div className="flex gap-1 mt-1">
                            {r.amenities.slice(0, 3).map((a) => (
                              <span
                                key={a}
                                className="text-[9.5px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-medium"
                              >
                                {a}
                              </span>
                            ))}
                      ...
                      <div className="flex gap-2 shrink-0">
                        ...
                        {r.status === "Occupied" && (
                          <button
                            onClick={() => notify("Housekeeping request sent for Room " + r.number)}
                            className="px-3 py-2 rounded-xl text-[12px] font-semibold border"
                            style={{ color: sc.text, borderColor: sc.border }}
                          >
                            Request Service
                          </button>
                        )}
              ...
              {rooms.rows.filter((r) => r.status === "Cleaning" || r.status === "Occupied")
                .length === 0 && (
      ...
      {tab === "reports" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ["Occupancy Rate", occupancy + "%", HTL_BLUE],
              ["ADR (Avg Daily Rate)", "USD " + adr, HTL_GOLD],
              ["RevPAR", "USD " + revPAR, "#7C3AED"],
              ["Total Revenue", "USD " + revenue, "#16A34A"],
            ].map(([l, v, col]) => (
              <div
                key={l}
                className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {roomsByType
                .map((type) => {
                  const typeRevenue = bookings.rows
                    ...
                    <div key={type} className="flex items-center gap-2 mb-2.5">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: pct + "%", background: HTL_BLUE }}
                        />
                      </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {["Direct", "Booking.com", "Expedia", "Airbnb", "Phone"]
                .map((src) => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div
                            className="h-full rounded-full"
                            style={{ width: (n / total) * 100 + "%", background: HTL_BLUE }}
                          />
                        </div>
                        <span className="text-[12px] font-bold" style={{ color: HTL_BLUE }}>
                          {n}
                        </span>
                      </div>
                    </div>
      ...
      {tab === "reports" &&
        (() => {
          const revByType = ["Single", "Double", "Suite", "Deluxe"]
            .map((type) => {
              const typeRevenue = bookings.rows
                .filter(
                  (b) => b.type === type && (b.status === "Checked Out" || b.status === "Active"),
                )
                .reduce((s, b) => s + (b.total || 0), 0);
              return {
                name: type,
                value: Math.round(typeRevenue / 1000),
                fill: { Single: "#1E3A8A", Double: "#2563EB", Suite: "#D97706", Deluxe: "#7C3AED" }[
                  type
                ],
              };
            })
          ...
          const sourceData = ["Direct", "Booking.com", "Expedia", "Airbnb", "Phone"]
            .map((src, i) => ({
              name: src,
              value: bookings.rows.filter((b) => b.source === src).length,
          ...
          return (
            <div className="space-y-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  [`Occupancy`, `${occupancy}%`, "#1E3A8A"],
                  ["Total Revenue", `TZS ${money(Math.round(revenue / 1000))}k`, "#16A34A"],
                  ["ADR (Avg Daily Rate)", `TZS ${money(adr)}`, "#D97706"],
                  ["RevPAR", `TZS ${money(revPAR)}`, "#7C3AED"],
                ].map(([l, v, col]) => (
                  <div
                    key={l}
                    className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  <h3 className="text-[13.5px] font-semibold text-[#111827] mb-3">
                    Revenue by Room Type (TZS k)
                  </h3>
                  {revByType.length === 0 ? (
                    <p className="text-slate-400 text-center py-6">No checkout data yet</p>
                  ) : (
                      ...
                      <div className="flex-1 space-y-1.5">
                        {sourceData.map((d) => (
                          <div key={d.name} className="flex items-center justify-between">
                            ...
                            <span className="text-[12.5px] font-bold" style={{ color: d.fill }}>
                              {d.value}
                            </span>
                          </div>
                        ))}
                    ...
                    <thead>
                      <tr className="bg-[#1E3A8A]">
                        {[
                          ...
                          <th
                            key={h}
                            className="px-3 py-2 text-left text-[10px] font-bold text-white"
                          >
                            {h}
                          </th>
                        ))}
                      ...
                      {bookings.rows.slice(0, 8).map((b, i) => (
                        <tr key={b.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                          ...
                          <td className="px-3 py-2">
                            <span className="text-[10.5px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                              {b.status}
                            </span>
                          </td>
                          <td className="px-3 py-2 font-mono font-bold text-[#1E3A8A]">
                            TZS {money(Math.round((b.total || 0) / 1000))}k
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <button
                  onClick={() => {
                    ...
                    const rows = bookings.rows
                      .map(
                        (b, i) => `<tr style="background:${i % 2 === 0 ? "#fff" : "#F8FAFB"}">
                    ...
                    <td style="padding:7px 12px;font-weight:700;font-family:monospace">TZS ${money(Math.round((b.total || 0) / 1000))}k</td>
                      ...
                      .join("");
                    win.document
                      .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Hotel Report</title>
                  ...
                  className="mt-3 flex items-center gap-1.5 text-[12.5px] font-bold text-white px-4 py-2.5 rounded-xl"
