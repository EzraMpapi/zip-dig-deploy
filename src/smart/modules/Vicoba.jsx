...
export const VICOBA_LOAN_SEED = [
  ...
  },
  {
    id: "VL-002",
    memberId: "MBR-003",
    memberName: "Fatuma Juma",
    amount: 1000,
    rate: 10,
    weeks: 24,
    disbursed: "2026-02-01",
    status: "Active",
    balance: 650,
  },
  {
...
export const VICOBA_MEETING_SEED = [
  {
    ...
    totalBuyIn: 240,
    loansGiven: 1,
    minutes: "Reviewed Q2 performance. Elected new treasurer. Approved 1 loan application.",
  },
  {
    id: "MTG-002",
    date: "2026-06-30",
    venue: "Chairperson&apos;s House",
    attendees: 6,
    totalBuyIn: 288,
    loansGiven: 0,
    minutes: "Annual dividend discussion. All members present. Fine collected from 1 member.",
  },
];

export function VicobaSaccosModule({ currentUser }) {
  const [tab, setTab] = useState("overview");
  const members = useCompanyTable("vicoba_members", VICOBA_MEMBER_SEED, {
    mapRow: (r) => ({ ...r, shares: r.shares || 0, contributions: r.contributions || 0 }),
  });
  const loans = useCompanyTable("vicoba_loans", VICOBA_LOAN_SEED, {
    mapRow: (r) => ({ ...r, balance: r.balance || 0 }),
  });
  ...
  async function addMember() {
    ...
    const row = {
      ...
      shares: Number(memberForm.shares) || 1,
      contributions: (Number(memberForm.shares) || 1) * SHARE_PRICE,
      joinedDate: TODAY.toISOString().slice(0, 10),
      status: "Active",
    };
  ...
  async function repayLoan(loan, amount) {
    ...
    loans.setRows((prev) =>
      prev.map((l) => (l.id === loan.id ? { ...l, balance: newBal, status: newStatus } : l)),
    );
  ...
  async function addMeeting() {
    ...
    const row = {
      id: docId("MTG"),
      ...meetingForm,
      attendees: Number(meetingForm.attendees) || 0,
      totalBuyIn: Number(meetingForm.totalBuyIn) || 0,
      loansGiven: 0,
    };
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div
        className="rounded-2xl overflow-hidden px-5 py-5 relative"
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[12px] mt-0.5" style={{ color: "rgba(255,255,255,.65)" }}>
              Community savings &amp; credit management &middot; {members.rows.length} members
              &middot; Cycle 2026
            </p>
          </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {
                label: "Total Members",
                value: members.rows.length,
                sub: members.rows.filter((m) => m.status === "Active").length + " active",
                icon: Users,
                color: "#2563EB",
              },
              {
                label: "Group Fund",
                value: "TZS " + money(totalFund) + "k",
              ...
              },
              {
                ...
                value: "TZS " + money(totalLoanPortfolio) + "k",
              ...
              {
                label: "Defaulted",
                value: "TZS " + money(defaulted.reduce((s, l) => s + l.balance, 0)) + "k",
                sub: defaulted.length + " loans at risk",
                icon: AlertCircle,
                color: "#EF4444",
              },
            ].map((k) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-[11px] mt-0.5" style={{ color: k.color }}>
                      {k.sub}
                    </p>
                  </div>
                  <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <p className="text-[13.5px] font-semibold text-[#111827] mb-3">
                Share Distribution — Top Members
              </p>
              {(() => {
                const shareData = [...members.rows]
                ...
                return shareData.length === 0 ? (
                  <p className="text-slate-400 text-center py-4">No members</p>
                ) : (
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const loanStatusData = [
                  ...
                  {
                    name: "Repaid",
                    value: loans.rows.filter((l) => l.status === "Repaid").length,
                    fill: "#16A34A",
                  },
                  {
                    name: "Defaulted",
                ...
                return loanStatusData.length === 0 ? (
                  <p className="text-slate-400 text-center py-4">No loans yet</p>
                ) : (
                      ...
                      <div className="pt-2 border-t border-slate-100">
                        <p className="text-[11.5px] text-slate-500">
                          ...
                          <strong className="text-[#16A34A]">
                            TZS {money(Math.round(totalInterestEarned))}k
                          </strong>
                        </p>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Initial Shares">
                  <input
                    ...
                    onChange={(e) => setMemberForm({ ...memberForm, shares: e.target.value })}
                    placeholder="1"
                  />
                </FormField>
              </div>
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Member", "Phone", "Gender", "Shares", "Fund (TZS k)", "Status", ""].map(
                    (h) => (
                      <th
                        key={h}
              ...
              <tbody>
                {members.rows.map((m) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => setLoanForm({ ...loanForm, memberId: m.id })}
          ...
          {showLoanForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Repayment Weeks">
                  <input
                    type="number"
                    min="1"
                    className={inputClass}
                    value={loanForm.weeks}
                    onChange={(e) => setLoanForm({ ...loanForm, weeks: e.target.value })}
                  />
                </FormField>
              </div>
              {loanForm.amount && (
                <p className="text-[12px] text-slate-500">
                  ...
                  <strong className="text-[#111827]">
                    TZS {money(Number(loanForm.amount) * (1 + loanForm.rate / 100))}k
                  </strong>{" "}
                  &middot; Weekly:{" "}
                  <strong>
                    TZS{" "}
                    {money((Number(loanForm.amount) * (1 + loanForm.rate / 100)) / loanForm.weeks)}k
                  </strong>
                </p>
              )}
              <div className="flex gap-2">
                    ...
                    <tr key={l.id} className="border-b border-slate-50 last:border-0">
                          ...
                          <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                ...
                                background: l.status === "Defaulted" ? "#EF4444" : "#2563EB",
                              }}
                            />
                          </div>
                          <span
                            className="font-mono text-[11.5px] font-bold"
                            style={{ color: l.status === "Defaulted" ? "#EF4444" : "#111827" }}
                          >
                            {money(l.balance)}k
                          </span>
                        </div>
                      ...
                      <td className="px-4 py-3">
                        {l.status === "Active" && (
                          <button
                            onClick={() => {
                              const amt = prompt("Amount received (TZS k):");
                              if (amt && Number(amt) > 0) repayLoan(l, Number(amt));
                            }}
                            className="text-[11.5px] font-semibold text-white bg-[#16A34A] px-2.5 py-1 rounded-lg"
                          >
                            Repay
                          </button>
                        )}
          ...
          {showMeetingForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Buy-in (TZS k)">
                  <input
                    type="number"
                    className={inputClass}
                    value={meetingForm.totalBuyIn}
                    onChange={(e) => setMeetingForm({ ...meetingForm, totalBuyIn: e.target.value })}
                  />
                </FormField>
              </div>
              <FormField label="Minutes / Notes">
                <textarea
                  ...
                  placeholder="Key decisions, resolutions, actions..."
                />
              </FormField>
              <div className="flex gap-2">
                <button
            ...
            {meetings.rows.map((m) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="text-[14px] font-semibold text-[#111827]">Meeting — {m.date}</p>
                    <p className="text-[12px] text-slate-400">{m.venue}</p>
                  </div>
                  <div className="flex gap-4 text-right">
                    <div>
                      <p className="text-[10.5px] text-slate-400">Attendees</p>
                      <p className="text-[14px] font-bold text-[#2563EB]">{m.attendees}</p>
                    </div>
                    <div>
                      <p className="text-[10.5px] text-slate-400">Buy-in</p>
                      <p className="text-[14px] font-bold text-[#16A34A]">
                        TZS {money(m.totalBuyIn)}k
                      </p>
                    </div>
                  </div>
                </div>
                {m.minutes && (
                  <p className="text-[12px] text-slate-500 leading-relaxed">{m.minutes}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            <p className="text-[12px] text-slate-500 mb-4">
              Based on current interest earned from all loans. Distributed proportional to shares
              held.
            </p>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                <p className="text-[11px] text-slate-400 mb-1">Total Interest</p>
                <p className="text-[18px] font-bold text-[#16A34A]">
                  TZS {money(totalInterestEarned)}k
                </p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                <p className="text-[11px] text-slate-400 mb-1">Total Shares</p>
                <p className="text-[18px] font-bold text-[#2563EB]">{totalShares}</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                <p className="text-[11px] text-slate-400 mb-1">Per Share</p>
                <p className="text-[18px] font-bold text-[#111827]">
                  TZS {money(dividendPerShare)}k
                </p>
              </div>
            </div>
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="py-2 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400">
                    Member
                  </th>
                  <th className="py-2 text-right text-[10.5px] font-medium uppercase tracking-wide text-slate-400">
                    Shares
                  </th>
                  <th className="py-2 text-right text-[10.5px] font-medium uppercase tracking-wide text-slate-400">
                    Dividend
                ...
                {[...members.rows]
                  ...
                  .map((m) => (
                    <tr key={m.id} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="py-2.5 text-right font-mono font-bold text-[#16A34A]">
                        TZS {money(m.shares * dividendPerShare)}k
                      </td>
                    </tr>
                  ))}
              </tbody>
