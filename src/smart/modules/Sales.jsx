...
export function Sales({
  ...
  const orders = useCompanyTable("sales_orders", ordersSeed, {
    select: "*,sales_order_items(*),sales_order_returns(*,sales_order_return_items(*))",
    order: { col: "order_date", ascending: false },
    mapRow: mapOrderRow,
  });
  ...
  const errorByTab = {
    quotations: quotations.error,
    orders: orders.error,
    invoices: invoices.error,
  };
  const loadingByTab = {
    quotations: quotations.loading,
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      result = result.filter(
        (r) => r.customer.toLowerCase().includes(q) || r.id.toLowerCase().includes(q),
      );
    }
  ...
  function printInvoice(doc) {
    ...
    const statusColor =
      { Paid: "#16A34A", Unpaid: "#F59E0B", Overdue: "#EF4444", Partial: "#3B82F6" }[doc.status] ||
      "#6B7280";
    ...
    const lineRows = (doc.items || [])
      ...
      .map((it, idx) => {
        ...
        return (
          ...
          (idx % 2 === 0 ? "#fff" : "#FAFBFC") +
          '">' +
          '<td style="padding:10px 12px;font-size:12.5px;color:#111827">' +
          (idx + 1) +
          ". " +
          it.name +
          (it.sku
            ? "<br><span style='font-size:10px;color:#9CA3AF;font-family:monospace'>SKU: " +
              it.sku +
              "</span>"
            : "") +
          "</td>" +
          '<td style="padding:10px 12px;text-align:center;font-size:12.5px;color:#374151;font-family:monospace">' +
          (it.qty || 1) +
          "</td>" +
          '<td style="padding:10px 12px;text-align:right;font-size:12.5px;color:#374151;font-family:monospace">' +
          fmt(it.rate || 0) +
          "</td>" +
          (hasDisc
            ? '<td style="padding:10px 12px;text-align:center;font-size:12px;color:#F59E0B;font-family:monospace">' +
              (disc > 0 ? disc + "%" : "—") +
              "</td>"
            : "") +
          '<td style="padding:10px 12px;text-align:right;font-size:12.5px;font-weight:600;color:#111827;font-family:monospace">' +
          fmt(lineAmt) +
          "</td>" +
          "</tr>"
        );
    ...
    const payRows = (doc.payments || [])
      .map(
        (p) =>
          '<tr><td style="padding:7px 12px;font-size:11.5px;color:#374151">' +
          p.date +
          "</td>" +
          '<td style="padding:7px 12px;font-size:11.5px;color:#374151">' +
          p.method +
          "</td>" +
          '<td style="padding:7px 12px;font-size:11px;color:#6B7280;font-family:monospace">' +
          (p.reference || "—") +
          "</td>" +
          '<td style="padding:7px 12px;text-align:right;font-size:12px;font-weight:600;color:#16A34A;font-family:monospace">' +
          fmt(p.amount) +
          "</td>" +
          "</tr>",
      )
    ...
    const logoHtml = co.logo
      ...
      : '<div style="width:52px;height:52px;border-radius:12px;background:' +
        ACCENT +
    ...
    win.document.write(
      "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'/>" +
        ...
        ".invoice-title{font-size:40px;font-weight:900;color:" +
        ACCENT +
        ";letter-spacing:-1px;text-align:right}" +
        ".invoice-badge{display:inline-flex;align-items:center;gap:6px;background:" +
        statusColor +
        "20;border:1px solid " +
        statusColor +
        "50;color:" +
        ...
        "table.items thead tr{background:" +
        ACCENT +
        "}" +
        ...
        ".tot-row.total{font-size:14px;font-weight:800;color:white;background:" +
        ACCENT +
        ";padding:12px 16px;border-radius:8px;margin-top:8px;border:none}" +
        ...
        ".bank-section{padding:24px 40px;background:#F0FDF4;border-top:2px solid " +
        ACCENT +
        "20}" +
        ".bank-title{font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:" +
        ACCENT +
        ";margin-bottom:10px}" +
        ...
        ".footer-brand{font-size:11px;font-weight:700;color:" +
        ACCENT +
        "}" +
        ...
        "<div style='font-family:monospace;font-size:15px;font-weight:700;color:rgba(255,255,255,0.85);margin-top:6px'>" +
        doc.id +
        "</div>" +
        "<div style='margin-top:8px'><span class='invoice-badge'>● " +
        (doc.status || "Unpaid") +
        "</span></div>" +
        "</div>" +
        "</div>" +
        ...
        (doc.status === "Overdue" ? "#EF4444" : doc.status === "Paid" ? "#16A34A" : "#111827") +
        "'>" +
        (doc.dueDate || "On receipt") +
        "</div>" +
        "</div>" +
        "</div>" +
        "</div>" +
        ...
        "<div class='tot-row'><span>Subtotal</span><span style='font-family:monospace'>" +
        fmt(subtotal) +
        "</span></div>" +
        (totalDiscount > 0
          ? "<div class='tot-row disc'><span>Discount Savings</span><span style='font-family:monospace'>− " +
            fmt(totalDiscount) +
            "</span></div>"
          : "") +
        "<div class='tot-row'><span>VAT (" +
        Math.round(18) +
        "%)</span><span style='font-family:monospace'>" +
        fmt(tax) +
        "</span></div>" +
        "<div class='tot-row total'><span>TOTAL DUE</span><span style='font-family:monospace'>TZS " +
        fmt(total) +
        "</span></div>" +
        (doc.amountPaid > 0
          ? "<div class='tot-row' style='padding:7px 0;font-size:12.5px;color:#16A34A'><span>Amount Paid</span><span style='font-family:monospace'>− " +
            fmt(doc.amountPaid) +
            "</span></div>"
          : "") +
        (balance > 0 && doc.amountPaid > 0
          ? "<div class='tot-row balance'><span>BALANCE DUE</span><span style='font-family:monospace'>TZS " +
            fmt(balance) +
            "</span></div>"
          : "") +
        "</div></div>" +
        ...
        (co.bankName
          ? "<div class='bank-section'>" +
            ...
            "<div class='bank-row'>" +
            (co.bankName
              ? "<div class='bank-field'><div class='bank-label'>Bank</div><div class='bank-value'>" +
                co.bankName +
                "</div></div>"
              : "") +
            (co.bankAccount
              ? "<div class='bank-field'><div class='bank-label'>Account Number</div><div class='bank-value' style='font-family:monospace'>" +
                co.bankAccount +
                "</div></div>"
              : "") +
            (co.bankBranch
              ? "<div class='bank-field'><div class='bank-label'>Branch</div><div class='bank-value'>" +
                co.bankBranch +
                "</div></div>"
              : "") +
            (co.mpesa
              ? "<div class='bank-field'><div class='bank-label'>M-Pesa / Mobile Money</div><div class='bank-value' style='font-family:monospace'>" +
                co.mpesa +
                "</div></div>"
              : "") +
            "</div></div>"
          : "<div style='padding:16px 40px 20px;border-top:1px solid #F3F4F6'>" +
            ...
            "Please transfer payment to the account details provided by " +
            (co.name || "SMART MANAGER") +
        ...
        // Notes
        (doc.notes
          ? "<div class='notes-bar'><div><div style='font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#9CA3AF;margin-bottom:4px'>Notes</div><div style='font-size:12.5px;color:#374151'>" +
            doc.notes +
            "</div></div></div>"
          : "") +
        // Footer
        "<div class='footer'>" +
        "<div class='footer-note'>Thank you for your business · " +
        (co.name || "SMART MANAGER") +
        "</div>" +
  ...
  async function addDocument(form) {
    ...
    if (tab === "invoices" && crm?.rows) {
      const lead = crm.rows.find(
        (l) => l.company?.toLowerCase() === form.customer.trim().toLowerCase(),
      );
      if (lead?.creditLimit > 0) {
        const outstanding = hooksByTab.invoices.rows
          .filter(
            (i) =>
              i.customer?.toLowerCase() === form.customer.trim().toLowerCase() &&
              i.status !== "Paid",
          )
        ...
        if (outstanding + newTotal > lead.creditLimit) {
          notify(
            `Credit limit exceeded — ${form.customer}: outstanding TZS ${money(Math.round(outstanding))}k + new TZS ${money(Math.round(newTotal))}k exceeds limit TZS ${money(lead.creditLimit)}k.`,
            "error",
          );
          return;
        }
    ...
    if (IS_CONFIGURED) {
      ...
      const itemsTable = {
        quotations: "sales_quotation_items",
        orders: "sales_order_items",
        invoices: "sales_invoice_items",
      }[tab];
  ...
  async function processOrderReturn(order, { items, reason }) {
    ...
    inventory.setRows((prev) =>
      prev.map((it) => {
        const line = items.find((ri) =>
          ri.sku ? ri.sku === it.sku : ri.name.toLowerCase() === it.name.toLowerCase(),
        );
        return line ? { ...it, qty: it.qty + line.qty } : it;
      }),
    );

    hooksByTab.orders.setRows((prev) =>
      prev.map((o) =>
        o.id === order.id ? { ...o, returns: [returnRecord, ...(o.returns || [])] } : o,
      ),
    );
    setSelected((s) =>
      s && s.id === order.id ? { ...s, returns: [returnRecord, ...(s.returns || [])] } : s,
    );
    const refundValue = items.reduce((s, it) => s + it.qty * it.rate, 0);
    notify(
      `Return processed for ${order.id} — TZS ${money(Math.round(refundValue))}k, stock restocked`,
    );

    if (IS_CONFIGURED && order.dbId) {
      try {
        const header = await sb("sales_order_returns")
          .insert({ order_id: order.dbId, reason })
          .single()
          .run();
        if (header?.id) {
          await sb("sales_order_return_items")
            .insert(
              items.map((it) => ({
                return_id: header.id,
                item_name: it.name,
        ...
        for (const it of items) {
          const invItem = inventory.rows.find((i) =>
            it.sku ? i.sku === it.sku : i.name.toLowerCase() === it.name.toLowerCase(),
          );
          if (invItem) {
            await sb("inventory_items")
              .eq("sku", invItem.sku)
              .update({ qty_on_hand: invItem.qty + it.qty })
              .run();
  ...
  async function advanceDocument(kind, id, nextStatus) {
    ...
    hooksByTab[kind].setRows((prev) =>
      prev.map((d) => {
        if (d.id !== id) return d;
        const patch = { status: nextStatus };
        if (kind === "invoices" && nextStatus === "Paid")
          patch.amountPaid = lineTotal(d.items).total;
        return { ...d, ...patch };
      }),
    );
    setSelected((s) => {
    ...
    if (kind === "orders" && nextStatus === "Fulfilled" && doc) {
      ...
      inventory.setRows((prev) =>
        prev.map((it) => {
          ...
          const line = doc.items.find((li) =>
            li.sku ? li.sku === it.sku : li.name.toLowerCase() === it.name.toLowerCase(),
          );
          if (!line) return it;
      ...
      if (IS_CONFIGURED) {
        try {
          for (const d of deductions) {
            ...
            await sb("inventory_items")
              .eq("sku", d.sku)
              .update({ qty_on_hand: Math.max(0, d.prevQty - d.qty) })
              .run();
            await sb("inventory_stock_movements")
              .insert({
                item_id: d.sku,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {tab !== "subscriptions" && (
          <>
            ...
            >
              <Plus size={15} /> New{" "}
              {tab === "quotations" ? "Quotation" : tab === "orders" ? "Order" : "Invoice"}
            </button>
            <button
              onClick={() =>
                downloadCSV("sales-" + tab, filtered, [
                  { key: "id", label: "ID" },
              ...
              className="flex items-center gap-1 text-[12.5px] font-medium text-slate-500 border border-slate-200 px-3 py-2 rounded-lg hover:border-[#16A34A] hover:text-[#16A34A] transition-colors"
            >
              <Download size={13} />
              CSV
            </button>
          ...
          {tab === "invoices" &&
            (() => {
              ...
              const statusData = [
                {
                  name: "Paid",
                  value: invRows.filter((i) => i.status === "Paid").length,
                  fill: "#16A34A",
                },
                {
                  name: "Partial",
                  value: invRows.filter((i) => i.status === "Partial").length,
                  fill: "#3B82F6",
                },
                {
                  name: "Unpaid",
                  value: invRows.filter((i) => i.status === "Unpaid").length,
                  fill: "#F59E0B",
                },
                {
                  name: "Overdue",
                  value: invRows.filter((i) => i.status === "Overdue").length,
                  fill: "#EF4444",
                },
              ].filter((d) => d.value > 0);
              const topCust = Object.entries(
                invRows.reduce((m, inv) => {
                  m[inv.customer] = (m[inv.customer] || 0) + lineTotal(inv.items || []).total;
                  return m;
                }, {}),
              )
                .sort((a, b) => b[1] - a[1])
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      <ResponsiveContainer width="55%" height={140}>
                        <PieChart>
                          ...
                          <Tooltip formatter={(v, n) => [v + " invoices", n]} />
                        </PieChart>
                      </ResponsiveContainer>
                  ...
                  <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                    <p className="text-[13px] font-semibold text-[#111827] mb-3">
                      Top 5 Customers by Revenue (TZS k)
                    </p>
                    ...
                    <>
                      {filtered.map((doc) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 font-mono text-[#111827] font-medium">
                              {doc.id}
                            </td>
...
export function DocPanel({
  ...
  const { subtotal, tax, total } = lineTotal(doc.items);
  const kindLabel = { quotations: "Quotation", orders: "Sales Order", invoices: "Invoice" }[
    doc.kind
  ];
  ...
  function generatePaymentLink() {
    ...
    setPayRef(ref);
    const link =
      "https://pay.businesssphere.co.tz/" +
      doc.id +
      "?ref=" +
      ref +
      "&amt=" +
        ...
        <div className="px-6 pt-6 pb-5 border-b border-slate-100">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <h2 className="text-[19px] font-semibold text-[#111827] font-mono mt-0.5">
                {doc.id}
              </h2>
            </div>
        ...
        <div className="px-6 py-5 flex-1">
          ...
          <div className="space-y-1.5 text-[13px]">
            ...
            <p className="text-[11px] text-slate-400 italic pt-0.5">
              {numberToWords(total * 1000)} shillings only
            </p>
            {isInvoice && (
              <>
                ...
                <div
                  className={`flex justify-between font-semibold text-[13px] ${balance > 0 ? "text-[#EF4444]" : "text-[#16A34A]"}`}
                >
          ...
          {isInvoice && payOpen && balance > 0 && (
            <div className="bg-[#F0FDF4] border border-[#16A34A]/20 rounded-xl p-4 mt-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[12px] font-mono font-bold text-[#16A34A]">
                  Balance: TZS {money(balance)}k
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    placeholder={"Up to " + money(balance)}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1 block">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1 block">
                  ...
                  <span className="text-[9px] font-normal normal-case">
                    (M-Pesa code, bank ref, cheque no.)
                  </span>
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. QK12AB3456 · leave blank to auto-generate"
                  />
                  <button
                    type="button"
                    onClick={() => setPayRef(genRef(payMethod))}
              ...
              {showPayLink && payRef && (
                <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl px-3 py-2.5">
                  <p className="text-[10.5px] font-bold text-[#2563EB] mb-0.5">
                    ✓ Link copied — send to customer
                  </p>
                  <p className="text-[10.5px] font-mono text-slate-600 break-all">
                    pay.businesssphere.co.tz/{doc.id}?ref={payRef}
                  </p>
                  <p className="text-[10px] text-slate-400 mt-1">
                    Ask customer to quote reference <strong>{payRef}</strong> when paying. Then
                    confirm here.
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 text-[12px] font-medium border border-slate-200 rounded-lg py-2.5 text-slate-500 hover:bg-slate-100"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 text-[12.5px] font-bold btn-primary text-white rounded-lg py-2.5 disabled:opacity-40"
                >
                  ✓ Confirm
                </button>
              </div>
                    ...
                    <div key={p.id} className="px-3 py-3 hover:bg-slate-50/60">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2.5">
                          <div className="w-8 h-8 rounded-lg bg-[#F0FDF4] flex items-center justify-center text-[16px] shrink-0">
                            {icons[p.method] || "💰"}
                          </div>
                          <div>
                        ...
                        <span className="text-[14px] font-mono font-bold text-[#16A34A] shrink-0 pt-0.5">
                          +TZS {money(p.amount)}k
                        </span>
                      </div>
                    </div>
                  );
...
_${co.name || "SMART MANAGER"}_`;
                  ...
                  if (phone)
                    ...
                    waBus.push({
                      ...
                      vars: {
                        docId: doc.id,
                        amount: money(Math.round(total)),
                        dueDate: doc.dueDate || "On receipt",
                        ref: doc.id,
                      },
                    });
                    notify("Open Collaboration → WhatsApp to send");
                  }
              ...
              <button
                onClick={() => {
                  ...
                  const subj = encodeURIComponent(
                    `Invoice ${doc.id} from ${co.name || "SMART MANAGER"}`,
                  );
...
${co.name || "SMART MANAGER"}`);
                  ...
                  if (email) window.location.href = `mailto:${email}?subject=${subj}&body=${body}`;
                    ...
                    emailBus.push({
                      subject: `Invoice ${doc.id} from ${co.name || ""}`,
                      body: decodeURIComponent(body),
                      tmpl: "invoice",
                    });
                    notify("Open Collaboration → Email to send");
                  }
                }}
          ...
          {onDelete && (
            <ConfirmDeleteButton
              label={`Delete ${kindLabel.toLowerCase()}`}
...
export function OrderReturnFormPanel({ order, onClose, onSubmit }) {
  ...
  const remaining = order.items
    .map((it) => {
      const alreadyReturned = (order.returns || []).reduce(
        (s, r) =>
          s + (r.items.find((ri) => (it.sku ? ri.sku === it.sku : ri.name === it.name))?.qty || 0),
        0,
      );
  ...
  const returnItems = remaining
    .filter((it) => qtys[it.name] > 0)
    .map((it) => ({ sku: it.sku, name: it.name, qty: qtys[it.name], rate: it.rate }));
      ...
      >
              ...
              {remaining.map((it) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-400 font-mono">
                      Up to {it.maxQty} returnable · {money(it.rate)}k each
                    </p>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      type="button"
                      onClick={() => setQty(it.name, qtys[it.name] - 1, it.maxQty)}
          ...
          {valid && (
            <div className="bg-slate-50 rounded-lg p-3 text-[13px]">
              <div className="flex justify-between font-semibold text-[#EF4444]">
                ...
                <span className="font-mono">TZS {money(Math.round(refundValue))}k</span>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          ...
          <button
            type="button"
            disabled={!valid}
            onClick={() => onSubmit({ items: returnItems, reason })}
...
export function DocFormPanel({ kind, onClose, onSubmit, inventory }) {
  ...
  const totals = usableItems.length
    ? lineTotal(
        usableItems.map((it) => ({
          ...it,
          qty: Number(it.qty) || 0,
      ...
      >
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Customer" required>
            ...
            />
            {touched && !form.customer.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Customer is required.</p>
            )}
          </FormField>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[12px] font-medium text-slate-600">
                Line items <span className="text-[#EF4444]">*</span>
              </label>
              <button
                type="button"
                onClick={addItemRow}
                className="text-[11.5px] font-medium text-[#16A34A] hover:text-[#15803D] flex items-center gap-1"
              >
            ...
            <datalist id="inventory-item-names">
              {inventory.rows.map((it) => (
                <option key={it.sku} value={it.name} />
              ))}
            </datalist>
                ...
                <div key={i}>
                  <div className="flex gap-2 items-start">
                    ...
                    {form.items.length > 1 && (
                      <button
                        ...
                        className="text-slate-300 hover:text-[#EF4444] mt-2.5 shrink-0"
                  ...
                  {it.sku && (
                    <p className="text-[10.5px] text-[#16A34A] font-mono mt-1 ml-0.5 flex items-center gap-1">
                      <CheckCircle2 size={10} /> Linked to stock item {it.sku} — fulfillment will
                      deduct inventory
                    </p>
                  )}
                </div>
            ...
            {touched && usableItems.length === 0 && (
              <p className="text-[11px] text-[#EF4444] mt-1.5">
                Add at least one item with a name and rate.
              </p>
            )}
          </div>
            ...
            <div className="bg-slate-50 rounded-lg p-3 space-y-1 text-[12.5px]">
              {discountSavings > 0 && (
                <div className="flex justify-between text-[#16A34A]">
                  ...
                  <span className="font-mono">- TZS {money(Math.round(discountSavings))}k</span>
                </div>
              )}
              <div className="flex justify-between text-slate-500">
                <span>Subtotal</span>
                <span className="font-mono">TZS {money(totals.subtotal)}k</span>
              </div>
              <div className="flex justify-between text-slate-500">
                <span>VAT ({Math.round(TAX_RATE * 100)}%)</span>
                <span className="font-mono">TZS {money(totals.tax)}k</span>
              </div>
              <div className="flex justify-between text-[#111827] font-semibold pt-1 border-t border-slate-200 mt-1">
                <span>Total</span>
                <span className="font-mono">TZS {money(totals.total)}k</span>
              </div>
            </div>
          )}
...
export function Subscriptions({ subscriptions, invoices }) {
  ...
  const filtered = useMemo(() => {
    ...
    const q = query.toLowerCase();
    return rows.filter(
      (s) => s.customer.toLowerCase().includes(q) || s.plan.toLowerCase().includes(q),
    );
  }, [rows, query]);
  ...
  async function addSubscription(form) {
    const draft = {
      ...
      nextBillingDate: form.startDate,
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("sales_subscriptions")
          .insert({
            doc_number: draft.id,
            ...
            next_billing_date: draft.startDate,
          })
          .single()
          .run();
        if (header?.id)
        ...
        notify("Subscription created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function setStatus(id, status) {
    ...
    if (IS_CONFIGURED && sub?.dbId) {
      try {
        await sb("sales_subscriptions").eq("id", sub.dbId).update({ status }).run();
      } catch (_e) {
        notify("Couldn't save the subscription status to the server.", "error");
      }
    }
  }
  ...
  async function deleteSubscription(id) {
    ...
    if (IS_CONFIGURED && sub?.dbId) {
      try {
        await sb("sales_subscriptions").eq("id", sub.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the subscription on the server.", "error");
      }
    }
  }
  ...
  async function generateInvoice(sub) {
    const draft = {
      ...
      items: [
        {
          name: `${sub.plan} (${sub.cycle})`,
          qty: 1,
          rate: Math.round(sub.amount / (1 + TAX_RATE)),
        },
      ],
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("sales_invoices")
          .insert({
            doc_number: draft.id,
            customer: draft.customer,
          ...
          .run();
        if (header?.id) {
          await sb("sales_invoice_items")
            .insert([
              {
                invoice_id: header.id,
                item_name: draft.items[0].name,
                qty: 1,
                rate: draft.items[0].rate,
                sort_order: 0,
              },
            ])
            .run();
          invoices.setRows((prev) =>
            prev.map((d) => (d.id === draft.id ? { ...d, dbId: header.id } : d)),
          );
        }
        if (sub.dbId)
          await sb("sales_subscriptions")
            .eq("id", sub.dbId)
            .update({ next_billing_date: nextDate })
            .run();
      } catch (_e) {
        notify("Invoice generated locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${SUBSCRIPTION_STATUS_COLOR[s.status]}14`,
                          color: SUBSCRIPTION_STATUS_COLOR[s.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: SUBSCRIPTION_STATUS_COLOR[s.status] }}
                        />
                        {s.status}
                      </span>
                    </td>
      ...
      {showForm && (
        <SubscriptionFormPanel onClose={() => setShowForm(false)} onSubmit={addSubscription} />
      )}
    </div>
  );
}
...
export function SubscriptionPanel({
  ...
  const isOverdue =
    ...
    subscription.nextBillingDate < TODAY.toISOString().slice(0, 10);
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${SUBSCRIPTION_STATUS_COLOR[subscription.status]}14`,
              color: SUBSCRIPTION_STATUS_COLOR[subscription.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: SUBSCRIPTION_STATUS_COLOR[subscription.status] }}
            />
            {subscription.status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="bg-slate-50 rounded-lg p-3">
            ...
            <p className="text-[15px] font-mono font-semibold text-[#111827]">
              TZS {money(subscription.amount)}k
            </p>
          </div>
          <div className="bg-slate-50 rounded-lg p-3">
        ...
        <div className="bg-slate-50 rounded-lg p-3 mb-6">
          ...
          <p
            className={`text-[15px] font-mono font-semibold ${isOverdue ? "text-[#F59E0B]" : "text-[#111827]"}`}
          >
        ...
        <div className="border-t border-slate-100 pt-4 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            {subscription.status !== "Active" && (
              <button
                onClick={() => onSetStatus(subscription.id, "Active")}
                className="flex-1 text-[12px] font-medium border border-slate-200 rounded-lg py-2 hover:bg-slate-50"
              >
                Resume
              </button>
            )}
            {subscription.status === "Active" && (
              <button
                onClick={() => onSetStatus(subscription.id, "Paused")}
                className="flex-1 text-[12px] font-medium border border-slate-200 rounded-lg py-2 hover:bg-slate-50"
              >
                Pause
              </button>
            ...
            {subscription.status !== "Cancelled" && (
              <button
                onClick={() => onSetStatus(subscription.id, "Cancelled")}
                className="flex-1 text-[12px] font-medium border border-[#EF4444]/25 text-[#EF4444] rounded-lg py-2 hover:bg-[#EF4444]/5"
              >
                Cancel
              </button>
            )}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Plan name" required>
            <input
              ...
              placeholder="e.g. Fleet GPS Monitoring"
            />
            {touched && !form.plan.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Plan name is required.</p>
            )}
          </FormField>
          ...
          <p className="text-[11.5px] text-slate-400">
            The first invoice isn&apos;t generated automatically — open the subscription and click
            Generate Invoice when the cycle is due.
          </p>
        </div>
