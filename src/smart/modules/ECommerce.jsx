  ...
  const ECOM_KPIS = [
    {
      ...
      value: `TZS ${money(stats.revenue)}k`,
      delta: "Last 7 days",
      up: true,
    ...
    },
    {
      ...
      delta: "Excl. cancelled",
      up: true,
      icon: ShoppingCart,
    },
    {
      label: "Avg Order Value",
      value: `TZS ${money(stats.avg)}k`,
      delta: "Per order",
      up: true,
      icon: Percent,
    },
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Your online storefront, priced from live Inventory stock
        </p>
      </div>
  ...
  async function togglePublished(sku) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Couldn't save the publish state to the server.", "error");
      }
    }
  }
            ...
            return (
              ...
              >
                ...
                <div className="p-3.5">
                  ...
                  <p className="text-[13px] font-medium text-[#111827] leading-snug mt-0.5 mb-2 line-clamp-2 min-h-[32px]">
                    {p.name}
                  </p>
                  <div className="flex items-center justify-between mb-2.5">
                    <span className="text-[14px] font-mono font-semibold text-[#111827]">
                      TZS {money(p.price)}k
                    </span>
                    {status && (
                      <span
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.sku} className="border-b border-slate-50 last:border-0">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          {p.featured && (
                            <p className="text-[10.5px] text-[#F59E0B] flex items-center gap-1">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[11px] font-medium px-2 py-1 rounded-full ${p.published ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-slate-100 text-slate-500"}`}
                      >
                        {p.published ? "Live" : "Draft"}
                      </span>
                    </td>
...
export function OnlineOrders({ orders }) {
  ...
  async function advanceOrder(id, next) {
    ...
    if (IS_CONFIGURED && order?.dbId) {
      try {
        await sb("ecommerce_orders").eq("id", order.dbId).update({ status: next }).run();
      } catch (_e) {
        notify("Couldn't save the order status to the server.", "error");
      }
    }
  }
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${ECOM_ORDER_STATUS_COLOR[o.status]}14`,
                          color: ECOM_ORDER_STATUS_COLOR[o.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: ECOM_ORDER_STATUS_COLOR[o.status] }}
                        />
                        {o.status}
                      </span>
                    </td>
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${ECOM_ORDER_STATUS_COLOR[order.status]}14`,
              color: ECOM_ORDER_STATUS_COLOR[order.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: ECOM_ORDER_STATUS_COLOR[order.status] }}
            />
            {order.status}
          </span>
        </div>
