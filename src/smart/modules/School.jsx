...
export function SchoolManagementModule({ currentUser, company }) {
  ...
  const nextAdmNo = () =>
    "ADM-" + new Date().getFullYear() + "-" + String(students.rows.length + 1).padStart(3, "0");
  ...
  async function addStudent() {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("sch_students").insert(row).run();
      } catch (_e) {}
    }
  }
  ...
  const filteredStudents = students.rows.filter(
    (s) =>
      !searchQ || s.name.toLowerCase().includes(searchQ.toLowerCase()) || s.admNo.includes(searchQ),
  );
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,.65)" }}>
              Students · Teachers · Classes · Exams · Fees · Library · Transport
            </p>
          </div>
          <div className="flex gap-3">
            {[
              ["Students", activeStudents, "#DBEAFE", "#1E40AF"],
              [
                "Teachers",
                teachers.rows.filter((t) => t.status === "Active").length,
                "#D1FAE5",
                "#065F46",
              ],
              ["Fee Rate", feeCollection + "%", "#FEF3C7", "#92400E"],
            ].map(([l, v, bg, col]) => (
              <div
                key={l}
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={{ background: tab === t.id ? SCH_BLUE : "transparent" }}
            >
              <I size={12} />
              {t.label}
            </button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              {
                l: "Fee Collected",
                v: "TZS " + money(collectedFees) + "k",
                sub: feeCollection + "% of total",
                c: SCH_GOLD,
                I: CircleDollarSign,
              },
              {
                l: "Outstanding Fees",
                v: "TZS " + money(outstanding) + "k",
                sub: fees.rows.filter((f) => f.status !== "Paid").length + " students",
                c: "#EF4444",
                I: AlertCircle,
              },
            ].map((k) => (
                  ...
                  <div key={level} className="flex items-center gap-2 mb-2.5">
                    ...
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: pct + "%", background: SCH_BLUE }}
                      />
                    </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const feeData = [
                  {
                    name: "Paid",
                    value: fees.rows.filter((f) => f.status === "Paid").length,
                    fill: "#16A34A",
                  },
                  {
                    name: "Partial",
                    value: fees.rows.filter((f) => f.status === "Partial").length,
                    fill: "#D97706",
                  },
                  {
                    name: "Unpaid",
                    value: fees.rows.filter((f) => f.status === "Unpaid").length,
                    fill: "#EF4444",
                  },
                ].filter((d) => d.value > 0);
                if (!feeData.length)
                  return <p className="text-slate-400 text-center py-4">No fee records</p>;
                return (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <p className="text-[11.5px] font-bold text-[#1E3A8A] pt-1">
                        {feeCollection}% collection rate
                      </p>
                    </div>
                  </div>
                );
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {exams.rows.slice(0, 4).map((e) => (
                  ...
                  >
                    <div>
                      <p className="text-[12.5px] font-medium text-[#111827]">{e.subject}</p>
                    ...
                    <div className="text-right">
                      ...
                      {e.avgScore > 0 && (
                        <p className="text-[11px] text-slate-500 mt-0.5">Avg: {e.avgScore}%</p>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {tab === "students" && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() =>
                downloadCSV("students", students.rows, [
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <FormField label="Gender">
                  <select
                    ...
                    onChange={(e) => setStuForm({ ...stuForm, gender: e.target.value })}
                  >
                ...
                <FormField label="Date of Birth">
                  <input
                    ...
                    className={inputClass}
                    value={stuForm.dob}
                    onChange={(e) => setStuForm({ ...stuForm, dob: e.target.value })}
                  />
                </FormField>
                <FormField label="Parent/Guardian">
                  <input
                    className={inputClass}
                    value={stuForm.parent}
                    onChange={(e) => setStuForm({ ...stuForm, parent: e.target.value })}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Adm No", "Student Name", "Gender", "Class", "Parent", "Phone", "Status"].map(
                    (h) => (
                      <th
                        key={h}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ background: s.gender === "F" ? "#DB2777" : SCH_BLUE }}
                        >
                          {s.name.charAt(0)}
                        </div>
            ...
            <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
              <p className="text-[11.5px] text-slate-400">
                Showing {filteredStudents.length} of {students.rows.length} students
              </p>
              <p className="text-[11.5px] font-medium text-slate-600">
                {students.rows.filter((s) => s.gender === "F").length} Female ·{" "}
                {students.rows.filter((s) => s.gender === "M").length} Male
              </p>
            </div>
          </div>
        </div>
      ...
      {tab === "teachers" && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ["Teaching Staff", teachers.rows.length, SCH_BLUE],
              ["Active", teachers.rows.filter((t) => t.status === "Active").length, "#059669"],
              [
                "Avg Experience",
                (
                  teachers.rows.reduce((s, t) => s + t.experience, 0) /
                  Math.max(teachers.rows.length, 1)
                ).toFixed(1) + " yrs",
                SCH_GOLD,
              ],
              [
                "Monthly Payroll",
                "TZS " + money(teachers.rows.reduce((s, t) => s + t.salary, 0)) + "k",
                "#7C3AED",
              ],
            ].map(([l, v, col]) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] font-semibold text-[#111827] truncate">{t.name}</p>
                    <p className="text-[12px] text-slate-400">{t.subject}</p>
                ...
                <div className="grid grid-cols-3 gap-2 border-t border-slate-100 pt-3">
                  <div className="text-center">
                    ...
                    <p className="text-[13px] font-bold text-[#111827]">{t.experience}y</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] text-slate-400">Salary</p>
                    <p className="text-[13px] font-bold" style={{ color: SCH_GOLD }}>
                      TZS {money(t.salary)}k
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] text-slate-400">Qual.</p>
                    <p className="text-[10px] text-slate-600 truncate" title={t.qualification}>
                      {t.qualification}
                    </p>
                  </div>
                </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-[16px] font-bold text-[#111827]">{cl.name}</p>
                  ...
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: pct + "%",
                        background: pct > 90 ? "#EF4444" : pct > 75 ? "#F59E0B" : SCH_BLUE,
                      }}
                    />
                  </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-[15px] font-semibold text-[#111827]">
                  Daily Attendance — {new Date().toDateString()}
                </h3>
                <p className="text-[12px] text-slate-400">Mark attendance for each class</p>
              </div>
              <button
                onClick={() => notify("Attendance saved successfully")}
                ...
                return (
                  <div key={cl.id} className="border border-slate-200 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-[13.5px] font-semibold text-[#111827]">{cl.name}</p>
                      <p className="text-[11.5px] text-slate-400">{cl.students} enrolled</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-green-50 rounded-lg p-2.5 text-center">
                        <p className="text-[10px] text-green-600 mb-0.5">Present</p>
                        <input
                          type="number"
                          className="w-full text-center text-[18px] font-bold text-green-700 bg-transparent border-none outline-none"
                          defaultValue={cl.students}
                          min="0"
                          max={cl.students}
                        />
                      </div>
                      <div className="bg-red-50 rounded-lg p-2.5 text-center">
                        <p className="text-[10px] text-red-500 mb-0.5">Absent</p>
                        <p className="text-[18px] font-bold text-red-600">0</p>
                      </div>
                    </div>
      ...
      {tab === "exams" && (
        <div className="space-y-3">
          <div className="flex justify-end">
            <button
              onClick={() => notify("Exam form — add via modal")}
              className="flex items-center gap-1.5 text-[12.5px] font-semibold text-white px-4 py-2.5 rounded-xl"
              style={{ background: SCH_BLUE }}
            >
              <Plus size={13} />
                ...
                {exams.rows.map((e) => (
                  ...
                  >
                    ...
                    <td
                      ...
                      style={{
                        color:
                          e.avgScore >= 70 ? "#16A34A" : e.avgScore >= 50 ? "#D97706" : "#EF4444",
                      }}
                    >
                      {e.avgScore > 0 ? e.avgScore + "%" : "—"}
                    </td>
                    <td
                      className="px-4 py-3 font-bold"
                      style={{
                        color:
                          e.passRate >= 80 ? "#16A34A" : e.passRate >= 60 ? "#D97706" : "#EF4444",
                      }}
                    >
                      {e.passRate > 0 ? e.passRate + "%" : "—"}
                    </td>
                    <td className="px-4 py-3">
                      <StatusChip s={e.status} />
                    </td>
      ...
      {tab === "fees" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Term Revenue", "TZS " + money(totalFees) + "k", SCH_BLUE],
              ["Collected", "TZS " + money(collectedFees) + "k", "#16A34A"],
                ...
                onClick={() =>
                  downloadCSV(
                    ...
                    [
                      ...
                      { key: "Class", label: "Class" },
                      { key: "Term", label: "Term" },
                      { key: "Amount_k", label: "Billed" },
                      { key: "Paid_k", label: "Paid" },
                      { key: "Balance_k", label: "Balance" },
                      { key: "Status", label: "Status" },
                    ],
                  )
                }
          ...
          {showFee && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <FormField label="Fee Amount (TZS k)">
                  <input
                    type="number"
                    className={inputClass}
                ...
                <FormField label="Amount Paid (TZS k)">
                  <input
                    type="number"
                    className={inputClass}
                    value={feeForm.paid}
                    onChange={(e) => setFeeForm({ ...feeForm, paid: e.target.value })}
                  />
                </FormField>
              </div>
              <div className="flex gap-2">
                ...
                {fees.rows.map((f) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <span className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">
                        {f.class}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-slate-500 text-[11.5px]">{f.term}</td>
                    <td className="px-3 py-3 font-mono">{money(f.amount)}k</td>
                    <td className="px-3 py-3 font-mono text-green-600 font-semibold">
                      {money(f.paid)}k
                    </td>
                    <td
                      className="px-3 py-3 font-mono font-bold"
                      style={{ color: f.balance > 0 ? "#EF4444" : "#16A34A" }}
                    >
                      {money(f.balance)}k
                    </td>
                        ...
                        <button
                          onClick={() => {
                            fees.setRows((p) =>
                              p.map((x) =>
                                x.id === f.id
                                  ? { ...x, paid: x.amount, balance: 0, status: "Paid" }
                                  : x,
                              ),
                            );
                            notify("Full payment recorded for " + f.student);
                          }}
                          className="text-[11px] font-semibold text-white px-2.5 py-1 rounded-lg bg-green-600"
                        >
                          Pay Full
                        </button>
                      )}
      ...
      {tab === "library" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Total Books", books.rows.reduce((s, b) => s + b.copies, 0), SCH_BLUE],
              ["Available", books.rows.reduce((s, b) => s + b.available, 0), "#16A34A"],
              ["On Loan", books.rows.reduce((s, b) => s + (b.copies - b.available), 0), SCH_GOLD],
            ].map(([l, v, col]) => (
              <div
                key={l}
                className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <p className="text-[13.5px] font-semibold text-[#111827]">Book Catalog</p>
              <button
                onClick={() => notify("Add book modal — coming next")}
              ...
              <tbody>
                {books.rows.map((b) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">
                        {b.category}
                      </span>
                    </td>
                    ...
                    <td
                      className="px-4 py-3 font-bold"
                      style={{ color: b.available === 0 ? "#EF4444" : "#16A34A" }}
                    >
                      {b.available}
                    </td>
                    <td className="px-4 py-3">
                      <StatusChip
                        s={b.available === 0 ? "Unpaid" : b.available < 3 ? "Partial" : "Active"}
                      />
                    </td>
                  </tr>
                ))}
      ...
      {tab === "transport" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Routes", transport.rows.length, SCH_BLUE],
              ["Students", transport.rows.reduce((s, r) => s + r.students, 0), "#16A34A"],
              [
                "Active Buses",
                transport.rows.filter((r) => r.status === "Active").length,
                SCH_GOLD,
              ],
            ].map(([l, v, col]) => (
              <div
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ background: SCH_BLUE + "18" }}
                    >
                      <Bus size={18} style={{ color: SCH_BLUE }} />
                    </div>
                    <div>
                      <p className="text-[13.5px] font-semibold text-[#111827]">{r.route}</p>
                      <p className="text-[11.5px] text-slate-400">{r.bus}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={l} className="flex justify-between">
                      <span className="text-[11.5px] text-slate-400">{l}</span>
                      <span className="text-[11.5px] font-medium text-[#111827]">{v}</span>
                    </div>
                  ))}
                </div>
              </div>
...
export const PHM_DISPENSE_SEED = [
  ...
  {
    id: "DIS-002",
    patient: "Noura Al Dossari",
    drug: "Amlodipine 5mg",
    qty: 30,
    dosage: "1 OD × 30 days",
    price: 27.0,
    prescriber: "Dr. Layla Al Zahrani",
    date: "2026-07-17",
    status: "Dispensed",
    rxNo: "RX-002",
  },
...
export const PHM_SUPPLIERS_SEED = [
  {
    id: "SUP-001",
    name: "Shelys Pharma Ltd",
    contact: "Charles Mwenda",
    phone: "022-123-4567",
    email: "orders@shelys.co.tz",
    terms: "Net 30",
    status: "Active",
  ...
  },
  {
    ...
    contact: "Miriam Wanjiku",
    phone: "022-234-5678",
    email: "supply@betahealthcare.co.tz",
    terms: "Net 21",
    status: "Active",
    lastOrder: "2024-07-01",
  },
  {
    id: "SUP-003",
    name: "Zenufa Laboratories",
    contact: "Hassan Salim",
    phone: "022-345-6789",
    email: "info@zenufa.co.tz",
    terms: "Net 45",
    status: "Active",
  ...
  {
    id: "SUP-004",
    name: "CiplaQCIL Tanzania",
    contact: "Grace Otieno",
    phone: "022-456-7890",
    email: "tz@ciplaQCIL.com",
    terms: "Net 30",
    status: "Active",
  ...
  {
    id: "SUP-005",
    name: "Novo Nordisk EA",
    contact: "Peter Kamau",
    phone: "022-567-8901",
    email: "orders@novonordisk.co.tz",
    terms: "Net 60",
    status: "Active",
    lastOrder: "2024-07-01",
  },
