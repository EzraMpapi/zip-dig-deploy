...
export function SupplyChain() {
  ...
  const shipments = useCompanyTable("scm_shipments", shipmentsSeed, {
    order: { col: "dispatch_date", ascending: false },
    mapRow: mapShipmentRow,
  });
  const vehicles = useCompanyTable("scm_vehicles", vehiclesSeed, {
    order: { col: "reg", ascending: true },
  ...
  const SCM_KPIS = [
    ...
    },
    {
      ...
      delta: "Vehicles ready",
      up: stats.availableVehicles > 0,
      icon: ClipboardList,
    },
  ];
  ...
  async function setVehicleStatus(reg, status) {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("scm_vehicles").eq("reg", reg).update({ status }).run();
      } catch (_e) {
        notify("Vehicle status saved locally, but the server update failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Deliveries and the fleet that carries them
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {(() => {
              ...
              const onTimeRate =
                shipments.rows.length > 0 ? Math.round((onTime / shipments.rows.length) * 100) : 0;
              return (
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="pt-2 border-t border-slate-100">
                      <p className="text-[11.5px] text-slate-500">
                        Delivery rate: <strong style={{ color: "#16A34A" }}>{onTimeRate}%</strong>
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {tab === "shipments" && (
        <Shipments shipments={shipments} vehicles={vehicles} onVehicleStatus={setVehicleStatus} />
      )}
...
export function Shipments({ shipments, vehicles, onVehicleStatus }) {
  ...
  async function addShipment(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("scm_shipments")
          .insert({
            order_ref: draft.orderRef,
            customer: draft.customer,
            destination: draft.destination,
            vehicle_reg: draft.vehicle,
        ...
        notify("Shipment created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function advanceShipment(id, next) {
    ...
    if (next === "Delivered")
      notify(id + " delivered" + (shp?.vehicle ? " — " + shp.vehicle + " is available again" : ""));

    if (IS_CONFIGURED && shp?.dbId) {
      try {
        await sb("scm_shipments").eq("id", shp.dbId).update({ status: next }).run();
      } catch (_e) {
        notify("Couldn't save the shipment status to the server.", "error");
      }
    }
  }
  ...
  async function deleteShipment(id) {
    ...
    if (IS_CONFIGURED && shp?.dbId) {
      try {
        await sb("scm_shipments").eq("id", shp.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the shipment on the server.", "error");
      }
    }
  }
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${SHIPMENT_STATUS_COLOR[s.status]}14`,
                          color: SHIPMENT_STATUS_COLOR[s.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: SHIPMENT_STATUS_COLOR[s.status] }}
                        />
                        {s.status}
                      </span>
                    </td>
      ...
      >
        ...
        <div className="mb-6">
          <span
            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
            style={{
              backgroundColor: `${SHIPMENT_STATUS_COLOR[shipment.status]}14`,
              color: SHIPMENT_STATUS_COLOR[shipment.status],
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: SHIPMENT_STATUS_COLOR[shipment.status] }}
            />
            {shipment.status}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2.5 text-[13px] text-slate-600 mb-6">
          ...
          {shipment.vehicle ? `Assigned to ${shipment.vehicle}` : "No vehicle assigned yet"}
        </div>
...
export function ShipmentFormPanel({ vehicles, onClose, onSubmit }) {
  const [form, setForm] = useState({
    ...
    destination: "",
    vehicle: "",
    dispatchDate: TODAY.toISOString().slice(0, 10),
    expectedDate: "",
  });
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Destination" required>
            <input
              ...
              placeholder="e.g. Dar es Salaam — Kurasini"
            />
            {touched && !form.destination.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Destination is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Expected delivery" required>
              ...
              />
              {touched && !form.expectedDate && (
                <p className="text-[11px] text-[#EF4444] mt-1">Expected date is required.</p>
              )}
            </FormField>
          </div>

          <p className="text-[11.5px] text-slate-400">
            Only vehicles currently marked{" "}
            <span className="font-medium text-slate-500">Available</span> can be assigned.
            Dispatching moves the vehicle On Route.
          </p>
        </div>
              ...
              <tr>
                <td colSpan={5}>
                  <EmptyState
                    ...
                    hint="Your delivery fleet will appear here once vehicles are added."
                  />
                </td>
              </tr>
            )}
                ...
                <tr key={v.reg} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                      style={{
                        backgroundColor: `${VEHICLE_STATUS_COLOR[v.status]}14`,
                        color: VEHICLE_STATUS_COLOR[v.status],
                      }}
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ backgroundColor: VEHICLE_STATUS_COLOR[v.status] }}
                      />
                      {v.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex gap-1.5 justify-end">
                      {v.status !== "Available" && (
                        <button
                          onClick={() => setStatus(v.reg, "Available")}
                          className="text-[11.5px] font-medium border border-slate-200 rounded-md px-2.5 py-1 hover:bg-slate-50 text-slate-600"
                        >
                          Set Available
                        </button>
                      )}
                      {v.status !== "Maintenance" && (
                        <button
                          onClick={() => setStatus(v.reg, "Maintenance")}
                          className="text-[11.5px] font-medium border border-[#F59E0B]/30 text-[#8a670a] rounded-md px-2.5 py-1 hover:bg-[#F59E0B]/5"
                        >
                          Maintenance
                        </button>
                      )}
