...
export function CustomerPortal({ currentUser, invoices, filesHook, onSignOut }) {
  ...
  const connections = useCompanyTable(
  ...
  const effectiveCustomer =
    currentUser.customerRef || invoices.rows[0]?.customer || "Demo Customer";
  ...
  const myDocuments = filesHook.rows.filter(
    (f) =>
      myInvoices.some((i) => i.id === f.linkedRecord) ||
      myOrders.some((o) => o.id === f.linkedRecord),
  );
    ...
    >
      <header className="bg-white border-b border-slate-200/80 px-4 sm:px-6 py-3.5 flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[13.5px] font-semibold text-[#111827] leading-tight">
              Customer Portal
            </p>
        ...
        <ProfileMenu
          ...
          session={{ demo: !currentUser.customerRef }}
          onSignOut={onSignOut}
        />
      </header>
      ...
      {!currentUser.customerRef && (
        <div className="bg-[#F59E0B]/10 border-b border-[#F59E0B]/20 px-4 sm:px-6 py-2">
          <p className="text-[11.5px] text-[#8a670a]">
            Demo mode — showing sample data for "{effectiveCustomer}" since there's no real
            signed-in customer identity yet. In live mode, this portal shows exactly one customer's
            own records, enforced by the database itself, not by this screen.
          </p>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-5">
          ...
          {PORTAL_TABS.map((t) => {
            ...
            return (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`text-[12px] font-medium px-3 py-1.5 rounded-md flex items-center gap-1.5 whitespace-nowrap transition-colors ${isActive ? "bg-[#16A34A] text-white" : "text-slate-500 hover:text-slate-700"}`}
              >
        ...
        {tab === "invoices" && (
          <CustomerInvoicesTab myInvoices={myInvoices} stripe={stripe} paypal={paypal} />
        )}
...
export function PortalFeedbackWidget({ currentUser }) {
  ...
  async function submit() {
    ...
    if (IS_CONFIGURED) {
      try {
        await sb("customer_feedback")
          .insert({
            customer_name: currentUser.customerRef || currentUser.name,
            nps_score: score,
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 mb-4">
      <p className="text-[12.5px] font-medium text-[#111827]">
        ...
        <span className="text-slate-400 font-normal">(0 = not at all, 10 = extremely)</span>
      </p>
      <div className="flex flex-wrap gap-1.5 mt-2">
        {Array.from({ length: 11 }, (_, i) => (
          <button
            key={i}
            onClick={() => setScore(i)}
            className={`w-8 h-8 rounded-lg text-[12px] font-mono font-medium border ${score === i ? "bg-[#16A34A] text-white border-[#16A34A]" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
          >
            {i}
          </button>
        ))}
...
export function CustomerInvoicesTab({ myInvoices, stripe, paypal }) {
  ...
  function downloadReceipt(inv) {
    ...
    const items = inv.items
      .map(
        (it) =>
          `<tr><td>${it.name}</td><td class="right">${it.qty}</td><td class="right">${money(it.rate)}k</td><td class="right">${money(Math.round(it.qty * it.rate))}k</td></tr>`,
      )
      .join("");
    const pays = (inv.payments || [])
      .map(
        (p) =>
          `<tr><td>${p.date}</td><td>${p.method || "—"}</td><td class="right">${money(Math.round(p.amount))}k</td></tr>`,
      )
      .join("");
    printAsPDF(
      `Receipt ${inv.id}`,
  ...
  const canPayOnline =
    (stripe?.enabled && stripe?.paymentLink) || (paypal?.enabled && paypal?.paypalMeLink);
  ...
  function payOnline(invoiceId) {
    ...
    notify(
      `Opening the payment page — reference invoice ${invoiceId} so ${stripe?.enabled ? "the business" : "PayPal"} can match your payment.`,
    );
      ...
      {!canPayOnline && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-3.5">
          <p className="text-[11.5px] text-slate-400">
            Online payment isn&apos;t configured yet — the business needs to connect Stripe or
            PayPal in their own Integrations settings first.
          </p>
        </div>
      )}
      {myInvoices.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
          <EmptyState
            ...
            hint="Invoices billed to you will appear here."
          />
        </div>
      )}
      {myInvoices.map((inv) => {
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="text-[13.5px] font-semibold text-[#111827]">{inv.id}</p>
                <p className="text-[11px] text-slate-400">Due {inv.dueDate}</p>
              </div>
              <span
                className={`text-[10.5px] font-medium px-2 py-0.5 rounded-full ${inv.status === "Paid" ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-[#EF4444]/10 text-[#EF4444]"}`}
              >
                {inv.status}
              </span>
              <button
                onClick={() => downloadReceipt(inv)}
                className="text-[11px] font-medium text-[#16A34A] hover:underline flex items-center gap-1 ml-2"
              >
                <Download size={11} /> Receipt
              </button>
            </div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-[20px] font-mono font-bold text-[#111827]">
                TZS {money(Math.round(total))}k
              </p>
              {balance > 0 && (
                <p className="text-[11.5px] text-[#EF4444]">
                  TZS {money(Math.round(balance))}k due
                </p>
              )}
            </div>
            ...
            {payingId === inv.id && (
              <p className="text-[11px] text-slate-400 mt-2 text-center">
                Paid already? It can take a moment to reflect here — check back shortly.
              </p>
            )}
          </div>
        );
                ...
                {ORDER_STEPS.map((step, i) => (
                  <React.Fragment key={step}>
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${i <= stepIndex ? "bg-[#16A34A] text-white" : "bg-slate-100 text-slate-400"}`}
                      >
                        {i <= stepIndex ? <CheckCircle2 size={13} /> : i + 1}
                      </div>
                      <span
                        className={`text-[10px] mt-1 ${i <= stepIndex ? "text-[#111827] font-medium" : "text-slate-400"}`}
                      >
                        {step}
                      </span>
                    </div>
                    {i < ORDER_STEPS.length - 1 && (
                      <div
                        className={`flex-1 h-0.5 mx-1 ${i < stepIndex ? "bg-[#16A34A]" : "bg-slate-100"}`}
                      />
                    )}
                  </React.Fragment>
                ))}
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
      {myDocuments.length === 0 ? (
        <EmptyState
          ...
          hint="Contracts and paperwork tied to your invoices or orders will appear here."
        />
      ) : (
  ...
  async function submitTicket(e) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Ticket saved locally, but sending to the server failed.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className="btn-primary text-white text-[12.5px] font-medium rounded-lg py-2.5 px-4 flex items-center gap-2 w-full sm:w-auto"
      >
      ...
      {showForm && (
        <form
          ...
          className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 space-y-3"
        >
          ...
          <FormField label="Category">
            ...
            >
              {["Billing", "Technical", "Product", "General"].map((c) => (
        ...
        {myTickets.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Tickets you open will show up here."
            />
          </div>
        )}
        {myTickets.map((t) => (
          <div
            key={t.id}
            className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 flex items-center justify-between"
          >
            <div className="min-w-0">
              <p className="text-[12.5px] font-medium text-[#111827] truncate">{t.subject}</p>
              <p className="text-[11px] text-slate-400">
                {t.id} · {t.category} · {t.createdDate}
              </p>
            </div>
            <span
              className={`text-[10.5px] font-medium px-2 py-0.5 rounded-full shrink-0 ml-2 ${t.status === "Resolved" || t.status === "Closed" ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
            >
              {t.status}
            </span>
          </div>
    ...
    try {
      const snapshot = {
        ...
        invoices: myInvoices.map((i) => ({
          ...
          status: i.status,
          due_date: i.dueDate,
          total_tzs_k: Math.round(lineTotal(i.items).total),
          balance_tzs_k: Math.round(lineTotal(i.items).total - (i.amountPaid || 0)),
        })),
      ...
      const text =
        data.content?.find((c) => c.type === "text")?.text || "Sorry, I couldn't process that.";
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm flex flex-col h-[480px]">
      <div className="px-4 py-3 border-b border-slate-100">
        ...
        <p className="text-[11px] text-slate-400">
          Scoped to your own invoices and orders only — nothing else about this business.
        </p>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <p className="text-[12.5px] text-slate-400 text-center py-8">
            Ask about your invoices or order status.
          </p>
        )}
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[80%] rounded-xl px-3.5 py-2.5 text-[13px] ${m.role === "user" ? "btn-primary text-white" : "bg-slate-50 text-slate-700"}`}
            >
              {m.content}
            </div>
          </div>
        ...
        {busy && (
          <div className="flex justify-start">
            <LoaderCircle size={16} className="animate-spin text-[#16A34A]" />
          </div>
        )}
      </div>
      <div className="p-3 border-t border-slate-100 flex gap-2">
        <input
          ...
          placeholder="Ask about your invoices or orders..."
          className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#16A34A]"
        />
        <button
          onClick={send}
