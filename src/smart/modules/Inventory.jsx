...
export function InventoryDashboard({ inventory, suppliersHook }) {
  ...
  const byCat = useMemo(() => {
    ...
    rows.forEach((it) => {
      ...
      map[c] = map[c] || { value: 0, count: 0 };
      map[c].value += it.qty * it.unitCost;
      map[c].count += 1;
    });
  ...
  const stockHealth = [
    {
      name: "Healthy",
      value: rows.filter((it) => it.qty > (it.reorderLevel || 5)).length,
      fill: "#16A34A",
    },
          ...
          <div className="flex-1">
            ...
            <p className="text-[11.5px] text-[#B45309] mt-0.5">
              {[...outOfStock, ...lowStock]
                ...
                .map((it) => it.name)
                .join(", ")}
              {[...outOfStock, ...lowStock].length > 4
                ? " +" + ([...outOfStock, ...lowStock].length - 4) + " more"
                : ""}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { l: "Low Stock", v: lowStock.length, c: lowStock.length > 0 ? "#F59E0B" : "#16A34A" },
          {
            l: "Out of Stock",
            v: outOfStock.length,
            c: outOfStock.length > 0 ? "#EF4444" : "#16A34A",
          },
        ].map((k) => (
                ...
                <div key={it.id || i} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      ...
                      <span className="text-[11.5px] font-mono font-bold text-[#16A34A] ml-2 shrink-0">
                        TZS {money(Math.round(val))}k
                      </span>
                    </div>
...
export function Inventory({ inventory, suppliersHook }) {
  ...
  const warehousesHook = useCompanyTable("inventory_warehouses", WAREHOUSES, {
  ...
  const INV_KPIS = [
    {
      label: "Stock Value",
      value: `TZS ${money(Math.round(stats.totalValue))}k`,
  ...
  async function adjustStock(sku, delta) {
    setItems((prev) =>
      prev.map((it) => (it.sku === sku ? { ...it, qty: Math.max(0, it.qty + delta) } : it)),
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Stock, warehouses, transfers, batches, and suppliers in one place
          </p>
        </div>
            ...
            onClick={() =>
              downloadCSV(
                ...
                inventory.rows.map((it) => ({
                  ...
                  Value_k: Math.round(((it.qty || 0) * (it.unitCost || 0)) / 1000),
                  ReorderPoint: it.reorder || 0,
                  Status:
                    it.qty <= 0 ? "Out of Stock" : it.qty <= (it.reorder || 0) ? "Low Stock" : "OK",
                })),
                [
                  { key: "SKU", label: "SKU" },
                  { key: "Name", label: "Name" },
      ...
      {tab === "reorder" && (
        <ReorderAlertsView inventory={inventory} suppliersHook={suppliersHook} />
      )}

      {tab === "dashboard" && (
        <InventoryDashboard inventory={inventory} suppliersHook={suppliersHook} />
      )}
      {tab === "stock" && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {INV_KPIS.map((k) => {
              ...
              return (
                <div
                  key={k.label}
                  className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-3"
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-[16px] sm:text-[18px] font-semibold text-[#111827] font-mono truncate">
                      {k.value}
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right font-mono">
                              {it.qty} <span className="text-slate-400">{it.unit}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span
                                className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                                style={{
                                  backgroundColor: `${STOCK_STATUS_COLOR[status]}14`,
                                  color: STOCK_STATUS_COLOR[status],
                                }}
                              >
                                <span
                                  className="w-1.5 h-1.5 rounded-full"
                                  style={{ backgroundColor: STOCK_STATUS_COLOR[status] }}
                                />
                                {status}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              {expiry && expiry !== "Fresh" ? (
                                <span
                                  className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                                  style={{
                                    backgroundColor: `${EXPIRY_STATUS_COLOR[expiry]}14`,
                                    color: EXPIRY_STATUS_COLOR[expiry],
                                  }}
                                >
                                  <span
                                    className="w-1.5 h-1.5 rounded-full"
                                    style={{ backgroundColor: EXPIRY_STATUS_COLOR[expiry] }}
                                  />
                                  {expiry}
                                </span>
                              ) : (
                            ...
                            <td className="px-4 py-3 text-right font-mono">
                              {money(Math.round(it.qty * it.unitCost))}
                            </td>
              ...
              (() => {
                ...
                return (
                  <div className="flex items-center gap-2.5">
                    ...
                    <span style={{ color: EXPIRY_STATUS_COLOR[exp] }} className="font-medium">
                      {exp}
                    </span>
              ...
              <div className="flex items-end gap-[1.5px] h-10 mb-1.5">
                {item.barcode.split("").map((digit, i) => (
                  <div
                    ...
                    className="bg-[#111827]"
                    style={{
                      width: parseInt(digit, 10) % 2 === 0 ? "2px" : "1px",
                      height: `${60 + parseInt(digit, 10) * 4}%`,
                    }}
                  />
                ))}
              </div>
          ...
          {adjusting && (
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 mb-6">
              <p className="text-[11px] font-medium text-slate-500 mb-2">
                Adjust quantity (use a negative number to remove stock)
              </p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="text-[12px] font-medium btn-primary text-white rounded-lg px-4 shrink-0"
                >
                  Apply
                </button>
              </div>
            ...
            {movements.length > 0 ? (
              <div className="border border-slate-100 rounded-lg overflow-hidden">
                {movements.map((mv, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between px-3 py-2.5 text-[13px] ${i !== movements.length - 1 ? "border-b border-slate-50" : ""}`}
                  >
                    <div>
                    ...
                    <span
                      className={`font-mono text-[13px] font-medium ${mv.type === "In" ? "text-[#16A34A]" : "text-[#EF4444]"}`}
                    >
                      {mv.type === "In" ? "+" : "−"}
                      {mv.qty}
                    </span>
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Item name" required>
            <input
              ...
              placeholder="e.g. Industrial water heater 50L"
            />
            {touched && !form.name.trim() && (
              <p className="text-[11px] text-[#EF4444] mt-1">Item name is required.</p>
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Category">
              <input
                ...
                placeholder="e.g. Hardware & Fixtures"
              />
            </FormField>
          </div>
          ...
          <FormField label="Expiry date">
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Only for items with a real shelf life — leave blank otherwise.
            </p>
          </FormField>
        </div>
...
export function Warehouses({ inventory }) {
  ...
  async function addWarehouse(form) {
    const draft = {
      id: `WH-${form.name.slice(0, 3).toUpperCase()}${Math.floor(Math.random() * 90)}`,
      name: form.name,
      city: form.city,
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("inventory_warehouses")
          .insert({ name: draft.name, city: draft.city })
          .single()
          .run();
        if (header?.id)
          setWarehouses((prev) =>
            prev.map((w) => (w.id === draft.id ? { ...w, dbId: header.id } : w)),
          );
      } catch (_e) {
        notify("Warehouse added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteWarehouse(id) {
    ...
    if (inUse) {
      notify(
        "Can't remove a warehouse that still holds stock — transfer or clear its items first.",
        "error",
      );
      return;
    }
    ...
    if (IS_CONFIGURED && w?.dbId) {
      try {
        await sb("inventory_warehouses").eq("id", w.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the warehouse on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
        ...
        {!loading &&
          stats.map((w) => (
            <button
              ...
              className="text-left bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 hover:border-[#16A34A]/50 hover:shadow-md transition-all"
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-[10.5px] text-slate-400 mb-0.5">SKUs</p>
                  <p className="text-[15px] font-mono font-semibold text-[#111827]">{w.skuCount}</p>
                </div>
                <div>
                  <p className="text-[10.5px] text-slate-400 mb-0.5">Stock Value</p>
                  <p className="text-[15px] font-mono font-semibold text-[#111827]">
                    {money(w.value)}k
                  </p>
                </div>
        ...
        {!loading && warehouses.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Add a location to start assigning stock to it."
      ...
      {selected && (
        <div className="fixed inset-0 z-30 flex justify-end">
          ...
          />
          <div
            className="relative w-full sm:w-[360px] bg-white h-full shadow-2xl p-6 overflow-y-auto flex flex-col"
            style={{ animation: "slideIn .15s ease-out" }}
          >
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-[17px] font-semibold text-[#111827]">{selected.name}</h2>
                <p className="text-[13px] text-slate-500">{selected.city}</p>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="text-slate-400 hover:text-slate-600"
            ...
            <ConfirmDeleteButton
              ...
              onConfirm={() => deleteWarehouse(selected.id)}
            />
          </div>
        </div>
      )}
      {showForm && (
        <WarehouseFormPanel onClose={() => setShowForm(false)} onSubmit={addWarehouse} />
      )}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Inventory</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Warehouse</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Name" required>
            <input
              ...
              value={form.name}
              onChange={(e) => set("name", e.target.value)}
              placeholder="e.g. Dodoma — Regional"
            />
          </FormField>
...
export function Transfers({ inventory }) {
  const transfers = useCompanyTable("inventory_transfers", transfersSeed, {
    order: { col: "created_at", ascending: false },
    mapRow: mapTransferRow,
  });
  ...
  async function addTransfer(form) {
    ...
    setShowForm(false);
    notify(
      `Transfer created: ${draft.itemName} → ${warehouses.find((w) => w.id === form.toWarehouse)?.city}`,
    );
    if (IS_CONFIGURED) {
      try {
        await sb("inventory_transfers")
          .insert({
            item_sku: draft.sku,
        ...
        notify("Transfer created locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function advance(id) {
    ...
    if (next === "Completed") {
      inventory.setRows((prev) =>
        prev.map((it) => (it.sku === t.sku ? { ...it, warehouse: t.toWarehouse } : it)),
      );
      ...
      if (IS_CONFIGURED) {
        try {
          await sb("inventory_items")
            .eq("sku", t.sku)
            .update({ warehouse_id: t.toWarehouse })
            .run();
        } catch (_e) {
          notify("Relocated locally, but the server update failed.", "error");
        }
      }
    }
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Couldn't save the transfer status to the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Each SKU is tracked in one warehouse at a time, so a transfer relocates its entire current
          stock. Splitting stock across two warehouses simultaneously isn&apos;t supported yet.
        </p>
      </div>
                ...
                rows.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-slate-50 last:border-0">
                      <td className="px-4 py-3">
                        <p className="font-medium text-[#111827]">{t.itemName}</p>
                        <p className="text-[11px] text-slate-400 font-mono">{t.sku}</p>
                      </td>
                      <td className="px-4 py-3 text-right font-mono">{t.qty}</td>
                      <td className="px-4 py-3 text-slate-500">
                        {warehouses.find((w) => w.id === t.fromWarehouse)?.city || t.fromWarehouse}
                      </td>
                      <td className="px-4 py-3 text-slate-500">
                        {warehouses.find((w) => w.id === t.toWarehouse)?.city || t.toWarehouse}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                          style={{
                            backgroundColor: `${TRANSFER_STATUS_COLOR[t.status]}14`,
                            color: TRANSFER_STATUS_COLOR[t.status],
                          }}
                        >
                          <span
                            className="w-1.5 h-1.5 rounded-full"
                            style={{ backgroundColor: TRANSFER_STATUS_COLOR[t.status] }}
                          />
                          {t.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        {next && (
                          <button
                            ...
                            className="text-[11.5px] font-medium text-[#16A34A] hover:text-[#15803D]"
                          >
                            Mark {next}
                          </button>
                        )}
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      hint="Relocate a SKU's stock between warehouses here."
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Inventory</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Transfer</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          {item && (
            <p className="text-[11.5px] text-slate-400">
              ...
              <span className="font-medium text-slate-600">
                {warehouses.find((w) => w.id === item.warehouse)?.city}
              </span>{" "}
              · {item.qty} {item.unit} will move in full
            </p>
          )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          ...
          <button
            type="submit"
            disabled={!valid}
            className="flex-1 text-[12px] font-medium btn-primary text-white rounded-lg py-2.5 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Create Transfer
          </button>
        </div>
...
export function Batches({ inventory }) {
  const batches = useCompanyTable("inventory_batches", batchesSeed, {
  ...
  async function addBatch(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Batch recorded locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          A traceability ledger for recalls and shelf-life tracking — informational, not the
          authoritative stock count. The aggregate quantity on each item (used by POS, Sales, and
          Manufacturing) doesn&apos;t derive from these rows.
        </p>
      </div>
                ...
                rows.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="border-b border-slate-50 last:border-0">
                      <td className="px-4 py-3">
                        <p className="font-medium text-[#111827]">{b.itemName}</p>
                        <p className="text-[11px] text-slate-400 font-mono">{b.sku}</p>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {exp ? (
                          <span
                            className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                            style={{
                              backgroundColor: `${EXPIRY_STATUS_COLOR[exp]}14`,
                              color: EXPIRY_STATUS_COLOR[exp],
                            }}
                          >
                            <span
                              className="w-1.5 h-1.5 rounded-full"
                              style={{ backgroundColor: EXPIRY_STATUS_COLOR[exp] }}
                            />
                            {b.expiryDate}
                          </span>
                        ) : (
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      hint="Record batch or lot numbers as deliveries arrive for traceability."
                      actionLabel="Log Batch"
                      onAction={() => setShowForm(true)}
                    />
                  </td>
                </tr>
...
export function BatchFormPanel({ inventory, onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.batchNumber.trim() || !(Number(form.qty) > 0)) return;
    onSubmit(form);
  }
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Inventory</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Log Batch</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Supplier">
            <input
              ...
              placeholder="e.g. Tanzania Portland Cement Co."
            />
          </FormField>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
...
export function Suppliers({ suppliersHook }) {
  ...
  async function addSupplier(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("inventory_suppliers")
          .insert({
            name: draft.name,
          ...
          .run();
        if (header?.id)
          setRows((prev) => prev.map((s) => (s.id === draft.id ? { ...s, dbId: header.id } : s)));
      } catch (_e) {
        notify("Supplier added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function toggleStatus(id) {
    ...
    if (IS_CONFIGURED && s?.dbId) {
      try {
        await sb("inventory_suppliers").eq("id", s.dbId).update({ status: next }).run();
      } catch (_e) {
        notify("Couldn't save supplier status to the server.", "error");
      }
    }
  }
  ...
  async function deleteSupplier(id) {
    ...
    if (IS_CONFIGURED && s?.dbId) {
      try {
        await sb("inventory_suppliers").eq("id", s.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the supplier on the server.", "error");
      }
    }
  }
                ...
                rows.map((s) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[11px] text-slate-400">{s.contactPerson}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-medium px-2 py-1 rounded-full inline-flex items-center gap-1.5"
                        style={{
                          backgroundColor: `${SUPPLIER_STATUS_COLOR[s.status]}14`,
                          color: SUPPLIER_STATUS_COLOR[s.status],
                        }}
                      >
                        <span
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: SUPPLIER_STATUS_COLOR[s.status] }}
                        />
                        {s.status}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={5}>
                    <EmptyState
                      ...
                      hint="Track vendors, contacts, and lead times for reordering."
        ...
        <div className="fixed inset-0 z-30 flex justify-end">
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-[17px] font-semibold text-[#111827]">{selected.name}</h2>
                <p className="text-[13px] text-slate-500">{selected.category}</p>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="text-slate-400 hover:text-slate-600"
            ...
            <div className="space-y-3 mb-6 text-[13px]">
              <div className="flex items-center gap-2.5 text-slate-600">
                <Users size={14} className="text-slate-400" /> {selected.contactPerson}
              </div>
              <div className="flex items-center gap-2.5 text-slate-600">
              ...
              <div className="flex items-center gap-2.5 text-slate-600">
                <Clock size={14} className="text-slate-400" /> {selected.leadTimeDays} day lead time
              </div>
            </div>
            <div className="flex-1" />
            <div className="flex flex-col gap-2">
              <button
                ...
                className="text-[12px] font-medium border border-slate-200 rounded-lg py-2 hover:bg-slate-50"
              >
                {selected.status === "Active" ? "Mark Inactive" : "Mark Active"}
              </button>
              <ConfirmDeleteButton
                label="Delete supplier"
                onConfirm={() => deleteSupplier(selected.id)}
              />
            </div>
          </div>
        </div>
...
export function InventoryAnalysisView({ inventory }) {
  ...
  const abc = useMemo(() => {
    const valued = inventory.rows
      .map((it) => ({ ...it, value: it.qty * it.unitCost }))
  ...
  const unassigned = inventory.rows.filter(
    (it) => !warehousesHook.rows.some((w) => w.id === it.warehouse),
  ).length;
  const CLS_META = {
    A: {
      color: "#16A34A",
      note: "Count often, protect hard — ~80% of your stock value lives here.",
    },
    B: { color: "#F59E0B", note: "Review monthly — meaningful value, moderate attention." },
    C: { color: "#94A3B8", note: "Order simply, count rarely — the long tail." },
  };
  ...
  const stockLevels = useMemo(
    () =>
      [...inventory.rows]
        ...
        .slice(0, 8)
        .map((it) => ({
          name: it.name?.length > 16 ? it.name.slice(0, 14) + "…" : it.name,
          qty: it.qty,
          value: Math.round(((it.qty || 0) * (it.unitCost || 0)) / 1000),
        })),
    [inventory.rows],
  );
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <h3 className="text-[13.5px] font-semibold text-[#111827] mb-3">
              Top Items by Stock Value (TZS k)
            </h3>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart
                data={stockLevels}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {catValue.length === 0 ? (
              <p className="text-slate-400 text-center py-6">No data</p>
            ) : (
              <div className="flex items-center gap-4">
                <ResponsiveContainer width="55%" height={150}>
                  <PieChart>
                    <Pie
                      data={catValue.slice(0, 6)}
                      dataKey="value"
                      cx="50%"
                      cy="50%"
                      outerRadius={58}
                      innerRadius={30}
                    >
                      {catValue.slice(0, 6).map((_, i) => (
                        <Cell
                          key={i}
                          fill={
                            ["#16A34A", "#2563EB", "#D97706", "#7C3AED", "#EF4444", "#0891B2"][i]
                          }
                        />
                      ))}
                    </Pie>
                    ...
                    <div key={d.name} className="flex items-center justify-between min-w-0">
                      <span className="flex items-center gap-1.5 text-[11px] truncate">
                        <span
                          ...
                          style={{
                            background: ["#16A34A", "#2563EB", "#D97706", "#7C3AED", "#EF4444"][i],
                          }}
                        />
      ...
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Each warehouse colored by its real share of total stock value — with its real low-stock
          count, so intensity means something you can check.
        </p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {heat.map((w) => (
          <div
            ...
            style={{
              backgroundColor: `rgba(22,163,74,${Math.max(0.06, Math.min(0.85, w.share))})`,
            }}
          >
            <p
              className={`text-[13px] font-semibold ${w.share > 0.45 ? "text-white" : "text-[#111827]"}`}
            >
              {w.name}
            </p>
            <p className={`text-[10.5px] ${w.share > 0.45 ? "text-white/80" : "text-slate-500"}`}>
              {w.city || "—"} · {w.items} items
            </p>
            <p
              className={`text-[12px] font-mono font-medium mt-1 ${w.share > 0.45 ? "text-white" : "text-[#111827]"}`}
            >
              TZS {money(Math.round(w.value))}k · {Math.round(w.share * 100)}%
            </p>
            {w.low > 0 && (
              <p
                className={`text-[10.5px] font-medium mt-0.5 ${w.share > 0.45 ? "text-white" : "text-[#EF4444]"}`}
              >
                {w.low} low-stock item(s)
              </p>
            )}
          </div>
        ))}
        {heat.length === 0 && !warehousesHook.loading && (
          <p className="col-span-full text-[12px] text-slate-400 text-center py-6">
            No warehouses yet.
          </p>
        )}
      </div>
      {unassigned > 0 && (
        <p className="text-[11px] text-slate-400">
          {unassigned} item(s) not assigned to any warehouse — excluded from the map above, stated
        ...
        return (
          <>
            <div className="pt-2">
              ...
              <p className="text-[12px] text-slate-500">
                Real expiry dates against the business date — what needs selling, discounting, or
                writing off first.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                <p className="text-[11px] text-slate-400 mb-1">Expired</p>
                <p
                  className={`text-[16px] font-mono font-bold ${expired.length > 0 ? "text-[#EF4444]" : "text-[#16A34A]"}`}
                >
                  {expired.length}
                </p>
              </div>
              <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                <p className="text-[11px] text-slate-400 mb-1">Within 30 days</p>
                <p
                  className={`text-[16px] font-mono font-bold ${soon30.length > 0 ? "text-[#F59E0B]" : "text-[#16A34A]"}`}
                >
                  {soon30.length}
                </p>
              </div>
              <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                <p className="text-[11px] text-slate-400 mb-1">31–90 days</p>
                <p className="text-[16px] font-mono font-bold text-[#111827]">{soon90.length}</p>
              </div>
            </div>
            {watch.length > 0 && (
                    ...
                    <div key={it.sku} className="flex items-center justify-between px-4 py-2.5">
                      <div className="min-w-0">
                        ...
                        <p className="text-[10.5px] text-slate-400">
                          {it.sku} · {it.qty} on hand
                        </p>
                      </div>
                      <span
                        className={`text-[11px] font-mono font-medium shrink-0 ml-3 ${days < 0 ? "text-[#EF4444]" : days <= 30 ? "text-[#F59E0B]" : "text-slate-500"}`}
                      >
            ...
            {noDate > 0 && (
              <p className="text-[10.5px] text-slate-400">
                {noDate} item(s) carry no expiry date — excluded from the buckets above and stated
                here, never silently treated as immortal.
              </p>
            )}
          </>
        );
      ...
      <div className="pt-2">
        ...
        <p className="text-[12px] text-slate-500">
          Standard Pareto classification by real stock value — A holds ~80% of cumulative value, B
          to 95%, C the tail.
        </p>
      </div>
      <div className="grid grid-cols-3 gap-3">
            ...
            <div key={k} className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <div className="flex items-center gap-2 mb-1">
                <span
                  ...
                  style={{ backgroundColor: CLS_META[k].color }}
                >
                  {k}
                </span>
                <span className="text-[12px] text-slate-500">{items.length} items</span>
              </div>
              <p className="text-[13px] font-mono font-semibold text-[#111827]">
                TZS {money(Math.round(v))}k
              </p>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            {abc.slice(0, 15).map((it) => (
              <tr key={it.sku} className="border-b border-slate-50 last:border-0">
                <td className="px-4 py-2.5 text-[#111827]">
                  {it.name} <span className="text-slate-400">({it.sku})</span>
                </td>
                <td className="px-4 py-2.5 text-right font-mono text-slate-600">
                  {money(Math.round(it.value))}k
                </td>
                ...
                <td className="px-4 py-2.5 text-center">
                  <span
                    className="inline-block w-5 h-5 rounded text-white text-[11px] font-bold leading-5"
                    style={{ backgroundColor: CLS_META[it.cls].color }}
                  >
                    {it.cls}
                  </span>
                </td>
        ...
        {abc.length > 15 && (
          <p className="text-[11px] text-slate-400 px-4 py-2 border-t border-slate-50">
            Top 15 shown of {abc.length} — the tail is C-class by construction.
          </p>
        )}
      </div>
      <p className="text-[10.5px] text-slate-400">
        Valuation here uses each item&apos;s current unit cost. True FIFO/LIFO costing needs per-lot
        purchase cost history this schema doesn&apos;t yet capture — real future work, named rather
        than a dropdown that changes nothing.
      </p>
    </div>
  );
}
...
export function ReorderAlertsView({ inventory, suppliersHook }) {
  ...
  function placeReorder(item) {
    ...
    notify(
      "Draft PO created for " +
        item.name +
        " — " +
        money(item.suggestedQty) +
        " units. Go to Procurement to approve.",
      "info",
    );
    logAudit(
      "Reorder initiated: " + item.name,
      "Inventory",
      "System",
    ...
    <div className="space-y-4">
      ...
      {inventory.rows.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="flex-1 space-y-2">
                {healthData.map((d) => (
                  <div key={d.name} className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-[13px]" style={{ color: d.fill }}>
                      {d.value}
                    </span>
                  </div>
                ))}
          ...
          {urgencyChart.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <h3 className="text-[13.5px] font-semibold text-[#111827] mb-2">
              ...
              <div className="flex gap-4 mt-1 text-[10.5px] text-slate-400">
                <span className="flex items-center gap-1">
                  <span className="w-3 h-2 rounded bg-[#E5E7EB]" />
                  Reorder Pt
                </span>
                <span className="flex items-center gap-1">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            SKUs at or below their reorder point. Create a draft PO with one click — it lands in
            Procurement for manager approval.
          </p>
        </div>
        {alerts.length > 0 && (
          <span className="text-[13px] font-bold text-[#EF4444] flex items-center gap-1.5">
            <Bell size={14} />
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      ...
                      style={{ color: item.qty === 0 ? "#EF4444" : "#F59E0B" }}
                    >
                      {item.qty}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {isOrdered ? (
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => placeReorder(item)}
                            className="text-[11.5px] font-semibold text-white px-2.5 py-1 rounded-lg"
                          ...
                          <button
                            onClick={() =>
                              setDismissed((d) => new Set([...d, item.sku || item.id]))
                            }
...
export function StockAuditView({ inventory }) {
  const audits = useCompanyTable("stock_audits", [], {
    order: { col: "audit_date", ascending: false },
    mapRow: (r) => ({
      id: r.id,
      dbId: r.id,
      date: r.audit_date,
      status: r.status,
      notes: r.notes || "",
      items: (r.stock_audit_items || []).map((it) => ({
        id: it.id,
        sku: it.sku,
        name: it.item_name,
        expectedQty: Number(it.expected_qty) || 0,
        countedQty: it.counted_qty === null ? null : Number(it.counted_qty),
      })),
    }),
  ...
  async function startAudit() {
    ...
    notify(
      `Stock audit started — ${draft.items.length} items to count, real quantities frozen at this moment.`,
    );
    if (IS_CONFIGURED) {
      try {
        const header = await sb("stock_audits")
          .insert({ audit_date: draft.date, status: "In Progress" })
          .single()
          .run();
        if (header?.id) {
          audits.setRows((prev) =>
        ...
        notify("Started locally, but saving to the server failed.", "error");
      }
    }
  }
        ...
        a.id === auditId
          ? {
              ...
              items: a.items.map((it) =>
                it.id === itemId ? { ...it, countedQty: value === "" ? null : Number(value) } : it,
              ),
            }
          : a,
      ),
  ...
  async function completeAudit(audit) {
    audits.setRows((prev) =>
      prev.map((a) => (a.id === audit.id ? { ...a, status: "Completed" } : a)),
    );
      ...
      try {
        ...
        for (const it of audit.items) {
          if (it.countedQty !== null)
            await sb("stock_audit_items")
              .eq("id", it.id)
              .update({ counted_qty: it.countedQty })
              .run();
        }
      } catch (_e) {
        notify("Completed locally, but the server update failed.", "error");
      }
    }
  }
  ...
  if (selectedAudit) {
    const variances = selectedAudit.items.filter(
      (it) => it.countedQty !== null && it.countedQty !== it.expectedQty,
    );
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <button
              onClick={() => setSelected(null)}
              className="text-[12px] text-slate-400 hover:text-slate-600 flex items-center gap-1 mb-1"
            >
              <ChevronLeft size={13} /> All audits
            </button>
            <h3 className="text-[15px] font-semibold text-[#111827]">
              Audit · {selectedAudit.date}
            </h3>
          </div>
          {selectedAudit.status === "In Progress" && (
            <button
              onClick={() => completeAudit(selectedAudit)}
              className="btn-primary text-white text-[12.5px] font-medium px-3.5 py-2 rounded-lg"
            >
              Complete Audit
            </button>
          )}
        ...
        {variances.length > 0 && (
          <div className="bg-[#FEE2E2] rounded-lg p-3">
            <p className="text-[12.5px] font-medium text-[#EF4444]">
              {variances.length} item{variances.length === 1 ? "" : "s"} with a real variance
              between recorded and counted stock.
            </p>
          </div>
        )}
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
              ...
              {selectedAudit.items.map((it) => {
                ...
                return (
                  <tr key={it.id} className="border-b border-slate-50 last:border-0">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {selectedAudit.status === "In Progress" ? (
                        <input
                          type="number"
                          value={it.countedQty ?? ""}
                          onChange={(e) => updateCount(selectedAudit.id, it.id, e.target.value)}
                          className="w-20 text-right bg-slate-50 border border-slate-200 rounded-md px-2 py-1 text-[12px]"
                        />
                      ) : (
                    ...
                    <td
                      className={`px-4 py-2.5 text-right font-mono font-medium ${variance === null ? "text-slate-300" : variance === 0 ? "text-[#16A34A]" : "text-[#EF4444]"}`}
                    >
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-slate-500">
            Real physical counts reconciled against real recorded quantities — variance surfaced per
            item, not silently trusted forever.
          </p>
        </div>
        <button
          onClick={startAudit}
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm divide-y divide-slate-50">
        {!audits.loading && audits.rows.length === 0 && (
          <EmptyState
            icon={ClipboardCheck}
            title="No audits yet"
            hint="Start one to freeze today's recorded quantities and reconcile them against a real physical count."
        ...
        {audits.rows.map((a) => (
          ...
          >
            <div>
              <p className="text-[13px] font-medium text-[#111827]">Audit · {a.date}</p>
              <p className="text-[11px] text-slate-400">{a.items.length} items</p>
            </div>
            <span
              className={`text-[10.5px] font-medium px-2 py-0.5 rounded-full ${a.status === "Completed" ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
            >
              {a.status}
            </span>
          </button>
    ...
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Inventory</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Supplier</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Company name" required>
            <input
              ...
              value={form.name}
              onChange={(e) => set("name", e.target.value)}
              placeholder="e.g. Coastal Steel & Hardware Ltd"
            />
            {touched && !form.name.trim() && (
