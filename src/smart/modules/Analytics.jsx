...
export function usePeriodCloses() {
  ...
  function isLocked(dateStr) {
    ...
    return periods.some(
      (p) => p.status === "Closed" && d >= new Date(p.startDate) && d <= new Date(p.endDate),
    );
  }
...
export function ManualJournalView({ currentUser }) {
  ...
  function removeLine(i) {
    if (lines.length > 2) setLines((prev) => prev.filter((_, idx) => idx !== i));
  }

  async function saveEntry() {
    if (!description.trim()) {
      setErr("Add a description for this journal entry.");
      return;
    }
    if (!balanced) {
    ...
    logAudit(
      "Manual journal entry: " + entry.id,
      "Finance",
      currentUser?.name || "System",
      description + " — TZS " + money(Math.round(totalDebit)) + "k",
    );
    if (IS_CONFIGURED) {
      try {
        ...
        notify("Saved locally — server sync failed.", "error");
      }
    }
  }
    ...
    <div className="space-y-4 max-w-3xl">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Double-entry bookkeeping — every entry must balance (debits = credits) before it can be
          posted. Entries are appended to the audit trail and cannot be deleted.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <FormField label="Description / Reference">
            <input
              ...
              placeholder="e.g. Accrued salaries July 2026"
            />
          </FormField>
        </div>
          ...
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="pb-2 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400 w-[40%]">
                  Account
                </th>
                <th className="pb-2 text-right text-[10.5px] font-medium uppercase tracking-wide text-slate-400 w-[20%]">
                  Debit (TZS k)
                </th>
                <th className="pb-2 text-right text-[10.5px] font-medium uppercase tracking-wide text-slate-400 w-[20%]">
                  Credit (TZS k)
                </th>
                <th className="pb-2 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400">
                  Memo
                </th>
                ...
                <tr key={i}>
                  ...
                  <td className="py-1.5 pl-1">
                    <button
                      ...
                      className="text-slate-300 hover:text-[#EF4444]"
                    >
                      <X size={13} />
                    </button>
                  </td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-200">
                ...
                <td
                  className="pt-2 text-right font-mono font-bold text-[13px]"
                  style={{ color: balanced ? "#16A34A" : "#EF4444" }}
                >
                ...
                <td colSpan={2} className="pt-2 text-right text-[11.5px]">
                  {balanced ? (
                    <span className="text-[#16A34A] font-semibold flex items-center justify-end gap-1">
                      <CheckCircle2 size={13} /> Balanced
                    </span>
                  ) : (
                    <span className="text-[#EF4444] font-semibold flex items-center justify-end gap-1">
                      <AlertCircle size={13} /> Out by TZS{" "}
                      {money(Math.abs(Math.round(totalDebit - totalCredit)))}k
                    </span>
                  )}
                </td>
              </tr>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={addLine}
            className="text-[12.5px] font-medium text-[#16A34A] hover:underline flex items-center gap-1"
          >
          ...
          <button
            onClick={saveEntry}
            disabled={!balanced || !description.trim()}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100">
            <p className="text-[13.5px] font-semibold text-[#111827]">
              Posted this session ({saved.length})
            </p>
          </div>
          {saved.map((entry) => (
            <div key={entry.id} className="px-4 py-3 border-b border-slate-50 last:border-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="font-mono text-[12px] text-[#16A34A] font-bold">
                  TZS {money(Math.round(entry.totalDebit))}k
                </span>
              </div>
              <div className="text-[11px] text-slate-400">
                {entry.date} · Posted by {entry.postedBy} · {entry.lines.length} lines
              </div>
            </div>
          ))}
        </div>
...
export function PeriodClosesView({ invoices, expenses, currentUser }) {
  ...
  function periodStats(p) {
    ...
    const rev = invoices.rows
      .filter((i) => inRange(i.issueDate) && i.status === "Paid")
      .reduce((s, i) => s + lineTotal(i.items).total, 0);
    const exp = expenses.rows
      .filter((e) => inRange(e.expenseDate) && e.status === "Paid")
  ...
  function addPeriod() {
    ...
    const overlap = periods.some(
      (p) => !(draft.endDate < p.startDate || draft.startDate > p.endDate),
    );
    if (overlap) {
      setErr("Period overlaps an existing one.");
      return;
    }
    const row = {
      id: docId("PC"),
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Closing a period locks it against backdating — no invoice or expense can be dated inside a
          closed period. Revenue and expenses shown are from real paid records only.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-[11.5px] text-slate-500 block mb-1">Period name</label>
            <input
              className={inputClass}
              value={draft.name}
          ...
          <div>
            <label className="text-[11.5px] text-slate-500 block mb-1">Start date</label>
            <input
              type="date"
              className={inputClass}
          ...
          <div>
            <label className="text-[11.5px] text-slate-500 block mb-1">End date</label>
            <input
              type="date"
              className={inputClass}
              value={draft.endDate}
        ...
        <button
          onClick={addPeriod}
          className="btn-primary text-white text-[12.5px] font-medium rounded-xl px-4 py-2.5"
        >
          Create period
        </button>
      </div>
        ...
        {periods.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[11.5px] text-slate-400 mt-1">
              Create your first accounting period above. Once closed, it cannot be backdated.
            </p>
          </div>
        )}
        {periods.length > 0 && (
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-slate-100">
                {["Period", "Dates", "Revenue", "Expenses", "Profit", "Status", "Action"].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {periods.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-[#16A34A]">
                      {money(Math.round(rev))}k
                    </td>
                    <td className="px-4 py-3 font-mono text-[#EF4444]">
                      {money(Math.round(exp))}k
                    </td>
                    <td
                      className="px-4 py-3 font-mono font-semibold"
                      style={{ color: profit >= 0 ? "#16A34A" : "#EF4444" }}
                    >
                      {profit >= 0 ? "+" : ""}
                      {money(Math.round(profit))}k
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className="text-[11px] font-semibold px-2 py-0.5 rounded-full"
                        style={{
                          backgroundColor: p.status === "Closed" ? "#FEE2E2" : "#DCFCE7",
                          color: p.status === "Closed" ? "#EF4444" : "#16A34A",
                        }}
                      >
                    ...
                    <td className="px-4 py-3">
                      {p.status === "Open" && (
                        <button
                          ...
                          className="text-[11.5px] font-medium text-[#EF4444] hover:underline flex items-center gap-1"
                        >
                          <Lock size={11} /> Close
                        </button>
                      )}
                      {p.status === "Closed" && (
                        <p className="text-[10.5px] text-slate-400">by {p.closedBy || "System"}</p>
                      )}
                    </td>
                  </tr>
                );
...
export function BankReconciliationView({ invoices, expenses }) {
  ...
  function reconcile() {
    ...
    const lines = statement
      ...
      .map((line) => {
        ...
        const amount = parseFloat(
          parts.find((p) => /^-?\d[\d,.]+$/.test(p.replace(/,/g, "")))?.replace(/,/g, "") || 0,
        );
    ...
    const ledgerCredits = [
      ...invoices.rows
        ...
        .map((i) => ({
          id: i.id,
          label: i.customer,
          amount: Math.round(lineTotal(i.items).total),
          type: "Receipt",
        })),
      ...expenses.rows
    ...
    for (const sl of lines) {
      ...
      const hit = ledgerCredits.find(
        (l) => !usedLedger.has(l.id) && Math.abs(l.amount - sl.amount) <= tolerance,
      );
      if (hit) {
    ...
    setRan(true);
    notify(
      `Reconciliation complete — ${matchedRows.length} matched, ${unmatchedStatement.length + unmatchedLedger.length} unmatched.`,
    );
  }
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Paste your bank statement lines (one per line: date, description, amount). The system
          matches them against your paid invoices and expenses by amount — within 1% tolerance.
        </p>
      </div>
        ...
        <div className="flex gap-2">
          ...
          {ran && (
            <button
              ...
              className="text-[12.5px] text-slate-500 border border-slate-200 rounded-xl px-4 py-2.5"
            >
              Clear
            </button>
          )}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            {matched.length === 0 && (
              <p className="text-[12px] text-slate-400 text-center py-6">
                No matches found — check amounts match your ledger.
              </p>
            )}
            {matched.map((m, i) => (
              <div
                key={i}
                className="flex items-center gap-4 px-4 py-3 border-b border-slate-50 last:border-0"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-[#111827] truncate">
                    {m.statement.desc || m.statement.raw}
                  </p>
                ...
                <div className="flex-1 min-w-0 text-right">
                  <p className="text-[12px] font-medium text-[#111827] truncate">
                    {m.ledger.label} · {m.ledger.type}
                  </p>
                  <p className="text-[10.5px] text-slate-400">{m.ledger.id}</p>
                </div>
                <span className="font-mono text-[12px] font-semibold text-[#16A34A] shrink-0">
                  TZS {money(m.ledger.amount)}k
                </span>
              </div>
            ))}
          </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
                ...
                <p className="text-[13px] font-semibold text-[#111827]">
                  Unmatched ({(unmatched.statement?.length || 0) + (unmatched.ledger?.length || 0)})
                </p>
              </div>
              {unmatched.statement?.map((sl, i) => (
                ...
                >
                  <span className="text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-[#FEF3C7] text-[#92400E]">
                    Statement
                  </span>
                  <p className="flex-1 text-[12px] text-[#111827] truncate">{sl.desc || sl.raw}</p>
                  <span className="font-mono text-[12px] text-slate-500">
                    TZS {money(sl.amount)}k
                  </span>
                </div>
              ))}
              {unmatched.ledger?.map((l, i) => (
                <div
                  ...
                  className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-50 last:border-0"
                >
                  <span className="text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-[#DBEAFE] text-[#1E40AF]">
                    Ledger
                  </span>
                  <p className="flex-1 text-[12px] text-[#111827] truncate">
                    {l.label} · {l.type}
                  </p>
...
export function MicrofinanceModule({ currentUser }) {
  ...
  const clients = useCompanyTable("mfi_clients", MFI_CLIENT_SEED, {
    mapRow: (r) => ({
      id: r.id,
      name: r.name,
      phone: r.phone,
      ...
      gender: r.gender,
      village: r.village,
      joinedDate: r.joined_date || r.joinedDate,
      status: r.status,
    }),
  });
  const loans = useCompanyTable("mfi_loans", MFI_LOAN_SEED, {
    mapRow: (r) => ({
      id: r.id,
      ...
      status: r.status,
      balance: r.balance || r.principal,
      collateral: r.collateral || "None",
    }),
  });
  ...
  const totalSavings =
    savings.rows.filter((r) => r.type === "Deposit").reduce((s, r) => s + r.amount, 0) -
  ...
  async function disburseLoan() {
    ...
    const row = {
      ...
      productName: loanForm.product,
      principal: Number(loanForm.principal),
      rate: Number(loanForm.rate),
      months: Number(loanForm.months),
      disbursed: TODAY.toISOString().slice(0, 10),
      status: "Active",
      balance: total,
      collateral: loanForm.collateral || "None",
    };
    ...
    logAudit(
      "Loan disbursed: " + row.id,
      "Microfinance",
      currentUser?.name || "System",
      "TZS " + money(row.principal) + "k to " + client?.name,
    );
    if (IS_CONFIGURED) {
      try {
        await sb("mfi_loans")
  ...
  function submitRepayment() {
    ...
    loans.setRows((prev) =>
      prev.map((l) => (l.id === repayModal.id ? { ...l, balance: newBal, status: newStatus } : l)),
    );
    notify("TZS " + money(amt) + "k repayment recorded");
    logAudit(
      "Repayment: " + repayModal.id,
      "Microfinance",
      currentUser?.name || "System",
      "TZS " + money(amt) + "k — Balance: TZS " + money(newBal) + "k",
    );
    setRepayModal(null);
    setRepayAmt("");
  }
  ...
  const monthlyRevenue = activeLoans.reduce(
    (s, l) => s + (l.principal * l.rate) / 100 / l.months,
    0,
  );
      ...
      >
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[12px] mt-0.5" style={{ color: "rgba(255,255,255,.65)" }}>
              Loans &middot; Savings &middot; Client Management &middot; PAR Monitoring
            </p>
          </div>
          <div className="flex gap-2">
            <button
        ...
        {MFI_TABS.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => setTab(t.id)}
              className={
                "flex items-center gap-1.5 flex-1 justify-center py-2 rounded-lg text-[12px] font-medium transition-colors whitespace-nowrap " +
                (tab === t.id
            ...
            >
              <I size={13} />
              {t.label}
              {t.id === "arrears" && atRisk.length > 0 && (
                <span className="ml-1 bg-[#EF4444] text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center shrink-0">
                  {atRisk.length}
                </span>
              )}
            </button>
        ...
        <div className="space-y-4">
            ...
            {[
              {
                l: "Active Clients",
                v: totalClients,
                sub: clients.rows.filter((c) => c.status === "Active").length + " active",
                c: "#059669",
                I: Users,
              },
              {
                l: "Loan Portfolio",
                v: "TZS " + money(totalPortfolio) + "k",
              ...
              {
                l: "Savings (Net)",
                v: "TZS " + money(totalSavings) + "k",
                sub: "Total deposits held",
                c: "#7C3AED",
                I: Wallet,
              },
              {
                l: "PAR > 30 days",
                v: PAR30_ratio + "%",
                sub: "TZS " + money(PAR30) + "k at risk",
                c: PAR30_ratio > 5 ? "#EF4444" : "#16A34A",
                I: AlertCircle,
              },
            ].map((k) => (
              <div
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-[20px] font-bold mt-1 text-[#111827]">{k.v}</p>
                    <p className="text-[11px] mt-0.5" style={{ color: k.c }}>
                      {k.sub}
                    </p>
                  </div>
                  <div
                    className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                    style={{ background: k.c + "18" }}
                  >
                    <k.I size={17} style={{ color: k.c }} />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              <p className="text-[13.5px] font-semibold text-[#111827] mb-3">
                Loan Portfolio by Product (TZS k)
              </p>
              {(() => {
                const prodData = [
                  "Personal Loan",
                  ...
                  .map((prod, i) => {
                    const bal = activeLoans
                      .filter((l) => l.productName === prod)
                      .reduce((s, l) => s + l.balance, 0);
                    return {
                      name: prod.replace(" Loan", ""),
                      value: Math.round(bal / 1000),
                      fill: ["#059669", "#2563EB", "#D97706", "#EF4444", "#7C3AED"][i],
                    };
                  })
                  .filter((d) => d.value > 0);
                if (!prodData.length)
                  return <p className="text-slate-400 text-center py-4">No active loans</p>;
                return (
                  <ResponsiveContainer width="100%" height={155}>
                    <BarChart
                      data={prodData}
                      layout="vertical"
                      margin={{ left: 5, right: 24, top: 0, bottom: 0 }}
                    >
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                <span className="text-[14px] font-mono font-bold text-[#059669]">
                  TZS {money(monthlyRevenue)}k
                </span>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const qualData = [
                  {
                    name: "Active",
                    value: loans.rows.filter((l) => l.status === "Active").length,
                    fill: "#059669",
                  },
                  {
                    name: "Closed",
                    value: loans.rows.filter((l) => l.status === "Closed").length,
                    fill: "#94A3B8",
                  },
                  {
                    name: "Defaulted",
                    value: loans.rows.filter((l) => l.status === "Defaulted").length,
                    fill: "#EF4444",
                  },
                  { name: "Overdue", value: atRisk.length, fill: "#F59E0B" },
                ].filter((d) => d.value > 0);
                return qualData.length === 0 ? (
                  <p className="text-slate-400 text-center py-4">No loans</p>
                ) : (
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-2">
                      PAR &gt; 30: <strong className="text-[#EF4444]">{PAR30_ratio}%</strong>
                    </p>
                  </div>
                );
              })()}
        ...
        <div className="space-y-3">
          {showClientForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <FormField label="Village / Ward">
                  <input
                    className={inputClass}
                    value={clientForm.village}
                    onChange={(e) => setClientForm({ ...clientForm, village: e.target.value })}
              ...
              <div className="flex gap-2">
                <button
                  onClick={saveClient}
                  className="btn-primary text-white text-[12.5px] rounded-xl px-4 py-2.5"
                >
                  Register Client
                </button>
                <button
          ...
          {!showClientForm && (
            <div className="flex justify-end">
              <button
                onClick={() => setShowClientForm(true)}
                className="flex items-center gap-1.5 btn-primary text-white text-[12.5px] rounded-xl px-4 py-2.5"
              >
                <UserPlus size={13} />
                Register Client
              </button>
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Client", "Phone", "National ID", "Village", "Status", "Loans"].map((h) => (
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400"
                    >
              ...
              <tbody>
                {clients.rows.map((cl) => {
                  const clLoans = loans.rows.filter(
                    (l) => l.clientId === cl.id && l.status === "Active",
                  );
                  return (
                    <tr
                      key={cl.id}
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-[#111827]">{cl.name}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={
                            clLoans.length > 0 ? "text-[#2563EB] font-semibold" : "text-slate-300"
                          }
                        >
                          {clLoans.length > 0 ? clLoans.length + " active" : "None"}
                        </span>
                      </td>
                    </tr>
                  );
          ...
          {showLoanForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <FormField label="Principal (TZS k)">
                  <input
                    type="number"
                    min="0"
                    className={inputClass}
                ...
                <FormField label="Collateral">
                  <input
                    ...
                    placeholder="e.g. Land title, Motorcycle"
                  />
                </FormField>
              </div>
              {loanForm.principal && (
                <div className="grid grid-cols-3 gap-3">
                  {[
                    [
                      "Total Repayable",
                      "TZS " +
                        money(Number(loanForm.principal) * (1 + Number(loanForm.rate) / 100)) +
                        "k",
                      "#111827",
                    ],
                    [
                      ...
                      "TZS " +
                        money(
                          (Number(loanForm.principal) * (1 + Number(loanForm.rate) / 100)) /
                            Number(loanForm.months),
                        ) +
                        "k",
                      "#2563EB",
                    ],
                    [
                      "Interest Income",
                      "TZS " +
                        money((Number(loanForm.principal) * Number(loanForm.rate)) / 100) +
                        "k",
                      "#059669",
                    ],
                  ].map(([l, v, col]) => (
                    <div key={l} className="bg-slate-50 rounded-lg p-2.5 text-center">
                      ...
                      <p className="text-[13.5px] font-bold mt-0.5" style={{ color: col }}>
                        {v}
                      </p>
                    </div>
                  ))}
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {[
                    ...
                    <th
                      key={h}
                      className="px-3 py-3 text-left text-[10px] font-medium uppercase tracking-wide text-slate-400"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {loans.rows.map((l) => (
                  <tr
                    ...
                    className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50"
                  >
                    <td className="px-3 py-3 font-mono text-[11px] font-medium text-[#059669]">
                      {l.id}
                    </td>
                    ...
                    <td
                      ...
                      style={{ color: l.balance > 0 ? "#2563EB" : "#16A34A" }}
                    >
                      {money(l.balance)}k
                    </td>
                    ...
                    <td className="px-3 py-3">
                      {l.status === "Active" && (
                        <button
                          ...
                          className="text-[11px] font-semibold text-white bg-[#059669] px-2.5 py-1 rounded-lg"
                        >
                          Repay
                        </button>
                      )}
      ...
      {tab === "savings" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
              <p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">Net Balance</p>
              <p className="text-[20px] font-bold text-[#059669]">TZS {money(totalSavings)}k</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
              <p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">Savers</p>
              <p className="text-[20px] font-bold text-[#111827]">
                {[...new Set(savings.rows.map((r) => r.clientId))].length}
              </p>
            </div>
          </div>
          {!showSavingForm && (
                ...
                {savings.rows.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-4 py-10 text-center text-slate-400 text-[13px]">
                      No savings transactions yet. Record the first one above.
                    </td>
                  </tr>
                ) : (
                  savings.rows.map((s) => (
                    <tr key={s.id} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="px-4 py-3">
                        <span
                          ...
                          style={{
                            background: s.type === "Deposit" ? "#DCFCE7" : "#FEE2E2",
                            color: s.type === "Deposit" ? "#16A34A" : "#EF4444",
                          }}
                        >
                          {s.type}
                        </span>
                      ...
                      <td
                        ...
                        style={{ color: s.type === "Deposit" ? "#059669" : "#EF4444" }}
                      >
                        {s.type === "Withdrawal" ? "-" : "+"} TZS {money(s.amount)}k
                      </td>
                    </tr>
                  ))
                )}
      ...
      {tab === "arrears" && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 rounded-xl border border-[#FCA5A5] bg-[#FEF2F2]">
            ...
            <div>
              <p className="text-[13px] font-semibold text-[#991B1B]">Portfolio at Risk (PAR30)</p>
              <p className="text-[12px] text-[#B91C1C]">
                TZS {money(PAR30)}k in {atRisk.length} loan{atRisk.length !== 1 ? "s" : ""} overdue
                &gt; 30 days. PAR ratio: {PAR30_ratio}%
              </p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {["Loan", "Client", "Disbursed", "Balance", "Status", "Collateral", "Action"].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400"
              ...
              <tbody>
                {atRisk.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-10 text-center text-[#16A34A] font-medium">
                      No loans at risk. Portfolio is healthy.
                    </td>
                  </tr>
                ) : (
                  atRisk.map((l) => (
                    <tr
                      key={l.id}
                      className="border-b border-slate-50 last:border-0 bg-[#FEF2F2]/30"
                    >
                      ...
                      <td className="px-4 py-3 font-mono font-bold text-[#EF4444]">
                        TZS {money(l.balance)}k
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full bg-[#FEE2E2] text-[#EF4444]">
                          {l.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="text-[11px] font-semibold text-white bg-[#EF4444] px-2.5 py-1 rounded-lg"
                        >
                          Collect
                        </button>
                      </td>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            <h3 className="text-[15px] font-semibold text-[#111827] mb-4">
              MFI Performance Summary
            </h3>
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ["Total Disbursed", money(loans.rows.reduce((s, l) => s + l.principal, 0)) + "k"],
                ["Loans Closed", loans.rows.filter((l) => l.status === "Closed").length],
                [
                  "Recovery Rate",
                  loans.rows.length > 0
                    ? (
                        (loans.rows.filter((l) => l.status === "Closed").length /
                          loans.rows.length) *
                        100
                      ).toFixed(0) + "%"
                    : "—",
                ],
                ["Monthly Revenue", "TZS " + money(monthlyRevenue) + "k"],
              ].map(([l, v]) => (
                <div key={l} className="bg-slate-50 rounded-xl p-3">
                  <p className="text-[11px] text-slate-400">{l}</p>
                  <p className="text-[18px] font-bold text-[#059669] mt-0.5">
                    TZS{" "}
                    {isNaN(Number(v.replace(/[^0-9]/g, "")))
                      ? ""
                      : v.includes("%") || v.includes("k") || !isNaN(Number(v))
                        ? v
                        : "TZS " + v}
                  </p>
                </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[12.5px] font-medium text-[#111827]">{cl.name}</span>
                    </div>
                    <div className="text-right">
                      <p
                        className="text-[12px] font-mono font-bold"
                        style={{ color: outstanding > 0 ? "#2563EB" : "#16A34A" }}
                      >
                        {outstanding > 0
                          ? "TZS " + money(outstanding) + "k outstanding"
                          : "No balance"}
                      </p>
                      <p className="text-[10.5px] text-slate-400">
                        {clLoans.length} loan{clLoans.length !== 1 ? "s" : ""} total
                      </p>
                    </div>
                  </div>
                );
        ...
        >
          <div className="bg-white rounded-2xl p-6 w-80 shadow-2xl">
            ...
            <p className="text-[12px] text-slate-500 mb-4">
              {repayModal.clientName} &middot; Balance: TZS {money(repayModal.balance)}k
            </p>
            <FormField label="Amount received (TZS k)">
              <input
                type="number"
                min="0"
                className={inputClass}
            ...
            <div className="flex gap-2 mt-4">
              <button
                ...
                className="flex-1 btn-primary text-white rounded-xl py-2.5 text-[13px] font-semibold"
              >
                Confirm
              </button>
              <button
                onClick={() => setRepayModal(null)}
                className="flex-1 text-[13px] text-slate-500 rounded-xl py-2.5 border border-slate-200"
              >
                Cancel
              </button>
            </div>
...
export function DataQualityView({ crm, invoices, expenses, inventory, employees }) {
  const findings = useMemo(() => {
    ...
    crm.rows.forEach((l) => {
      ...
      (seen[k] = seen[k] || []).push(l.company);
    });
    const dupCustomers = Object.values(seen).filter((v) => v.length > 1);
    out.push({
      label: "Duplicate customers",
      count: dupCustomers.length,
      sample: dupCustomers
        .slice(0, 3)
        .map((v) => v.join(" / "))
        .join("; "),
      fix: "CRM > Leads — merge by hand; auto-merging lookalikes is how data gets eaten.",
    });

    const badPhones = [
      ...crm.rows.filter((l) => !phoneOk(l.phone)).map((l) => l.company),
      ...employees.rows.filter((e) => !phoneOk(e.phone)).map((e) => e.name),
    ];
    out.push({
      label: "Invalid phone numbers",
      count: badPhones.length,
      sample: badPhones.slice(0, 3).join(", "),
      fix: "Open the named record in CRM or HR — the pattern tolerates +255/07 formats, spaces, and dashes.",
    });
    ...
    out.push({
      ...
      sample: noEmail
        ...
        .map((e) => e.name)
        .join(", "),
      fix: "HR > Employees — payslips and notifications need a real address.",
    });

    const noExpiry = inventory.rows.filter((it) => !it.expiryDate);
    out.push({
      label: "Items without expiry dates",
      count: noExpiry.length,
      sample: noExpiry
        .slice(0, 3)
        .map((it) => it.name)
        .join(", "),
      fix: "Inventory > Stock — undated stock is invisible to Expiry Tracking (section 106's blind-spot rule).",
    });

    const noDue = invoices.rows.filter((i) => i.status !== "Paid" && !i.dueDate);
    out.push({
      label: "Unpaid invoices missing due date",
      count: noDue.length,
      sample: noDue
        .slice(0, 3)
        .map((i) => i.id)
        .join(", "),
      fix: "Sales > Invoices — no due date means invisible to aging, budgets, and the Risk Center.",
    });

    const noMethod = expenses.filter((e) => e.status === "Paid" && !e.method);
    out.push({
      label: "Paid expenses missing payment method",
      count: noMethod.length,
      sample: noMethod
        .slice(0, 3)
        .map((e) => e.vendor)
        .join(", "),
      fix: "Finance > Payables — method gaps weaken the Cash Flow statement's honesty.",
    });
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Six real scans over live rows — every finding names its record and its door.{" "}
          {clean ? "All clean right now." : ""}
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {findings.map((f) => (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              <span
                ...
                style={{ color: f.count === 0 ? "#16A34A" : "#F59E0B" }}
              >
                {f.count}
              </span>
            </div>
            {f.count > 0 && f.sample && (
              <p className="text-[10.5px] text-slate-500 truncate">{f.sample}</p>
            )}
            <p className="text-[10.5px] text-slate-600 mt-1.5">
              <span className="font-medium text-[#16A34A]">Fix:</span> {f.fix}
            </p>
          </div>
        ))}
      </div>
...
export function FirstRunGuide({ invoices, inventory, crm, company, onNavigate }) {
  ...
  const steps = [
    {
      done: Boolean(company.tin || company.address),
      label: "Complete company profile",
      hint: "Settings → Company profile — add your TIN, address, and logo",
      nav: "settings",
    },
    {
      done: inventory.rows.length > 0,
      label: "Add your first product or service",
      hint: "Inventory → Add Item — products appear in Sales, POS, and the AI assistant",
      nav: "inventory",
    },
    {
      done: crm.rows.length > 0,
      label: "Add your first customer",
      hint: "CRM → Add Lead — customers appear in Sales invoices, quotations, and Customer 360",
      nav: "crm",
    },
    {
      done: invoices.rows.length > 0,
      label: "Create your first invoice",
      hint: "Sales → Invoices → New Invoice — this activates the Revenue KPI and Business Health",
      nav: "sales",
    },
  ];
    ...
    <div className="bg-white rounded-2xl border border-[#16A34A]/20 shadow-md p-6 mb-4">
      <div className="flex items-start gap-4">
        <div
          ...
          style={{ background: "linear-gradient(135deg,#052614,#16A34A)" }}
        >
        ...
        <div className="flex-1">
          ...
          <p className="text-[12.5px] text-slate-500 mt-0.5 mb-4">
            Complete these steps to activate your dashboard. Each one adds real data — your KPIs and
            Business Health score will update automatically.
          </p>
          <div className="space-y-2.5">
            {steps.map((step, i) => (
              <div
                key={i}
                className={`flex items-start gap-3 p-3 rounded-xl border ${step.done ? "border-[#16A34A]/20 bg-[#F0FDF4]" : "border-slate-200 bg-slate-50/60"}`}
              >
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-[11px] font-bold ${step.done ? "bg-[#16A34A] text-white" : "bg-slate-200 text-slate-500"}`}
                >
                  {step.done ? "✓" : i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[12.5px] font-semibold ${step.done ? "text-[#16A34A] line-through opacity-60" : "text-[#111827]"}`}
                  >
                    {step.label}
                  </p>
                ...
                {!step.done && (
                  <button
                    ...
                    className="text-[11.5px] font-medium text-[#16A34A] border border-[#16A34A]/30 rounded-lg px-2.5 py-1 shrink-0 hover:bg-[#16A34A]/5"
                  >
                    Go →
                  </button>
                )}
...
export function RiskCenterView({ invoices, expenses, inventory, employees }) {
  const training = useCompanyTable("hr_training", [], {
    mapRow: mapTrainingRow,
  ...
  const risks = useMemo(() => {
    ...
    out.push({
      ...
      basis:
        unpaid.length === 0
          ? "No unpaid invoices"
          : `${overdue.length} of ${unpaid.length} unpaid invoices overdue`,
      fix: "Chase the Receivables Aging list oldest-first; Cmd+K a customer name to jump straight to them.",
    });
    ...
    out.push({
      ...
      basis:
        inventory.rows.length === 0
          ...
          : `${low.length} of ${inventory.rows.length} items at/below reorder`,
      fix: "Raise POs from Procurement; the Inventory Replenishment workflow template automates the alert.",
    });
    ...
    out.push({
      cat: "Cybersecurity",
      pct: cyPct,
      basis: `This device: App Lock ${lock ? "on" : "OFF"}, biometric unlock ${bio ? "enrolled" : "not enrolled"} — RLS and RBAC hold platform-wide regardless`,
      fix: lock
        ? "Enroll biometric unlock in Settings > App Lock; enable TOTP 2FA when wired (GoTrue MFA)."
        : "Turn on App Lock in Settings — one PIN, this device, right now.",
    });

    // Compliance — overdue mandatory/compliance training + tax proximity
    const compOverdue = training.rows.filter(
      (r) =>
        (r.mandatory || r.compliance) && r.status !== "Completed" && r.dueDate && r.dueDate < t,
    );
    const compAll = training.rows.filter((r) => r.mandatory || r.compliance);
    const cpPct =
      compAll.length === 0 ? 0.2 : Math.min(1, compOverdue.length / compAll.length + 0.1);
    out.push({
      cat: "Compliance",
      pct: cpPct,
      basis:
        compAll.length === 0
          ? "No mandatory/compliance training assigned yet — itself a mild exposure"
          : `${compOverdue.length} of ${compAll.length} mandatory/compliance training(s) overdue`,
      fix: "Assign compliance courses in HR > Training; the Tax Center's deadline strip covers TRA filing dates.",
    });
    ...
    const scPct =
      inventory.rows.length === 0
        ? 0
        : Math.min(1, (outOfStock.length * 2 + low.length) / Math.max(1, inventory.rows.length));
    out.push({
      cat: "Supply Chain",
      pct: scPct,
      basis: `${outOfStock.length} item(s) fully out of stock — proxy measure: per-item supplier links don't exist yet, so concentration risk isn't computable`,
      fix: "Add second suppliers for A-class items (Smart Analysis names them); track lead times in Suppliers.",
    });
    ...
    out.push({
      ...
      pct: repPct,
      basis:
        "Proxy: overdue obligations correlate with disputes — support-ticket volume will sharpen this when threaded here",
      fix: "Resolve oldest overdue items first; portal support tickets give customers a real channel before they go public.",
    });
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[12px] text-slate-500">
          Six categories, every score computed from real signals, every mitigation pointing at a
          screen that exists — {high > 0 ? `${high} currently High` : "none currently High"}.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {risks.map((r) => (
          <div key={r.cat} className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            <p className="text-[10.5px] text-slate-600 mt-1.5">
              <span className="font-medium text-[#16A34A]">Mitigation:</span> {r.fix}
            </p>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          Role-focused dashboards, custom KPIs, real external market data, and transparent
          statistical projections — all over live data every module reads
        </p>
      </div>
      ...
      {tab === "benchmarking" && (
        <Benchmarking data={{ invoices, expenses, crm, inventory, employees }} />
      )}
      {tab === "risk" && (
        <RiskCenterView
          invoices={invoices}
      ...
      {tab === "periods" && (
        <PeriodClosesView invoices={invoices} expenses={expenses} currentUser={currentUser} />
      )}
      ...
      {items.map((s) => {
        ...
        return (
          ...
          >
            ...
            <p className={`text-[17px] font-mono font-semibold ${s.color || "text-[#111827]"}`}>
              {s.value}
            </p>
      ...
      {items.map((i) => (
        <div key={i.label}>
          <div className="flex justify-between text-[12px] mb-1">
            ...
            <span className="font-mono text-[#111827] font-medium shrink-0">
          ...
          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
            <div
              className="h-full rounded-full"
              style={{ width: `${(i.value / max) * 100}%`, backgroundColor: i.color || "#16A34A" }}
            />
          </div>
        </div>
      ))}
...
export function ExecutiveDashboard({
  ...
  // ── Live KPIs ────────────────────────────────────────────────────────
  const revenue = invoices.rows.reduce(
    (s, inv) => s + (inv.status === "Paid" ? lineTotal(inv.items).total : inv.amountPaid || 0),
    0,
  );
  const expenseTotal = expenses.rows.reduce((s, e) => s + e.amount, 0);
  const profit = revenue - expenseTotal;
  const margin = revenue > 0 ? ((profit / revenue) * 100).toFixed(1) : 0;
  const openPipeline = crm.rows
  ...
  const overdueInvs = invoices.rows.filter(
    (inv) => inv.status !== "Paid" && inv.dueDate && new Date(inv.dueDate) < TODAY,
  );
  const overdueValue = overdueInvs.reduce(
    (s, inv) => s + (lineTotal(inv.items).total - (inv.amountPaid || 0)),
    0,
  );
  ...
  trendData[5] = {
    ...
    expenses: Math.round(expenseTotal / 1000),
  ...
  const KPIS = [
    {
      label: "Revenue",
      value: "TZS " + money(Math.round(revenue)) + "k",
      sub: "Collected",
      col: "#2563EB",
      mod: "finance",
    },
    {
      label: "Net Profit",
      value: "TZS " + money(Math.round(Math.abs(profit))) + "k",
      sub: (profit >= 0 ? "Profit" : "Loss") + " · " + margin + "%",
      col: profit >= 0 ? "#16A34A" : "#EF4444",
      mod: "reports",
    },
    {
      label: "Pipeline",
      value: "TZS " + money(Math.round(openPipeline)) + "k",
      sub: crm.rows.filter((l) => !["Won", "Lost"].includes(l.stage)).length + " open deals",
      col: "#7C3AED",
      mod: "crm",
    },
    {
      label: "Overdue AR",
      value: "TZS " + money(Math.round(overdueValue)) + "k",
      sub: overdueInvs.length + " invoices overdue",
      col: overdueValue > 0 ? "#EF4444" : "#16A34A",
      mod: "finance",
    },
    {
      label: "Stock Value",
      value: "TZS " + money(Math.round(stockValue)) + "k",
    ...
    },
    {
      ...
      value: "TZS " + money(Math.round(expenseTotal)) + "k",
      sub: "Period to date",
      col: "#F59E0B",
      mod: "finance",
    },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-[12.5px] text-slate-400 mt-0.5">
                Live metrics across all 33 modules · Click any card to drill down
              </p>
            </div>
            <div className="text-right shrink-0 ml-4">
              <p className="text-[10.5px] text-slate-400 uppercase tracking-wide mb-1">
                Business Health
              </p>
              <p className="text-[32px] font-black leading-none" style={{ color: healthColor }}>
                {healthScore}
              </p>
              <p className="text-[11px] font-semibold mt-0.5" style={{ color: healthColor }}>
                {healthLabel}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {KPIS.map((k) => (
              <div
                key={k.label}
                onClick={() => nav(k.mod)}
                className="rounded-xl border border-slate-100 p-3 cursor-pointer hover:border-slate-300 hover:shadow-sm transition-all group"
              >
                <p className="text-[10.5px] text-slate-400 uppercase tracking-wide mb-0.5">
                  {k.label}
                </p>
                <p className="text-[16px] font-bold" style={{ color: k.col }}>
                  {k.value}
                </p>
                <p className="text-[10.5px] text-slate-400 mt-0.5 group-hover:text-slate-600 transition-colors">
                  {k.sub}
                </p>
              </div>
            ))}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <div className="flex items-center justify-center mb-4">
            <div className="relative w-28 h-28">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-[24px] font-black" style={{ color: healthColor }}>
                  {healthScore}
                </p>
          ...
          {[
            ...
            [
              "Stock Healthy",
              inventory.rows.filter((it) => it.qty <= (it.reorderLevel || 5)).length === 0,
              "No low-stock items",
            ],
            ...
            >
              <div
                className={
                  "w-4 h-4 rounded-full flex items-center justify-center shrink-0 text-[9px] font-bold text-white " +
                  (ok ? "bg-[#16A34A]" : "bg-slate-200")
                }
              >
                {ok ? "✓" : "✗"}
              </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <button
            key={k.l}
            onClick={() => nav(k.mod)}
            className="bg-white rounded-xl border border-slate-200/80 p-4 text-left hover:border-slate-300 hover:shadow-sm transition-all group"
          >
            <div
              className="w-9 h-9 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform"
              style={{ background: k.col + "15" }}
            >
...
export function FinancialDashboard({ invoices, expenses, posTransactions, onNavigate }) {
  ...
  const posRevenue = posTransactions.rows.reduce(
    (s, t) =>
      s + Math.round(t.items.reduce((si, it) => si + it.qty * it.price, 0) * (1 + TAX_RATE)),
    0,
  );
  const expenseTotal = expenses.rows.reduce((s, e) => s + e.amount, 0);
  const gross = revenue + posRevenue;
  const profit = gross - expenseTotal;
  const outstanding = invoices.rows.filter(
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          {
            l: "Total Expenses",
            v: "TZS " + money(Math.round(expenseTotal)) + "k",
            col: "#F59E0B",
          },
          {
            l: "Net Profit",
            v: "TZS " + money(Math.round(Math.abs(profit))) + "k",
            col: profit >= 0 ? "#16A34A" : "#EF4444",
          },
          ...
          <div key={k.l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
            ...
            <p className="text-[20px] font-bold" style={{ color: k.col }}>
              {k.v}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <p className="text-[11.5px] text-slate-400 mb-3">
            TZS thousands · All expense categories
          </p>
          {byCategory.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <p className="text-[11.5px] text-slate-400 mb-3">
            Invoice collection performance · TZS thousands
          </p>
          {statusData.length === 0 ? (
                    ...
                    <div>
                      <div className="flex justify-between text-[11.5px] mb-1.5">
                        ...
                        <span
                          className="font-bold"
                          style={{
                            color: rate >= 80 ? "#16A34A" : rate >= 60 ? "#F59E0B" : "#EF4444",
                          }}
                        >
                          {rate}%
                        </span>
                      ...
                      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: rate + "%",
                            background: rate >= 80 ? "#16A34A" : rate >= 60 ? "#F59E0B" : "#EF4444",
                          }}
                        />
                      </div>
                    </div>
...
export function HRDashboard({ employees, leaveRequests, onNavigate }) {
  ...
  const leaveTypes = useMemo(() => {
    ...
    leaveRequests.rows.forEach((l) => {
      map[l.leaveType || "Annual"] = (map[l.leaveType || "Annual"] || 0) + 1;
    });
  ...
  const radarData = [
    {
      subject: "Active",
      value: active > 0 ? Math.round((active / (employees.rows.length || 1)) * 100) : 0,
    },
    {
      subject: "Retention",
      value:
        employees.rows.length > 0
          ? Math.round((1 - inactive / (employees.rows.length || 1)) * 100)
          : 90,
    },
    {
      subject: "Leave Mgmt",
      value: pendingLeave === 0 ? 100 : Math.round((1 - pendingLeave / 10) * 80),
    },
    {
      subject: "Payroll",
      value: payroll > 0 ? Math.min(100, Math.round(payroll / employees.rows.length / 20)) : 0,
    },
    {
      subject: "Diversity",
      value: (() => {
        const f = employees.rows.filter((e) => e.gender === "F").length;
        return employees.rows.length > 0 ? Math.round((f / employees.rows.length) * 200) : 50;
      })(),
    },
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          [
            "Pending Leave",
            pendingLeave + " request" + (pendingLeave !== 1 ? "s" : ""),
            pendingLeave > 0 ? "#EF4444" : "#16A34A",
          ],
        ].map(([l, v, col]) => (
          <div
            key={l}
            onClick={() => nav("hr")}
            className="bg-white rounded-xl border border-slate-200/80 p-4 text-center cursor-pointer hover:shadow-sm"
          >
      ...
      {leaveRequests.rows.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <button onClick={() => nav("hr")} className="text-[12px] text-[#16A34A] font-medium">
              View all →
            </button>
          </div>
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/50">
                {["Employee", "Type", "Dates", "Status"].map((h) => (
                  <th
                    key={h}
                    className="px-4 py-2.5 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400"
                  >
                    {h}
                  </th>
                ))}
...
export function SalesDashboard({ invoices, crm, onNavigate }) {
  ...
  const weightedForecast = openLeads.reduce(
    (s, l) => s + l.value * ((STAGE_PROBABILITY[l.stage] || 0) / 100),
    0,
  );
  const wonCount = crm.rows.filter((l) => l.stage === "Won").length;
  ...
  const monthlyRevenue = months.map((m, i) => {
    const base = invoices.rows.reduce(
      (s, inv) => s + (inv.status === "Paid" ? lineTotal(inv.items).total : inv.amountPaid || 0),
      0,
    );
    ...
    return {
      month: m,
      revenue: Math.round((base * factor) / 1000),
      invoices: Math.max(1, Math.round(invoices.rows.length * factor)),
    };
  });
  monthlyRevenue[5].revenue = Math.round(
            ...
            {stageData.map((s, i) => {
              ...
              return (
                <div key={s.stage} className="flex items-center gap-3">
                  <span className="text-[12px] font-medium text-slate-600 w-24 shrink-0">
                    {s.stage}
                  </span>
                  <div
                    className="flex-1 h-6 rounded-lg overflow-hidden bg-slate-100"
                    style={{ paddingLeft: i * 8 }}
                  >
                    <div
                      className="h-full rounded-lg flex items-center px-2.5 transition-all"
                      style={{ width: Math.max(pct, 8) + "%", background: s.fill }}
                    >
                      {s.count > 0 && (
                  ...
                  <span className="text-[11.5px] font-mono font-bold text-slate-700 w-20 text-right">
                    TZS {money(Math.round(s.value / 1000))}k
                  </span>
                </div>
              );
            })}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <ResponsiveContainer width="100%" height={180}>
            ...
            >
              ...
              <Tooltip
                formatter={(v, name) => [
                  name === "revenue" ? "TZS " + money(v) + "k" : v + " inv",
...
export function OperationsDashboard({ inventory, workOrders, onNavigate }) {
  ...
  const lowStock = inventory.rows.filter(
    (it) => stockStatus(it.qty, it.reorder) === "Low Stock",
  ).length;
  const outOfStock = inventory.rows.filter(
    (it) => stockStatus(it.qty, it.reorder) === "Out of Stock",
  ).length;
  const activeOrders = workOrders.rows.filter((w) =>
    ["In Progress", "Planned"].includes(w.status),
  ).length;
  const nav = onNavigate || (() => {});
  ...
  // Work order status
  const woStatus = ["Planned", "In Progress", "Completed", "Cancelled"]
    .map((status) => ({
      name: status,
  ...
  const stockHealth = [
    {
      name: "In Stock",
      value: inventory.rows.filter((it) => stockStatus(it.qty, it.reorder) === "In Stock").length,
      fill: "#16A34A",
    },
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          {
            l: "Stock Value",
            v: "TZS " + money(Math.round(stockValue)) + "k",
            c: "#2563EB",
            mod: "inventory",
          },
          {
            l: "Low Stock Items",
            v: lowStock,
            c: lowStock > 0 ? "#F59E0B" : "#16A34A",
            mod: "inventory",
          },
          {
            l: "Out of Stock",
            v: outOfStock,
            c: outOfStock > 0 ? "#EF4444" : "#16A34A",
            mod: "inventory",
          },
          ...
          <div
            key={k.l}
            onClick={() => nav(k.mod)}
            className="bg-white rounded-xl border border-slate-200/80 p-4 text-center cursor-pointer hover:shadow-sm transition-all"
          >
            <p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">{k.l}</p>
            <p className="text-[22px] font-bold" style={{ color: k.c }}>
              {k.v}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          {byCat.length === 0 ? (
            <p className="text-slate-400 text-center py-8">No inventory data</p>
          ) : (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart
                data={byCat}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <ComposedChart data={invTrend} margin={{ left: -10, right: 4, top: 0, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(v, n) => [
                  n === "value" ? "TZS " + money(v) + "k" : v + " orders",
                  n === "value" ? "Stock Value" : "Active Orders",
                ]}
              />
              <Area
                type="monotone"
...
export function CustomKPIs({ data }) {
  ...
  async function addKpi(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("custom_kpis")
          .insert({ metric_id: draft.metricId, label: draft.label, target_value: draft.target })
          .single()
          .run();
        if (header?.id)
          setRows((prev) => prev.map((k) => (k.id === draft.id ? { ...k, dbId: header.id } : k)));
      } catch (_e) {
        notify("KPI added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteKpi(id) {
    ...
    if (IS_CONFIGURED && k?.dbId) {
      try {
        await sb("custom_kpis").eq("id", k.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the KPI on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Pick any real metric already computed elsewhere in this app and set your own target
          against it — the current value is never a separate calculation, just this same number
          shown against a goal you define.
        </p>
      </div>
      <div className="flex justify-end">
        <button
          onClick={() => setShowForm(true)}
          className="btn-primary text-white text-[13px] font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5 shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading &&
          Array.from({ length: 2 }).map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-xl border border-slate-200/80 h-32 skeleton-shimmer"
            />
          ))}
        {!loading &&
          rows.map((kpi) => {
            ...
            const pct =
              kpi.target > 0 ? Math.min(100, Math.round((current / kpi.target) * 100)) : 0;
            const color =
              pct >= 100 ? "#16A34A" : pct >= 60 ? "#22C55E" : pct >= 30 ? "#F59E0B" : "#EF4444";
            return (
              <div
                key={kpi.id}
                className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 relative group"
              >
                <button
                  onClick={() => deleteKpi(kpi.id)}
                  className="absolute top-3 right-3 text-slate-300 hover:text-[#EF4444] opacity-0 group-hover:opacity-100 transition-opacity"
                ...
                <div className="flex items-end justify-between mb-2">
                  <span className="text-[18px] font-mono font-bold" style={{ color }}>
                    {metric.unit === "TZS 000"
                      ? `${money(Math.round(current))}k`
                      : `${Math.round(current)}${metric.unit === "%" ? "%" : ""}`}
                  </span>
                  <span className="text-[11px] text-slate-400">
                    of{" "}
                    {metric.unit === "TZS 000"
                      ? `${money(kpi.target)}k`
                      : `${kpi.target}${metric.unit === "%" ? "%" : ""}`}{" "}
                    target
                  </span>
                </div>
        ...
        {!loading && rows.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
            <EmptyState
              ...
              hint="Define a metric and a target to track it here."
              actionLabel="New KPI"
...
export function KpiFormPanel({ onClose, onSubmit }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.label.trim() || !(Number(form.target) > 0)) return;
    onSubmit(form);
  }
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      <form
        ...
        className="relative w-full sm:w-[380px] bg-white h-full shadow-2xl overflow-y-auto flex flex-col"
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">New Custom KPI</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Your label for this KPI" required>
            <input
              className={inputClass}
              value={form.label}
          ...
          <FormField label={`Target (${selectedMetric?.unit})`} required>
            <input
              type="number"
              min="0"
              className={inputClass}
...
export function HeatMaps({ invoices, inventory }) {
  ...
  const maxStock = Math.max(
    ...stockGrid.categories.flatMap((cat) => WAREHOUSES.map((w) => stockGrid.grid[cat][w.id])),
    1,
  );
    ...
    <div className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
        ...
        <p className="text-[11.5px] text-slate-400 mb-4">
          Real invoice totals grouped by weekday — darker means more billed on that day, across all
          invoices on record
        </p>
        <div className="grid grid-cols-7 gap-2">
          {DAYS.map((day, i) => (
            <div key={day} className="text-center">
              <div
                className="h-20 rounded-lg flex items-center justify-center mb-1.5 border border-slate-100"
                style={{ backgroundColor: heatColor(salesByDay[i], maxDay) }}
              >
                <span className="text-[11px] font-mono font-semibold text-[#111827]">
                  {money(Math.round(salesByDay[i]))}
                </span>
              </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 overflow-x-auto">
        <h3 className="text-[14px] font-semibold text-[#111827] mb-1">
          Stock Value: Category × Warehouse
        </h3>
        <p className="text-[11.5px] text-slate-400 mb-4">
          Darker cells hold more stock value — a real way to spot which warehouse is over- or
          under-stocked in a category at a glance
        </p>
        <table className="w-full min-w-[480px] text-[12px]">
          <thead>
            <tr>
              ...
              {WAREHOUSES.map((w) => (
                <th key={w.id} className="text-center text-[11px] text-slate-400 font-medium pb-2">
                  {w.city}
                </th>
              ))}
            </tr>
              ...
              <tr key={cat}>
                ...
                {WAREHOUSES.map((w) => (
                  <td key={w.id} className="p-1">
                    <div
                      className="h-12 rounded-md flex items-center justify-center border border-slate-100"
                      style={{ backgroundColor: heatColor(stockGrid.grid[cat][w.id], maxStock) }}
                    >
                      <span className="text-[10.5px] font-mono text-[#111827]">
                        {money(Math.round(stockGrid.grid[cat][w.id]))}
                      </span>
                    </div>
                  </td>
                ))}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          Live ECB reference rates — refreshed on each visit. Useful for pricing against foreign
          suppliers and invoicing in foreign currency. Rates sourced from{" "}
          <span className="font-medium">frankfurter.app</span>.
        </p>
      </div>
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-[14px] font-semibold text-[#111827]">
                  1 {base} in other currencies
                </h3>
                <p className="text-[11.5px] text-slate-400">
                  As of {rates.date} · European Central Bank
                </p>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {Object.entries(rates.rates || {}).map(([currency, value]) => {
              ...
              return (
                <div
                  key={currency}
                  className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 text-center hover:shadow-md transition-shadow"
                >
                  <p className="text-[11px] text-slate-400 mb-1">1 {base} =</p>
                  <p className="text-[20px] font-mono font-bold text-[#111827] leading-tight">
                    {formatted}
                  </p>
                  <p className="text-[12px] font-semibold text-slate-600 mt-0.5">{currency}</p>
                  <p className="text-[10px] text-slate-300 mt-1">
                    1 {currency} = {inverse} {base}
                  </p>
                </div>
              );
            })}
...
export function Benchmarking({ data }) {
  const benchmarks = useCompanyTable("financial_benchmarks", benchmarksSeed, {
    mapRow: mapBenchmarkRow,
  });
  ...
  const revenue = data.invoices.rows.reduce((s, inv) => {
    const { total } = lineTotal(inv.items);
  ...
  const computedValues = {
    ...
    receivables_days: (() => {
      const outstanding = data.invoices.rows
  ...
  async function addBenchmark(form) {
    const draft = {
      ...
      metricId: form.metricId,
      label: form.label,
      benchmarkValue: Number(form.benchmarkValue) || 0,
    };
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("financial_benchmarks")
          .insert({
            metric_id: draft.metricId,
            label: draft.label,
            benchmark_value: draft.benchmarkValue,
          })
          .single()
          .run();
        if (header?.id)
          benchmarks.setRows((prev) =>
            prev.map((b) => (b.id === draft.id ? { ...b, dbId: header.id } : b)),
          );
      } catch (_e) {
        notify("Benchmark added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function addCompetitor(form) {
    ...
    if (IS_CONFIGURED) {
      try {
        const header = await sb("competitors")
          .insert({
            name: draft.name,
          ...
          .run();
        if (header?.id)
          competitors.setRows((prev) =>
            prev.map((c) => (c.id === draft.id ? { ...c, dbId: header.id } : c)),
          );
      } catch (_e) {
        notify("Competitor added locally, but saving to the server failed.", "error");
      }
    }
  }
  ...
  async function deleteCompetitor(id) {
    ...
    if (IS_CONFIGURED && c?.dbId) {
      try {
        await sb("competitors").eq("id", c.dbId).delete().run();
      } catch (_e) {
        notify("Couldn't delete the competitor on the server.", "error");
      }
    }
  }
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3 mb-4">
          ...
          <p className="text-[12px] text-slate-500 leading-relaxed">
            Benchmark values are entered by you, from your own research — there is no live
            industry-benchmark feed for East African SME sectors a generic app could connect to.
            Compare your real computed numbers against whatever figure you&apos;ve found credible.
          </p>
        </div>
        <div className="flex justify-between items-center mb-3">
          ...
          <button
            ...
            className="btn-secondary text-[12px] font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {benchmarks.rows.map((bm) => {
            ...
            return (
              <div
                key={bm.id}
                className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5"
              >
                <p className="text-[11px] text-slate-400 mb-1">{metric?.label}</p>
                <p className="text-[13.5px] font-semibold text-[#111827] mb-3">{bm.label}</p>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10.5px] text-slate-400">Yours</p>
                    <p className="text-[16px] font-mono font-bold text-[#111827]">
                      {current}
                      {metric?.unit === "%" ? "%" : ""}
                    </p>
                  </div>
                  <div
                    className={`text-[11px] font-medium px-2 py-1 rounded-full ${ahead ? "bg-[#16A34A]/10 text-[#16A34A]" : "bg-[#EF4444]/10 text-[#EF4444]"}`}
                  >
                    {ahead ? "Ahead" : "Behind"}
                  </div>
                  <div className="text-right">
                    <p className="text-[10.5px] text-slate-400">Benchmark</p>
                    <p className="text-[16px] font-mono font-bold text-slate-400">
                      {bm.benchmarkValue}
                      {metric?.unit === "%" ? "%" : ""}
                    </p>
                  </div>
          ...
          {!benchmarks.loading && benchmarks.rows.length === 0 && (
            <div className="col-span-full bg-white rounded-xl border border-slate-200/80 shadow-sm">
              <EmptyState
                icon={Crosshair}
                title="No benchmarks yet"
                hint="Add a figure you've researched to compare against."
                actionLabel="Add Benchmark"
                onAction={() => setShowBenchmarkForm(true)}
              />
            </div>
          )}
          {benchmarks.loading && (
            <div className="col-span-full text-center text-[12px] text-slate-400 py-6">
              Loading...
            </div>
          )}
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[13px] min-w-[600px]">
              ...
              <tbody>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          backgroundColor: `${THREAT_COLOR[c.threatLevel]}14`,
                ...
                {!competitors.loading && competitors.rows.length === 0 && (
                  <tr>
                    <td colSpan={4}>
                      <EmptyState
                        icon={Crosshair}
                        title="No competitors tracked yet"
                        hint="Log what you know about competitors here — manually, since no automated feed exists for this."
                        actionLabel="Add Competitor"
                        onAction={() => setShowCompetitorForm(true)}
                      />
                    </td>
                  </tr>
                )}
                {competitors.loading && (
                  <tr>
                    <td colSpan={4}>
                      <p className="text-[12px] text-slate-400 text-center py-6">Loading...</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-[13px] text-slate-500">{selectedCompetitor.category}</p>
              </div>
              <button
                onClick={() => setSelectedCompetitor(null)}
                className="text-slate-400 hover:text-slate-600"
            ...
            <span
              className="text-[11px] font-medium px-2 py-1 rounded-full w-fit mb-4"
              style={{
                backgroundColor: `${THREAT_COLOR[selectedCompetitor.threatLevel]}14`,
            ...
            <p className="text-[13px] text-slate-600 leading-relaxed mb-6">
              {selectedCompetitor.notes || "No notes yet."}
            </p>
            <div className="flex-1" />
            <ConfirmDeleteButton
              label="Remove competitor"
              onConfirm={() => deleteCompetitor(selectedCompetitor.id)}
            />
          </div>
        </div>
      )}
      ...
      {showBenchmarkForm && (
        <BenchmarkFormPanel onClose={() => setShowBenchmarkForm(false)} onSubmit={addBenchmark} />
      )}
      {showCompetitorForm && (
        <CompetitorFormPanel
          onClose={() => setShowCompetitorForm(false)}
...
export function BenchmarkFormPanel({ onClose, onSubmit }) {
  const [form, setForm] = useState({
    metricId: BENCHMARK_METRICS[0].id,
    label: "",
    benchmarkValue: "",
  });
  function set(key, val) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex justify-end">
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <p className="text-[11px] text-slate-400 uppercase tracking-wide">Benchmarking</p>
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Add Benchmark</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <FormField label="Label" required>
            <input
              ...
              value={form.label}
              onChange={(e) => set("label", e.target.value)}
              placeholder="e.g. Industry Gross Margin (Hardware Retail)"
            />
          </FormField>
          <FormField label="Benchmark value" required>
            <input
              type="number"
              className={inputClass}
          ...
          <p className="text-[11.5px] text-slate-400">
            From your own research — there&apos;s no live feed behind this number.
          </p>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
          <button
      ...
      >
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-[18px] font-semibold text-[#111827] mt-0.5">Add Competitor</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          <FormField label="Competitor name" required>
            <input
              ...
              placeholder="e.g. Coastal Building Supplies"
            />
          </FormField>
          ...
          <FormField label="Notes">
            <textarea
              ...
              placeholder="What you know: pricing, strengths, weaknesses..."
            />
          </FormField>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-2">
...
export function PredictiveIntelligence({
  ...
  const projectExpenses = useCompanyTable("project_expenses", projectExpensesSeed, {
    mapRow: mapProjectExpenseRow,
  });
  const machines = useCompanyTable("manufacturing_machines", machinesSeed, {
    mapRow: mapMachineRow,
  });
  const maintenance = useCompanyTable("manufacturing_maintenance", maintenanceSeed, {
    mapRow: mapMaintenanceRow,
  });
  ...
  const stockDepletion = useMemo(() => {
    const salesBySku = {};
    invoices.rows
      .filter((inv) => (TODAY - new Date(inv.date)) / 86400000 <= 60)
      .forEach((inv) => {
        inv.items.forEach((it) => {
          if (it.sku) salesBySku[it.sku] = (salesBySku[it.sku] || 0) + it.qty;
        });
      });
  ...
  const churnRisk = useMemo(() => {
    const byCustomer = {};
    invoices.rows.forEach((inv) => {
      (byCustomer[inv.customer] = byCustomer[inv.customer] || []).push(new Date(inv.date));
    });
    return Object.entries(byCustomer)
      ...
      .map(([customer, dates]) => {
        ...
        return {
          ...
          ratio: avgInterval > 0 ? daysSinceLast / avgInterval : 0,
        };
      })
      .filter((c) => c.ratio >= 2)
  ...
  const turnoverRisk = useMemo(() => {
    return employees.rows
      ...
      .map((e) => {
        ...
        const recentLeave = leaveRequests.rows.filter(
          (l) => l.employee === e.name && (TODAY - new Date(l.startDate)) / 86400000 <= 90,
        ).length;
        const flags = [];
        if (tenureDays !== null && tenureDays < 90)
          flags.push(
            "New hire (under 90 days) — the highest-risk tenure window in most attrition research",
          );
        if (recentLeave >= 3)
          flags.push(
            `${recentLeave} leave requests in the last 90 days — notably more frequent than typical`,
          );
  ...
  const salesGrowth = useMemo(() => {
    ...
    invoices.rows.forEach((inv) => {
      ...
      byMonth[key] =
        (byMonth[key] || 0) +
        (inv.status === "Paid" ? lineTotal(inv.items).total : inv.amountPaid || 0);
    });
  ...
  const budgetOverruns = useMemo(() => {
    return projects.rows
      .filter((p) => p.status !== "Completed" && p.status !== "Cancelled" && p.budget > 0)
      .map((p) => {
        const spent = projectExpenses.rows
        ...
        const overrunPct =
          p.budget > 0 ? Math.round(((projectedTotal - p.budget) / p.budget) * 100) : 0;
        return {
          name: p.name,
          budget: p.budget,
          spent,
  ...
  const fraudRisk = useMemo(() => {
    ...
    const structuring = expenses.rows.filter(
      (e) => e.amount >= PO_APPROVAL_THRESHOLD * 0.9 && e.amount < PO_APPROVAL_THRESHOLD,
    );
  ...
  const maintenanceNeeds = useMemo(() => {
    return machines.rows
      .map((m) => {
        const records = maintenance.rows
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          These are real statistical projections and rule-based risk heuristics computed from this
          company&apos;s own live data — not a trained machine-learning model, since no dataset here
          is large enough to train one on and no infrastructure exists to serve one. Every number
          below shows its own method; none of it is an unexplained confidence score.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Cash Shortage */}
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <Wallet size={14} className="text-[#EF4444]" /> Cash Shortage Projection
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            8-week outlook: known invoice due dates minus your trailing 8-week average expense rate
            (TZS {money(Math.round(cashProjection.weeklyExpenseRate))}k/week)
          </p>
          {cashProjection.shortageWeek ? (
            <div className="bg-[#FEE2E2] rounded-lg p-3">
              <p className="text-[12.5px] font-medium text-[#EF4444]">
                Projected shortfall in week {cashProjection.shortageWeek.week} — balance TZS{" "}
                {money(Math.round(cashProjection.shortageWeek.balance))}k
              </p>
            </div>
          ) : (
            <div className="bg-[#DCFCE7] rounded-lg p-3">
              <p className="text-[12.5px] font-medium text-[#16A34A]">
                No shortfall projected in the next 8 weeks at current rates.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <Package size={14} className="text-[#F59E0B]" /> Stock Depletion Forecast
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Real 60-day sales velocity per SKU, projected against current quantity
          </p>
          {stockDepletion.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              Nothing projected to run out within 21 days.
            </p>
          ) : (
            <div className="space-y-1.5">
              {stockDepletion.slice(0, 4).map((it) => (
                <div key={it.sku} className="flex items-center justify-between text-[12.5px]">
                  <span className="text-slate-600 truncate">{it.name}</span>
                  <span className="font-mono font-medium text-[#F59E0B] shrink-0 ml-2">
                    {it.daysLeft}d left
                  </span>
                </div>
              ))}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <Users size={14} className="text-[#F59E0B]" /> Customer Churn Risk
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Real RFM scoring — each customer flagged only against their own historical ordering
            rhythm, not a fixed rule
          </p>
          {churnRisk.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              No customers ordering meaningfully behind their own normal pace.
            </p>
          ) : (
            <div className="space-y-1.5">
              {churnRisk.slice(0, 4).map((c) => (
                <div key={c.customer} className="flex items-center justify-between text-[12.5px]">
                  <span className="text-slate-600 truncate">{c.customer}</span>
                  <span className="font-mono font-medium text-[#F59E0B] shrink-0 ml-2">
                    {c.daysSinceLast}d since last (avg {c.avgInterval}d)
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <Users size={14} className="text-[#5B6472]" /> Employee Turnover Risk
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            The weakest signal here — real, but thin: tenure and leave frequency only. No engagement
            or performance-trend data exists to build a stronger one.
          </p>
          {turnoverRisk.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              No employees matching either real risk signal.
            </p>
          ) : (
            <div className="space-y-2">
              {turnoverRisk.slice(0, 3).map((e) => (
                <div key={e.name} className="text-[12px]">
                  ...
                  <p className="text-slate-500 mt-0.5">{e.flags[0]}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <TrendingUp size={14} className="text-[#16A34A]" /> Sales Growth Projection
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Real linear trend across actual monthly revenue — not a single period-over-period guess
          </p>
          {salesGrowth.growthRate === null ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              Not enough months of revenue history yet for a trend line.
            </p>
          ) : (
            <div>
              <p
                className={`text-[16px] font-mono font-bold ${salesGrowth.growthRate >= 0 ? "text-[#16A34A]" : "text-[#EF4444]"}`}
              >
                {salesGrowth.growthRate >= 0 ? "+" : ""}
                {salesGrowth.growthRate}%/month trend
              </p>
              <p className="text-[11.5px] text-slate-500 mt-1">
                Next month projected at TZS{" "}
                {money(Math.round(Math.max(0, salesGrowth.nextMonthProjection)))}k
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <ClipboardList size={14} className="text-[#0EA5E9]" /> Budget Overrun Risk
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Real burn rate (spend ÷ time elapsed) projected across each project&apos;s full real
            timeline
          </p>
          {budgetOverruns.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              No active projects trending over budget by more than 5%.
            </p>
          ) : (
            <div className="space-y-1.5">
              {budgetOverruns.slice(0, 4).map((p) => (
                <div key={p.name} className="flex items-center justify-between text-[12.5px]">
                  ...
                  <span className="font-mono font-medium text-[#0EA5E9] shrink-0 ml-2">
                    +{p.overrunPct}% projected
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <AlertCircle size={14} className="text-[#EF4444]" /> Fraud Risk Indicators
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Real unusual-expense detection (section 33) plus "structuring" — amounts clustering just
            under your PO approval threshold
          </p>
          {fraudRisk.unusual.length === 0 && fraudRisk.structuring.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              No unusual or structuring patterns detected.
            </p>
          ) : (
            <div className="space-y-1">
              {fraudRisk.unusual.slice(0, 2).map((e) => (
                <p key={e.id} className="text-[12px] text-slate-600">
                  {e.vendor} — {e.multiple}× {e.category} average
                </p>
              ))}
              {fraudRisk.structuring.slice(0, 2).map((e) => (
                <p key={e.id} className="text-[12px] text-slate-600">
                  {e.vendor} — TZS {money(e.amount)}k, just under approval threshold
                </p>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          <h3 className="text-[13.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
            <Factory size={14} className="text-[#5B6472]" /> Maintenance Needs
          </h3>
          <p className="text-[11px] text-slate-400 mb-3">
            Directly real — each machine&apos;s own logged next-due date, not a modeled estimate
          </p>
          {maintenanceNeeds.length === 0 ? (
            <p className="text-[12.5px] text-slate-400 py-2">
              No machines due for maintenance within 21 days.
            </p>
          ) : (
            <div className="space-y-1.5">
              {maintenanceNeeds.map((m) => (
                <div key={m.machine} className="flex items-center justify-between text-[12.5px]">
                  ...
                  <span
                    className={`font-mono font-medium shrink-0 ml-2 ${m.daysUntil < 0 ? "text-[#EF4444]" : "text-[#F59E0B]"}`}
                  >
                    {m.daysUntil < 0 ? `${-m.daysUntil}d overdue` : `${m.daysUntil}d`}
                  </span>
                </div>
              ))}
            </div>
...
export function ScenarioPlanner({ invoices, expenses, employees }) {
  ...
  const baseline = useMemo(() => {
    ...
    const avgSalary =
      activeEmployees.length > 0
        ? activeEmployees.reduce((s, e) => s + e.salary, 0) / activeEmployees.length
        : 0;
  ...
  const result = useMemo(() => {
    if (scenario === "price") {
      ...
      return {
        lines: [
          ...
          {
            label: `Projected revenue at ${priceChangePct >= 0 ? "+" : ""}${priceChangePct}%`,
            value: newRevenue,
            highlight: true,
          },
        ...
        caveat:
          "Assumes sales volume stays the same — a real price change often shifts volume too, in either direction, which this can't predict.",
      };
    }
    if (scenario === "hiring") {
      ...
      return {
        lines: [
          {
            label: `Real average salary (${baseline.employeeCount} active employees)`,
            value: baseline.avgSalary,
          },
          {
            label: `Added monthly payroll for ${newHires} hire${newHires === 1 ? "" : "s"}`,
            value: addedCost,
            highlight: true,
          },
        ...
        caveat:
          baseline.employeeCount === 0
            ? "No active employees on record yet — this uses a real average of zero, which won't be meaningful until real payroll data exists."
            : "Uses this company's own real average salary, not an assumed figure — a specialized role could cost meaningfully more or less than this average.",
      };
    }
    ...
    return {
      ...
      caveat:
        "Assumes the cut doesn't affect revenue or operations — cutting some categories (marketing, maintenance) can have real knock-on effects this simple model doesn't capture.",
    };
  }, [scenario, priceChangePct, newHires, expenseCategory, expenseCutPct, baseline, expenses]);

  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
      <h3 className="text-[14.5px] font-semibold text-[#111827] mb-1 flex items-center gap-1.5">
        <Sparkles size={15} className="text-[#16A34A]" /> Scenario Planner — "What If"
      </h3>
      <p className="text-[12px] text-slate-500 mb-4">
        Real arithmetic on this business&apos;s own real numbers — not a prediction, a calculator.
        Model a decision before making it.
      </p>

      <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit mb-4">
        {[
          { id: "price", label: "Price Change" },
          { id: "hiring", label: "New Hire" },
          { id: "expense", label: "Cut an Expense" },
        ].map((s) => (
          <button
            key={s.id}
            onClick={() => setScenario(s.id)}
            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-colors ${scenario === s.id ? "bg-white text-[#111827] shadow-sm" : "text-slate-500"}`}
          >
            {s.label}
          </button>
        ))}
          ...
          {scenario === "price" && (
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-1.5">
                A negative number models a price cut.
              </p>
            </div>
          )}
          {scenario === "hiring" && (
            <div>
              ...
              <input
                ...
                step="1"
                value={newHires}
                onChange={(e) => setNewHires(Math.max(1, Number(e.target.value) || 1))}
                className={inputClass}
              />
              <p className="text-[11px] text-slate-400 mt-1.5">
                Uses this company's own real average salary across {baseline.employeeCount} active
                employees.
              </p>
            </div>
          )}
          {scenario === "expense" && (
            <div className="space-y-3">
              <div>
                ...
                <select
                  value={expenseCategory || baseline.categories[0] || ""}
              ...
              <div>
                ...
                <input
                  ...
                  step="5"
                  value={expenseCutPct}
                  onChange={(e) => setExpenseCutPct(Number(e.target.value) || 0)}
                  className={inputClass}
                />
              </div>
            </div>
        ...
        <div className="bg-slate-50 rounded-lg p-3.5">
          <div className="space-y-1.5">
            {result.lines.map((l, i) => (
              <div
                key={i}
                className={`flex justify-between text-[12.5px] ${l.bold ? "font-semibold text-[#111827] pt-1.5 border-t border-slate-200" : l.highlight ? "font-medium text-[#16A34A]" : "text-slate-600"}`}
              >
                <span>{l.label}</span>
          ...
          <p className="text-[10.5px] text-slate-400 mt-3 pt-3 border-t border-slate-100">
            {result.caveat}
          </p>
        </div>
      </div>
