    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        ...
        <p className="text-[12px] text-slate-400 mb-4">
          For an invoice reference, a payment link, or any text — genuinely real and scannable
        </p>
        ...
        {qrUrl && (
          <div className="mt-4 flex flex-col items-center gap-3 bg-slate-50 rounded-lg p-4">
            <img
              ...
              width={180}
              height={180}
              className="rounded-lg bg-white p-2 border border-slate-200"
            />
            <a
              href={qrUrl}
              download="qrcode.png"
              className="text-[12px] font-medium text-[#16A34A] hover:text-[#15803D] flex items-center gap-1.5"
            >
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        ...
        <p className="text-[12px] text-slate-400 mb-4">
          Already real and live — every Inventory item gets a deterministic barcode automatically
        </p>
        <p className="text-[13px] text-slate-600 leading-relaxed mb-4">
          Inventory generates an EAN-13-style code from each item&apos;s SKU — the same code every
          time, searchable at checkout in POS. No separate barcode system needed here; it&apos;s
          already wired into Inventory and POS.
        </p>
        {onNavigate && (
          <button
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          A real, working signature pad for lightweight internal sign-off — not a certified
          e-signature platform. It doesn&apos;t verify identity, seal against tampering, or produce
          the legal audit trail DocuSign or Adobe Sign provide; use one of those for anything
          requiring that.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <FormField label="Document reference">
            <input
              ...
              placeholder="e.g. QT-1043 or PC-02"
            />
          </FormField>
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button
            ...
            disabled={!hasDrawn || !documentRef.trim() || !signerName.trim()}
            ...
            {rows.map((s) => (
              <div key={s.id} className="flex items-center gap-4 px-4 sm:px-5 py-3">
                <img
                  ...
                  className="h-10 bg-white border border-slate-100 rounded"
                />
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-medium text-[#111827]">{s.signerName}</p>
                  <p className="text-[11px] text-slate-400">
                    {s.documentRef} · {new Date(s.signedAt).toLocaleString()}
                  </p>
                </div>
              </div>
            ))}
