...
export function PostCreateDispatch({ company, crm }) {
  ...
  const lead = (crm?.rows || []).find(
    (l) => (l.company || l.contact || "").toLowerCase() === (inv.customer || "").toLowerCase(),
  );
  ...
  const waMsg = [
    ...
    inv.items
      ?.slice(0, 3)
      .map((it) => `  • ${it.name}  ×${it.qty}  @ TZS ${fmt(it.rate)}`)
      .join("\n"),
    inv.items?.length > 3
      ? `  • …and ${inv.items.length - 3} more item${inv.items.length - 3 > 1 ? "s" : ""}`
      : null,
    ``,
  ...
  const emailBody = [
    ...
    `Items:`,
    ...(inv.items || []).map(
      (it) => `  ${it.name}  ×${it.qty}  TZS ${fmt((it.qty || 1) * (it.rate || 0))}`,
    ),
    ``,
    `Payment Details:`,
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 transition-all duration-280 ${closing ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}
      ...
      <div className="w-[340px] bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-slate-100" style={{ background: "#0D2214" }}>
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-[#16A34A] font-mono font-bold text-[15px] mt-0.5">
                TZS {fmt(total)}
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-[rgba(255,255,255,.3)] text-[10px] mt-0.5">
          ...
          <div className="flex gap-1.5 mt-2.5 flex-wrap">
            {(inv.items || []).slice(0, 3).map((it, i) => (
              <span
                key={i}
                className="text-[10.5px] text-[rgba(255,255,255,.6)] bg-[rgba(255,255,255,.08)] px-2 py-0.5 rounded-full"
              >
                {it.name.length > 18 ? it.name.slice(0, 16) + "…" : it.name}
              </span>
            ))}
            {(inv.items?.length || 0) > 3 && (
              <span className="text-[10.5px] text-[rgba(255,255,255,.4)] px-1">
        ...
        <div className="px-4 pt-3 pb-1">
          ...
          <p className="text-[10.5px] text-slate-400">
            Choose one or more channels — each can be used independently
          </p>
        </div>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <p
                    ...
                    style={{ color: done ? a.color : "#111827" }}
                  >
                ...
                {done && (
                  <span className="text-[11px] font-black shrink-0" style={{ color: a.color }}>
                    ✓
                  </span>
                )}
              </button>
...
export function SendReceiptPanel() {
  ...
  const msg = encodeURIComponent(
    "✅ Receipt — " +
      ...
      receipt.customer +
      ",\n\nPayment of TZS " +
      money(Math.round(receipt.amount)) +
      "k received on " +
      receipt.date +
      " via " +
      receipt.method +
  ...
  function sendViaWhatsApp() {
    ...
    setSent((s) => ({ ...s, whatsapp: true }));
    notify(
      `WhatsApp opened for ${receipt.customer} — hit Send in WhatsApp to deliver the receipt.`,
    );
  }
  ...
  function printReceipt() {
    ...
    const fmt = (n) => new Intl.NumberFormat("en-US").format(Math.round(n));
    const subtotalVal = (receipt.items || []).reduce(
    ...
    const itemRows = (receipt.items || [])
      .map(
        (it) =>
          '<tr><td style="padding:7px 10px;border-bottom:1px solid #F3F4F6">' +
          it.name +
          "</td>" +
          '<td style="padding:7px 10px;border-bottom:1px solid #F3F4F6;text-align:center;font-family:monospace">' +
          (it.qty || 1) +
          "</td>" +
          '<td style="padding:7px 10px;border-bottom:1px solid #F3F4F6;text-align:right;font-family:monospace">' +
          fmt(it.rate || 0) +
          "k</td>" +
          '<td style="padding:7px 10px;border-bottom:1px solid #F3F4F6;text-align:right;font-family:monospace;font-weight:600">' +
    ...
    const logoHtml = co.logo
      ? '<img src="' +
        co.logo +
        '" style="height:44px;object-fit:contain;filter:brightness(0) invert(1)" alt="logo"/>'
    ...
    printAsPDF(
      ...
      '<div style="font-family:Inter,sans-serif;max-width:520px;margin:0 auto;background:white">' +
        ...
        logoHtml +
        '<div><div style="font-size:16px;font-weight:800;color:white">' +
        (co.name || "Smart Manager") +
        "</div>" +
        (co.tagline
          ? '<div style="font-size:10px;color:rgba(255,255,255,.7);font-style:italic">' +
            co.tagline +
            "</div>"
          : "") +
        "</div></div>" +
        ...
        '<div style="font-size:15px;font-weight:900;color:white;margin-top:2px">' +
        (receipt.id || docId("RCT")) +
        "</div></div>" +
        "</div>" +
        (detailStrip
          ? '<div style="background:#F0FDF4;padding:8px 28px;font-size:10.5px;color:#166534">' +
            detailStrip +
            "</div>"
          : "") +
        ...
        '<div style="font-size:11.5px;color:#111827">Invoice: <strong>' +
        receipt.invoiceId +
        "</strong></div>" +
        '<div style="font-size:11px;color:#6B7280">Date: ' +
        receipt.date +
        "</div>" +
        '<div style="font-size:11px;color:#6B7280">Method: ' +
        receipt.method +
        "</div>" +
        (receipt.reference
          ? '<div style="font-size:11px;color:#6B7280">Ref: ' + receipt.reference + "</div>"
          : "") +
        "</div>" +
        "</div>" +
        ...
        '<div style="display:flex;justify-content:space-between;font-size:11.5px;color:#6B7280;margin-bottom:4px"><span>Subtotal</span><span>TZS ' +
        fmt(subtotalVal) +
        "k</span></div>" +
        '<div style="display:flex;justify-content:space-between;font-size:11.5px;color:#6B7280;margin-bottom:10px"><span>VAT (18%)</span><span>TZS ' +
        fmt(taxVal) +
        "k</span></div>" +
        ...
        '<span style="font-size:18px;font-weight:900;color:#4ADE80">TZS ' +
        fmt(receipt.amount) +
        "k</span>" +
        "</div>" +
        "</div>" +
        ...
        (co.website
          ? '<div style="font-size:11px;color:#16A34A;margin-bottom:4px">' + co.website + "</div>"
          : "") +
        '<div style="font-size:10px;color:#9CA3AF">Generated by Smart Manager · ' +
  ...
  const CHANNELS = [
    {
      ...
      fn: sendViaWhatsApp,
      need: phone,
      hint: "wa.me link — opens WhatsApp, you tap Send",
    },
    {
      id: "email",
      label: "Email",
      ...
      fn: sendViaEmail,
      need: email,
      hint: "mailto: link — opens your email client",
    },
    {
      id: "sms",
      label: "SMS",
      ...
      fn: sendViaSMS,
      need: phone,
      hint: "sms: link — opens your SMS app",
    },
  ];

  return (
    <div
      className="fixed bottom-24 sm:bottom-6 left-4 z-50 w-[calc(100vw-2rem)] sm:w-[400px]"
      style={{ animation: "toastIn .25s cubic-bezier(.34,1.4,.64,1)" }}
    >
      <div className="bg-white rounded-2xl shadow-2xl border border-[#16A34A]/20 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10.5px] text-white/70">
                {receipt.customer} · TZS {money(Math.round(receipt.amount))}k · {receipt.invoiceId}
              </p>
            </div>
          </div>
          <button onClick={() => setReceipt(null)} className="text-white/60 hover:text-white">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10.5px] text-slate-500 block mb-1">
                Phone (WhatsApp / SMS)
              </label>
              <input
            ...
            <div>
              <label className="text-[10.5px] text-slate-500 block mb-1">Email</label>
              <input
                className={inputClass}
                value={email}
            ...
            {CHANNELS.map((ch) => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  title={
                    disabled ? `Enter ${ch.id === "email" ? "email" : "phone"} first` : ch.hint
                  }
                  ...
                  style={{
                    backgroundColor: done ? ch.bg : "white",
                    borderColor: done ? ch.border : "#E2E8F0",
                    color: done ? ch.color : "#6B7280",
                  }}
                >
          ...
          <p className="text-[10px] text-slate-400 text-center">
            Each button opens your device&apos;s app — confirm send there. Automated sending needs
            AfricasTalking/SendGrid backend.
          </p>

          {/* Print PDF */}
          <button
            onClick={printReceipt}
            className="w-full flex items-center justify-center gap-2 text-[12.5px] font-medium border border-slate-200 rounded-xl py-2.5 hover:bg-slate-50 transition-colors text-slate-600"
          >
...
export function ConfirmDialog() {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-slate-200/60"
        onClick={(e) => e.stopPropagation()}
      >
        <div
          className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 ${danger ? "bg-red-50" : "bg-[#DCFCE7]"}`}
        >
          {danger ? (
            <AlertCircle size={22} className="text-[#EF4444]" />
          ) : (
            <AlertCircle size={22} className="text-[#16A34A]" />
          )}
        </div>
        <h3
          className="text-[16px] font-bold text-[#111827] text-center mb-2"
        ...
        <div className="flex gap-3">
          <button
            onClick={() => setDialog(null)}
            className="flex-1 py-2.5 rounded-xl text-[13px] font-medium border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            <div className="h-[2px] w-full" style={{ background: `${s.accent}30` }}>
              <div
                ...
                style={{
                  ...
                  animation: `toastDrain ${TOAST_DURATION}ms linear forwards`,
                }}
              />
            </div>
