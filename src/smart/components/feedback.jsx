...
// ─── PostCreateDispatch ──────────────────────────────────────────────
export const PostCreateDispatch = memo(function PostCreateDispatch({ company, crm }) {
  const [inv, setInv] = useState(null);
  const [sent, setSent] = useState({});
  const [visible, setVis] = useState(false);
  const [closing, setClg] = useState(false);
  ...
  useEffect(() => {
    ...
    return () => {
      invoiceCreatedBus.listeners.delete(handler);
      if (dismissTimerRef.current) {
        clearTimeout(dismissTimerRef.current);
        dismissTimerRef.current = null;
      }
  ...
  useEffect(() => {
    if (!visible) return;
    dismissTimerRef.current = setTimeout(dismiss, 60000);
    return () => {
      if (dismissTimerRef.current) {
        clearTimeout(dismissTimerRef.current);
        dismissTimerRef.current = null;
  ...
  const co = company || (typeof window !== "undefined" ? window.__smartManagerCompany : {}) || {};
  ...
  const waMsg = [
    ...
    ``,
    ...(inv.items || []).slice(0, 3).map((it) => `  • ${it.name}  ×${it.qty}  @ TZS ${fmt(it.rate)}`),
  ...
  }, [email, emailSubject, emailBody, inv, co.owner]);
  ...
  const ACTIONS = [
    { id: "whatsapp", label: phone ? "Send via WhatsApp" : "WhatsApp Center", sub: phone ? phone : "No phone — opens WA Center", icon: "📱", color: "#25D366", bg: "#F0FFF4", border: "#D1FAE5", fn: sendWhatsApp },
    { id: "email", label: email ? "Send via Email" : "Email Center", sub: email ? email : "No email — opens Email Center", icon: "✉️", color: "#2563EB", bg: "#EFF6FF", border: "#BFDBFE", fn: sendEmail },
    { id: "print", label: "Print / Save PDF", sub: "Professional invoice PDF", icon: "🖨", color: "#374151", bg: "#F8FAFB", border: "#E5E7EB", fn: printNow },
    { id: "link", label: "Copy Payment Link", sub: `pay.… / ${inv.id}`, icon: "🔗", color: "#7C3AED", bg: "#F5F3FF", border: "#DDD6FE", fn: copyPayLink },
  ];
    ...
    >
      <div className="w-[340px] bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden">
          ...
          {ACTIONS.map((a) => {
            ...
            return (
              <button
                ...
                style={{ background: done ? a.bg : "white", borderColor: done ? a.color + "40" : "#E5E7EB" }}
              >
                <div className="w-9 h-9 rounded-xl flex items-center justify-center text-[19px] shrink-0" style={{ background: a.bg, border: `1.5px solid ${a.border}` }}>
                  {done ? "✓" : a.icon}
                </div>
                ...
                {done && <span className="text-[11px] font-black shrink-0" style={{ color: a.color }}>✓</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="px-3 pb-3 flex gap-2">
          <button onClick={dismiss} className="flex-1 text-[12px] font-medium text-slate-500 border border-slate-200 rounded-xl py-2 hover:bg-slate-50">Dismiss</button>
          {Object.keys(sent).length > 0 && (
            <button onClick={dismiss} className="flex-1 text-[12px] font-bold text-white rounded-xl py-2 bg-[#16A34A]">✓ Done · Close</button>
          )}
        </div>
      </div>
      <style>{`@keyframes slideUp{from{opacity:0;transform:translateY(24px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}`}</style>
    </div>
  );
});

// ─── SendReceiptPanel ──────────────────────────────────────────────────
export const SendReceiptPanel = memo(function SendReceiptPanel() {
  ...
  const msg = encodeURIComponent(
    ...
    "\n\nDear " + receipt.customer + ",\n\nPayment of TZS " + money(Math.round(receipt.amount)) +
    "k received on " + receipt.date + " via " + receipt.method + refPart +
    ".\n\nThank you for your business!\n\n— SmartManager"
  );
  ...
  }, [email, subject, msg, receipt.customer]);
  ...
  const printReceipt = useCallback(() => {
    const co = typeof window !== "undefined" ? window.__smartManagerCompany || {} : {};
    const fmt = (n) => new Intl.NumberFormat("en-US").format(Math.round(n));
    ...
    printAsPDF("Receipt " + receipt.invoiceId,
      ...
      (co.tagline ? '<div style="font-size:10px;color:rgba(255,255,255,.7);font-style:italic">' + co.tagline + "</div>" : "") +
      "</div></div>" +
      ...
      (receipt.reference ? '<div style="font-size:11px;color:#6B7280">Ref: ' + receipt.reference + "</div>" : "") +
      "</div></div>" +
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-[#16A34A]/20 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10.5px] text-slate-500 block mb-1">Phone (WhatsApp / SMS)</label>
              <input className={inputClass} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+255 7XX XXX XXX" />
            </div>
            <div>
              <label className="text-[10.5px] text-slate-500 block mb-1">Email</label>
              <input className={inputClass} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="customer@email.com" />
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" onClick={() => setDialog(null)}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-slate-200/60" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-3">
          <button onClick={() => setDialog(null)} className="flex-1 py-2.5 rounded-xl text-[13px] font-medium border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">Cancel</button>
          <button
            ...
            style={{
              background: danger ? "linear-gradient(135deg,#EF4444,#DC2626)" : "linear-gradient(135deg,#16A34A,#22C55E)",
              boxShadow: danger ? "0 4px 12px rgba(239,68,68,0.3)" : "0 4px 12px rgba(22,163,74,0.3)"
            }}
          >
...
export const Toasts = memo(function Toasts() {
  ...
  useEffect(() => {
    const onToast = (t) => {
      ...
      const toast = { ...t, id, born: Date.now() };
    ...
    return () => {
      toastBus.listeners.delete(onToast);
      ...
      {toasts.map((t) => {
        ...
        return (
          <div
            ...
            style={{ animation: "toastIn .22s cubic-bezier(.34,1.4,.64,1)", backdropFilter: "blur(16px)", background: s.bg, border: `1px solid ${s.accent}28` }}
          >
