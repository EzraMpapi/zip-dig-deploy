...
export function DailyBriefing({
  ...
  const data = useMemo(() => {
    ...
    const newLeads = leads.filter(
      (l) => l.createdAt?.slice(0, 10) === today || l.date?.slice(0, 10) === today,
    );
    const openOpps = leads.filter((l) => !["Won", "Lost"].includes(l.stage));
    const pipeVal = openOpps.reduce((s, l) => s + (l.value || 0), 0);

    // HR
    const emps = Array.isArray(employees)
      ? employees
      : Array.isArray(employees?.rows)
        ? employees.rows
        : [];
    const activeEmps = emps.filter((e) => e.status === "Active");
    const onLeave = (leaveRequests?.rows || []).filter(
      (l) => l.status === "Approved" && l.startDate <= today && l.endDate >= today,
    );
    const expContracts = emps.filter(
      (e) =>
        e.contractEndDate &&
        e.contractEndDate <= new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10),
    );

    // MANUFACTURING
    const wos = workOrders?.rows || [];
    const overdueWO = wos.filter(
      (w) => w.status !== "Completed" && w.status !== "Cancelled" && w.dueDate < today,
    );
    ...
    const subsDue = subs.filter(
      (s) =>
        s.status === "Active" &&
        s.nextBillingDate &&
        s.nextBillingDate <= new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10),
    );
    const MRR = subs
  ...
  function printBriefing() {
    ...
    const lowStockRows = lowStock
      .slice(0, 10)
      .map(
        (it, i) => `<tr style="background:${i % 2 === 0 ? "#fff" : "#FEF2F2"}">
      ...
      <div class="page">
        ...
        ${
          ...
          <table class="data"><thead><tr>
            ...
            : `<div class="section"><div class="sec-hdr"><span class="sec-icon">✅</span><span class="sec-title" style="color:#16A34A">No Overdue Invoices</span></div></div>`
        }
        ...
        ${
          ...
          <table class="data"><thead><tr>
            ...
            : `<div class="section"><div class="sec-hdr"><span class="sec-icon">✅</span><span class="sec-title" style="color:#16A34A">All Stock Levels Healthy</span></div></div>`
        }
        ...
        <div class="section">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">
            ...
            <div style="background:${onLeave.length > 0 ? "#FFFBEB" : "#F8FAFB"};border-radius:10px;padding:14px">
              ...
              ${onLeave
                ...
                .map(
                  (l) =>
                    `<div style="font-size:10.5px;color:#92400E;margin-top:3px">${l.employeeName || l.employee || "—"}</div>`,
                )
                .join("")}
            </div>
          ...
          ${
            ...
            ${overdueWO
              ...
              .map(
                (w) =>
                  `<div style="font-size:11.5px;color:#374151;margin-bottom:2px">• ${w.productName || w.title || w.id} — due ${w.dueDate}</div>`,
              )
              .join("")}
          </div>`
              : ""
        ...
        <div class="section">
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
            ${[
              ...
              .map(
                ([l, v, col]) => `<div style="background:#F8FAFB;border-radius:10px;padding:14px">
          ...
          ${
            subsDue.length > 0
              ? `<div style="margin-top:12px;background:#FFFBEB;border-radius:10px;padding:12px">
            ...
            ${subsDue
              ...
              .map(
                (s) =>
                  `<div style="font-size:11.5px;color:#374151;margin-bottom:2px">• ${s.customer} — ${s.plan} — ${s.nextBillingDate}</div>`,
              )
              .join("")}
          </div>`
              : ""
  ...
  return (
    <div
      ...
      style={{ background: "rgba(13,34,20,0.7)", backdropFilter: "blur(4px)" }}
    >
      <div
        className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] mx-4 flex flex-col overflow-hidden"
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[rgba(255,255,255,.5)] text-[12.5px] mt-1.5">
                Here is your daily business briefing for {co.name || "your company"}
              </p>
            </div>
          ...
          {(criticals.length > 0 || highs.length > 0) && (
            <div className="flex gap-2 mt-3 flex-wrap">
              {criticals.length > 0 && (
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-white bg-[#EF4444] px-3 py-1 rounded-full">
                  🚨 {criticals.length} Critical
                </span>
              )}
              {highs.length > 0 && (
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-[#111827] bg-[#F59E0B] px-3 py-1 rounded-full">
                  ⚠ {highs.length} High
                </span>
              )}
              {lowStock.length > 0 && (
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-[#EF4444] bg-[#FEF2F2] border border-[#FECACA] px-3 py-1 rounded-full">
                  📦 {lowStock.length} Low Stock
                </span>
              )}
              {overdueInvs.length > 0 && (
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-[#F59E0B] bg-[#FFFBEB] border border-[#FDE68A] px-3 py-1 rounded-full">
                  ⏰ {overdueInvs.length} Overdue Invoices
                </span>
              )}
            </div>
          )}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <span
                            ...
                            style={{ background: ac.col, color: "white" }}
                          >
                            {ac.label}
                          </span>
