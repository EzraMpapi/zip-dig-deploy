  ...
  expenseRows.forEach((e) => {
    ...
    if (avg > 0 && e.amount > avg * 2) {
      flagged.push({
        ...
        category_average_tzs_k: Math.round(avg),
        date: e.date,
        multiple: Math.round((e.amount / avg) * 10) / 10,
      });
    }
  });
...
export function useBusinessAlerts({
  ...
  return useMemo(() => {
    ...
    const overdue = invoices.rows.filter(
    ...
    if (unusualExpenses.length) {
      alerts.push({
        ...
        subtitle: unusualExpenses
          .slice(0, 2)
          .map((e) => `${e.vendor} (${e.multiple}× ${e.category} average)`)
          .join(", "),
      });
    }
      ...
      alerts.push({
        ...
        subtitle:
          pendingLeave
            .slice(0, 2)
            .map((l) => l.employee)
            .join(", ") + (pendingLeave.length > 2 ? "…" : ""),
      });
    }

    const overdueOrders = workOrders.rows.filter(
      (w) =>
        w.status !== "Completed" && w.status !== "Cancelled" && w.dueDate && w.dueDate < todayStr,
    );
      ...
      alerts.push({
        ...
        subtitle:
          overdueOrders
            .slice(0, 2)
            .map((w) => w.product)
            .join(", ") + (overdueOrders.length > 2 ? "…" : ""),
      });
    }

    const dueSubscriptions = subscriptions.rows.filter(
      (s) => s.status === "Active" && s.nextBillingDate < todayStr,
    );
      ...
      alerts.push({
        ...
        subtitle:
          dueSubscriptions
            .slice(0, 2)
            .map((s) => s.customer)
            .join(", ") + (dueSubscriptions.length > 2 ? "…" : ""),
      });
    }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {alerts.length > 0 && (
          <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 px-0.5 bg-[#EF4444] rounded-full ring-1 ring-white flex items-center justify-center text-[9px] font-bold text-white leading-none">
        ...
        <>
          ...
          >
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              {alerts.length > 0 && (
                <span className="text-[11px] text-slate-400 font-mono">{alerts.length}</span>
              )}
            </div>
            ...
            <div className="px-3 py-2.5 border-t border-slate-100">
              <button
                onClick={() => {
                  setOpen(false);
                  if (window.__openDailyBrief) window.__openDailyBrief();
                }}
...
export const PALETTE_ACTIONS = [
  {
    ...
    module: "sales",
    intent: { tab: "invoices", openForm: true },
  },
...
export function CommandPalette({
  ...
  const actionResults = PALETTE_ACTIONS.filter((a) => modules.some((m) => m.id === a.module)) // only real actions for modules this user can actually reach
    ...
    .map((a) => ({
      ...
      icon: Zap,
      kind: "Quick action",
      action: () => (a.intent ? onNavigateWithIntent(a.module, a.intent) : onNavigate(a.module)),
    }));
  ...
  for (const t of q.split(/\s+/).filter(Boolean)) {
    ...
    if (mi >= 0 && monthKey === null)
      monthKey = `${TODAY.getFullYear()}-${String(mi + 1).padStart(2, "0")}`;
      ...
      : [
          ...(canSee("crm")
            ? (crm?.rows || [])
                .filter(
                  (l) =>
                    l.company.toLowerCase().includes(q) || (l.name || "").toLowerCase().includes(q),
                )
                .slice(0, 4)
                .map((l) => ({
                  id: `rec-lead-${l.id}`,
                  ...
                  icon: Building2,
                  kind: "Customer / Lead",
                  action: () => onNavigateWithIntent("crm", { tab: "leads" }),
                }))
            : []),
          ...(canSee("sales")
            ? (invoices?.rows || [])
                ...
                .map((inv) => ({
                  id: `rec-inv-${inv.id}`,
                  label: inv.id,
                  sub: `${inv.customer} · ${inv.status}`,
                  icon: ReceiptText,
                  kind: "Invoice",
                  action: () => onNavigateWithIntent("sales", { tab: "invoices" }),
                }))
            : []),
          ...(canSee("inventory")
            ? (inventory?.rows || [])
                ...
                .map((it) => ({
                  id: `rec-item-${it.sku}`,
                  label: it.name,
                  sub: `${it.sku} · ${it.qty} in stock`,
                  icon: Package,
                  kind: "Product",
                  action: () => onNavigateWithIntent("inventory", { tab: "stock" }),
                }))
            : []),
          ...(canSee("finance")
            ? (expenses?.rows || [])
                .filter((e) => {
                  const matchText =
                    !expText ||
                    e.vendor.toLowerCase().includes(expText) ||
                    (e.category || "").toLowerCase().includes(expText);
                ...
                .map((e) => ({
                  id: `rec-exp-${e.id}`,
                  label: e.vendor,
                  sub: `${e.category} · ${e.date} · TZS ${money(Math.round(e.amount))}k`,
                  icon: ClipboardList,
                  kind: "Expense",
                  action: () => onNavigateWithIntent("finance", { tab: "expenses" }),
                }))
            : []),
        ];
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh]">
      ...
      <div
        ...
        style={{ animation: "fadeInUp .15s ease-out" }}
      >
        ...
        <div className="max-h-80 overflow-y-auto py-1.5">
          {results.length === 0 && (
            <p className="text-[12.5px] text-slate-400 text-center py-8">No matches.</p>
          )}
          {results.map((r, i) => {
            const Icon = r.icon;
            return (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: i === selectedIndex ? "#DCFCE7" : "#F3F4F6" }}
                >
                  <Icon size={14} style={{ color: i === selectedIndex ? "#16A34A" : "#94A3B8" }} />
                </div>
                <span
                  className={`text-[13px] flex-1 min-w-0 ${i === selectedIndex ? "font-medium text-[#111827]" : "text-slate-600"}`}
                >
                  <span className="block truncate">{r.label}</span>
                  {r.sub && (
                    <span className="block text-[10.5px] text-slate-400 truncate font-normal">
                      {r.sub}
                    </span>
                  )}
                </span>
    ...
    <div className="relative">
      <button
        ...
        style={{
          ...
          boxShadow: "0 3px 10px rgba(34,197,94,.4)",
        }}
          ...
          <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl border border-slate-200/80 shadow-lg z-40 overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              ...
              {session && !session.demo && (
                <p className="text-[10.5px] text-slate-400 truncate mt-0.5">{session.email}</p>
              )}
            </div>
            {(!session || session.demo) && (
              <p className="px-4 py-2.5 text-[11px] text-slate-400 border-b border-slate-100">
                Demo session — not a real signed-in account.
              </p>
            )}
            <button
              onClick={onSignOut}
              className="w-full flex items-center gap-2 text-[12.5px] text-[#EF4444] hover:bg-[#FEE2E2] px-4 py-2.5 text-left transition-colors"
            >
