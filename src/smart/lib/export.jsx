...
import { ChevronDown, Download, FileCheck, FileSpreadsheet, FileText } from "lucide-react";
...
export function printAsPDF(title, bodyHtml, opts) {
  ...
  win.document.write(
    '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>' +
      ...
      ".doc-hdr{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:36px;padding-bottom:20px;border-bottom:3px solid " +
      accent +
      "}" +
      ".brand{font-size:22px;font-weight:800;color:#111827}.brand-sub{font-size:11px;color:#6B7280;margin-top:3px}" +
      ".doc-title{font-size:28px;font-weight:900;color:" +
      accent +
      ...
      "h2{font-size:10.5px;font-weight:700;color:" +
      accent +
      ";text-transform:uppercase;letter-spacing:.1em;margin:28px 0 10px;padding-bottom:5px;border-bottom:1px solid #E5E7EB}" +
      ...
      ".total-row td{font-weight:700;background:#F0FDF4!important;border-top:2px solid " +
      accent +
      ";font-size:13px}" +
      ".summary{background:" +
      accent +
      "0D;border:1px solid " +
      accent +
      "30;border-radius:10px;padding:20px 24px;margin-top:24px}" +
      ".sum-row{display:flex;justify-content:space-between;padding:5px 0;font-size:12.5px;border-bottom:1px solid " +
      accent +
      "18}" +
      ...
      (logo
        ? '<img src="' +
          logo +
          '" style="height:44px;object-fit:contain;margin-bottom:8px;display:block"/>'
        : "") +
      '<div class="brand">' +
      (companyName || "SMART MANAGER") +
      "</div>" +
      ...
      "</div>" +
      bodyHtml +
      '<div class="footer">Powered by SMART MANAGER · Generated ' +
      new Date().toLocaleString() +
      " · Computer-generated document — no signature required</div>" +
      "</div>" +
      '<div class="print-btn">' +
      '<button onclick="window.print()" style="background:' +
      accent +
      ';color:#fff">🖨 Print / PDF</button>' +
  ...
  const bodyHtml = rows
    .map(
      (r) =>
        "<tr>" +
        r
          .map((c) => '<td class="' + (typeof c === "number" ? "right" : "") + '">' + c + "</td>")
          .join("") +
        "</tr>",
    )
    .join("");
...
export function ExportMenu({ title, filename, sheetName, headers, rows }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 text-[12px] font-medium border border-slate-200 rounded-lg px-3 py-2 hover:bg-slate-50 transition-colors text-slate-600"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1.5 w-44 bg-white rounded-lg border border-slate-200/80 shadow-lg z-40 overflow-hidden">
            <button
              onClick={() => run(() => exportCSV(`${filename}.csv`, headers, rows))}
              className="w-full flex items-center gap-2 text-[12.5px] text-slate-600 hover:bg-slate-50 px-3 py-2.5 text-left"
            >
              <FileSpreadsheet size={13} className="text-slate-400" /> CSV
            </button>
            <button
              onClick={() => run(() => exportExcel(`${filename}.xlsx`, sheetName, headers, rows))}
              className="w-full flex items-center gap-2 text-[12.5px] text-slate-600 hover:bg-slate-50 px-3 py-2.5 text-left"
            >
              <FileSpreadsheet size={13} className="text-[#16A34A]" /> Excel
            </button>
            <button
              onClick={() => run(() => exportWord(`${filename}.doc`, title, html))}
              className="w-full flex items-center gap-2 text-[12.5px] text-slate-600 hover:bg-slate-50 px-3 py-2.5 text-left"
            >
              <FileText size={13} className="text-[#0EA5E9]" /> Word
            </button>
            <button
              onClick={() => run(() => printAsPDF(title, html))}
              className="w-full flex items-center gap-2 text-[12.5px] text-slate-600 hover:bg-slate-50 px-3 py-2.5 text-left"
            >
...
export function printCustomerStatement(customer, invoices) {
  ...
  const custInvoices = invoices
    .filter((inv) => inv.customer === customer)
  ...
  const rows = custInvoices.map((inv) => {
    ...
    running += bal;
    const statusColor =
      { Paid: "#16A34A", Unpaid: "#F59E0B", Overdue: "#EF4444", Partial: "#3B82F6" }[inv.status] ||
      "#6B7280";
    return (
      '<tr style="border-bottom:1px solid #F3F4F6">' +
      '<td style="padding:8px 10px;font-size:11.5px;font-family:monospace">' +
      inv.date +
      "</td>" +
      '<td style="padding:8px 10px;font-size:11.5px;font-weight:600;color:#16A34A">' +
      inv.id +
      "</td>" +
      '<td style="padding:8px 10px;font-size:11.5px">Invoice</td>' +
      '<td style="padding:8px 10px;text-align:right;font-family:monospace;font-size:11.5px">TZS ' +
      fmt(total) +
      "k</td>" +
      '<td style="padding:8px 10px;text-align:right;font-family:monospace;font-size:11.5px;color:#16A34A">TZS ' +
      fmt(paid) +
      "k</td>" +
      '<td style="padding:8px 10px;text-align:right;font-family:monospace;font-weight:700;font-size:11.5px;color:' +
      (bal > 0 ? "#EF4444" : "#16A34A") +
      '">TZS ' +
      fmt(bal) +
      "k</td>" +
      '<td style="padding:8px 10px;text-align:center"><span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:20px;color:white;background:' +
      statusColor +
      '">' +
      inv.status +
      "</span></td>" +
  ...
  const totalPaid = custInvoices.reduce((s, inv) => {
    const t = lineTotal(inv.items || []).total;
    return s + (inv.amountPaid || (inv.status === "Paid" ? t : 0));
  }, 0);
  ...
  printAsPDF(
    ...
    '<div style="font-family:Inter,sans-serif;max-width:680px;margin:0 auto;padding:32px;color:#111827">' +
      ...
      "</div>" +
      (co.tin ? '<div style="font-size:11px;color:#6B7280">TIN: ' + co.tin + "</div>" : "") +
      "</div>" +
      '<div style="text-align:right">' +
      '<div style="font-size:22px;font-weight:900;color:#16A34A">ACCOUNT STATEMENT</div>' +
      '<div style="font-size:11px;color:#6B7280;margin-top:4px">As at ' +
      ...
      (rows.length
        ? rows.join("")
        : '<tr><td colspan="7" style="padding:20px;text-align:center;color:#9CA3AF">No invoices found for this customer.</td></tr>') +
      "</tbody>" +
      "</table>" +
      '<div style="display:flex;justify-content:flex-end">' +
      '<div style="width:300px">' +
      '<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #E5E7EB;font-size:12px"><span style="color:#6B7280">Total Invoiced</span><span>TZS ' +
      fmt(totalCharged) +
      "k</span></div>" +
      '<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #E5E7EB;font-size:12px;color:#16A34A"><span>Total Received</span><span>TZS ' +
      fmt(totalPaid) +
      "k</span></div>" +
      '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:' +
      (totalOutstand > 0 ? "#052614" : "#F0FDF4") +
      ';border-radius:8px;margin-top:8px">' +
      '<span style="font-size:13px;font-weight:700;color:' +
      (totalOutstand > 0 ? "white" : "#16A34A") +
      '">Outstanding Balance</span>' +
      '<span style="font-size:18px;font-weight:900;color:' +
      (totalOutstand > 0 ? "#4ADE80" : "#16A34A") +
      '">TZS ' +
      fmt(totalOutstand) +
      "k</span>" +
      "</div>" +
      "</div>" +
      "</div>" +
      '<div style="text-align:center;margin-top:24px;font-size:10px;color:#9CA3AF">Generated by Smart Manager · ' +
      (co.name || "") +
      " · This is a computer-generated statement.</div>" +
      "</div>",
  );
}
