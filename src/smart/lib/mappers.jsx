...
export function mapInventoryRow(r) {
  return {
    ...
    unit: r.unit || "unit",
    barcode: r.barcode || generateBarcode(r.sku),
...
export function mapPurchaseOrderRow(r) {
  return {
    ...
    items: mapPoItems(r.purchase_order_items),
  };
}
...
export function mapOrderReturnRow(rr) {
  return {
    ...
    items: (rr.sales_order_return_items || []).map((it) => ({
      name: it.item_name,
      sku: it.item_sku,
      qty: Number(it.qty) || 0,
      rate: Number(it.rate) || 0,
...
export function mapOrderRow(r) {
  return {
    ...
    customer: r.customer,
    date: r.order_date,
    quotationRef: r.quotation_id ? "linked" : "—",
    status: r.status,
    owner: r.owner_id || "Unassigned",
...
export function mapInvoiceRow(r) {
  return {
    ...
    date: r.issue_date,
    dueDate: r.due_date,
    orderRef: r.order_id ? "linked" : "—",
    status: r.status,
    amountPaid: Number(r.amount_paid) || 0,
    items: mapDocItems(r.sales_invoice_items),
    payments: (r.sales_payments || [])
      .map(mapPaymentRow)
...
export function mapEmployeeRow(r) {
  return {
    ...
    contractType: r.contract_type || "Permanent",
...
export function mapChatRow(r) {
  return {
    ...
    customer: r.customer,
    status: r.status,
    messages: mapChatMessages(r.support_chat_messages),
  };
}
...
export function mapKbArticleRow(r) {
  return {
    ...
    updatedDate: r.updated_at?.slice(0, 10),
  };
}
...
export function mapBenchmarkRow(r) {
  return {
    ...
    metricId: r.metric_id,
    label: r.label,
    benchmarkValue: Number(r.benchmark_value) || 0,
  };
}
...
export function mapMarketplaceTemplateRow(r) {
  return {
    ...
    trigger: r.trigger_type,
    steps: r.steps || [],
    publisherName: r.published_by_company_name || "Official",
